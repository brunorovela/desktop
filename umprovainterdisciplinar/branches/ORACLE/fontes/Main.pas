...
uses
  ...
  ToolWin, ZConnection,
  UCrpe32, UCrpeClasses, Mask, DBCtrls, uRelutils, CropImage, uUsuario,
  uLoginManager, uTrataExcecao, uTratamentoExcecaoPI;
...
type
  TPrincipalForm = class(TForm)
    ...
    Report: TCrpe;
    Sobre1: TMenuItem;
    procedure Importaodedadosdaleitora1Click(Sender: TObject);
    procedure Impressodeprovasecartes1Click(Sender: TObject);
...
uses
   uDataHoje, uDM, uErro, ufPIntGeracao, uFPintImportacao,
   Masks, uSobre, uFPIntRelatorios, uIUMDataModule;
...
begin
  ...
  // Verificar se a base de dados está conectada
   ...
   if not DM.GetUsuarioLogado.TemPermissao(
         DM.GetUsuarioLogado.Pessoa.Codigo, 'ProvaInterdisciplinar.Modulo.Acesso', npAcesso, True) then
   begin
...
//   ArquivoMorto1.Visible := PasswordDlg.Autorizar( 7777, taAcessar, False );
...
begin
  ...
  if FecharForcado then begin
   ...
   if (Mensagem('Deseja realmente sair do sistema ?', '', MB_YESNO + MB_ICONQUESTION) = IDYES) then
   begin
      CanClose := True;
      DM.db.Disconnect;
...
begin
   ...
   Application.OnException :=
      TTrataExcecao.Create(TTratamentoExcecaoPI.Create()).TrataExcecao;
   { Parâmetros Gerais }
...
begin
  ...
  if DM.GetUsuarioLogado.TemPermissao( 1026, npAcesso, False ) then Begin
      Application.CreateForm(TfrmDataHoje, frmDataHoje);
      frmDataHoje.ShowModal;
  end
  else
  begin
      DataHoje := Trunc(DM.DataAtual());
  end;
...
begin
   if DM.GetUsuarioLogado.TemPermissao(15002, npAcesso, true) then
   begin
      Application.CreateForm(TfPIntRelatorios, fPIntRelatorios);
      fPIntRelatorios.ShowModal;
   end;
end;
...
procedure TPrincipalForm.Importaodedadosdaleitora1Click(Sender: TObject);
begin
   if DM.GetUsuarioLogado.TemPermissao(15001, npAcesso, true) then
   begin
      if not ProcuraForm(fPIntImportacao) then
         Application.CreateForm(TfPIntImportacao, fPIntImportacao);
      fPIntImportacao.ShowModal;
   end;
end;

procedure TPrincipalForm.Impressodeprovasecartes1Click(Sender: TObject);
begin
   if DM.GetUsuarioLogado.TemPermissao(15000, npAcesso, true) then
   begin
      if not ProcuraForm(fPIntGeracao) then
         Application.CreateForm(TfPIntGeracao, fPIntGeracao);
      fPIntGeracao.ShowModal;
   end;
end;
