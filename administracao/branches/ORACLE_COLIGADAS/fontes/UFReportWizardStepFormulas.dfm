inherited frmReportWizardStepFormulas: TfrmReportWizardStepFormulas
  object dbgFormulas: TDBGrid
    Left = 0
    Top = 0
    Width = 451
    Height = 304
    Align = alClient
    DataSource = dsFormulas
    TabOrder = 0
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'Tahoma'
    TitleFont.Style = []
    Columns = <
      item
        Expanded = False
        FieldName = 'FormulaName'
        Title.Caption = 'F'#243'rmula'
        Width = 256
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'PropertyName'
        Title.Caption = 'Filtro'
        Width = 256
        Visible = True
      end>
  end
  object dsFormulas: TDataSource
    DataSet = cdsFormulas
    Left = 288
    Top = 232
  end
  object cdsFormulas: TClientDataSet
    Aggregates = <>
    Params = <>
    Left = 240
    Top = 232
    object cdsFormulasName: TStringField
      FieldName = 'Name'
      Size = 255
    end
    object cdsFormulasProperty: TStringField
      FieldName = 'Property'
      Size = 32
    end
    object cdsFormulasPropertyName: TStringField
      FieldKind = fkLookup
      FieldName = 'PropertyName'
      LookupDataSet = cdsProperties
      LookupKeyFields = 'Kind'
      LookupResultField = 'Name'
      KeyFields = 'Property'
      Size = 255
      Lookup = True
    end
    object cdsFormulasFormulaName: TStringField
      FieldKind = fkLookup
      FieldName = 'FormulaName'
      LookupDataSet = cdsNamesFormulas
      LookupKeyFields = 'Name'
      LookupResultField = 'Name'
      KeyFields = 'Name'
      Size = 255
      Lookup = True
    end
  end
  object cdsProperties: TClientDataSet
    Active = True
    Aggregates = <>
    Params = <>
    Left = 240
    Top = 264
    Data = {
      
      6F546974756C6F000005005475726D61055475726D61}
    object cdsPropertiesName: TStringField
      FieldName = 'Name'
      ReadOnly = True
      Size = 255
    end
    object cdsPropertiesKind: TStringField
      FieldName = 'Kind'
      Size = 32
    end
  end
  object cdsNamesFormulas: TClientDataSet
    Aggregates = <>
    Params = <>
    Left = 240
    Top = 296
    object cdsNamesFormulasName: TStringField
      FieldName = 'Name'
      Size = 255
    end
  end
end
