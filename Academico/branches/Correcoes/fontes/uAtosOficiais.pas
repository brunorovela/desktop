...
procedure TfrmAtosOficiais.RadioButton1Click(Sender: TObject);
begin
  cbUnidadeCertificadora.ItemIndex := -1;
  cbUnidadeCertificadora.Enabled := false;
end;

procedure TfrmAtosOficiais.RadioButton2Click(Sender: TObject);
begin
  cbUnidadeCertificadora.ItemIndex := 0;
  cbUnidadeCertificadora.Enabled := True;
end;
...
begin
   if (frmCursos.qyAtosOficiais.State in [dsInsert]) or (frmCursos.qyAtosOficiais.IsEmpty) then
      ...
      umdtAto.Date := frmCursos.qyAtosOficiais.FieldByName('dt_ato').AsDateTime;
   ...
   qyConsulta.SQL.Add('SELECT cd_instituicao, nm_instituicao FROM instituicoes_ensino WHERE sn_unidade_certificadora = 1');
   qyConsulta.Open;

   for i := 0 to qyConsulta.RecordCount - 1 do
   begin
      cbUnidadeCertificadora.addItem(
         qyConsulta.Fields[1].Text,
         TItemCombo.Create(
           qyConsulta.Fields[0].Text,
           qyConsulta.Fields[1].Text
         )
      );


      if StrToInt(TItemCombo(cbUnidadeCertificadora.Items.Objects[i]).getCodigo()) = frmCursos.qyAtosOficiaiscd_unidade_certificadora.AsInteger then
      begin
   ...
   if ( frmCursos.qyAtosOficiais.FieldByName('cd_coligada').AsInteger = 0 ) OR ( cbUnidades.ItemIndex < 0  )then
   begin
...
begin
   ...
   if cbUnidadeCertificadora.ItemIndex > -1 then
     frmCursos.qyAtosOficiaiscd_unidade_certificadora.AsInteger := StrToInt(TitemCombo(cbUnidadeCertificadora.Items.Objects[cbUnidadeCertificadora.ItemIndex]).getCodigo())
   else
     frmCursos.qyAtosOficiaiscd_unidade_certificadora.Clear;
