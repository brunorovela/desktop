...
type
   TFControlePerm = class(TForm)
    ...
    cdsPermissoesACAO: TIntegerField;
    cdsPermissoesCHAVE: TStringField;
    cdsPermissoesACESSO: TLargeintField;
    cdsPermissoesINCLUSAO: TLargeintField;
    cdsPermissoesALTERACAO: TLargeintField;
    cdsPermissoesEXCLUSAO: TLargeintField;
    cdsPermissoesESPECIAL: TLargeintField;
    cdsPermissoesTODAS: TLargeintField;
    ...
    procedure lbBuscaChange(Sender: TObject);
      procedure dbPermissaoClick(Sender: TObject);
   ...
   public
      ...
      procedure GravaLogPermissao(const nr_permissao : Integer);
      procedure RegistrarLogPermissao(const Ds_acao, Ds_chave, Ds_log : String);      
...
procedure TFControlePerm.GravaLogPermissao(const nr_permissao: integer);
var
   cont_permissao : integer;
   ds_chave, ds_log_inserir, ds_log_excluir : String;
begin
   ds_chave :=  intToStr(getCodGrupo)
            +':'+qryPermissoesCD_ACAO.AsString
            +':'+qryPermissoesPERMISSAO.AsString
            +'-'+DateToStr(Now()) ;

   ds_log_inserir := 'Foi adicionada a perm '
         + qryPermissoesCD_ACAO.AsString
         + '({$nr_permissao$}) para o grupo ' 
         + intToStr(getCodGrupo)
         + ' '+DateTimeToStr(Now);
  
  ds_log_excluir :=    'Foi excluida a perm '
         +qryPermissoesCD_ACAO.AsString
         +'({$nr_permissao$}) para o grupo '
         + intToStr(getCodGrupo)
         +' '+DateTimeToStr(Now);
   ...
   // if para ACESSO
   // if para ACESSO
   if ( (nr_permissao AND 1) = 1 ) then
   begin
      RegistrarLogPermissao(
         'Inclusao',
         ds_chave,
         StringReplace( ds_log_inserir,'{$nr_permissao$}', '1', [rfReplaceAll, rfIgnoreCase])
      );      
   end else
   if (qryPermissoesACESSO.AsInteger = 1) then   
   begin
      RegistrarLogPermissao(
         'Exclusao',
         ds_chave,
         StringReplace( ds_log_excluir,'{$nr_permissao$}', '1', [rfReplaceAll, rfIgnoreCase])
      );
   end;
   
   // if para INCLUSAO
   // if para INCLUSAO
   if ( nr_permissao AND 2 = 2) then
   begin
      RegistrarLogPermissao(
         'Inclusao',
         ds_chave,
         StringReplace( ds_log_inserir,'{$nr_permissao$}', '2', [rfReplaceAll, rfIgnoreCase])
      );      
   end else
   if (qryPermissoesINCLUSAO.AsInteger = 1) then
   begin
      RegistrarLogPermissao(
         'Exclusao',
         ds_chave,
         StringReplace( ds_log_excluir,'{$nr_permissao$}', '2', [rfReplaceAll, rfIgnoreCase])
      );
   end;

   // if para ALTERACAO
   // if para ALTERACAO
   if ( nr_permissao AND 4 = 4) then
   begin
      RegistrarLogPermissao(
         'Inclusao',
         ds_chave,
         StringReplace( ds_log_inserir,'{$nr_permissao$}', '4', [rfReplaceAll, rfIgnoreCase])
      );      
   end else
   if (qryPermissoesALTERACAO.AsInteger = 1) then
   begin
      RegistrarLogPermissao(
         'Exclusao',
         ds_chave,
         StringReplace( ds_log_excluir,'{$nr_permissao$}', '4', [rfReplaceAll, rfIgnoreCase])
      );
   end;
   
   // if para EXCLUSAO
   // if para EXCLUSAO
   if ( nr_permissao AND 8 = 8) then
   begin
      RegistrarLogPermissao(
         'Inclusao',
         ds_chave,
         StringReplace( ds_log_inserir,'{$nr_permissao$}', '8', [rfReplaceAll, rfIgnoreCase])
      );      
   end else
   if (qryPermissoesEXCLUSAO.AsInteger = 1) then
   begin
      RegistrarLogPermissao(
         'Exclusao',
         ds_chave,
         StringReplace( ds_log_excluir,'{$nr_permissao$}', '8', [rfReplaceAll, rfIgnoreCase])
      );
   end;

   // if para ESPECIAL   
   // if para ESPECIAL
   if ( nr_permissao AND 16 = 16) then
   begin
      RegistrarLogPermissao(
         'Inclusao',
         ds_chave,
         StringReplace( ds_log_inserir,'{$nr_permissao$}', '16', [rfReplaceAll, rfIgnoreCase])
      );      
   end else
   if (qryPermissoesESPECIAL.AsInteger = 1) then
   begin
      RegistrarLogPermissao(
         'Exclusao',
         ds_chave,
         StringReplace( ds_log_excluir,'{$nr_permissao$}', '16', [rfReplaceAll, rfIgnoreCase])
      );
   end;

   // if para TODAS SELECIONADAS   
   // if para TODAS SELECIONADAS  
   if ( nr_permissao = 31) then
   begin
      RegistrarLogPermissao(
         'Inclusao',
         ds_chave,
         StringReplace( ds_log_inserir,'{$nr_permissao$}', '31', [rfReplaceAll, rfIgnoreCase])
      );      
   end else
   if (qryPermissoesTODAS.AsInteger = 1) then
   begin
      RegistrarLogPermissao(
         'Exclusao',
         ds_chave,
         StringReplace( ds_log_excluir,'{$nr_permissao$}', '31', [rfReplaceAll, rfIgnoreCase])
      );
   end;
end;
...
begin
   ...
   // grava log de alteração da permissao
   GravaLogPermissao(nr_permissao);
...
const
   SQL_COLIGADAS =
      ...
      ' ORDER BY sn_matriz DESC ,nm_coligada ASC ';
var
...
begin
   ...
   begin
      ...
      sql := StringReplace(sql, '[nm_pessoa]', 'AND p.nm_pessoa LIKE ' + QuotedStr('%' + filtro_nm_pessoa + '%'), []);
   end;
...
procedure TFControlePerm.dbPermissaoClick(Sender: TObject);
var
   Permissoes: Integer;
begin
   if (not carregando) and
      (not selecionandoTodos) and
      (cdsPermissoes.State in [dsInsert, dsEdit]) then
   begin

      Permissoes := 0;

      if dbAcesso.Checked then Inc(Permissoes, 1);
      if dbInclusao.Checked then Inc(Permissoes, 2);
      if dbAlteracao.Checked then Inc(Permissoes, 4);
      if dbExclusao.Checked then Inc(Permissoes, 8);
      if dbEspecial.Checked then Inc(Permissoes, 16);
...
procedure TFControlePerm.RegistrarLogPermissao(const Ds_acao, Ds_chave,
  Ds_log: String);
begin
      // Salva o log
      DM.setLog(
