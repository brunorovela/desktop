...
type
  Tfrm_Relatorios_impressos = class(TForm)
    ...
    LVersao: TLabel;
    procedure LVersaoClick(Sender: TObject);
...
const
   SQL_LISTA_RELATORIOS =
      ...
      ' WHERE ( t.codigo IS NULL OR t.cd_coligada IN ( %s ) ) AND r.cd_modulo IN (%s) ' +
...
begin
   ...
   if modulo = 2 then
      ...
      modulos := IntToStr(modulo) ;
   

   qyRelatorios.Close;
   qyRelatorios.SQL.Text := Format(SQL_LISTA_RELATORIOS, [DM.UsuarioLogado.GetColigadasFilhaSelecionada(), modulos]);
  // qyRelatorios.ParamByName('modulo').AsInteger := modulo;
   qyRelatorios.Open;
end;


procedure Tfrm_Relatorios_impressos.LVersaoClick(Sender: TObject);
var
  URL: string;
begin
