...
begin
   ...
   else if cmCurso.ItemIndex = 2 then
      qyCursosTurmasCampossn_curso.AsInteger := 2
   else
      qyCursosTurmasCampossn_curso.AsInteger := 3;
   ...
   if cmTipoCampo.ItemIndex = 0 then
      ...
      qyCursosTurmasCamposDS_TIPO.AsString := 'X'
   else
      qyCursosTurmasCamposds_tipo.AsString := 'P';
...
const
   SQL_VERIFICA_CONTEUDO_TEXTO_MEMO = ''+
      'SELECT '+
      '  GROUP_CONCAT(ctc.ds_conteudo) AS conteudo_texto, '+
	   '  GROUP_CONCAT(ctcm.ds_conteudo) AS conteudo_memo '+
      'FROM '+
	   '  cursos_turmas_extras cte '+
      '  LEFT JOIN cursos_turmas_cadastro ctc ON (ctc.cd_campo = cte.cd_campo) '+
      '  LEFT JOIN cursos_turmas_cadastro_memo ctcm ON (ctcm.cd_campo = cte.cd_campo) '+
      'WHERE '+
      '  cte.cd_campo = :cd_campo';
var
...
begin
   ...
   if (((qyCursosTurmasCamposds_tipo.AsString = 'A') AND (cmTipoCampo.Text = 'MEMO'))) then
   begin
      ...
      qryBuscaConteudoCampo.ParamByName('cd_campo').AsInteger := qyCursosTurmasCamposCD_CAMPO.AsInteger;
   ...
   if (((qyCursosTurmasCamposds_tipo.AsString = 'M') AND (cmTipoCampo.Text = 'TEXTO'))) then
   begin
   ...
   if (cmTipoCampo.ItemIndex = 1) OR (cmTipoCampo.ItemIndex = 5) then
   begin
...
begin
      ...
      if qyCursosTurmasCampossn_curso.AsInteger = 0 then
         ...
         cmCurso.ItemIndex := 0
      else if qyCursosTurmasCampossn_curso.AsInteger = 2 then
         cmCurso.ItemIndex := 2
      else
         cmCurso.ItemIndex := 3;
      ...
      if (qyCursosTurmasCamposds_tipo.AsString ='A')  then
         ...
         cmTipoCampo.ItemIndex := 2
      else if (qyCursosTurmasCamposds_tipo.AsString ='E') then
         cmTipoCampo.ItemIndex := 3
      else if (qyCursosTurmasCamposds_tipo.AsString ='X') then
...
begin
  ...
  if (cmTipoCampo.ItemIndex = 1 ) OR (cmTipoCampo.ItemIndex = 5) then begin
      dblNomeOpcao.Visible := true;
      lblOpcao.Visible := true;
   end
   else begin
...
begin
   qyCursosTurmasCampos.Open;
   qyOpcoes.Open;
   if (qyCursosTurmasCamposds_tipo.AsString = 'S')
   or
      (qyCursosTurmasCamposds_tipo.AsString = 'X')
   then
   begin
