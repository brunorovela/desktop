...
uses
  ...
  UMDateTimePicker, uMensalidades_Baixar, ZDbcIntfs, UZDbcFuncs;

type
  TSituacao = (sAguardandoProcessamento, sOcorrenciaInexistente, sNNNaoEncontrado, sValorDivergente, sJaPago, sNegociado, sCancelado, sNNDuplicado, sRegistroInformativo, sRegistroProcessado, sDesconto, sAlunoEmCobranca );
  {
  ...
  11: sAlunoEmCobranca = Aluno com a opca fin_cobranca.sn_spc ligada
  ...
  TfrmRetornoBanco = class(TForm)
    ...
    qryItensds_situacao_remessa: TStringField;
    qyVerificaArquivoRemessa: TUMZQuery;
    qyVerificaArquivoRemessacd_arquivo: TIntegerField;
    qyVerificaArquivoRemessanr_pendentes: TLargeintField;
    qyAtualizaArquivoRemessa: TUMZQuery;
    sbAluno: TSpeedButton;
    sbNossoNumero: TSpeedButton;
    pnSPC: TPanel;
    qryItensTotalqtd_spc: TLargeintField;
    procedure sbNossoNumeroClick(Sender: TObject);
  ...
  private
    ...
    filtroCdPessoa: Integer;
    filtroNossoNumero: String;
    ...
    function BuscaNossoNumero( strNumero : String; sFiltro : String; snAlunoEmCobranca : Boolean = false) : Boolean;
...
uses uDM, uCamposPlanilhas, uSelCaixa, Main, uClassMovimento, uRetornoTitulos, uUsuario,
  uEstorno, uRepRecibo, uRemessaClass, uFSelecionarPessoa, uNossoNumero;
...
function TfrmRetornoBanco.BuscaNossoNumero(strNumero, sFiltro: String; snAlunoEmCobranca : Boolean = false) : Boolean;
const
   SIntToStrFmt = '%d';
Var
   NovoNN : String;
...
begin
  ...
  if Trim(strNumero) <> '' then begin
      ...
      // Verificar situação de cobrança

      if snAlunoEmCobranca then begin

         Dm.tblMensalidades.SQL.Clear();
         Dm.tblMensalidades.SQL.Text :=
                 ' SELECT Mensalidades.* from Mensalidades    ' +
                 '  INNER JOIN fin_cobranca ON (fin_cobranca.cd_pessoa = Mensalidades.codigoaluno ) '+
                 ' WHERE ' +
                 ' Mensalidades.nossonumero = ''' + NovoNN + ''' AND ' +
                 ' Mensalidades.cd_coligada in (' + Dm.GetColigadas() + ') AND ' +
                 ' ( IFNULL(fin_cobranca.sn_spc,0) = 1 OR IFNULL(fin_cobranca.sn_serasa,0) = 1 OR IFNULL(fin_cobranca.sn_cartorio,0) = 1 ) AND ' +
                 sFiltro +
                 ' ORDER BY Mensalidades.valorbruto DESC ';
         ...
         exit;
            ...
            End else Begin
               ...
               Dm.tblMensalidades.SQL.Text :=
                 ' SELECT * from Mensalidades WHERE ' +
                 ' nossonumero = ''' + NovoNN + ''' AND ' +
                 ' cd_coligada in (' + Dm.GetColigadas() + ') AND ' +
            ...
            if not Result AND Dm.isTrue(qryItenssn_baixar.AsString) then begin
               ...
               Dm.tblMensalidades.SQL.Text :=
                 ' SELECT * from Mensalidades WHERE ' +
                 ' nossonumero = ''' + NovoNN + ''' AND  (situacao = 0 OR situacao = 1) AND ' +
                 ' cd_coligada in (' + Dm.GetColigadas() + ')' +
                 ' ORDER BY valorbruto DESC ';
               Dm.tblMensalidades.Open();
               Dm.tblMensalidades.FetchAll();
               ...
               qBuscaBoleto.SQL.Text :=
                    ...
                    ' ) '+
                    ' WHERE '+
                    '    b.nr_nossonumero = :nossonumero AND '+
                    '    b.cd_coligada in (' + Dm.GetColigadas() + ') AND ' +
                    '    situacao IN (2, 10)  ';
     ...
     if (sColCPF <> '') AND (sColVencimento <> '') then begin
         ...
         if (tblBancocd_banco.AsString = 'FIE') OR (qryRetornocd_banco.AsString = 'FIE') then begin
         ...
         // Somente vai olhar a cobranca se a baixa for diferente de FIES
         if (snAlunoEmCobranca)  then begin
            Dm.tblMensalidades.SQL.Add('  INNER JOIN fin_cobranca ON (fin_cobranca.cd_pessoa = Mensalidades.codigoaluno AND ( IFNULL(fin_cobranca.sn_spc,0) = 1 OR IFNULL(fin_cobranca.sn_serasa,0) = 1 OR IFNULL(fin_cobranca.sn_cartorio,0) = 1 ) ) ');
         end;

         Dm.tblMensalidades.SQL.Add(
           ' WHERE ' +
...
begin
   ...
   btBaixar.Enabled := ((qryItenscd_situacao.AsInteger in [0, 2, 3, 4, 5, 6, 7, 8, 11]) AND (qryItenssn_baixar.AsString = 'S'));
...
const
   ...
   S_SQL_FILTRO_CD_PESSOA =
       '  AND r.codigo IN (SELECT DISTINCT cd_retorno FROM retorno_itens WHERE cd_pessoa = %d) ';

   S_SQL_FILTRO_NOSSONUMERO =
       '  AND r.codigo IN (SELECT DISTINCT cd_retorno FROM retorno_itens WHERE nr_nossonumero = "%s") ';

   S_SQL_ORDER = ' ORDER BY r.codigo DESC ';

   function GetSQLByProtocol: string;
   begin
      Result := S_SQL_RETORNO;

      if filtroCdPessoa > 0 then
      begin
        Result := Result + Format(S_SQL_FILTRO_CD_PESSOA, [filtroCdPessoa]);
      end;

      if Length(filtroNossoNumero) > 0 then
      begin
        Result := Result + Format(S_SQL_FILTRO_NOSSONUMERO, [filtroNossoNumero]);
      end;

      Result := Result + S_SQL_ORDER;     
...
CONST
  ...
  S_SQL_FILTRO_CD_PESSOA = '  AND ri.cd_pessoa = :cd_pessoa ';

  S_SQL_FILTRO_NOSSONUMERO = '  AND ri.nr_nossonumero LIKE :nr_nossonumero ';

  SQL_GROUP_BY = ' GROUP BY ri.cd_retorno, ri.nr_sequencia ';
...
begin
   ...
   if filtroCdPessoa > 0 then
   begin
      qryItens.SQL.Add(S_SQL_FILTRO_CD_PESSOA);
      qryItens.ParamByName('cd_pessoa').AsInteger := filtroCdPessoa;
   end;
   

   if Length(filtroNossoNumero) > 0 then
   begin
      qryItens.SQL.Add(S_SQL_FILTRO_NOSSONUMERO);
      qryItens.ParamByName('nr_nossonumero').AsString := filtroNossoNumero;
   end;

   qryItens.SQL.Add( SQL_GROUP_BY  );
   ...
   pnSPC.Visible := qryItensTotalqtd_spc.AsInteger > 0;
...
begin
  ...
  if tblBancoconta_inicio.AsInteger = 0 then begin
      ...
      frm_SelCAixa.iForcarColigada :=TColigada(cbColigadas.Items.Objects[cbColigadas.ItemIndex]).Codigo;
  ...
  qyRetorno.SQL.Add('VALUES(:codigo, now(), :baixados, :rejeitados, :outras_ocorrencias, :usuario, :nomeArquivo, :cd_banco, :cd_coligada, :md5_arquivo ) ');
...
//       Trim(slColunas[ ]);
   ...
   { Alterar Ocorrencia se usar remessa }
    ...
    try
       ...
       qyInsert.ParamByName('dt_pagamento').AsSTring := ''
    end;

    try
       qyInsert.ParamByName('dt_credito').AsDateTime :=  StrToDate(pega_data_credito());
    except
       qyInsert.ParamByName('dt_credito').AsSTring := ''
    end;
...
// fim registro na conta banco
  ...
  // Deve ser posicionado no último retorno lido, e não no primeiro, pois em caso de duplo processamento, vaidar problema.
  qryRetorno.Locate('Codigo',qyMaxRetorno.FieldByName('maximo').AsString, []);
  // qryRetorno.First;
...
var
  ...
  mov : TMovimento;
  iTpConta : Word;
  iCodAberturaAtual, cdLayout: Integer;
  objRemessa: TRemessa;
...
begin
  ...
  // Se possuir itens a serem processados cria a classe de remessa
  ...
  while not qryItens.EOF do begin
   ...
   freeandnil(mov);
    ...
    if qryItenscd_situacao.AsInteger <> 10 then begin
        ...
        {
          ATUALIZAÇÃO DOS DADOS DA REMESSA RECUPERADOS ATRAVÉS DA TELA DE RETORNO
        }

        if DM.variavel_parametro('sn_processo_remessa_novo') = '1' then
        begin
        
          // Busca a situação do NN que representa o registro que recebeu do retorno
          situacaoNN := objRemessa.getSituacaoNossoNumero(qryItens.FieldByName('ds_situacao_remessa').AsString);

          // Se o registro que foi recebido é válido
          if situacaoNN <> snnNone then
          begin
            // Atualiza o status do NN na tabela de status com o recebido do retorno
            objRemessa.alterarStatusNossoNumero(strNossoNumero, situacaoNN);
          end;
        ...
        End ELSE Begin
           ...
           if Dm.isTrue(qryItenssn_baixar.AsString) then Begin
              ...
              // Somente registrar mensalidades em cobrança para titulos que não sejam do fies.
              if (tblBancocd_banco.AsString <> 'FIE') AND (qryRetornocd_banco.AsString <> 'FIE') AND BuscaNossoNumero(strNossoNumero, '(situacao = 2 OR situacao = 8 OR situacao = 10)', true) then begin
                 // O último parametro = true indica que será feita a busca de uma mensalidade pendente com o aluno em situação de cobrança = SPC (fin_cobranca.sn_spc = 1)

                 AtualizarSituacaoItem(qryItenscd_retorno.AsInteger, qryItensnr_sequencia.AsInteger, sAlunoEmCobranca, Dm.tblMensalidadescd_resp.AsInteger, Dm.tblMensalidadesCodigoAluno.AsInteger, dm.tblMensalidadesDataVencimento.AsDateTime,  'Cobrança em SPC' );
              end else if BuscaNossoNumero( strNossoNumero, ' (situacao = 2 OR situacao = 8 OR situacao = 10) ' ) then Begin
  ...
  {
    ATUALIZAÇÃO DOS ARQUIVOS DA REMESSA ATRAVÉS DA TELA DE RETORNO
  }

  if DM.variavel_parametro('sn_processo_remessa_novo') = '1' then
  begin

    qyVerificaArquivoRemessa.Close;
    qyVerificaArquivoRemessa.Open;

    while not qyVerificaArquivoRemessa.Eof do
    begin
      // Atualiza o arquivo de remessa (situação para confirmado pelo retorno)
      qyAtualizaArquivoRemessa.Close;
      qyAtualizaArquivoRemessa.ParamByName('cd_arquivo').AsInteger := qyVerificaArquivoRemessa.FieldByName('cd_arquivo').AsInteger;
      qyAtualizaArquivoRemessa.ExecSQL;

      qyVerificaArquivoRemessa.Next;
    end;
...
procedure TfrmRetornoBanco.sbAlunoClick(Sender: TObject);
var
  resultadoFiltro: TResultadoFiltroPessoa;
begin

  if not sbAluno.Down then
  begin
    filtroCdPessoa := -1;
    FiltrarArquivos();
    FiltrarConteudo();
  ...
  filtroCdPessoa := resultadoFiltro.cd_pessoa;
...
procedure TfrmRetornoBanco.sbNossoNumeroClick(Sender: TObject);
begin

  if not sbNossoNumero.Down then
  begin
    filtroNossoNumero := '';
    FiltrarArquivos();
    FiltrarConteudo();
  ...
  try
    ...
    frmNossoNumero.pcBusca.ActivePageIndex := 0;

    frmNossoNumero.pcBusca.Pages[0].TabVisible := True;
    frmNossoNumero.pcBusca.Pages[1].TabVisible := False;
    frmNossoNumero.pcBusca.Pages[2].TabVisible := False;

    if frmNossoNumero.ShowModal <> mrOk then
    begin
      filtroNossoNumero := '';
      sbNossoNumero.Down := False;
      FiltrarArquivos();
      FiltrarConteudo();
      Exit;
    end;

    filtroNossoNumero := frmNossoNumero.txtNossoNumero.Text;
    FiltrarArquivos();
    FiltrarConteudo();
...
const
   SSQLSelectRetornoComBaixa = ''
      + 'SELECT '
      + '   t.cd_mensalidade '
      + 'FROM '
      + '   retorno r '
      + '      JOIN retorno_itens i ON '
      + '         (r.codigo = i.cd_retorno) '
      + '      JOIN retorno_itens_titulos t ON '
      + '         (i.cd_retorno = t.cd_retorno) AND '
      + '         (i.nr_sequencia = t.nr_sequencia) '
      + '      JOIN banco_codigo_retorno b ON '
      + '         (r.cd_banco = b.cd_banco) AND '
      + '         (i.cd_ocorrencia = b.cd_retorno) '
      + 'WHERE '
      + '   r.codigo = ? AND '
      + '   i.nr_sequencia = ? AND '
      + '   b.cd_origem = 1 AND '
...
begin
   ...
   try
      ...
      Stmt := PrepareStatement(SSQLSelectRetornoComBaixa); 
      try
         while not qryItens.Eof do
         begin
            Stmt.ClearParameters;
            Stmt.SetInt(1, qryItenscd_retorno.AsInteger);
            Stmt.SetInt(2, qryItensnr_sequencia.AsInteger);
            Rs := Stmt.ExecuteQueryPrepared;
            try
               if Rs.Next then
                  try
                     oRecibo.setMensalidade(Rs.GetInt(1));
...
Var
   ...
   iCodCaixa, iCodAbertura,
   iCodAberturaAtual, cdLayout : integer;
   iTpConta :word;
   Movimento : TMovimento;
   ds_historico, sCodCheques, sOpCheques,
   sAux, sCodMensalidadeOrigem, dsNossoNumero : String;
   i : Integer;
...
begin
  ...
  while not qryTitulos.Eof do begin
      ...
      qyAltera.SQL.Text :=
          ...
          ' WHERE cd_movimento_te in (' + IntToStr(qyTemp2.FieldByName('cd_movimento_te').AsInteger) + ', ' + IntToStr(Movimento.CodigoMovimento) + ') ';
      qyAltera.ExecSQL();
      ...
      iCodCaixa := DM.tblMensalidades.FieldByName('cd_caixa').AsInteger;
      dsNossoNumero := DM.tblMensalidades.FieldByName('nossonumero').AsString;
      ...
      DM.estornarAcoesMovimentoDescCondicional(DM.tblMensalidadescd_mensalidade.AsInteger);
      DM.tblMensalidades.Close();
