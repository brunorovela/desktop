...
uses
   uCadEquiv, uNotaDiscEquiv, uFSelecionarPessoa, uDM, uUsuario, Main,
   UFRegistraEliminEqui, UFHistoricoEliminEqui, UFObservacoes;
         ...
         begin
            ...
            try
               ...
               begin
                ...
                fRegistraEliminEqui.SetCurso(tblDiscEquivCurso.AsString);
...
var
   ...
   qryGetNomeDisciplina : TUMZQuery;
begin
      ...
      if not Dm.tblFichaIndividual.Eof then begin
         ...
         if DM.isSituacaoValida([0], DM.tblFichaIndividualSituacao.AsInteger) then
         Begin
         ...
         // O aluno não tinha a disciplina no anosemestre que irá registrar. Verificar se tem a disciplina em outro ano/semestre
          ...
          if DM.tblFichaIndividual.Eof then begin
             ...
             if DM.isSituacaoValida([0], DM.tblFichaIndividualSituacao.AsInteger) then begin
      ...
      if DM.tblFichaIndividual.State = dsEdit then begin
         ...
         DM.CriarConsulta(qryGetNomeDisciplina);

         qryGetNomeDisciplina.SQL.Clear();            
         qryGetNomeDisciplina.SQL.Add(
            ' SELECT '
            + '    d.descricao '
            + ' FROM '
            + ' disciplinas d '
         ...
         qryGetNomeDisciplina.ParamByName('codigoDisciplina').AsInteger :=
            tblEquivDisciplina.AsInteger;

         qryGetNomeDisciplina.Open();
         ...
         DM.setLog(
           ...
           '(' + DM.tblFichaIndividualdisciplina.AsString +')' +
            qryGetNomeDisciplina.FieldByName('descricao').AsString,
            DM.getColigadaByTurma(
               DM.tblFichaIndividualTurma.AsString,
               DM.tblFichaIndividualAnoSemestre.AsInteger
            ),
            'Registrado equivalência sobre esta disciplina'
         );
      end;
      ...
      // Salva a data de inclusão do registro na tabela
...
begin
  ...
  try
    ...
    frmCadEquiv.tblGrade.SQL.Text :=
            ...
            '        GD.CD_DISCIPLINA = :DISCIPLINA  AND                                                        '+
            '        g.cd_curso = :CURSO                                                                        '+
            '    ORDER BY GD.NR_SERIE, d.Ordem                                                                     ';
    ...
    if(tblDiscEquivcd_professor.AsString > '0') then
    begin
      frmCadEquiv.eProfessor.Text := tblDiscEquivcd_professor.AsString;
    end;
