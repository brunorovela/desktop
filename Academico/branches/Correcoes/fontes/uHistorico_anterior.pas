...
type
  Tfrm_historico_anterior = class(TForm)
    ...
    GroupBox1: TGroupBox;
    tblColegiosSeriesnr_mes_inicio: TSmallintField;
    tblColegiosSeriesnr_ano_inicio: TSmallintField;
    tblColegiosSeriesnr_mes_conclusao: TSmallintField;
    pnGrid: TPanel;
    DBGrid3: TDBGrid;
    pnDados: TPanel;
    Label22: TLabel;
    ...
    DBEdit10: TDBEdit;
    DBEdit13: TDBEdit;
    tblColegiosSeriesconclusao: TStringField;
    tblColegiosSeriesinicio: TStringField;
    cbSituacoes: TComboBox;
    tblColegiosGrau1NR_MES_CONCLUSAO: TSmallintField;
    tblColegiosGrau2NR_MES_CONCLUSAO: TSmallintField;
    qryColegiosGrau3NR_MES_CONCLUSAO: TSmallintField;
    lblFundamentalMesConclusao: TLabel;
    dbeFundamentalMesConclusao: TDBEdit;
    lblMedioMesConclusao: TLabel;
    dbeMedioMesConclusao: TDBEdit;
    lblSuperiorMesConclusao: TLabel;
    dbeSuperiorMesConclusao: TDBEdit;
    SpeedButton4: TSpeedButton;
    procedure SpeedButton4Click(Sender: TObject);
    procedure dsColegiosSeriesDataChange(Sender: TObject; Field: TField);
...
procedure Tfrm_historico_anterior.SpeedButton4Click(Sender: TObject);
begin
   qyVestibular.Edit;
   edEscolaVest.Clear;
end;
...
begin
   ...
   qyVestibular.ParamByName('curso').AsString := txtCurso.Text;
   qyVestibular.Open;
...
procedure Tfrm_historico_anterior.DBEdit13Exit(Sender: TObject);
begin
   if(tblColegiosSeriesnr_mes_inicio.AsString <> '') AND
      ((tblColegiosSeriesnr_mes_inicio.AsInteger < 1 ) OR (tblColegiosSeriesnr_mes_inicio.AsInteger > 12)) then
   begin
      Mensagem('Preencha um mês de inicio valido.',  'Atenção', MB_OK + MB_ICONWARNING);
      tblColegiosSeriesnr_mes_inicio.AsString := '';
   end;
end;
...
procedure Tfrm_historico_anterior.DBEdit9Exit(Sender: TObject);
begin
   if(tblColegiosSeriesnr_mes_conclusao.AsString <> '') AND
      ((tblColegiosSeriesnr_mes_conclusao.AsInteger < 1 ) OR (tblColegiosSeriesnr_mes_conclusao.AsInteger > 12)) then
   begin
      Mensagem('Preencha um mês de conclusão valido.',  'Atenção', MB_OK + MB_ICONWARNING);
      tblColegiosSeriesnr_mes_conclusao.AsString := '';
   end;
end;

procedure Tfrm_historico_anterior.DBEdit9KeyPress(Sender: TObject;
  var Key: Char);
begin
  // apenas números
...
begin
 ...
 // Agora pega da grade do curso
 //  qryInsertFicha.ParamByName('cd_matricula_curso').AsInteger := codigoMatriculaCurso;
   try
     qryInsertFicha.ExecSQL;
     //log
     chave := IntToStr(codigoaluno) + ';' + IntToStr(iAno) + ';' + IntToStr(iSerie);
     DM.setLog(30005, 'Inclusao', chave, DM.getColigadaByTurma(tblHistoricoturma.AsString,tblHistoricoanosemestre.AsInteger), 'Inclusão (Historico anterior do estudante > Disciplina anteriores > Incluir todas as disciplinas da grade) com o codigo aluno ' + IntToStr(codigoaluno) + ' e serie ' + IntToStr(iSerie) );
   except on E: Exception do
   end;
...
begin
  ...
  0 : Begin
        ...
        //log
        chave := IntToStr(tblHistoricocodigoaluno.AsInteger) + ';' + tblHistoricoturma.AsString + ';' + tblHistoricocurso.AsString;
        DM.setLog(30005, 'Exclusao', chave, DM.getColigadaByTurma(tblHistoricoturma.AsString,tblHistoricoanosemestre.AsInteger), 'Excluiu Disciplina anteriores codigo aluno ' + IntToStr(tblHistoricocodigoaluno.AsInteger) + ' e turma ' + tblHistoricoturma.AsString);
        tblHistorico.Delete;
      End;
  1 : Begin
        if Mensagem('Tem certeza que deseja apagar a informação do histórico anterior do aluno?',  'Confirmação', MB_YESNO + MB_ICONQUESTION ) <> mrYes
        then Exit;
         ...
         //log
         chave := IntToStr(codigoaluno) + ';' + txtCurso.Text;;
         DM.setLog(30005, 'Exclusao', chave, DM.getColigadaByTurma(tblHistoricoturma.AsString,tblHistoricoanosemestre.AsInteger), 'Exclusao historico anterior na aba colegios ' + ' com o codigo do aluno ' + IntToStr(codigoaluno) );
  ...
  2 : Begin
        ...
        //log
        chave := IntToStr(qyVestibularcodigoaluno.AsInteger) + ';' + qyVestibularcurso.AsString + ';' + IntToStr(qyVestibularanosemestre.AsInteger);
        DM.setLog(30005, 'Exclusao', chave, DM.getColigadaByTurma(tblHistoricoturma.AsString,tblHistoricoanosemestre.AsInteger), 'Exclusao historico anterior na aba exames/selecao com o codigo do aluno ' + IntToStr(qyVestibularcodigoaluno.AsInteger) );
        qyVestibular.Delete;
      End;
...
Var
   serie, disciplina,anosemestre : Integer;
   chave: string;
begin
...
begin
  ...
  Abort;
   ...
   0 :  Begin
          ...
          if tblHistorico.state in [dsInsert] then
          begin
             //log
          chave := IntToStr(codigoaluno) + ';' + tblHistoricoturma.AsString + ';' + tblHistoricocurso.AsString;
          DM.setLog(30005, 'Inclusao', chave, DM.getColigadaByTurma(tblHistoricoturma.AsString,tblHistoricoanosemestre.AsInteger), 'Inseriu Disciplina anteriores codigo aluno ' + IntToStr(codigoaluno) + ' e turma ' + tblHistoricoturma.AsString);
          end else if tblHistorico.state in [dsEdit] then
          begin
           //log
          chave := IntToStr(tblHistoricocodigoaluno.AsInteger) + ';' + tblHistoricoturma.AsString + ';' + tblHistoricocurso.AsString;
          DM.setLog(30005, 'Alteracao', chave, DM.getColigadaByTurma(tblHistoricoturma.AsString,tblHistoricoanosemestre.AsInteger), 'Alteracao Disciplina anteriores codigo aluno ' + IntToStr(codigoaluno) + ' e turma ' + tblHistoricoturma.AsString);  
          end;
        ...
        End;
   1 :  Begin
          if (qryColegiosGrau3.State in [dsInsert]) OR (qryColegiosGrau3.State in [dsInsert]) OR (tblColegiosGrau2.State in [dsInsert]) OR  (tblColegiosGrau1.State in [dsInsert]) OR (tblColegiosSeries.State in [dsInsert]) then
          begin
           //log
          chave := IntToStr(codigoaluno) + ';' + txtCurso.Text;
          DM.setLog(30005, 'Inclusao', chave, DM.getColigadaByTurma(tblHistoricoturma.AsString,tblHistoricoanosemestre.AsInteger), 'Inseriu historico anterior na aba colegios ' + ' com o codigo do aluno ' + IntToStr(codigoaluno) );
          end else if (qryColegiosGrau3.State in [dsEdit]) OR (tblColegiosGrau2.State in [dsEdit]) OR  (tblColegiosGrau1.State in [dsEdit]) OR (tblColegiosSeries.State in [dsEdit]) then
          begin
             //log
          chave := IntToStr(codigoaluno) + ';' + txtCurso.Text;;
          DM.setLog(30005, 'Alteracao', chave, DM.getColigadaByTurma(tblHistoricoturma.AsString,tblHistoricoanosemestre.AsInteger), 'Alteracao historico anterior na aba colegios ' + ' com o codigo do aluno ' + IntToStr(codigoaluno) );
          end;
          ...
          begin
             ...
             begin
               tblColegiosSeriesgrau.AsInteger := StrToInt(TItemCombo(cbSituacoes.Items.Objects[cbSituacoes.ItemIndex]).getCodigo);
             end;
   ...
   2 :  Begin
          ...
          if (trim(edEscolaVest.Text) = '') then
          begin
            qyVestibularcd_instituicao.Clear;
          end;

          if qyVestibular.state in [dsInsert] then
            begin
               //log
               chave := IntToStr(codigoaluno) + ';' + qyVestibularcurso.AsString + ';' + IntToStr(qyVestibularanosemestre.AsInteger);
               DM.setLog(30005, 'Inclusao', chave, DM.getColigadaByTurma(tblHistoricoturma.AsString,tblHistoricoanosemestre.AsInteger), 'Inseriu historico anterior na aba exames/selecao com o codigo do aluno ' + IntToStr(qyVestibularcodigoaluno.AsInteger) );
            end else if qyVestibular.state in [dsEdit] then
            begin
               //log
               chave := IntToStr(codigoaluno) + ';' + qyVestibularcurso.AsString + ';' + IntToStr(qyVestibularanosemestre.AsInteger);
               DM.setLog(30005, 'Alteracao', chave, DM.getColigadaByTurma(tblHistoricoturma.AsString,tblHistoricoanosemestre.AsInteger), 'Alteracao historico anterior na aba exames/selecao com o codigo do aluno ' + IntToStr(qyVestibularcodigoaluno.AsInteger) );
            end;
   ...
   3 :  Begin
          ...
          //log
          chave := IntToStr(codigoaluno) + ';' + tblObsHistoricocurso.AsString;
          DM.setLog(30005, 'Alteracao', chave, DM.getColigadaByTurma(tblHistoricoturma.AsString,tblHistoricoanosemestre.AsInteger), 'Log de registrando alteracao ou inclusao no registro historico anterior na aba observacao_para_historico com o codigo do aluno '+ IntToStr(qyVestibularcodigoaluno.AsInteger) );
...
procedure Tfrm_historico_anterior.CarregaComboGrau;
const
...
procedure Tfrm_historico_anterior.cbSituacoesChange(Sender: TObject);
begin
   if not (tblColegiosSeries.State in [dsInsert, dsEdit]) then
   begin
      tblColegiosSeries.Edit();
...
procedure Tfrm_historico_anterior.dsColegiosSeriesDataChange(Sender: TObject;
  Field: TField);
var
   i:integer;
   entrou:Boolean;
begin
   if tblColegiosSeries.State in [dsbrowse] then
   begin
      entrou := false;
      ...
      begin
         ...
         if(TItemCombo(cbSituacoes.Items.Objects[cbSituacoes.ItemIndex]).getCodigo = tblColegiosSeriesgrau.AsString) and not(entrou) then
         begin
            entrou := true;
         end;
      end;
...
begin
   if ( qyVestibularcd_instituicao.AsInteger > 0) then
   begin
