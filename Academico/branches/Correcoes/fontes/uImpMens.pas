...
uses
  ...
  UMAjuda, UMDateTimePicker, Spin, Mask, UDateTimePicker, ShellAPi,
...
type
  ...
  TfrmImpMens = class(TForm)
    ...
    chkVencto: TCheckBox;
    dtpVenctoDe: TDatePickerEx;
    lblVenctoAte: TLabel;
    dtpVenctoAte: TDatePickerEx;
    bvlSep3: TBevel;
    ckbDataBoleto: TCheckBox;
    txtDtBoleto: TDatePickerEx;
    UMAjuda1: TUMAjuda;
    pnlExtrato: TPanel;
    bvlSep4: TBevel;
    lblExtrato: TLabel;
    ...
    btnImprimir: TButton;
    btnFechar: TButton;
    qryBoletoRegistroOnline: TUMZQuery;
    ckbRemessaOnline: TCheckBox;
  ...
  private
    ...
    function VerificaDDA(const NossoNumero: AnsiString): Boolean;
    function gerar_random() : String;
   function HasRegistroOnline() : Boolean;
  public
    { Public declarations }
    ...
    Procedure VerificaImprimirOnline();
...
uses
   uRepBloquetos, uRepRecibo, Main, uDM, Math, uUsuario, uMensalidades,
   IniFiles, ClassComprasEntradaSaida, ClassRegistros, uImpressaoDOS, uCobranca,
   UFMensalidadesDDA, uRemessaClass, IdCoderMIME;
...
begin
   ...
   qyContas.SQL.Clear;
   qyContas.SQL.Add(' SELECT cc.cd_caixa, cc.ds_caixa, cc.tp_conta, cc.cd_coligada  ');
   qyContas.SQL.Add(' FROM fin_cadastro_contas as cc            ');
   qyContas.SQL.Add(' INNER JOIN coligadas as c ON (c.cd_coligada = cc.cd_coligada) ');
   qyContas.SQL.Add(' WHERE cc.sn_ativa = ''S''               ');
   qyContas.SQL.Add(Format('   AND (cc.cd_coligada IN (%s) OR cc.sn_todas_coligadas = 1 ) ', [DM.UsuarioLogado.GetColigadasFilhaSelecionada()]));
   qyContas.SQL.Add('   AND cc.nm_arquivo_bloqueto is NOT NULL AND cc.nm_arquivo_bloqueto <> ''''   ');
   qyContas.SQL.Add('   AND c.cd_coligada_matriz = '+ IntToStr(DM.GetUsuarioLogado.GetColigadaLogada.Codigo) );
...
begin
   ...
   while not SQLParametro.EOF do begin
   ...
   // Fim da repetiração da impressão de recibos
...
const
   ...
   SFilterVencto = ''
      ...
      + '	m.anosemestre = %d AND '
      + '	m.situacao IN (2, 10) AND '
      + '	(m.datavencimento BETWEEN ''%s'' AND ''%s'') '
      + ') ';
   ...
   SFilterParcVencto = ''
      ...
      + '	m.anosemestre = %d AND '
      + '	m.situacao IN (2, 10) AND '
      + '	(m.parcela BETWEEN %d AND %d) AND '
      + '	(m.datavencimento BETWEEN ''%s'' AND ''%s'') '
      + ') ';

   SFilterSQLParcela = ''
      + '(mensalidades.codigoaluno = :codigo) AND '
      + '( '
      + '	mensalidades.turma = :turma AND '
      + '	mensalidades.situacao IN (2, 10) AND '
      + '	(mensalidades.parcela BETWEEN %d AND %d) '
      + '   %s'
      + ') ';
   ...
   SFilterSQLVencto = ''
      ...
      + '	mensalidades.turma = :turma AND '
      + '	mensalidades.situacao IN (2, 10) AND '
      + '	(mensalidades.datavencimento BETWEEN ''%s'' AND ''%s'') '
      + '   %s'
      + ') ';

   SFilterSQLParcVencto = ''
      + '(mensalidades.codigoaluno = :codigo) AND '
      + '( '
      + '	mensalidades.turma = :turma AND '
      + '	mensalidades.situacao IN (2, 10) AND '
      + '	(mensalidades.parcela BETWEEN %d AND %d) AND '
      + '	(mensalidades.datavencimento BETWEEN ''%s'' AND ''%s'') '
      + '   %s'
      + ') ';

   SFilterSQLAnoSem = 'AND mensalidades.anosemestre = :anosemestre';
   ...
   SFilterCRParcela = ''
      ...
      + '{mensalidades.situacao} in [2, 10] and '
      + '{mensalidades.parcela} >= %d and '
      + '{mensalidades.parcela} <= %d %s';
   ...
   SFilterCRVencto = ''
      ...
      + '{mensalidades.turma} = %s and '
      + '{mensalidades.situacao} in [2, 10] and '
      + '{mensalidades.datavencimento} >= %s and '
      + '{mensalidades.datavencimento} <= %s %s';
   ...
   SFilterCRAnoSem = 'and {mensalidades.anosemestre} = %d';

   SDateCR = 'Date(%d, %d, %d)';
Var
   NomeRel, selecao : string;
   ano, mes, dia : word;
   sFiltroAux, S, S1, S2, S3: String;
   bAgrupado  : Boolean;
...
begin
  ...
  if (radExtratoTotal.Checked) then begin
      ...
      PrincipalForm.ImprimeRpt(
         ...
         sNomeRelatorio,
         -1,
         '{Extrato_de_Mensalidades.CodigoAluno} = ' + Format('%d', [FUMRecMensalidade.CodigoAluno]), '',slFormulas,nil,nil, nil, true);
      Exit;
  end;
  ...
  begin
     ...
     if ckbDataBoleto.Checked then begin
        dtBoleto := txtDtBoleto.SelectedDate;
     end else begin
        dtBoleto := 0;
     end;
      ...
      begin
           ...
           qryAjuste.ParamByName('cd_mensalidade').AsInteger := FUMRecMensalidade.Cd_Mensalidade;
           qryAjuste.Open;

           bCancelarDDA := false;
           iCancelamento := 0;
           while not qryAjuste.Eof do
           begin
            if qryAjuste.FieldByName('cd_origem').AsInteger = 3 then
              bCancelarDDA := true;
            if qryAjuste.FieldByName('cd_acao').AsInteger = 101 then
     ...
     if radSelecionado.Checked then begin
        ...
        if not VerificaDDA(FUMrecMensalidade.NossoNumero) then
        begin
           Exit;
        end;
     ...
     begin
         if chkParcelas.Checked and chkVencto.Checked then
         begin
            S1 := FormatDateTime(SDateToISOFmt, dtpVenctoDe.SelectedDate);
            S2 := FormatDateTime(SDateToISOFmt, dtpVenctoAte.SelectedDate);

            sFiltroAux := Format(SFilterParcVencto, [
               FUMRecMensalidade.CodigoAluno,
               QuotedStr(FUMRecMensalidade.CodTurma),
               FUMRecMensalidade.AnoSemestre,
               speParcelaDe.Value,
               speParcelaAte.Value,
               S1,
               S2]);
         ...
         if chkParcelas.Checked and (not chkVencto.Checked) then
         begin
            sFiltroAux := Format(SFilterParcela, [
               FUMRecMensalidade.CodigoAluno,
               QuotedStr(FUMRecMensalidade.CodTurma),
               FUMRecMensalidade.AnoSemestre,
               speParcelaDe.Value,
         ...
         if (not chkParcelas.Checked) and chkVencto.Checked then
         begin
            S1 := FormatDateTime(SDateToISOFmt, dtpVenctoDe.SelectedDate);
            S2 := FormatDateTime(SDateToISOFmt, dtpVenctoAte.SelectedDate);

            sFiltroAux := Format(SFilterVencto, [
               FUMRecMensalidade.CodigoAluno,
               QuotedStr(FUMRecMensalidade.CodTurma),
               FUMRecMensalidade.AnoSemestre,
               S1,
         ...
         if DM.variavel_parametro('sn_processo_remessa_novo') = '1' then
         begin
            TfrmMensalidadesDDA.CancelaDDA(
               speParcelaDe.Value,
               speParcelaAte.Value,
               FUMRecMensalidade.CodigoAluno,
               FUMRecMensalidade.CodTurma);
         end;
     end;
     ...
     if radSelecionado.Checked then begin
        { Mesmo em bloqueto agrupado, repassar para o relatório o cd_mensalidade }
         if chkAgrupar.Checked then begin
            // Buscar o nosso número do boleto, e passar apenas esse nosso número.
            // Isso resolve a questão do cliente ter vários boletos no mesmo periodo, com tipos de titulos diferentes que não se agrupam:
            // Programação Antiga

            // Programação Nova
            DM.CriarConsulta(qryAjuste);
            qryAjuste.SQL.Text := 'SELECT nossonumero FROM mensalidades as m WHERE ' + sFiltroAux;
            qryAjuste.Open();
            strNNAux := qryAjuste.FieldByName('nossonumero').AsString;
            if (strNNAux <> '') then begin
               selecao := '{mensalidades.nossonumero} = "' + strNNAux + '"' ;
            end else begin
               selecao := '{mensalidades.codigoaluno} = ' + Format('%d', [FUMRecMensalidade.CodigoAluno]);
               selecao := selecao + ' AND ToText({mensalidades.datavencimento}, "yyyyMMdd") = "' + FormatDateTime( 'yyyymmdd', FUMRecMensalidade.DtVencto ) +'" ';
            end;
            selecao := selecao + ' and {fin_cadastro_contas.sn_ativa} = "S"';
        ...
        begin
         ...
         S3 := Format(SFilterCRAnoSem, [FUMRecMensalidade.AnoSemestre]);

         if chkParcelas.Checked and chkVencto.Checked then
         begin
            DecodeDate(dtpVenctoDe.SelectedDate, Year, Month, Day);
            S1 := Format(SDateCR, [Year, Month, Day]);
            
            DecodeDate(dtpVenctoAte.SelectedDate, Year, Month, Day);
            S2 := Format(SDateCR, [Year, Month, Day]);

            selecao := Format(SFilterCRParcVencto, [
               FUMRecMensalidade.CodigoAluno,
               QuotedStr(FUMRecMensalidade.CodTurma),
               speParcelaDe.Value,
               speParcelaAte.Value,
         ...
         if chkParcelas.Checked and (not chkVencto.Checked) then
         begin
            selecao := Format(SFilterCRParcela, [
               FUMRecMensalidade.CodigoAluno,
               QuotedStr(FUMRecMensalidade.CodTurma),
               speParcelaDe.Value,
               speParcelaAte.Value,
         ...
         begin
            ...
            selecao := Format(SFilterCRVencto, [
               FUMRecMensalidade.CodigoAluno,
               QuotedStr(FUMRecMensalidade.CodTurma),
               S1,
               S2,
     ...
     if radSelecionado.Checked AND HasRegistroOnline() AND ckbRemessaOnline.Checked then
     begin
       VerificaImprimirOnline();
       Exit;
     end;
     ...
     if PrincipalForm.ImprimeRpt(Self, NomeRel, -1, selecao, '',nil,nil,nil,nil, true, False, nil, True) then begin
        ...
        if radSelecionado.Checked then begin
           if strNNAux <> '' then begin
              Self.qyUpdMens.SQL.Add( '(mensalidades.nossonumero = :nossonumero)');
              Self.qyUpdMens.ParamByName('nossonumero').AsString := strNNAux;
           end else begin
            ...
            if frmMensalidades.cbTurma.ItemIndex > 0 then
            begin
               S3 := SFilterSQLAnoSem;
            end;
            ...
            if chkParcelas.Checked and chkVencto.Checked then
            begin
               S := Format(SFilterSQLParcVencto, [speParcelaDe.Value, speParcelaAte.Value, S1, S2, S3]);
               qyUpdMens.SQL.Add(S);
               qyUpdMens.ParamByName('Codigo').AsInteger := FUMRecMensalidade.CodigoAluno;
               qyUpdMens.ParamByName('Turma').AsString := FUMRecMensalidade.CodTurma;
            end;

            if chkParcelas.Checked and (not chkVencto.Checked) then
            begin
               S := Format(SFilterSQLParcela, [speParcelaDe.Value, speParcelaAte.Value, S3]);
               qyUpdMens.SQL.Add(S);
               qyUpdMens.ParamByName('Codigo').AsInteger := FUMRecMensalidade.CodigoAluno;
               qyUpdMens.ParamByName('Turma').AsString := FUMRecMensalidade.CodTurma;
            end;
            ...
            if (not chkParcelas.Checked) and chkVencto.Checked then
            begin
               S := Format(SFilterSQLVencto, [S1, S2, S3]);
               qyUpdMens.SQL.Add(S);
               qyUpdMens.ParamByName('Codigo').AsInteger := FUMRecMensalidade.CodigoAluno;
               qyUpdMens.ParamByName('Turma').AsString := FUMRecMensalidade.CodTurma;
            end;
            ...
            if S3 <> '' then
            begin
               qyUpdMens.ParamByName('anosemestre').AsInteger := FUMRecMensalidade.AnoSemestre;
            end;
        end;
...
const
   SMsgErroValidacao = ''
      + 'Para efetuar a impressão utilizando a opção "Intervalo de parcelas", '
      + 'é necessário selecionar a opção parcela, vencimento ou ambas.';
   SCaptionWarn = 'Atenção';
begin

   if radSelecionado.Checked AND HasRegistroOnline() AND ckbRemessaOnline.Checked AND NOT AlterouConta then
   begin
      VerificaImprimirOnline();
      Exit;
   end;

   if radParcela.Checked and
      (not chkParcelas.Checked) and
      (not chkVencto.Checked) then
   begin
      MessageBox(Handle, SMsgErroValidacao, SCaptionWarn,
...
procedure TfrmImpMens.cbContasChange(Sender: TObject);
begin

   AlterouConta := true;

   if radSelecionado.Checked AND HasRegistroOnline() then
   begin
      ckbRemessaOnline.Visible := HasRegistroOnline();
   end else begin
      ckbRemessaOnline.Visible := False;
   end;
...
function TfrmImpMens.HasRegistroOnline : Boolean;
begin
   qryBoletoRegistroOnline.Close;
   qryBoletoRegistroOnline.ParamByName('cd_caixa').AsInteger
      := StrToInt(ListaContas[cbContas.ItemIndex]);
   qryBoletoRegistroOnline.Open;

   Result := False;

   if ( qryBoletoRegistroOnline.RecordCount > 0 ) then
   begin
      Result := True;
   end;
end;
...
begin
   ...
   dtpVenctoDe.SelectedDate := Date;
   dtpVenctoAte.SelectedDate := Date;

   ckbRemessaOnline.Visible := false;

   if cbContas.ItemIndex > -1  then
   begin
     // Verifica se a mensalidade selecionada possui registro online
     ckbRemessaOnline.Visible := HasRegistroOnline();
   end;
...
procedure TfrmImpMens.chkParcelasClick(Sender: TObject);
begin
   UpdateControlsStatus;
end;

procedure TfrmImpMens.chkVenctoClick(Sender: TObject);
begin
   UpdateControlsStatus;
end;
...
procedure TfrmImpMens.UpdateControlsStatus;
begin
   chkParcelas.Enabled := radParcela.Checked;
   speParcelaDe.Enabled := radParcela.Checked and chkParcelas.Checked;
   speParcelaAte.Enabled := radParcela.Checked and chkParcelas.Checked;
   chkVencto.Enabled := radParcela.Checked;
   dtpVenctoDe.Enabled := radParcela.Checked and chkVencto.Checked;
   dtpVenctoAte.Enabled := radParcela.Checked and chkVencto.Checked;
   Label1.Enabled := radParcela.Checked and chkParcelas.Checked;
   lblVenctoAte.Enabled := radParcela.Checked and chkVencto.Checked;
end;
...
begin
   ...
   if (txtDtBoleto.SelectedDate < apenasData) then
   begin
      txtDtBoleto.SelectedDate := apenasData;
   end;

   if ((txtDtBoleto.SelectedDate -apenasData ) > nrDiasMaxData) then
   begin
      novaData:= apenasData + nrDiasMaxData;
      txtDtBoleto.SelectedDate := novaData;
...
function TfrmImpMens.VerificaDDA(const NossoNumero: AnsiString): Boolean;
const
   TextQ = 'O título selecionado possui um pedido de inclusão no Débito Automático. Para efetuar a impressão do boleto, é necessário cancelar a sua inclusão no Débito Automático. Deseja efetuar o cancelamento do Débito Automático e efetuar a impressão do boleto?';
   CaptionQ = 'Atenção';
   TextI = 'O pedido de cancelamento do Débito Automático foi registrado. O boleto será impresso.';
   CaptionI = 'Informação';
var
   Remessa: TRemessa;
   UserChoice: Integer;
begin
   ...
   begin
      ...
      try
         if Remessa.verificaAcaoEstaFila(NossoNumero, tarBaixarDDA) then
         begin

            Result := False;

            UserChoice := MessageBox(0, TextQ, CaptionQ,
               MB_ICONWARNING + MB_YESNO + MB_DEFBUTTON2);

            if UserChoice = ID_YES then
            begin
               Result := True;
               Remessa.CancelaDDA(NossoNumero);

               MessageBox(0, TextI, CaptionI, MB_ICONINFORMATION + MB_OK);
            end;
...
function TfrmImpMens.gerar_random: String;
var
...
procedure TfrmImpMens.VerificaImprimirOnline;
CONST
  SQL_BUSCA_MENSALIDADE =
    'SELECT me.ds_autentica_impressao '
    + ' FROM mensalidades me WHERE me.cd_mensalidade = ?';
var
   ds_link, ds_link_portal, dtvencimento_base64 : String;
   Stmt: IZPreparedStatement;
   Rs: IZResultSet;
   ds_autentica_impressao, ds_chave : String;
   Encoder  : TIdEncoderMime;
begin
   ds_link_portal := trim(DM.variavel_parametro('online_endereco_local'));

   ds_autentica_impressao := '';
   dtvencimento_base64 := '';

   if ( Length(ds_link_portal) < 1 ) then
   begin
      Mensagem(
         'Link do portal online não informado!',
         'Aviso',
   ...
   Stmt := PrepareStatement(SQL_BUSCA_MENSALIDADE);

   try
      Stmt.SetInt(1, FUMRecMensalidade.CodigoMensalidade );
      Rs := Stmt.ExecuteQueryPrepared;
      while Rs.Next do
      begin
         ds_autentica_impressao := Rs.GetString(1);

           ds_link :=
            ds_link_portal
            + 'projetos/unimestre/financeiro/boleto.php?ds_autentica_impressao='
            + ds_autentica_impressao ;

            // Manda a nova data vencimento em base64 para impressão do boleto
            if ( ckbDataBoleto.checked ) then
            begin
               // Codifica base64
               Encoder := TIdEncoderMime.Create(nil);
               dtvencimento_base64 :=
                  Encoder.Encode(FormatDateTime('yyyy-mm-dd', txtDtBoleto.SelectedDate));

               // Adiciona ao link a dt_vencimento em base64
               ds_link := ds_link +
                '&dtvencimento='
                + dtvencimento_base64;
            end;
   ...
   // Chama o boleto do portal online
   ShellExecute(
      Application.Handle,
      'open',
      PChar(ds_link),
      nil,
      PChar('--user-data-dir=' + ds_chave),
      SW_SHOWNORMAL
   );
