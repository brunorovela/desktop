...
const
   ...
   SQL_BUSCA_RATEIO_LANCAMENTO = ' SELECT '+
      ...
      ' )'+
      ' INNER JOIN coligadas as c ON (c.cd_coligada = fc.cd_coligada) ' +
      ' INNER JOIN fin_config_centro_custos fccc ON ( '+
      '  fcc.cd_centro = fccc.cd_centro AND '+
      '  c.cd_coligada_matriz = fccc.cd_coligada_matriz '+
      ' ) '+
      ' WHERE '+
...
begin
   ...
   { Retirado por Claudionor - entendido que podemos ter vários debitos ou créditos iguais
   if (edCodigoCredito.Text <> '') and
...
var
   i, x, k, j, p, indexAtual, codigoLancamento: Integer;
   NovoValor: Double;
   ...
   listaExecucaoRateiosAgrupado: TList;
   iPosicaoEncontrou : Integer;
   bEncontrou : Boolean;
...
begin
   ...
   begin
      ...
      begin
         ...
         ExecutaLancamento.Coligada := TColigada(cbColigadasLanc.Items.Objects[cbColigadasLanc.ItemIndex]).Codigo; // Errado DM.GetUsuarioLogado.GetColigadaLogada.Codigo;
         ...
         begin
            ...
            listaExecucaoRateiosAgrupado := TList.Create();

            bEncontrou := false;
            for p := 0 to listaExecucaoRateios.Count - 1 do begin

                for x := 0 to listaExecucaoRateiosAgrupado.Count - 1 do begin
                    IF (TExecucaoRateioCP(listaExecucaoRateios.Items[p]).Conta = TExecucaoRateioCP(listaExecucaoRateiosAgrupado.Items[x]).Conta) AND
                       (TExecucaoRateioCP(listaExecucaoRateios.Items[p]).Centro = TExecucaoRateioCP(listaExecucaoRateiosAgrupado.Items[x]).Centro)
                    then begin
                         iPosicaoEncontrou := x;
                         bEncontrou := true;
                    end;
                ...
                if bEncontrou then  begin

                    NovoValor := TExecucaoRateioCP(listaExecucaoRateiosAgrupado.Items[iPosicaoEncontrou]).Valor;
                    NovoValor := NovoValor + TExecucaoRateioCP(listaExecucaoRateios.Items[p]).Valor;

                    TExecucaoRateioCP(listaExecucaoRateiosAgrupado.Items[iPosicaoEncontrou]).Valor := NovoValor;

                                 
                end else begin

                    listaExecucaoRateiosAgrupado.Add( listaExecucaoRateios[p] ) ;

                end;


            end;
            listaExecucaoRateios := listaExecucaoRateiosAgrupado ;
...
begin
  ...
  { Comentado por Claudionor -- Erro de arredondamento percentual. O que vale é fechar o valor.
