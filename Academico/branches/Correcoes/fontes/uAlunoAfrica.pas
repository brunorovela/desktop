...
type
  TfAlunosAfrica = class(TfPessoasAfrica)
    ...
    Label227: TLabel;
    Label62: TLabel;
    dbCheckInMemorian: TDBCheckBox;
    ds_forma_academica: TDBEdit;
    ds_profissao: TDBEdit;
    tblPessoasn_obito: TIntegerField;
...
procedure TfAlunosAfrica.btDesvincularClick(Sender: TObject);
var
   ds_campo : String;
begin
  inherited;
...
begin
  ...
  End
     ...
     btAcad_Mae.Visible := True;
     btDesvincular.Visible := True;
     btDesvincular.Hint := 'Desvincular Financeiro.';
  end
...
procedure TfAlunosAfrica.cbFormaConheceChange(Sender: TObject);
begin
  inherited;
...
begin
  ...
  if not(DM.CamposObrigatoriosPreenchidos(self, pgPessoa.ActivePage.Name)) then
   ...
   if (tblPessoacd_resp_finan.AsInteger = 0 ) and
      not(bMenorIdade) then
   begin
     tblPessoa.Edit;
     tblPessoacd_resp_finan.AsInteger := tblPessoacd_pessoa.asInteger ;
     tblPessoa.Post;
...
procedure TfAlunosAfrica.Button1Click(Sender: TObject);
begin
   inherited;
   tblPessoa.Refresh;
end;
