...
uses
  ...
  UZDataset, UMComboBox, StrUtils, UMEditNumerico, Menus, jpeg, ExtDlgs;
...
type
  TfrmComprasCadastroProdutos = class(TForm)
    ...
    qyProdutosSN_LIMIT_POR_ANO_SEM: TSmallintField;
    qyProdutosNR_PARCELAS: TIntegerField;
    qyCategoriassn_cantina: TSmallintField;
    qyProdutosds_adicionais: TMemoField;
    qyCodBarrasn_cantina: TSmallintField;
    pmFoto: TPopupMenu;
    mIAlterar: TMenuItem;
    mIExcluir: TMenuItem;
    MenuItem3: TMenuItem;
    ...
    qryImagem: TUMZQuery;
    qryImagemim_produto: TBlobField;
    qryImagemcd_produto: TStringField;
    AbrirFoto: TSavePictureDialog;
    qryUpdateImagem: TUMZQuery;
    qyProdutosSN_PRODUTO_INCLUSO: TSmallintField;
    qyProdutosSN_CANTINA_ESTOQUE: TSmallintField;
    Label13: TLabel;
    pnCadastro: TPanel;
    Label1: TLabel;
    Label2: TLabel;
    ...
    dbNrParcelas: TDBEdit;
    dbAdicionais: TDBMemo;
    Panel3: TPanel;
    imageFoto: TImage;
    ckbProdutoIncluso: TDBCheckBox;
    DBCheckBox2: TDBCheckBox;
    qyProdutosNR_ESTOQUE_MIN: TLargeintField;
    edEstoqueMinimo: TDBEdit;
    Label14: TLabel;
    qyProdutosSN_BAIXAR_TITULO_AUTO: TSmallintField;
    dbchBaixarTitulo: TDBCheckBox;
    qyCategoriassn_venda_produto: TSmallintField;
    DBCheckBox3: TDBCheckBox;
    qyProdutosSN_ESTOQUE_POR_COLIGADA: TSmallintField;
    procedure ckbProdutoInclusoClick(Sender: TObject);
    procedure qyProdutosAfterPost(DataSet: TDataSet);
    procedure qyProdutosBeforeCancel(DataSet: TDataSet);
    ...
    procedure dbNrParcelasKeyPress(Sender: TObject; var Key: Char);
...
begin
   ...
   begin
      ...
      qyCategorias.SQL.Add( Format( 'WHERE ccc.cd_coligada IN ( %s )', [ DM.UsuarioLogado.GetColigadasFilhaSelecionada() ] ) );
   end;
...
begin
   ...
   qyCodBarra.ParamByName('cantina').AsString := qyCategoriassn_cantina.AsString;
   qyCodBarra.Open;
end;
...
const
   ...
   SQL_PRODUTOS_GROUP = ' GROUP BY cp.CD_PRODUTO ';

var
   strRetorno, strOriginal, strEncontrar : pchar;
begin
   qyProdutos.Close();
   ...
   begin
      qyProdutos.SQL.Add('AND ccc.CD_COLIGADA = :CD_COLIGADA');
      ...
      qyProdutos.SQL.Add(Format('AND ccc.CD_COLIGADA IN ( %s )', [DM.GetColigadas()]));
   end;   
   ...
   strOriginal := Pchar(fieldOrdem);
   strEncontrar := pchar('DESC');
   strRetorno := strpos(strOriginal, strEncontrar );

   qyProdutos.SQL.Add(SQL_PRODUTOS_GROUP);
   ...
   begin
      if (strRetorno <> nil) then begin
         qyProdutos.SQL.Add( SQL_PRODUTOS_ORDER + 'cp.' + fieldOrdem );
      end
      else begin
          qyProdutos.SQL.Add( SQL_PRODUTOS_ORDER + ' CAST( ' + fieldOrdem + ' as SIGNED) ASC' );
      end;
   end;
...
begin
   ...
   if(qyCategoriassn_venda_produto.AsInteger = 0) then
   begin
      qyProdutosNR_PARCELAS.AsInteger := 1;
   end;

   if ( qyProdutos.State IN [dsInsert,dsEdit] ) AND ( qyProdutosNR_ESTOQUE_MIN.AsInteger < 0 ) then
   begin
      Mensagem( 'O Estoque mínimo não pode ser menor que zero.', Application.Title, MB_OK + MB_ICONWARNING, Handle );
   ...
   if (qyProdutosNR_PARCELAS.AsInteger < 1 ) then
   begin
      Mensagem( 'O número de parcelas deve ser no minimo 1.', Application.Title, MB_OK + MB_ICONWARNING, Handle );
      dbNrParcelas.SetFocus;
      Exit;
   end;
   ...
   begin
      ...
      begin
         qyCodBarrasn_cantina.AsInteger := qyCategoriassn_cantina.AsInteger;
...
procedure TfrmComprasCadastroProdutos.ckbProdutoInclusoClick(Sender: TObject);
begin
   if (ckbProdutoIncluso.Checked) then
   begin
      edValor.Text := '0,00';
   end;
end;

procedure TfrmComprasCadastroProdutos.dbNrParcelasKeyPress(Sender: TObject;
  var Key: Char);
begin
  // apenas números
...
begin
   ...
   dbchBaixarTitulo.Enabled := ckbGerarTitulo.Enabled;
   ...
   dbNrParcelas.Enabled := (qyCategoriassn_venda_produto.AsInteger = 1);
   edLimite.Enabled := (qyCategoriassn_venda_produto.AsInteger = 1);
   DBCheckBox1.Enabled := (qyCategoriassn_venda_produto.AsInteger = 1);
   ckbGerarTitulo.Enabled :=  ckbGerarTitulo.Enabled and (qyCategoriassn_venda_produto.AsInteger = 1);
   dbchBaixarTitulo.Enabled := ckbGerarTitulo.Enabled;
   ckbAcumularValores.Enabled := ckbAcumularValores.Enabled and (qyCategoriassn_venda_produto.AsInteger = 1);
   ckbImprimirCupom.Enabled := (qyCategoriassn_venda_produto.AsInteger = 1);
   ckbProdutoIncluso.Enabled := (qyCategoriassn_cantina.AsInteger = 1);
   DBCheckBox2.Enabled := (qyCategoriassn_cantina.AsInteger = 1);

   if not (qryImagem.State in [dsInsert,dsEdit]) then
   begin
      qryImagem.Close;
      qryImagem.ParamByName('cd_produto').AsString := qyProdutoscd_produto.AsString;
      qryImagem.Open;
   end;
   { Se houver alguma informação gravada no campo foto da tabela }
   imageFoto.visible := false;
   Panel3.Caption := 'FOTO';
   if qryImagemim_produto.AsString <> '' Then
   begin

     imageFoto.visible := True;
     Panel3.Caption := '';
     try
         { Cria o componente BLOBStream}
         BS := qryImagem.CreateBlobStream(qryImagemim_produto, bmRead);
...
begin
   ...
   begin
      ...
      observacao :=  observacao +
                    ...
                    ' , Controlar limite de retirada por ano/sem: ' + IntToStr(qyProdutosSN_LIMIT_POR_ANO_SEM.AsInteger) +
   ...
   begin
      ...
      if (qyProdutosSN_LIMIT_POR_ANO_SEM.AsFloat <> qyProdutosSN_LIMIT_POR_ANO_SEM.OldValue) then
      begin
         observacao := observacao + ' Controlar limite de retirada por ano/sem, de ' + FloatToStr(qyProdutosSN_LIMIT_POR_ANO_SEM.OldValue) + ' para ' +  FloatToStr(qyProdutosSN_LIMIT_POR_ANO_SEM.AsFloat) + ' ; ';
      end;
...
procedure TfrmComprasCadastroProdutos.imageFotoDblClick(Sender: TObject);
var
  Imagem:TJPEGImage;
  BS: TStream;
begin
   ...
   if AbrirFoto.FileName <> '' Then Begin
      ...
      Imagem := TJPEGImage.Create;
      BS := qryImagem.CreateBlobStream(qryImagemim_produto, bmWrite);
      ...
      FreeAndNil(Imagem);
      TBlobField(qryImagemim_produto).LoadFromStream(BS);
      AbrirFoto.FileName:='';
      exit;
   end;
end;
...
const
   SQL_COD_BARRA_EXISTE =
      ' SELECT COUNT(*) AS registros FROM COMP_CODIGO_BARRAS WHERE CD_CODIGO_BARRAS = :cd_cod_barra and (cd_produto <> :produto or cd_produto is null) AND sn_cantina = :cantina ';
var
   qyIsCodigoBarraExiste: TUMZReadOnlyQuery;
   vCodigoBarraAntigo : String;
begin
   DM.CriarConsulta(qyIsCodigoBarraExiste);

   try
      vCodigoBarraAntigo:= qyProdutoscd_produto.OldValue;
   Except
      vCodigoBarraAntigo := '-9999999999999999999999999999999999999999999999999';
   end;
   ...
   qyIsCodigoBarraExiste.ParamByName('produto').AsString := vCodigoBarraAntigo;
   qyIsCodigoBarraExiste.ParamByName('cantina').AsInteger := qyCategoriassn_cantina.AsInteger;
...
procedure TfrmComprasCadastroProdutos.mICancelarClick(Sender: TObject);
begin
   //cancela
end;

procedure TfrmComprasCadastroProdutos.mIExcluirClick(Sender: TObject);
begin
    { Coloca a tabela em modo de edição }
   qryImagem.Edit;
   qryImagemim_produto.AsString := '';
   imageFoto.Visible := False;
   Panel3.Caption := 'FOTO';
   qryImagem.Post();
end;
...
procedure TfrmComprasCadastroProdutos.qyProdutosAfterPost(DataSet: TDataSet);
begin
   if(qryImagem.State in [dsInsert,dsEdit]) then
   begin
      qryUpdateImagem.Close;
      qryUpdateImagem.ParamByName('img').AsString := qryImagemim_produto.AsString;
      qryUpdateImagem.ParamByName('produto').AsString := qyProdutoscd_produto.AsString;
      qryUpdateImagem.ExecSQL;

      qryImagem.Cancel;
   end;
end;

procedure TfrmComprasCadastroProdutos.qyProdutosBeforeCancel(DataSet: TDataSet);
begin
   qryImagem.Cancel;
   AbrirFoto.FileName:='';
end;
...
procedure TfrmComprasCadastroProdutos.ResizeFoto(var oJPG: TJPEGImage; iMaxWidth, iMaxHeigth: Integer);
var
  oBmp: TBitmap;
  iW, iH: Integer;
  lP: Double;
