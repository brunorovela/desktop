...
uses
  ...
  ImgList, Main, DateUtils, General, ClassRegistros, UMDateTimePicker;
...
begin
   ...
   begin
      ...
      frmFacturasGerar.umdtInicio.Date   := Self.umdtInicio.Date;
      frmFacturasGerar.umdtFim.Date    := Self.umdtFim.Date;
   end;
...
begin
   ...
   begin
      filtro   := filtro + ' AND ( ToText( {fin_facturas.dt_emissao}, "yyyyMMdd") >= ' + QuotedStr(FormatDateTime('yyyyMMdd', umdtInicio.Date));
      filtro   := filtro + ' AND ToText( {fin_facturas.dt_emissao}, "yyyyMMdd") <= ' + QuotedStr(FormatDateTime('yyyyMMdd', umdtFim.Date)) + ' )';
   end
   else
   begin
      if umdtInicio.Date > umdtFim.Date then
...
begin
   ...
   qyFacturas.ParamByName('dt_inicio').AsString    := FormatDateTime('yyyy-mm-dd', umdtInicio.Date)+' 00:00:00';
   qyFacturas.ParamByName('dt_fim').AsString       := FormatDateTime('yyyy-mm-dd', umdtFim.Date)+' 23:59:59';
