...
type
  ...
  TFDigDocumentos = class(TForm)
    ...
    btnConferirDoc: TToolButton;
    qyConfereDocumento: TUMZQuery;
    btnRejeitarDoc: TToolButton;
    qyVerificaStatusDoc: TUMZQuery;
    qyVerificaStatusDoccd_conferencia: TIntegerField;
    procedure btnRejeitarDocClick(Sender: TObject);
    procedure btnConferirDocClick(Sender: TObject);
  ...
  private
    ...
    procedure AtualizarStatusDoc(cd_conferencia:Integer);
...
begin
   ...
   AtualizarStatusDoc(1);

end;

procedure TFDigDocumentos.btnConferirDocClick(Sender: TObject);
begin
  AtualizarStatusDoc(1);
  Mensagem('Documento Conferido.', '',  MB_OK);
end;

procedure TFDigDocumentos.AtualizarStatusDoc(cd_conferencia:Integer);
begin

  if Self.pcDocumentos.ActivePage = tsDocsAlunos then
   qyConfereDocumento.Params.ParamByName('cod_documento').Value := qyDocsAluno.FieldByName('codigo').AsInteger
  else
   qyConfereDocumento.Params.ParamByName('cod_documento').Value := qyDocumentoscodigo.AsInteger;

   qyDocumentos.FieldByName('codigo').AsInteger;
  qyConfereDocumento.Params.ParamByName('cd_conferencia').Value := cd_conferencia;
  qyConfereDocumento.Params.ParamByName('codigoaluno').Value := iCodAluno;
  qyConfereDocumento.Params.ParamByName('usuario_conferencia').Value := DM.UsuarioLogado.Pessoa.Codigo;
  qyConfereDocumento.ExecSQL;
  carregaConferencia;
end;
...
procedure TFDigDocumentos.carregaConferencia;
begin

  qyVerificaStatusDoc.Close();

  if Self.pcDocumentos.ActivePage = tsDocsAlunos then
  begin
    Self.qyVerificaStatusDoc.Params.ParamByName('cod_documento').Value := Self.qyDocsAluno.FieldByName('codigo').AsInteger;
  end
  else
  begin
    qyVerificaStatusDoc.Params.ParamByName('cod_documento').Value:= qyDocumentos.FieldByName('codigo').AsInteger;
  end;

  qyVerificaStatusDoc.Params.ParamByName('codigoaluno').Value := iCodAluno;

  qyVerificaStatusDoc.Open();

  if qyVerificaStatusDoc.FieldByName('cd_conferencia').AsInteger = 1 then
  begin
    btnConferirDoc.Enabled := false;
    btnRejeitarDoc.Enabled := true;
  end
  else if qyVerificaStatusDoc.FieldByName('cd_conferencia').AsInteger = 2 then
  begin
    btnConferirDoc.Enabled := true;
    btnRejeitarDoc.Enabled := false;
  end
  else
  begin
    btnConferirDoc.Enabled := true;
         ...
         img2.Assign(qyDocumentosAlunos.FieldByName('im_documento'));
            ...
            try
               ...
               if b.Size > 0 then begin
                  ...
                  itemAux.Indent:= qyDocumentosAlunos.FieldByName('cd_documento_aluno').AsInteger;
      ...
      try
         ...
         try
            qyExcluirExtArquivo.SQL.Text := '' 
              ...
              + ' WHERE '
              + '  cd_documento_aluno = :cd_documento_aluno';
            ...
            qyDoc.ParamByName('cd_documento_aluno').AsInteger := lvwDocumentos.Selected.Indent;
            qyExcluirExtArquivo.ParamByName('cd_documento_aluno').AsInteger   := lvwDocumentos.Selected.Indent;
...
begin
   ...
   try
      ...
      try
         qyDoc.SQL.Text :=
            'SELECT im_documento FROM documentos_alunos WHERE cd_documento_aluno = :cd_documento_aluno';
         ...
         begin
            ...
            begin
               btnAbrirPDFClick(nil);
            end
         else
            Mensagem('Nenhum documento pôde ser carregado!', '', MB_ICONERROR + MB_OK);
      except
...
begin
   ...
   begin
      Mensagem(
         'Selecione um documento para efetuar essa operação.',
         '',
         MB_ICONERROR + MB_OK
      );
      Abort();
...
procedure TFDigDocumentos.btnRejeitarDocClick(Sender: TObject);
begin
  AtualizarStatusDoc(2);
  Mensagem('Documento Rejeitado.', '',  MB_OK);
end;
