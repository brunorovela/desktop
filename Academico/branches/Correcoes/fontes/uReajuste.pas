...
uses
  ...
  Mask, DB, ZConnection, uDM, UMDateTimePicker;
...
procedure TfReajusteMensa.umdtVencInicialChangeDate(Sender: TObject);
begin
...
begin
   ...
   // Iniciar o Filtro
    ...
    if (ckbData.Checked) then begin
         sFiltro := sFiltro + ' AND (datavencimento >=' + QuotedStr(FormatDateTime('yyyy-mm-dd', umdtVencInicial.getDate)) + '';
         sFiltro := sFiltro + ' AND datavencimento <=' + QuotedStr(FormatDateTime('yyyy-mm-dd', umdtVencFinal.getDate)) + ' )';
    end;
...
begin
   ...
   umdtVencInicial.setDate(DM.DataAtual());
   umdtVencFinal.setDate(umdtVencInicial.getDate);
