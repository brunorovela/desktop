...
begin
   ...
   if snPermissaoRestrita then begin
      qryPlanosConta.ParamByName('sn_permissao_restrita').AsInteger := 1;
   end else begin
      qryPlanosConta.ParamByName('sn_permissao_restrita').AsInteger := 0;
   end;
...
begin
   ...
   if cbSelecao.ItemIndex = 3 then
   begin
      anosemestre :=
         StrToIntDef(FMensGerarAnoSem.meAnoSem.Text, DM.GetAnoSemestreAtual);
   end;
   ...
   case cbSelecao.ItemIndex of
      0:
         begin
            ...
            qryTurmasPessoa.ParamByName('CD_COLIGADA_MATRIZ').AsInteger := DM.UsuarioLogado.ColigadaLogada;
...
const
   ...
   SMsgValorZerado = ''
      + 'O valor líquido da parcela não pode ser menor ou igual a zero.';
   SMsgDescontoAbsurdo = ''
      + 'O valor do desconto condicional não pode ser maior que o valor '
      + 'líquido da parcela.';
var
...
begin
   ...
   ValorLiquido :=
      cdsTituloVL_BRUTO.AsCurrency +
      cdsTituloVL_EXTRA.AsCurrency -
      cdsTituloVL_DESC_EXTRA.AsCurrency;

   if ValorLiquido <= 0 then
   begin
      Result := False;
      MessageBox(Handle, SMsgValorZerado, SWarnCaption, MB_ICONWARNING or MB_OK);
   end;

   if (cdsTituloVL_DESCONTO.AsCurrency > 0) and
      (cdsTituloVL_DESCONTO.AsCurrency > ValorLiquido) then
   begin
      Result := False;
      MessageBox(Handle, SMsgDescontoAbsurdo, SWarnCaption, MB_ICONWARNING or MB_OK);
   end;
