object frmMensalidades: TfrmMensalidades
  ...
  object Panel7: TPanel
    ...
    object sbCobranca: TSpeedButton
      Left = 685
      Top = 6
      Width = 182
      Height = 25
      Hint = 'Acessar planilha de cobran'#231'a'
      Caption = '&Planilha de Cobran'#231'a'
      Flat = True
      OnClick = sbCobrancaClick
    end
  ...
  object Panel3: TPanel
    ...
    BevelOuter = bvLowered
    Caption = 
      'PLANILHA DE RECEBIMENTOS. Esta tela n'#227'o recebe mais atualiza'#231#245'es' +
      '. Utilize o Gest'#227'o Online.'
    Color = clBlue
    Font.Charset = ANSI_CHARSET
    Font.Color = clWhite
    Font.Height = -13
  ...
  object pnlRespPendente: TPanel
    ...
    BevelOuter = bvNone
    Caption = 
      'O respons'#225'vel financeiro deste aluno possui outras parcelas em a' +
      'berto.'
    Color = clInfoBk
    TabOrder = 6
    Visible = False
  ...
  object tblMensalidades: TUMZQuery
    ...
    SQL.Strings = (
      ...
      '   CASE'
      '      WHEN fp.CD_FINANCIAMENTO_PESSOA IS NOT NULL THEN '#39'1'#39
      '      WHEN fp2.CD_FINANCIAMENTO_PESSOA IS NOT NULL THEN '#39'1'#39
      '      ELSE '#39'0'#39
      ...
      '            JOIN rem_ocorrencias ON'
        ...
        '              (rem_dados.cd_ocorrencia = rem_ocorrencias.cd_ocor' +
        'rencia)'
      '        WHERE'
      ...
      '                JOIN fin_cheques_mensalidades ON'
        ...
        '                  (fin_cheques.CD_CHEQUE = fin_cheques_mensalida' +
        'des.CD_CHEQUE)'
      '            WHERE'
      '              fin_cheques.cd_situacao = 0 AND'
      
        '              fin_cheques_mensalidades.cd_mensalidade = me.cd_me' +
        'nsalidade'
      '          )'
      '        THEN'
      '          CONCAT(sf.ds_sigla_situacao, '#39'(C)'#39')'
      '        ELSE'
      ...
      '      JOIN situacoes_financeiras as sf ON'
        ...
        '         (IF(me.situacao = 2 AND me.datavencimento < current_dat' +
        'e(), -2, me.situacao)  =  sf.cd_situacao)'
      ...
      '       LEFT JOIN pessoas AS pfc ON'
      '         (pfc.cd_pessoa = c.CD_RESPONSAVEL )'#9' '
      ...
      '      LEFT JOIN fin_financiamento_mensalidade fm ON'
      '        (me.cd_mensalidade = fm.CD_MENSALIDADE)'
      '      LEFT JOIN fin_financiamento_pessoa fp ON'
      
        '        (fm.CD_FINANCIAMENTO_PESSOA = fp.CD_FINANCIAMENTO_PESSOA' +
        ')'
      '      LEFT JOIN fin_financiamento_mensalidade fm2 ON'
      '        (me.cd_mensalidade = fm2.CD_MENSALIDADE_ORIGEM)'
      '      LEFT JOIN fin_financiamento_pessoa fp2 ON'
      
        '        (fm2.CD_FINANCIAMENTO_PESSOA = fp2.CD_FINANCIAMENTO_PESS' +
        'OA)'
      'WHERE'
    ...
    object tblMensalidadesPercDescExtra: TFloatField
      Tag = 1
    ...
    object tblMensalidadesPercDescCond: TFloatField
      Tag = 1
    ...
    object tblMensalidadesValorDescontoCalc: TFloatField
      Tag = 1
      DisplayLabel = 'Desc. Cond.'
      FieldKind = fkCalculated
    ...
    object tblMensalidadessn_cartorio: TIntegerField
      FieldName = 'sn_cartorio'
    end
    object tblMensalidadesds_descontos: TStringField
      Tag = 1
      DisplayLabel = 'Descontos'
      FieldName = 'ds_descontos'
      Size = 255
    end
    object tblMensalidadessn_plano: TStringField
      Tag = 1
      DisplayLabel = 'Tem plano'
      FieldName = 'sn_plano'
    end
    object tblMensalidadesstatus_remessa: TStringField
      Tag = 1
      DisplayLabel = 'Status da Remessa'
      DisplayWidth = 5
      FieldName = 'status_remessa'
      Size = 255
    end
    object tblMensalidadesSN_FINANCIAMENTO: TStringField
      FieldName = 'SN_FINANCIAMENTO'
      ReadOnly = True
      Size = 1
    end
    object tblMensalidadesVL_PERCENTUAL_DIVISAO: TFloatField
      Tag = 1
      DisplayLabel = '% Div.'
    ...
    object tblMensalidadesresp: TStringField
      Tag = 1
  ...
  object pmOutrasOpcoes: TPopupMenu
    ...
    object miCreditosVisualizar: TMenuItem
      Caption = 'Visualizar cr'#233'ditos'
      OnClick = miCreditosVisualizarClick
    end
    object N3: TMenuItem
      Caption = '-'
    end
    ...
    object SaldodaCantina1: TMenuItem
      Caption = 'Saldo da Cantina'
      OnClick = SaldodaCantina1Click
    end
    object TMenuItem
    end
    object N4: TMenuItem
    ...
    object ConfirmarNegociao1: TMenuItem
      Caption = 'Confirmar Negocia'#231#227'o'
      OnClick = ConfirmarNegociao1Click
    end
  end
  ...
  object qryNegocia: TUMZQuery
    ...
    SQL.Strings = (
      ...
      ' INNER JOIN Pessoas AS p ON (men.CodigoAluno = p.cd_pessoa )'
        ...
        ' LEFT JOIN pessoas AS r ON (IFNULL(men.cd_resp, p.cd_resp_finan)' +
        ' = r.cd_pessoa)'
      
        ' LEFT JOIN matriculas mat ON (mat.codigoaluno = men.codigoaluno ' +
        'AND mat.turma = men.turma AND mat.anosemestre = men.anosemestre)' +
        ' '
      ' WHERE men.codigoAluno = :codigoaluno'
      
        '       AND ( CASE WHEN r.cd_pessoa IS NOT NULL THEN r.sn_nao_blo' +
        'quear_financeiro <> 1 ELSE 1 = 1 END )')
    Params = <
      item
        DataType = ftUnknown
        Name = 'codigoaluno'
  ...
  object qryAlteraCompraAcumulada: TUMZQuery
    Connection = DM.db
    Params = <>
    Left = 232
    Top = 296
  end
  object qryPendenciasResp: TUMZReadOnlyQuery
    Connection = DM.db
    SQL.Strings = (
      'SELECT'
      'COUNT(DISTINCT m.cd_mensalidade) AS NR_TITULOS'
      'FROM'
      'mensalidades m'
      'join pessoas a ON'
      '(m.cd_resp = a.cd_resp_finan)'
      'WHERE'
      'a.cd_pessoa != a.cd_resp_finan AND'
      'a.cd_pessoa = :codigoaluno AND'
      'm.codigoaluno <> :codigoaluno AND'
      'm.datavencimento < now() AND'
      'm.situacao IN ('
      
        'select cd_situacao from situacoes_financeiras where sn_protesto ' +
        '= 1 OR cd_situacao in (2,10)'
      #9')')
    Params = <
      item
        DataType = ftUnknown
    ...
    object qryPendenciasRespNR_TITULOS: TLargeintField
      FieldName = 'NR_TITULOS'
      ReadOnly = True
    end
  end
  object updMensalidade: TZUpdateSQL
    DeleteSQL.Strings = (
      ...
      #9'fin_mov_cr f'
      'WHERE'
      #9'f.cd_mensalidade = :CD_MENSALIDADE AND'
      #9'EXISTS ('
      #9#9'SELECT'
      #9#9#9'm.cd_mensalidade'
      #9#9'FROM'
      #9#9#9'mensalidades m'
      #9#9'WHERE'
      #9#9#9'f.cd_mensalidade = m.cd_mensalidade AND'
      
        #9#9#9'm.cd_mensalidade = COALESCE(m.cd_mensalidade_origem, m.cd_men' +
        'salidade)'
      #9');'
      ''
      'DELETE'
      #9'f'
      'FROM'
      #9'fin_negocia_parc_nova f'
      'WHERE'
      '        f.cd_mensalidade = :CD_MENSALIDADE AND'
      #9'EXISTS ('
      #9#9'SELECT'
      #9#9#9'm.cd_mensalidade'
      #9#9'FROM'
      ...
      #9#9#9'e.CD_COMPRA'
      #9#9'FROM'
      #9#9#9'comp_estoque_mensalidade e'
      #9#9#9#9'JOIN mensalidades m ON'
      #9#9#9#9#9'(e.CD_MENSALIDADE = m.cd_mensalidade)'
      #9#9'WHERE'
      #9#9#9'a.CD_COMPRA = e.CD_COMPRA AND'
      #9#9#9'e.CD_MENSALIDADE = :CD_MENSALIDADE AND'
      ...
      '                                JOIN mensalidades m ON'
        ...
        '                                        (em.CD_MENSALIDADE = m.c' +
        'd_mensalidade)'
      '                WHERE'
      '                        e.CD_COMPRA = em.CD_COMPRA AND'
      '                        em.CD_MENSALIDADE = :CD_MENSALIDADE AND'
      
        '                        m.cd_mensalidade = COALESCE(m.cd_mensali' +
        'dade_origem, m.cd_mensalidade)'
      '        );'
      ''
      'DELETE'
      '        e'
      ...
      'WHERE'
      '        e.CD_MENSALIDADE = :CD_MENSALIDADE AND'
      '        EXISTS ('
      '                SELECT'
      '                        m.cd_mensalidade'
      '                FROM'
      ...
      #9'mensalidades'
      'WHERE'
      #9'cd_mensalidade = :CD_MENSALIDADE')
  ...
  object qryTurmasAluno: TUMZReadOnlyQuery
    Connection = DM.db
    SQL.Strings = (
      '('
      'SELECT DISTINCT'
      ...
      #9'0 CD_FINANCIAMENTO,'
      #9'0 SN_FINANCIAMENTO'
      'FROM'
      #9'mensalidades m'
      'WHERE'
      ...
      #9'('
      #9#9'SELECT'
      #9#9#9'f.CD_FINANCIAMENTO_PESSOA'
      #9#9'FROM'
      #9#9#9'fin_financiamento_mensalidade f'
      #9#9'WHERE'
      #9#9#9'm.cd_mensalidade = f.CD_MENSALIDADE'
      #9')'
      ')'
      'UNION'
      '('
      'SELECT DISTINCT'
      #9'f.DS_FINANCIAMENTO CD_TURMA,'
      #9'p.NR_ANOSEMESTRE NR_ANOSEMESTRE,'
      #9'p.CD_FINANCIAMENTO,'
      #9'1 SN_FINANCIAMENTO'
      'FROM'
      #9'fin_financiamento_mensalidade m'
      #9#9'JOIN fin_financiamento_pessoa p ON'
      #9#9#9'(m.CD_FINANCIAMENTO_PESSOA = p.CD_FINANCIAMENTO_PESSOA)'
      #9#9'JOIN fin_config_financiamento f ON'
      #9#9#9'(p.CD_FINANCIAMENTO = f.CD_FINANCIAMENTO)'
      'WHERE'
      #9'p.CD_PESSOA = :CD_PESSOA'
      ')'
      'ORDER BY'
      #9'SN_FINANCIAMENTO DESC,'
      #9'NR_ANOSEMESTRE DESC,'
      #9'CD_TURMA')
    Params = <
      item
        DataType = ftUnknown
    ...
    object qryTurmasAlunoCD_TURMA: TStringField
      FieldName = 'CD_TURMA'
      Size = 255
    end
    object qryTurmasAlunoNR_ANOSEMESTRE: TSmallintField
      FieldName = 'NR_ANOSEMESTRE'
    end
    object qryTurmasAlunoCD_FINANCIAMENTO: TLargeintField
      FieldName = 'CD_FINANCIAMENTO'
    end
    object qryTurmasAlunoSN_FINANCIAMENTO: TLargeintField
      FieldName = 'SN_FINANCIAMENTO'
    end
  end
  object qryTurmasResp: TUMZReadOnlyQuery
    Connection = DM.db
    SQL.Strings = (
      '('
      'SELECT DISTINCT'
      ...
      #9'('
      #9#9'SELECT'
      #9#9#9'e.cd_mensalidade'
      #9#9'FROM'
      #9#9#9'fin_financiamento_mensalidade m'
      #9#9#9#9'JOIN mensalidades e ON'
      #9#9#9#9#9'(m.CD_MENSALIDADE = e.cd_mensalidade)'
      #9#9'WHERE'
      #9#9#9'p.CD_FINANCIAMENTO_PESSOA = m.CD_FINANCIAMENTO_PESSOA AND'
      #9#9#9'e.cd_resp = :CD_PESSOA'
      #9')'
      ')'
      'ORDER BY'
      #9'SN_FINANCIAMENTO DESC,'
    ...
    object qryTurmasRespCD_TURMA: TStringField
      FieldName = 'CD_TURMA'
      Size = 255
    end
    object qryTurmasRespNR_ANOSEMESTRE: TSmallintField
      FieldName = 'NR_ANOSEMESTRE'
    end
    object qryTurmasRespCD_FINANCIAMENTO: TLargeintField
      FieldName = 'CD_FINANCIAMENTO'
    end
    object qryTurmasRespSN_FINANCIAMENTO: TLargeintField
      FieldName = 'SN_FINANCIAMENTO'
    end
  end
end
