unit uFAplicarCreditoCP;

interface

uses
  Windows, Messages, SysUtils, Variants, Classes, Graphics, Controls, Forms,
  Dialogs, ExtCtrls, uCompromissos, StdCtrls, Buttons, Grids, Contnrs,
  UMEditMonetario;

type
  TfrmAplicarCreditoCP = class(TForm)
      paCabecalho: TPanel;
      Panel4: TPanel;
...
uses uCreditoCP, uGeneral;

{$R *.dfm}

procedure TfrmAplicarCreditoCP.Button2Click(Sender: TObject);
begin
   ...
   else if (edValorCredito.getValorDouble() > Self.objCompromisso.getValorTotalPendente()) then
   begin
      TGeneral.Mensagem('Valor inválido. Informe um valor inferior ou igual ao valor do compromisso. Valor: ' +
      FormatFloat( '###,###,##0.00',  Self.objCompromisso.getValorTotalPendente()), '', MB_OK + MB_ICONEXCLAMATION, Handle);
      Self.ModalResult := mrNone;
      Exit;
   end;

   Self.objCompromisso.AplicarDescontoCredito(Self.listaCreditos, edValorCredito.getValorDouble());
end;

procedure TfrmAplicarCreditoCP.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
   Action := caFree;
end;

procedure TfrmAplicarCreditoCP.FormCloseQuery(Sender: TObject;
...
procedure TfrmAplicarCreditoCP.FormCreate(Sender: TObject);
begin
   Self.objCompromisso := nil;
   Self.listaCreditos := nil;
end;

procedure TfrmAplicarCreditoCP.FormKeyDown(Sender: TObject;
...
procedure TfrmAplicarCreditoCP.FormShow(Sender: TObject);
var
   I : Integer;
   valorCreditoUsado, valorCreditoDisponivel, Devolvido: Double;
begin
   Self.listaCreditos := TCreditoCP.GeTCreditoCPDisponiveisPessoa(Self.objCompromisso.Fornecedor);
   ...
   sgCreditosDisponiveis.Cells[3, 0] := 'Valor devolvido';
   sgCreditosDisponiveis.Cells[4, 0] := 'Valor disponível';
   ...
   sgCreditosDisponiveis.Cells[4, 1] := EmptyStr;
   ...
   sgCreditosDisponiveis.ColWidths[4] := 130;

   valorTotalCreditoDisponivel := 0;

   for I := 0 to Self.listaCreditos.Count - 1 do
   begin
      valorCreditoUsado := TCreditoCP(Self.listaCreditos.Items[I]).GetValorCreditoCPUsado();
      valorCreditoDisponivel := TCreditoCP(Self.listaCreditos.Items[I]).GetValorCreditoCPDisponivel();
      Devolvido := TCreditoCP(listaCreditos[I]).GetValorCreditoCPDevolvido;

      // Código do título
      sgCreditosDisponiveis.Cells[0, I+1] := IntToStr(TCreditoCP(Self.listaCreditos.Items[I]).Compromisso.Codigo);

      // Valor do crédito
      sgCreditosDisponiveis.Cells[1, I+1] := FloatToStrF(TCreditoCP(Self.listaCreditos.Items[I]).ValorCreditoCP, ffNumber, 18, 2, TGeneral.GetFormatSettings());
      ...
      sgCreditosDisponiveis.Cells[3, I + 1] := FloatToStrF(Devolvido, ffNumber,
         18, 2, TGeneral.GetFormatSettings);

      // Valor do crédito disponível
      sgCreditosDisponiveis.Cells[4, I+1] := FloatToStrF(valorCreditoDisponivel, ffNumber, 18, 2, TGeneral.GetFormatSettings());
   ...
   lbInformativoValorMaximo.Caption := Format( lbInformativoValorMaximo.Caption,
      [Self.objCompromisso.getValorTotalPendente], TGeneral.GetFormatSettings() );
end;

procedure TfrmAplicarCreditoCP.Panel1Resize(Sender: TObject);
begin
   pnResize.Left := Trunc(TPanel(Sender).Width / 2) - Trunc(pnResize.Width/2);
end;

procedure TfrmAplicarCreditoCP.sgCreditosDisponiveisDrawCell(
