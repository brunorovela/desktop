...
uses
  ...
  Dialogs, General,Clipbrd, UMComboBox;
...
type
  TFrmGradesExtraCad = class(TForm)
    ...
    Label15: TLabel;
    procedure dsExtraAlunoDataChange(Sender: TObject; Field: TField);
  ...
  private
    sPath : String;
    bAnexar, sn_aviso_geral : Boolean;
...
begin
   ...
   if((Length(qyExtraAlunonr_anosem.AsString) < 5) AND (TUMNucleo.GetParametroUsaAnosemestre)) then
   begin
      Mensagem('O Ano/Semestre deve ser preenchido corretamente.', Application.Title, MB_OK + MB_ICONWARNING, Handle);
      exit;
   end;

   if(qyExtraAlunocd_pessoa.AsInteger <= 0) then
   begin
      Mensagem('Deve ser selecionado uma pessoa.', Application.Title, MB_OK + MB_ICONWARNING, Handle);
      exit;
   end;

   if(qyExtraAlunocd_ge_atividade.AsInteger <= 0) then
   begin
      Mensagem('Deve ser selecionado uma atividade.', Application.Title, MB_OK + MB_ICONWARNING, Handle);
      exit;
   end;

   if (qyExtraAlunocd_situacao.AsInteger = 1) and
      (qyExtraAlunonr_horas.IsNull) then
   begin
      Mensagem(
         'Você não pode deferir uma atividade sem definir um número de horas deferidas.',
         'Aviso',
   ...
   if sn_aviso_geral then
   begin
      Mensagem(
         'ATENÇÃO: As horas (deferidas) ultrapassaram a quantidade de horas máxima permitida. '
         + 'Foi ajustado para para que sejam deferidas apenas as horas restantes.',
         'Aviso',
         MB_OK + MB_ICONEXCLAMATION,
         Handle
      );
...
procedure TFrmGradesExtraCad.cbSituacaoChange(Sender: TObject);
begin
   ...
   qyExtraAlunocd_situacao.AsInteger := cbSituacao.ItemIndex;
end;
...
procedure TFrmGradesExtraCad.DBEdit3Enter(Sender: TObject);
begin
   if (Pos('/',DBEdit3.Text) > 0) then
   begin
      DBEdit3.Text := ReplaceStr(DBEdit3.Text,'/','');
   end;
end;

procedure TFrmGradesExtraCad.DBEdit3KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
   ...
   begin
      ...
      begin
         DBEdit3.Text := ReplaceStr(Clipboard.AsText,'/','');
         ClipBoard.Clear;
         Key := 0;
      end;
   end;
...
procedure TFrmGradesExtraCad.dsExtraAlunoDataChange(Sender: TObject;
  Field: TField);
begin
   cbSituacao.ItemIndex := qyExtraAlunocd_situacao.AsInteger;
end;
...
begin
   ...
   nrHorasRequisitadasOriginais := -1;
   nrHorasRequisitadas := -1;
   ...
   else if  ((cbSituacao.ItemIndex = 1) or (qyExtraAlunocd_situacao.AsInteger = 1))
            and
            ( qyExtraAlunonr_horas.IsNull ) then
   begin
   ...
   begin
      ...
      begin
         mensagemHoras := 'ATENÇÃO: As horas requisitadas ('+FloatToStr(nrHorasRequisitadasOriginais)+') ultrapassaram a quantidade de horas que é possível digitar para essa atividade ('+FloatToStr(nrHorasPorAtividade)+'). Foi ajustado para que sejam deferidas apenas as horas restantes.';
         aviso := True;
         sn_aviso_geral := True;
   ...
   begin
      ...
      begin
         mensagemHoras := 'ATENÇÃO: As horas requisitadas ('+FloatToStr(nrHorasRequisitadasOriginais)+') ultrapassaram a quantidade de horas limite na atividade ('+FloatToStr(nrHorasNaAtividade)+'). Foi ajustado para que sejam deferidas apenas as horas restantes.';
         aviso := True;
         sn_aviso_geral := True;
   ...
   begin
      ...
      // VERIFICAÇÃO PARA NÃO GERAR HORAS NEGATIVAS
      if ( nrHorasRequisitadas < 0 ) then
      begin
        nrHorasRequisitadas := 0;
      end;
...
begin
   ...
   DM.qyAux.SQL.Clear;
   DM.qyAux.SQL.Text := 'SELECT ' +
                        '  COALESCE(SUM(CASE WHEN al.cd_ge_aluno <> :cd_ge_aluno AND al.cd_curso = :cd_curso THEN al.nr_horas ELSE 0 END), 0) AS nr_horas_acumuladas_aluno, '+
...
begin
    Mensagem('Essa função foi desabilitada. Para anexar arquivos utilize o gestão online ou o portal online.', Application.Title);
...
var
   ...
   mbAnexo          : String;
begin
    Mensagem('Essa função foi desabilitada. Para baixar arquivos utilize o gestão online ou o portal online.', Application.Title);
...
var
   retorno : Integer;
begin
    Mensagem('Essa função foi desabilitada. Para apagar arquivos utilize o gestão online ou o portal online.', Application.Title);
