...
//    INSERINDO_CENTRO: Boolean;
    ...
    procedure carregaColigadas(const sn_inserir : Integer = 0);
    procedure carregaAreas(const sn_inserir : Integer = 0);
...
begin
   ...
   codigoAntigo := StrToIntDef(edOrdemClassificacao.Text, 0);
   FORCA_ATUALIZACAO := False;
end;
...
procedure Tfrm_CadCentros.carregaAreas(const sn_inserir: Integer = 0);
var
   i: Integer;
...
procedure Tfrm_CadCentros.carregaColigadas(const sn_inserir : Integer = 0);
var
   i: Integer;
...
begin
   ...
   begin
      ...
      if ( sn_inserir <> 1 ) then
      begin
         if qryColigadasCentros.Locate(
            'cd_coligada',
            DM.GetUsuarioLogado.GetColigadasFilhas.Items[i].Codigo,
            []
         ) then
         begin
...
begin
   ...
   begin
      ...
      if qryDepartamentosSelecionados.Locate('cd_departamento', qryDepartamentos.FieldByName('codigo').AsString, []) then
      begin
...
begin
   ...
   begin
      qyCentrosCustoscd_centro_pai.AsInteger := StrToIntDef(TItemCombo(cbCentrosSinteticos.Items.Objects[cbCentrosSinteticos.ItemIndex]).getCodigo, 0);

//      if qyCentrosCustos.State in [dsInsert] then
//      begin
         grauParentesco := getGrauParentesco(
            StrToIntDef(TItemCombo(cbCentrosSinteticos.Items.Objects[cbCentrosSinteticos.ItemIndex]).getCodigo, 0)
         );
         
         qyCentrosCustoscd_classificacao.AsString := getClassificacao(
            grauParentesco,
            StrToIntDef(TItemCombo(cbCentrosSinteticos.Items.Objects[cbCentrosSinteticos.ItemIndex]).codigo, 0)
         );
//      end;
...
const
   ...
   SQL_VERIFICA_TURMAS_VINCULADAS =
      ...
      ' WHERE '+
      ' 	cd_centro = :cd_centro ' +
      '  AND c.CD_COLIGADA_MATRIZ = :cd_coligada_matriz ';
...
begin
   ...
   qryVerificaTurmasVinculadas.ParamByName('cd_centro').AsInteger :=
      cd_centro;
    qryVerificaTurmasVinculadas.ParamByName('cd_coligada_matriz').AsInteger := DM.GetUsuarioLogado.GetColigadaLogada.Codigo;
   ...
   qryVerificaExclusao.ParamByName('cd_coligada_matriz').AsInteger := DM.GetUsuarioLogado.GetColigadaLogada.Codigo;
...
begin
   ...
   // Carrega os componente de coligadas/Unidades
   carregaColigadas(1);
   carregaAreas(1);
end;
