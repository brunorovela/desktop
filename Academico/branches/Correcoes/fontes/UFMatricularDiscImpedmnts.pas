...
uses
   ...
   UZDbcFuncs, ZDbcIntfs, ComCtrls, ExtCtrls, StdCtrls, Buttons, uDM;
...
type
   ...
   private
      ...
      FCurso: string;
      FTurma: string;
      FPorcentagemAluno: Double;
      FSerieAluno: Integer;
      function GetPorcentagemAluno: Double;
      procedure SetPorcentagemAluno(const Value: Double);
      function GetSerieAluno: Integer;
      procedure SetSerieAluno(const Value: Integer);
   ...
   public
      ...
      property SerieAluno: Integer read GetSerieAluno write SetSerieAluno;
      property PorcentagemAluno: Double read GetPorcentagemAluno write SetPorcentagemAluno;
   end;
...
begin
   ...
   cdsRestricoes.Close;
   cdsRestricoes.Fields.Clear;
   cdsRestricoes.FieldDefs.Clear;
...
function TfMatricularDiscImpedmnts.GetPorcentagemAluno: Double;
begin
   Result := FPorcentagemAluno;
end;
...
function TfMatricularDiscImpedmnts.GetSerieAluno: Integer;
begin
   Result := FSerieAluno;
end;
...
begin
   ...
   begin
      if (Self.PorcentagemAluno > qyBuscaValorGrade.FieldByName('NR_PERC_MAX_REPROVACAO').AsInteger) and
         (qyBuscaValorGrade.FieldByName('nr_serie').AsInteger >= Self.SerieAluno) then
      begin
         cdsRestricoes.Insert;
         cdsRestricoesDS_OBSERVACAO.AsString := 'Aluno ultrapassou os '+qyBuscaValorGrade.FieldByName('NR_PERC_MAX_REPROVACAO').AsString+'% máximo de disciplinas reprovadas no curso';
         cdsRestricoesNR_TIPO.AsInteger := 1;
      end;
   end;
end;
...
procedure TfMatricularDiscImpedmnts.SetPorcentagemAluno(const Value: Double);
begin
   FPorcentagemAluno := Value;
end;
...
procedure TfMatricularDiscImpedmnts.SetSerieAluno(const Value: Integer);
begin
   FSerieAluno := Value;
end;
