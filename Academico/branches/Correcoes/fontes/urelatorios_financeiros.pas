...
uses
  ...
  UZDataset, UMComboBox, UMDateTimePicker, Contnrs, Variants, uItemCombo, shellapi;
...
type
  ...
  private
    filtroAtualizaMensalidades: String;
...
begin
  ...
  umdtBase.Enabled := (Pos('periodo_base', xField) > 0) OR (Pos('periodo_base', lowercase(xFormula)) >0 );
  lbDataBase.Enabled := umdtBase.Enabled;

  umdtInicial.Enabled := (Pos('data', xField) > 0) or (Pos('dt_', xField) > 0) OR (Pos('data', lowercase(xFormula)) >0 ) OR (Pos('dt_', lowercase(xFormula)) >0 );
...
begin
   ...
   if umdtBase.Enabled then
   begin
      S := Format('%d,%d,%d %d:%d:%d', [
         YearOf(umdtBase.Date),
         MonthOf(umdtBase.Date),
         DayOf(umdtBase.Date),
         HourOf(umdtBase.Date),
         MinuteOf(umdtBase.Date),
         SecondOf(umdtBase.Date)
      ]);
  ...
  // sFiltro é utilizado para SQL
   ...
   while (Length(Trim(sParametros)) > 1) do begin
      ...
      if (Pos('periodo_base', pCampo) > 0)then begin
          ...
          DecodeDate(umdtBase.Date, wAno, wMes, wDia);

          sFSelecao := sFSelecao + sAux + '( {' + pAtual +'} = ' + 'Date(' + intToStr(wano) + ', ' + intToStr(wmes) + ', ' + intToStr(wDia) + ') ' + ' )';
          sFiltro := sFiltro + sAux + '( ' + pAtual + ' = ''' + FormatDateTime( 'yyyy-mm-dd', umdtBase.Date ) + ''' ) ';
...
// Feito pelo CLAU      sFiltro := sFiltro + sAux + qRelatorios.FieldByName('ds_Especial').AsString;
   ...
   if (Pos('boleto', LowerCase(qRelatorios.FieldByName('nm_relatorio').AsString)) > 0) OR (Pos('bloqueto', LowerCase(qRelatorios.FieldByName('nm_relatorio').AsString)) > 0) then begin
      ...
      begin
         ...
         frmRepBloquetos.qryUpdate.SQL.Text := 'update mensalidades AS m set m.bloqueto = ''S'', m.ocorrencia_remessa = 1, m.ocorrencia_retorno = 0 ';
         frmRepBloquetos.qryUpdate.SQL.Add(' where ');

         frmRepBloquetos.qryUpdate.SQL.Add( filtroAtualizaMensalidades );
...
var
   ...
   sMensagemOp : String;
   cd_tipo_titulo_filtrado: String;
begin
   ...
   if txtDepartamento.Enabled then begin
      if txtDepartamento.Text <> '' then begin
         sFiltroAux := sFiltroAux + ' and (m.depto IN (' + txtDepartamento.Text + ')) ';
   ...
   if (umdtInicial.Enabled) AND (ckDtFiltrar.Checked) Then Begin
      sFiltroAux := sFiltroAux + ' and (m.datavencimento >= "' + FormatDateTime( 'yyyy-mm-dd', umdtInicial.Date ) + '") ';
      sFiltroAux := sFiltroAux + ' and (m.datavencimento <= "' + FormatDateTime( 'yyyy-mm-dd', umdtFinal.Date ) + '") ';
   end;
   ...
   begin
      ...
      if (cbUnidadeEnsino.ItemIndex = 0)  then    // Todas as coligadas que tem permissao apenas
      begin
           sFiltroAux := sFiltroAux + ' and (m.cd_coligada IN (' + DM.GetColigadas() + ')) ';
      end ;

      if cbUnidadeEnsino.ItemIndex <> 0 then
      begin
        sFiltroAux := sFiltroAux + ' and (m.cd_coligada IN (' + IntToStr(TColigada(cbUnidadeEnsino.Items.Objects[cbUnidadeEnsino.ItemIndex]).Codigo) + ')) ';
      end;
   end;

   if DM.variavel_parametro('sn_processo_remessa_novo') <> '1' then
   begin
      // Na remessa nova pode imprimir filtros maaiores
   ...
   cd_tipo_titulo_filtrado := '';
   if (txtTipoTitulo.Text <> '') then
   begin
      cd_tipo_titulo_filtrado := txtTipoTitulo.Text;
   end;
   ...
   begin
      ...
      begin
         sConta := slFormulas.Values['iContaImpressao'];
         sConta := sConta + '=' + slFormulas.Values['iContaColigada'];
         // Prepara boleto para impressão para empresa
         if not frmRepBloquetos.PrepararBloquetosParaImpressao(sFiltroAux, bAgrupado, sConta, True, 0, qRelatorios.FieldByName('ds_filtro_bloquetos').AsString, cd_tipo_titulo_filtrado, pbImpressaoEmpresa) then
         begin
            result := false;
            exit;
         end;
      end else begin
         if not frmRepBloquetos.PrepararBloquetosParaImpressao(sFiltroAux, bAgrupado, sConta, True, 0, qRelatorios.FieldByName('ds_filtro_bloquetos').AsString, cd_tipo_titulo_filtrado)  then
         begin
            result := false;
            exit;
         end;
   ...
   begin
      if bEmpresa OR bResponsavel then begin
         sConta := slFormulas.Values['iContaImpressao'];
         sConta := sConta + '=' + slFormulas.Values['iContaColigada'];
         // Prepara boleto para impressão para aluno
         if not frmRepBloquetos.PrepararBloquetosParaImpressao(sFiltroAux, bAgrupado, sConta, True, 0, qRelatorios.FieldByName('ds_filtro_bloquetos').AsString, cd_tipo_titulo_filtrado, pbImpressaoAluno) then
         begin
            result := false;
            exit;
         end;
      ...
      begin
         if not frmRepBloquetos.PrepararBloquetosParaImpressao(sFiltroAux, bAgrupado, sConta, True, 0, qRelatorios.FieldByName('ds_filtro_bloquetos').AsString, cd_tipo_titulo_filtrado)  then begin
            result := false;
            exit;
         end;
      end;
   ...
   filtroAtualizaMensalidades := sFiltroAux;
...
function Tfrm_relatorios_financeiros.isGrupoPai(selecionado: String): Boolean;
var
  i: Integer;
begin
  Result := False;
...
procedure Tfrm_relatorios_financeiros.Label3Click(Sender: TObject);
var
  URL: string;
begin
  URL := 'https://unimestre.com/basedeconhecimento/projeto-relatorios/';
