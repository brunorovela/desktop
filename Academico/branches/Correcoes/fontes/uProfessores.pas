...
uses
  ...
  UMComboBox, uColigada, uUMNucleo, Menus, Variants, UZDbcFuncs, UZSortedGrid,
  ZSqlUpdate;
...
type
  ...
  TfrmProfessores = class(TForm)
    ...
    UPDTurmasProfessores: TZUpdateSQL;
...
uses
   uDM, Main, uTableFields, uFSelecionarPessoa, ZDbcIntfs, uGeneral,
   uObservacoesProfessor, uItemCombo, uTrocarProfessorTurma, uUsuario,
   uFSelecionarColigada, uFSelecionarSala, Types, uSala, uTurma,
   uTurmaProfessorAceite, uProfessor, uProfessorAfrica, uPessoas,
   UFDisciplinasSelecao;
...
begin
   ...
   chave := IntToStr(tblTurmasProfessorescd_turmaprofessor.AsInteger) + '-' + IntToStr(tblProfessorescd_pessoa.AsInteger);
   DM.setLog(30005, 'Alteracao', chave, DM.UsuarioLogado.ColigadaLogada, ' Foi limpo a carga horária do cd_turmaprofessor: ' + IntToStr(tblTurmasProfessorescd_turmaprofessor.AsInteger) + ' professor: ' + IntToStr(tblProfessorescd_pessoa.AsInteger) );
...
var
   qyAtualizachEfetiva: TUMZQuery;
   cargaHorariaEfetiva,chave: String;
...
begin
   ...
   chave := IntToStr(tblTurmasProfessorescd_turmaprofessor.AsInteger) + '-' + IntToStr(tblProfessorescd_pessoa.AsInteger);
   DM.setLog(30005, 'Alteracao', chave, DM.UsuarioLogado.ColigadaLogada, ' Alterado a carga horária do professor: ' + IntToStr(tblProfessorescd_pessoa.AsInteger) + ' com carga horaria de: ' + cargaHorariaEfetiva );
...
begin
   ...
   begin
      ...
      chave := IntToStr(tblTurmasProfessorescd_sala.AsInteger) + '-' + tblTurmasProfessoresds_sala.AsString;
      DM.setLog(30005, 'Alteracao', chave, DM.UsuarioLogado.ColigadaLogada, ' Alteração da sala : ' + tblTurmasProfessoresds_sala.AsString + ', codigo professor: ' + IntToStr(tblTurmasProfessoresProfessor.AsInteger) + ' e código da disciplina: ' + IntToStr(tblTurmasProfessoresdisciplina.AsInteger) );
...
begin
   ...
   chave := IntToStr(tblTurmasProfessoresProfessor.AsInteger) + '-' + IntToStr(tblTurmasProfessoresdisciplina.AsInteger);
   DM.setLog(30005, 'Alteracao', chave, DM.UsuarioLogado.ColigadaLogada, ' Limpo o vínculo de sala: codigo professor: ' + IntToStr(tblTurmasProfessoresProfessor.AsInteger) + ' e disciplina: ' + IntToStr(tblTurmasProfessoresdisciplina.AsInteger));
...
procedure TfrmProfessores.DBGrid1DragDrop(Sender, Source: TObject; X,
  Y: Integer);
const
   SMsgDisciplinaDup = 'A disciplina selecionada já está vinculada ao professor.';
var
   SelectedRecords: TList;
   I, DiscID: Integer;
   CursoID: AnsiString;
   Found: Boolean;
begin
   ...
   begin
      ...
      SelectedRecords := frmDisciplinasSelecao.SelectedRecords;
      try
         for I := 0 to SelectedRecords.Count - 1 do
         begin
            frmDisciplinasSelecao.qryDisciplinas.GotoBookmark(SelectedRecords[I]);

            DiscID := frmDisciplinasSelecao.qryDisciplinasCD_DISCIPLINA.AsInteger;
            CursoID := frmDisciplinasSelecao.qryDisciplinasCD_CURSO.AsString;

            Found := tblProfessoresDisciplinas.Locate('disciplina;curso',
               VarArrayOf([DiscID, CursoID]), [loCaseInsensitive]);

            if Found and (SelectedRecords.Count = 1) then
               MessageDlg(SMsgDisciplinaDup, mtWarning, [mbOK], -1);

            if not Found then
            begin
               tblProfessoresDisciplinas.Append;
               tblProfessoresDisciplinasdisciplina.AsInteger := DiscID;
               tblProfessoresDisciplinasCurso.AsString := CursoID;
            end;
         end;

         if tblProfessoresDisciplinas.State = dsInsert then
  ...
  try
     ...
     begin
       ...
       try
        ...
        chave := IntToStr(tblTurmasProfessoresProfessor.AsInteger) + '-' + IntToStr(tblTurmasProfessoresdisciplina.AsInteger);
        DM.setLog(30005, 'Inclusao', chave, DM.UsuarioLogado.ColigadaLogada, ' Inclusão da disciplina do professor. Disciplina: ' + IntToStr(tblTurmasProfessoresdisciplina.AsInteger) + ', codigo professor: ' + IntToStr(tblTurmasProfessoresProfessor.AsInteger));
...
const
   SQLSelectProfessores = ''
      ...
      + ' 			u.sn_professor = 1 AND '
      + ' 			f.CD_PESSOA = u.CD_PROFESSOR '
      + ' 	) ';
var
   I: Integer;
   S: AnsiString;
begin
   ...
   begin
      S := Format('%d', [ColigadaFiltrada.Codigo]);
   end;

   tblProfessores.Close;
   tblProfessores.SQL.Text := Format(SQLSelectProfessores, [S]);
...
const
   SQLSelectColigadas = ''
      ...
      + ' 			NU_GRUPOS_PESSOAS P '
      + ' 		WHERE '
      + ' 			U.CD_COLIGADA = P.CD_COLIGADA AND '
      + ' 			P.CD_PESSOA = ? '
      + ' 	) '
      + ' ORDER BY '
      + ' 	C.NM_COLIGADA ';
...
begin
   Found := False;
   cbUnidadeEnsinoProfessor.Items.BeginUpdate;
   cbUnidadeEnsinoProfessor.Clear;

   Stmt := PrepareStatement(SQLSelectColigadas);
   try
      Stmt.SetInt(1, tblProfessorescd_pessoa.AsInteger);
      Stmt.SetInt(2, DM.UsuarioLogado.Pessoa.Codigo);
      Rs := Stmt.ExecuteQueryPrepared;
      try
         while Rs.Next do
         begin
            Coligada := TColigada.Create(Rs.GetInt(1), Rs.GetString(2));

            Index := cbUnidadeEnsinoProfessor.Items.AddObject(Coligada.Nome, Coligada);

            if (ColigadaFiltrada <> nil) and
               (Coligada.Codigo = ColigadaFiltrada.Codigo) then
            begin
               cbUnidadeEnsinoProfessor.ItemIndex := Index;
            end;
         end;
      finally
         Rs.Close;
   ...
   cbUnidadeEnsinoProfessor.Items.EndUpdate;

   Panel3.Height := 65;
   cbUnidadeEnsinoProfessor.Visible := True;
   lbUnidadeEnsinoProfessor.Visible := True;

   if cbUnidadeEnsinoProfessor.Items.Count = 1 then
   begin
...
var
   ...
   msgAceiteExclusao,chave : String;
begin
   ...
   begin
      ...
      begin
         ...
         chave := IntToStr(tblTurmasProfessoresProfessor.AsInteger) + '-' + IntToStr(tblTurmasProfessoresdisciplina.AsInteger);
         DM.setLog(30005, 'Exclusao', chave, DM.UsuarioLogado.ColigadaLogada, ' Exclusão da disciplina do professor. Disciplina: ' + IntToStr(tblTurmasProfessoresdisciplina.AsInteger) + ', codigo professor: ' + IntToStr(tblTurmasProfessoresProfessor.AsInteger));
...
begin
   if DM.UsuarioLogado.TemPermissao(1117, npIncluir, true) then
   begin
      if frmDisciplinasSelecao = nil then
         Application.CreateForm(TfrmDisciplinasSelecao, frmDisciplinasSelecao);

      frmDisciplinasSelecao.Show;
   end;
end;
   ...
   begin
      ...
      begin
         ...
         if Mensagem('Deseja confirmar a exclusão deste registro?',Application.Title,
            ...
            chave := tblProfessoresDisciplinas.FieldByName('Sigla').AsString + tblProfessoresDisciplinas.FieldByName('Curso').AsString + '_' + IntToStr(tblProfessoresDisciplinas.FieldByName('disciplina').AsInteger);
            DM.setLog(30005, 'Exclusao', chave, DM.UsuarioLogado.ColigadaLogada, 'Exclusão de discplina  ' + IntToStr(tblProfessoresDisciplinas.FieldByName('disciplina').AsInteger) + ' do curso ' + tblProfessoresDisciplinas.FieldByName('Curso').AsString );
            end;
...
begin
   ...
   begin
     ...
     with DM.qyAux do begin
       ...
       chave := IntToStr(cd_professor_substituto) + '_' + IntToStr(cd_professor_substituido) + '_' + IntToStr(nr_anosemestre) + '_' + IntToStr(cd_disciplina);
       DM.setLog(30005, 'Alteracao', chave, DM.UsuarioLogado.ColigadaLogada, 'Troca de professor de: ' + IntToStr(cd_professor_substituto) + ' para ' + IntToStr(cd_professor_substituido) );
