...
uses
  ...
  Mask, ZDataset, UMDateTimePicker, UDateTimePicker;
...
procedure TfCopiaTurma.bbtnSalvarClick(Sender: TObject);
begin
...
begin
   ...
   dtpInicio.SelectedDate := dt_inicio_finan;
   dtpFim.SelectedDate := dt_fim_finan;
...
begin
   ...
   begin
      ...
      campos :=
         ...
         ' tp.professor, NULL, tp.situacao, tp.cd_categoria, tp.ds_sala ';
      SQL.Add( ' SELECT '
               + campos
               + ' FROM turmas t '
      );
      SQL.Add( ' INNER JOIN turmasprofessores tp ON ( '
             + ' tp.curso = t.curso AND  '
             + ' tp.turma = t.codigo AND  '
             + ' tp.anosemestre = t.anosemestre )'
      );
      SQL.Add( ' WHERE t.anosemestre = :nr_anosemestre AND ' );
      SQL.Add( '  t.codigo = :cd_turma AND '
         ...
         begin
            ...
            begin
               ...
               begin
                  tem_conflito := True;
                  if DM.UsaAnosemestre then begin
                     if InputQueryUpperCase('Código Inválido','A turma de código "' + cd_turma_nova + '"  já existe na unidade de ensino "' + TItemCombo(FArrayColigadas.Objects[IColigada]).getDescricao() + '".'+Chr(13)+'Informe um novo código:',cd_turma_nova) then
                     begin
...
begin
   ...
   begin
      ...
      qyCopiaTurma.ParamByName('datainicio').AsDate := dtpInicio.SelectedDate;
      qyCopiaTurma.ParamByName('datafim').AsDate := dtpFim.SelectedDate;
      ...
      SQL.Clear;
      campos :=
         ' cd_coligada, cd_curso, cd_turma, cd_campo, nm_arquivo, me_arquivo, tipo_arquivo, nr_anosemestre ';
      SQL.Add( ' INSERT '+ignore+' INTO cursos_turmas_cadastro_arq ( ' + campos + ' ) ' );

      campos :=
         ' ctca.cd_coligada, ctca.cd_curso, :cd_turma_nova, ctca.cd_campo, ctca.nm_arquivo, ctca.me_arquivo, ctca.tipo_arquivo, :nr_anosemestre_novo ';
      SQL.Add( ' SELECT ' + campos + ' FROM turmas t ' );
...
begin
   CopiarGradesHorarios := CopiarDisciplinasTurmasProfessor and CopiarGradesHorarios;
   rgCopiaGradesHorarios.Enabled := CopiarGradesHorarios;
end;
...
begin
   ...
   rgCopiaDisciplinasTurmasProfessor.Enabled := True;
   rgCopiaGradesHorarios.Enabled := (not CopiarMaisColigadas);

   CopiarDisciplinasTurmasProfessor := True;
   CopiarGradesHorarios := not CopiarMaisColigadas;
end;
