object frmRepasseParceiro: TfrmRepasseParceiro
  Left = 0
  Top = 0
  Caption = 'Cadastro da Comiss'#227'o'
  ClientHeight = 598
  ClientWidth = 1057
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  FormStyle = fsMDIChild
  KeyPreview = True
  OldCreateOrder = False
  Position = poScreenCenter
  Visible = True
  WindowState = wsMaximized
  OnClose = FormClose
  OnCreate = FormCreate
  OnKeyDown = FormKeyDown
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object Panel2: TPanel
    Left = 297
    Top = 25
    Width = 760
    Height = 548
    Align = alClient
    Caption = 'Panel2'
    TabOrder = 2
    object grdRepasseParceiro: TZSortedGrid
      Left = 1
      Top = 138
      Width = 758
      Height = 409
      Align = alClient
      DataSource = dsRepasseParceiro
      Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit]
      TabOrder = 0
      TitleFont.Charset = DEFAULT_CHARSET
      TitleFont.Color = clWindowText
      TitleFont.Height = -11
      TitleFont.Name = 'Tahoma'
      TitleFont.Style = []
      OnCellClick = grdRepasseParceiroCellClick
      OnKeyDown = grdRepasseParceiroKeyDown
      OnKeyUp = grdRepasseParceiroKeyUp
      Columns = <
        item
          Expanded = False
          FieldName = 'cd_aluno'
          Title.Caption = 'C'#243'digo'
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'ds_aluno'
          Width = 230
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'ds_cidade'
          Width = 120
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'ds_curso'
          Width = 137
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'ds_turma'
          Width = 230
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'ds_vendedor'
          Width = 230
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'vl_percentual'
          Visible = True
        end>
    end
    object pnFiltro: TPanel
      Left = 1
      Top = 1
      Width = 758
      Height = 137
      Align = alTop
      BevelOuter = bvNone
      TabOrder = 1
      object lbAluno: TLabel
        Left = 486
        Top = 27
        Width = 27
        Height = 13
        Caption = 'Aluno'
      end
      object lbTurma: TLabel
        Left = 48
        Top = 81
        Width = 30
        Height = 13
        Caption = 'Turma'
      end
      object btnFiltroTurma: TSpeedButton
        Left = 394
        Top = 81
        Width = 23
        Height = 21
        Cursor = crHandPoint
        Glyph.Data = {
          
          333333773FF77333333333370007333333333333777333333333}
        NumGlyphs = 2
        OnClick = btnFiltroTurmaClick
      end
      object btnFiltroAluno: TSpeedButton
        Left = 829
        Top = 27
        Width = 23
        Height = 21
        Cursor = crHandPoint
        Glyph.Data = {
          
          333333773FF77333333333370007333333333333777333333333}
        NumGlyphs = 2
        OnClick = btnFiltroAlunoClick
      end
      object lbAnosemestre: TLabel
        Left = 11
        Top = 27
        Width = 67
        Height = 13
        Caption = 'Ano/semestre'
      end
      object lbVendedor: TLabel
        Left = 467
        Top = 54
        Width = 46
        Height = 13
        Caption = 'Vendedor'
      end
      object btnFiltroVendedor: TSpeedButton
        Left = 829
        Top = 54
        Width = 23
        Height = 21
        Cursor = crHandPoint
        Glyph.Data = {
          
          333333773FF77333333333370007333333333333777333333333}
        NumGlyphs = 2
        OnClick = btnFiltroVendedorClick
      end
      object lbPeriodo: TLabel
        Left = 477
        Top = 81
        Width = 36
        Height = 13
        Caption = 'Per'#237'odo'
        Visible = False
      end
      object lbCurso: TLabel
        Left = 48
        Top = 54
        Width = 28
        Height = 13
        Caption = 'Curso'
      end
      object btnFiltroCurso: TSpeedButton
        Left = 394
        Top = 54
        Width = 23
        Height = 21
        Cursor = crHandPoint
        Glyph.Data = {
          
          333333773FF77333333333370007333333333333777333333333}
        NumGlyphs = 2
        OnClick = btnFiltroCursoClick
      end
      object pnFiltroTitulo: TPanel
        Left = 0
        Top = 0
        Width = 758
        Height = 17
        Align = alTop
        BevelOuter = bvNone
        Caption = 'FILTROS'
        Color = clGray
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindow
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        TabOrder = 0
      end
      object edAluno: TEdit
        Left = 519
        Top = 27
        Width = 304
        Height = 21
        BevelInner = bvNone
        BevelKind = bkFlat
        BorderStyle = bsNone
        ReadOnly = True
        TabOrder = 1
      end
      object edTurma: TEdit
        Left = 84
        Top = 81
        Width = 304
        Height = 21
        BevelInner = bvNone
        BevelKind = bkFlat
        BorderStyle = bsNone
        ReadOnly = True
        TabOrder = 2
      end
      object btnFiltrar: TBitBtn
        Left = 81
        Top = 108
        Width = 75
        Height = 21
        Cursor = crHandPoint
        Caption = 'Filtrar'
        TabOrder = 3
        OnClick = btnFiltrarClick
        Glyph.Data = {
          
          3000333333333333377733333333333333333333333333333333}
        NumGlyphs = 2
      end
      object btnLimpar: TBitBtn
        Left = 162
        Top = 108
        Width = 75
        Height = 21
        Cursor = crHandPoint
        Caption = 'Limpar'
        TabOrder = 4
        OnClick = btnLimparClick
        Glyph.Data = {
          
          FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
        NumGlyphs = 2
      end
      object edAnosemestre: TMaskEdit
        Left = 84
        Top = 27
        Width = 52
        Height = 21
        BevelInner = bvNone
        BevelKind = bkFlat
        BorderStyle = bsNone
        EditMask = '9999/9;0;_'
        MaxLength = 6
        ReadOnly = True
        TabOrder = 5
      end
      object udAnosemestre: TUpDown
        Left = 139
        Top = 27
        Width = 17
        Height = 21
        Cursor = crHandPoint
        Min = -100
        TabOrder = 6
        OnClick = udAnosemestreClick
      end
      object edPeriodoDe: TDateTimePicker
        Left = 519
        Top = 81
        Width = 130
        Height = 21
        BevelInner = bvNone
        BevelKind = bkFlat
        Date = 42895.000000000000000000
        Time = 42895.000000000000000000
        ShowCheckbox = True
        Checked = False
        TabOrder = 7
        Visible = False
      end
      object edPeriodoAte: TDateTimePicker
        Left = 693
        Top = 81
        Width = 130
        Height = 21
        BevelInner = bvNone
        BevelKind = bkFlat
        Date = 42895.000000000000000000
        Time = 42895.000000000000000000
        ShowCheckbox = True
        Checked = False
        TabOrder = 8
        Visible = False
      end
      object edVendedor: TEdit
        Left = 519
        Top = 54
        Width = 304
        Height = 21
        BevelInner = bvNone
        BevelKind = bkFlat
        BorderStyle = bsNone
        ReadOnly = True
        TabOrder = 9
      end
      object edCurso: TEdit
        Left = 84
        Top = 54
        Width = 304
        Height = 21
        BevelInner = bvNone
        BevelKind = bkFlat
        BorderStyle = bsNone
        ReadOnly = True
        TabOrder = 10
      end
      object BitBtn1: TBitBtn
        Left = 423
        Top = 54
        Width = 23
        Height = 21
        Cursor = crHandPoint
        TabOrder = 11
        OnClick = BitBtn1Click
        Glyph.Data = {
          
          FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
        NumGlyphs = 2
      end
      object BitBtn2: TBitBtn
        Left = 423
        Top = 81
        Width = 23
        Height = 21
        Cursor = crHandPoint
        TabOrder = 12
        OnClick = BitBtn2Click
        Glyph.Data = {
          
          FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
        NumGlyphs = 2
      end
      object BitBtn3: TBitBtn
        Left = 858
        Top = 27
        Width = 23
        Height = 21
        Cursor = crHandPoint
        TabOrder = 13
        OnClick = BitBtn3Click
        Glyph.Data = {
          
          FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
        NumGlyphs = 2
      end
      object BitBtn4: TBitBtn
        Left = 858
        Top = 54
        Width = 23
        Height = 21
        Cursor = crHandPoint
        TabOrder = 14
        OnClick = BitBtn4Click
        Glyph.Data = {
          
          FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
        NumGlyphs = 2
      end
    end
  end
  object toolPessoa: TToolBar
    Left = 0
    Top = 573
    Width = 1057
    Height = 25
    Align = alBottom
    ButtonWidth = 84
    Caption = 'toolPessoa'
    EdgeBorders = [ebTop, ebBottom]
    Images = ImageList1
    List = True
    ShowCaptions = True
    TabOrder = 0
    object ToolButton1: TToolButton
      Left = 0
      Top = 0
      Width = 8
      Caption = 'ToolButton1'
      ImageIndex = 9
      Style = tbsSeparator
    end
    object btnInserir: TToolButton
      Left = 8
      Top = 0
      Caption = 'F2 Incluir'
      ImageIndex = 0
      OnClick = btnInserirClick
    end
    object btnAlterar: TToolButton
      Left = 92
      Top = 0
      Caption = 'F3 Alterar'
      ImageIndex = 1
      OnClick = btnAlterarClick
    end
    object btnExcluir: TToolButton
      Left = 176
      Top = 0
      Caption = 'F9 Excluir'
      ImageIndex = 2
      OnClick = btnExcluirClick
    end
    object btnSalvar: TToolButton
      Left = 260
      Top = 0
      Caption = 'F5 Salvar'
      ImageIndex = 3
      OnClick = btnSalvarClick
    end
    object ToolButton4: TToolButton
      Left = 344
      Top = 0
      Width = 8
      Caption = 'ToolButton4'
      ImageIndex = 3
      Style = tbsSeparator
    end
    object btnCancelar: TToolButton
      Left = 352
      Top = 0
      Caption = 'F6 Cancelar'
      ImageIndex = 4
      OnClick = btnCancelarClick
    end
    object ToolButton8: TToolButton
      Left = 436
      Top = 0
      Width = 8
      Caption = 'ToolButton8'
      ImageIndex = 6
      Style = tbsSeparator
    end
    object btnSair: TToolButton
      Left = 444
      Top = 0
      Caption = 'F12 Fechar'
      ImageIndex = 7
      OnClick = btnSairClick
    end
    object ToolButton2: TToolButton
      Left = 528
      Top = 0
      Width = 8
      Caption = 'ToolButton2'
      ImageIndex = 8
      Style = tbsSeparator
    end
  end
  object Panel1: TPanel
    Left = 0
    Top = 25
    Width = 297
    Height = 548
    Align = alLeft
    TabOrder = 1
    object lbCadComissao: TLabel
      Left = 8
      Top = 237
      Width = 59
      Height = 13
      Caption = '% Comiss'#227'o'
      OnClick = BitBtn1Click
    end
    object lbCadVendedor: TLabel
      Left = 8
      Top = 190
      Width = 46
      Height = 13
      Caption = 'Vendedor'
      OnClick = BitBtn1Click
    end
    object lbCadAluno: TLabel
      Left = 8
      Top = 12
      Width = 27
      Height = 13
      Caption = 'Aluno'
      OnClick = BitBtn1Click
    end
    object lbCadTurma: TLabel
      Left = 8
      Top = 144
      Width = 30
      Height = 13
      Caption = 'Turma'
    end
    object lbCadCurso: TLabel
      Left = 8
      Top = 101
      Width = 28
      Height = 13
      Caption = 'Curso'
    end
    object Label2: TLabel
      Left = 8
      Top = 58
      Width = 67
      Height = 13
      Caption = 'Ano/semestre'
    end
    object btnBuscarCurso: TSpeedButton
      Left = 257
      Top = 117
      Width = 23
      Height = 21
      Cursor = crHandPoint
      Glyph.Data = {
        
        333333773FF77333333333370007333333333333777333333333}
      NumGlyphs = 2
      OnClick = btnBuscarCursoClick
    end
    object btnBuscarTurma: TSpeedButton
      Left = 257
      Top = 163
      Width = 23
      Height = 21
      Cursor = crHandPoint
      Glyph.Data = {
        
        333333773FF77333333333370007333333333333777333333333}
      NumGlyphs = 2
      OnClick = btnBuscarTurmaClick
    end
    object btnBuscarAluno: TSpeedButton
      Left = 257
      Top = 31
      Width = 23
      Height = 21
      Cursor = crHandPoint
      Glyph.Data = {
        
        333333773FF77333333333370007333333333333777333333333}
      NumGlyphs = 2
      OnClick = btnBuscarAlunoClick
    end
    object btnBuscarVendedor: TSpeedButton
      Left = 257
      Top = 209
      Width = 23
      Height = 21
      Cursor = crHandPoint
      Glyph.Data = {
        
        333333773FF77333333333370007333333333333777333333333}
      NumGlyphs = 2
      OnClick = btnBuscarVendedorClick
    end
    object edCadComissao: TEdit
      Left = 7
      Top = 256
      Width = 60
      Height = 21
      BevelInner = bvNone
      BevelKind = bkFlat
      BorderStyle = bsNone
      TabOrder = 0
    end
    object edCadVendedor: TEdit
      Left = 7
      Top = 209
      Width = 244
      Height = 21
      BevelInner = bvNone
      BevelKind = bkFlat
      BorderStyle = bsNone
      ReadOnly = True
      TabOrder = 1
    end
    object edCadAluno: TEdit
      Left = 8
      Top = 31
      Width = 243
      Height = 21
      BevelInner = bvNone
      BevelKind = bkFlat
      BorderStyle = bsNone
      ReadOnly = True
      TabOrder = 2
    end
    object edCadTurma: TEdit
      Left = 8
      Top = 163
      Width = 243
      Height = 21
      BevelInner = bvNone
      BevelKind = bkFlat
      BorderStyle = bsNone
      ReadOnly = True
      TabOrder = 3
    end
    object edCadCurso: TEdit
      Left = 8
      Top = 117
      Width = 243
      Height = 21
      BevelInner = bvSpace
      BevelKind = bkFlat
      BorderStyle = bsNone
      ReadOnly = True
      TabOrder = 4
    end
    object edCadAnosemestre: TMaskEdit
      Left = 8
      Top = 74
      Width = 52
      Height = 21
      BevelInner = bvNone
      BevelKind = bkFlat
      BorderStyle = bsNone
      EditMask = '9999/9;0;_'
      MaxLength = 6
      ReadOnly = True
      TabOrder = 5
    end
    object UpDown1: TUpDown
      Left = 63
      Top = 74
      Width = 17
      Height = 21
      Cursor = crHandPoint
      Min = -100
      TabOrder = 6
      OnClick = UpDown1Click
    end
  end
  object pnRemessaTit: TPanel
    Left = 0
    Top = 0
    Width = 1057
    Height = 25
    Align = alTop
    BevelOuter = bvLowered
    Caption = 'CADASTRO DE COMISS'#195'O'
    Color = clBlue
    Font.Charset = ANSI_CHARSET
    Font.Color = clWhite
    Font.Height = -13
    Font.Name = 'Arial'
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 3
  end
  object qyRepasseParceiro: TUMZQuery
    Connection = DM.db
    Params = <>
    Left = 388
    Top = 217
    object qyRepasseParceirocd_repasse_parceiro: TIntegerField
      FieldName = 'cd_repasse_parceiro'
    end
    object qyRepasseParceironm_aluno: TStringField
      DisplayLabel = 'Aluno'
      FieldName = 'ds_aluno'
      Size = 255
    end
    object qyRepasseParceirods_cidade: TStringField
      DisplayLabel = 'Cidade'
      FieldName = 'ds_cidade'
      Size = 255
    end
    object qyRepasseParceirods_curso: TStringField
      DisplayLabel = 'Curso'
      FieldName = 'ds_curso'
      Size = 255
    end
    object qyRepasseParceirods_turma: TStringField
      DisplayLabel = 'Turma'
      DisplayWidth = 255
      FieldName = 'ds_turma'
      Size = 255
    end
    object qyRepasseParceironm_vendedor: TStringField
      DisplayLabel = 'Vendedor'
      FieldName = 'ds_vendedor'
      Size = 255
    end
    object qyRepasseParceirovl_percentual: TSmallintField
      DisplayLabel = '% Comiss'#227'o'
      FieldName = 'vl_percentual'
    end
    object qyRepasseParceiroid_matricula: TIntegerField
      FieldName = 'id_matricula'
    end
    object qyRepasseParceirocd_curso: TStringField
      FieldName = 'cd_curso'
      Size = 255
    end
    object qyRepasseParceirocd_turma: TStringField
      FieldName = 'cd_turma'
      Size = 255
    end
    object qyRepasseParceironr_anosemestre: TIntegerField
      FieldName = 'nr_anosemestre'
    end
    object qyRepasseParceirocd_aluno: TIntegerField
      FieldName = 'cd_aluno'
    end
    object qyRepasseParceirocd_vendedor: TIntegerField
      FieldName = 'cd_vendedor'
    end
  end
  object dsRepasseParceiro: TDataSource
    DataSet = qyRepasseParceiro
    Left = 388
    Top = 249
  end
  object ImageList1: TImageList
    Left = 388
    Top = 282
    Bitmap = {
      
      000000000000}
  end
end
