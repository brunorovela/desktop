...
type
  TfRemessaCP = class(TForm)
    ...
    qryRemessasdescLayout: TStringField;
    ...
    qryLayouts: TUMZReadOnlyQuery;
    qryLayoutsNM_LAYOUT: TStringField;
    ...
    qryRemessasCD_LAYOUT: TIntegerField;
    qryLayoutsCD_LAYOUT: TLargeintField;
    qryLayoutsCD_CAIXA: TLargeintField;
    procedure qryRemessasdescLayoutChange(Sender: TField);
...
begin
    ...
    qLotes.SQL.Text :=
       ...
       '   AND cd_layout = ' + qryRemessasCD_LAYOUT.AsString   ;
...
begin
   ...
   if qryLayouts.RecordCount = 1 then
   begin
      qryRemessascd_caixa.AsInteger := qryLayoutscd_caixa.AsInteger;
      qryRemessascd_layout.AsInteger := qryLayoutscd_layout.AsInteger;
   end;
...
procedure TfRemessaCP.qryRemessasdescLayoutChange(Sender: TField);
begin
   // qryRemessasCD_CAIXA.AsInteger := qryLayoutsCD_CAIXA.AsInteger;
   // qryRemessasCD_LAYOUT.AsInteger := qryLayoutsCD_LAYOUT.AsInteger;
end;
...
begin
   ...
   if qryRemessascd_layout.AsInteger = 0 then begin
       // Se não selecionou uma conta, então não pode salvar
       Mensagem('Você precisa selecionar uma conta/layout de pagamento para continuar.', 'Informação', MB_OK + MB_ICONINFORMATION, Handle);
       Exit;
   end;
...
const
   SQL_CARREGAR_CONTAS =
      ' SELECT DISTINCT L.CD_LAYOUT, L.NM_LAYOUT, L.CD_CAIXA ' +
      ...
      '         L.CD_ORIGEM = 2 ' +
      '      ) ' +
      ' WHERE C.CD_COLIGADA IN (%s) AND U.CD_USUARIO = :CD_USUARIO AND L.SN_ATIVO = 1' +
      ' ORDER BY L.NM_LAYOUT ';
begin
   qryLayouts.Close;
   qryLayouts.SQL.Text := Format( SQL_CARREGAR_CONTAS, [DM.UsuarioLogado.GetColigadasFilhaSelecionada()] );
   qryLayouts.ParamByName('CD_USUARIO').AsInteger :=  DM.iCdPessoaLogado;
   qryLayouts.Open();
end;
