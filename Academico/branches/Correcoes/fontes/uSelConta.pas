...
type
   TTipoPlanoContas = (tpcTodas, tpcEntradas, tpcSaidas);
   ...
   Tfrm_SelConta = class(TForm)
    ...
    lblCdContabilidade: TLabel;
    txtCd_contabilidade: TEdit;
    qyPlanosContasCD_COLIGADA_MATRIZ: TIntegerField;
  ...
  public
    ...
    SomenteAnalitico,
    buscaTudo: Boolean;
    class var filtroContas : TTipoPlanoContas;
...
// Checar permissao para contas restritars
   ...
   if not snPermissaoRestrita then  begin
       qyPlanosContas.SQL.Add(' AND IFNULL(sn_restringir_permissao,0) = 0  ');
   end;
   ...
   if txtCd_contabilidade.Text <> '' then
   begin
      qyPlanosContas.SQL.Add(' AND cd_conta_contabil = ' + txtCd_contabilidade.Text);
   end;
   ...
   if filtroContas = tpcEntradas then
   begin
      qyPlanosContas.SQL.Add('   AND tp_entrada_saida in (0,1) ');
   end;

   if filtroContas = tpcSaidas then
   begin
      qyPlanosContas.SQL.Add('   AND tp_entrada_saida in (0,2) ');
   end;
...
begin
   ...
   // Sempre volta essa tela para todas, pois ao abrir as que não tem que ser para todas estarei especificando
   Self.filtroContas := tpcTodas;
end;
