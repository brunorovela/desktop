...
uses
  ...
  ZAbstractDataset, ZDataset, UZDataset, DBCtrls, ZSysUtils, ZDbcIntfs, UZDbcFuncs;
...
type
  TformMonografias = class(TForm)
    ...
    qyMonografiasDS_TEMA_HTML: TMemoField;
    qyMonografiasDS_IDEIA_INICIAL_HTML: TMemoField;
    dbreTema: TDBRichEdit;
    qyMonografiasds_chave_grupo: TStringField;
    procedure DateTimeFieldOnValidate(Sender: TField);
    procedure qyMonografiasds_ideia_inicialGetText(Sender: TField;
...
uses uUsuario, uDM, uCadMonografias, uFSelecionarPessoa, uFSelecionarTurma, bde, uCamposPlanilhas;
...
const
   SSQLSelectLastId = 'SELECT MAX(CD_MONOGRAFIA) FROM MONOGRAFIAS';
var
   Rs: IZResultSet;
   LastID: Integer;
   OriginalSQL: AnsiString;
   Params: TParams;
begin
   ...
   Rs := ExecuteQuery(SSQLSelectLastId);
   try
      if Rs.Next then
         LastID := Rs.GetInt(1);
   finally
   ...
   OriginalSQL := qyMonografias.SQL.Text;
   Params := TParams.Create;
   Params.Assign(qyMonografias.Params);
   
   Application.CreateForm(TformCadMonografias, formCadMonografias);
   
   DBGrid1.DataSource := nil;
   
   qyMonografias.SQL.Text := 'SELECT * FROM monografias WHERE cd_monografia > :cd_monografia';
   qyMonografias.ParamByName('cd_monografia').AsInteger := LastID;
   qyMonografias.Close;
   qyMonografias.Open;
   ...
   if formCadMonografias.ShowModal = mrOk then
   begin
      qyMonografias.SQL.Text := 'SELECT * FROM monografias WHERE cd_monografia > :cd_monografia';
      qyMonografias.ParamByName('cd_monografia').AsInteger := LastID;
      qyMonografias.Close;
   ...
   begin
      qyMonografias.SQL.Text := OriginalSQL;
      qyMonografias.Params.Assign(Params);
      qyMonografias.Open;
   end;
...
procedure TformMonografias.DateTimeFieldOnValidate(Sender: TField);
const
   MsgDataInvalida = 'A data informada para o campo "%s" é inválida.';
var
   S: AnsiString;
   D: TDateTime;
   FmtSettings: TFormatSettings;
   OnValidateField: TFieldNotifyEvent;
begin
   S := Sender.AsString;
   S := Trim(S);
   ...
   if S = EmptyStr then
   begin
      OnValidateField := Sender.OnValidate;
      Sender.OnValidate := nil;
      Sender.Clear;
      Sender.OnValidate := OnValidateField;
   end;
   ...
   begin
      ...
      if not TryStrToDate(S, D, FmtSettings) then
      begin
         S := Format(MsgDataInvalida, [Sender.DisplayLabel]);
...
begin
   ...
   DM.MontarPlanilha(DBGrid1, 'planilha_monografias_campos');
end;
...
procedure TformMonografias.qyMonografiasds_ideia_inicialGetText(Sender: TField;
  var Text: string; DisplayText: Boolean);
begin
   Text := dbreTema.Text;
end;

procedure TformMonografias.sbSelecionarColunaClick(Sender: TObject);
begin
   Application.CreateForm(TfrmSelColunas, frmSelColunas);
   frmSelColunas.SelecionaColunas(DBGrid1, 'planilha_monografias_campos');
end;
