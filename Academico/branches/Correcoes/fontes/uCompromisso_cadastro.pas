...
type
   TfrmCompromissoCadastro = class(TForm)
    ...
    qryFinanceiro: TUMZQuery;
    qryFinanceirovl_credito: TFloatField;
   ...
   public
      ...
      procedure setSituacao( codigo: string );
    ...
    function getPlanoContaPadraoFornecedor(Const cd_pessoa: string ) : string;
...
var
   ...
   dt_pagamento      : string;
   cd_coligada       : string;
   cd_conta,data_1   : string;
   cd_centro,data_2  : string;
   vl_total_despesa  : string;
   bAlterar          : boolean;
   qyUpdateFinContasPagar : TUMZQuery;
begin
   ...
   begin
       ...
       Dm.CriarConsulta(qyUpdateFinContasPagar);

      qyUpdateFinContasPagar.Close();
      qyUpdateFinContasPagar.SQL.Clear();
      qyUpdateFinContasPagar.SQL.Add('UPDATE FIN_CONTAS_PAGAR SET ');
      qyUpdateFinContasPagar.SQL.Add('dt_vencimento = :dt_vencimento, dt_provavel_pgto = :dt_provavel_pgto ');
      qyUpdateFinContasPagar.SQL.Add(' WHERE CD_TITULO = :cd_titulo ');
      qyUpdateFinContasPagar.SQL.Add(' AND nr_parcela = :parcela ');

      qyUpdateFinContasPagar.ParamByName('dt_vencimento').AsString := formatdatetime('yyyy-mm-dd',StrToDate(txtParVencimento.Text));
      qyUpdateFinContasPagar.ParamByName('dt_provavel_pgto').AsString := formatdatetime('yyyy-mm-dd',StrToDate(txtParProvavelPgto.Text));

      qyUpdateFinContasPagar.ParamByName('cd_titulo').AsString := objParcelaSelecionada.getValorCampo('cd_titulo');
      qyUpdateFinContasPagar.ParamByName('parcela').AsString := objParcelaSelecionada.getValorCampo('nr_parcela');

      qyUpdateFinContasPagar.ExecSQL();
      ...
      begin
         ...
         begin
            ...
            begin
               ...
               if objParcelaSelecionada.getValorCampo('ds_observacao') <>
                  objParcelaCopiaSelecionada.getValorCampo('ds_observacao') then
               begin
                  AlteraObservacaoParcela(
                     objParcelaSelecionada.getValorCampo('ds_observacao'),
...
//   txtParHistorico.ReadOnly               := valor;
...
var
   ...
   data_format: TDate;
   dataProvavelPgto,resultado: String;
   dt_vencimento,dt_bloqueio: TDateTime;
   baixar_checked: Boolean;
   qyValidaFornecedorDoc : TUMZQuery;
   qyValidaDtVencimento : TUMZQuery;
begin
   ...
   begin
      ...
      dt_bloqueio := Dm.DataBloqueioFinanceiro(0);
      if (formatdatetime('yyyy/mm/dd',StrToDate(txtParVencimento.Text)) < formatdatetime('yyyy/mm/dd',(dt_bloqueio)) ) OR
      (formatdatetime('yyyy/mm/dd',StrToDate(txtParProvavelPgto.Text)) < formatdatetime('yyyy/mm/dd',(dt_bloqueio)) )  then
      begin
        Mensagem('Você não pode efetuar alterações de títulos com data inferior a ' + DateToStr(dt_bloqueio), 'Atenção', MB_OK + MB_ICONWARNING);
        exit;
      end;
      ...
      begin
        try
          qyValidaFornecedorDoc := nil;
          DM.CriarConsulta( qyValidaFornecedorDoc );

          with qyValidaFornecedorDoc do
          begin
            Close();
            SQL.Clear();
            SQL.Add('SELECT');
            SQL.Add('	* ');
            SQL.Add('FROM');
            SQL.Add('	fin_contas_pagar fcp');
            SQL.Add('	INNER JOIN fin_config_tipos_titulo fctt ON (fctt.cd_tipo_titulo = fcp.cd_tipo_titulo) ');
            SQL.Add('WHERE');
            SQL.Add('	fcp.nr_documento = :nr_documento ');
            SQL.Add('	AND fcp.cd_pessoa = :cd_pessoa ');
            SQL.Add(' AND fctt.sn_numero_doc_distinto = 1');
            ParamByName('nr_documento').AsString := txtNrDocumento.Text;
            ParamByName('cd_pessoa').AsString := txtCdFornecedor.Text;

            Open();
            if(RecordCount > 0) then
            begin
              Mensagem('Já existe um documento com este número cadastrado para este fornecedor.', 'Atenção', MB_OK + MB_ICONWARNING);
              exit;
            end;
          end;
        except on E: EZDatabaseError do
          TrataErroBanco( qyValidaFornecedorDoc, E );
        end;
...
begin
   ...
   begin
      ...
      begin
         ...
         txtParVencimento.Enabled               := true;
         txtParProvavelPgto.Enabled             := true;
         Self.BloquearAlteracaoParcela( true );
      end;
   end;
end;
...
begin
  ...
  if (Self.getFormEstado() = 'inserir') OR (Self.getFormEstado() = 'alterar')  then
   begin
      if (Self.cd_situacao = '1') OR (Self.getFormEstado() = 'inserir')   then
      begin
         txtParVencimento.Enabled               := true;
         txtParProvavelPgto.Enabled             := true;
      end
         ...
         begin
         txtParVencimento.Enabled               := false;
         txtParProvavelPgto.Enabled             := false;
      end;
   end;
...
//   Self.getListaCentroCustos();
   ...
   begin
      ...
      begin
         ...
         txtDtEmissao.text :=  DateTimeToStr(qryColigadasdt_bloqueio_financeiro.AsDateTime);
      end;

   


      // Retirado por Claudionor
      // dt_bloqueio_boleto não deve ser utilizado no contas a pagar
...
begin
   ...
   qyFormaPgto.SQL.Add('WHERE sn_ativo = 1 AND cd_coligada = ' + IntToStr(self.cd_coligada_seleciona) );
...
begin
   ...
   // Checar permissao para contas restritars
   snPermissaoRestrita := ( DM.UsuarioLogado.TemPermissao(DM.iCdPessoaLogado, 'UMFinanceiro.PlanoContasRestrito', npAcesso, False) );
   ...
   qyPlanoConta.SQL.Add(' AND cd_coligada_matriz = ' + IntToStr( cd_coligada )  );
   qyPlanoConta.SQL.Add(' AND sn_ativo = 1                              ');
   qyPlanoConta.SQL.Add(' AND tp_entrada_saida IN (0,2)                 ');

   if not snPermissaoRestrita then  begin
       qyPlanoConta.SQL.Add(' AND IFNULL(sn_restringir_permissao,0) = 0  ');
   end;
...
function TfrmCompromissoCadastro.getPlanoContaPadraoFornecedor(Const cd_pessoa: string ): string;
CONST
   SQL_PLANO_CONTA =
    ' SELECT'
   +'    pc.cd_conta, '
   ...
   +'       pc.cd_coligada = c.cd_coligada OR '
   +'       pc.cd_coligada_matriz = c.CD_COLIGADA_MATRIZ '
   +'    ) '
   +'    INNER JOIN fin_plano_contas_fornecedor pcf ON ( '
   +'       pcf.cd_conta = pc.cd_conta AND '
   +'       ( '
   +'          pcf.cd_coligada = pc.cd_coligada OR '
   +'          pcf.cd_coligada = pc.cd_coligada_matriz '
   +'       ) '
   +'    ) '
   +' WHERE '
   +'    pcf.cd_pessoa = :cd_pessoa '
   +' GROUP BY '
   +'    pc.cd_conta ';            
var
   qyPlanoConta   : TUMZQuery;
...
begin
   ...
   qyPlanoConta.Close();
   qyPlanoConta.SQL.Clear();
   qyPlanoConta.SQL.Add(SQL_PLANO_CONTA);
...
begin
   ...
   begin
      ...
      begin
         ...
         //Alteração feita devido ao fato de quando se tenta limpar o campo Observação, não dá update
         if (txtObservacao.Text = '') then
            (ListaParcelas.Items[i] as TFinContasPagar).setValorCampo('ds_observacao',             ' ' );
...
begin
   ...
   begin
      ...
      qryFinanceiro.ParamByName('CD_PESSOA').AsInteger := resultado_filtro.cd_pessoa;
       qryFinanceiro.Open;
       Credito := qryFinanceiro.FieldByName('VL_CREDITO').AsFloat;
       if Credito > 0 then
       Mensagem('Este fornecedor possui crédito a ser descontado.');
...
procedure TfrmCompromissoCadastro.setSituacao(codigo: string);
begin
   Self.cd_situacao := codigo;
end;
...
begin
   ...
   begin
      if(DM.convertDateTimeToDate(qryColigadasdt_bloqueio_financeiro.AsDateTime) > StrToDate(txtDtEmissao.Text)) then
      begin
