...
type
   Tfrm_Horarios = class(TForm)
    ...
    DBEdit7: TDBEdit;
    procedure tblHorariosqtd_aulasChange(Sender: TField);
...
begin
   ...
   else if horario.TemDiarioCronogramaVinculado() then
   begin
      Mensagem(
         'Você não pode excluir este horário, pois ele está vinculado à um cronograma.',
         'Aviso',
...
procedure Tfrm_Horarios.tblHorariosqtd_aulasChange(Sender: TField);
const
   SMsgQuantidadeInsuficiente = 'A quantidade de aulas deve ser maior que zero.';
begin
   if tblHorariosqtd_aulas.AsInteger <= 0 then
   begin
      MessageDlg(SMsgQuantidadeInsuficiente, mtWarning, [mbOK], 0);
      tblHorariosqtd_aulas.AsInteger := 1;
   end;
end;
