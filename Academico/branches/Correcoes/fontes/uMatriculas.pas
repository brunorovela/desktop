...
uses
  ...
  UFMatriculasTotais, uHistorico_anterior, DateUtils;
...
type
  ...
  TfrmMatriculas = class(TForm)
    ...
    tblFichaIndividualCodigo: TIntegerField;
    ...
    miHistorico: TMenuItem;
    tblMatriculasds_departamento: TStringField;
    tblMatriculasDS_CURSO: TStringField;
    tblMatriculasCD_SITUACAO_CURSO: TLargeintField;
    tblFichaIndividualdt_base_ficha: TDateTimeField;
    ToolButton7: TToolButton;
    qryTransfereFreqs: TUMZQuery;
    qryTransfereNotas: TUMZQuery;
    tblFichaIndividualdt_inclusao: TDateTimeField;
    tblMatriculasCD_PESSOA_MATRICULOU: TIntegerField;
    tblDocdt_conferencia: TDateField;
    tblDoccd_conferencia: TStringField;
    tblDocusuario_conferencia: TStringField;
    tblMatriculasds_obs: TStringField;
    tblMatriculasdataentrada: TDateTimeField;
    tblDocobservacao: TStringField;
    procedure ToolButton7Click(Sender: TObject);
    procedure miHistoricoClick(Sender: TObject);
    procedure grdMouseUp(Sender: TObject; Button: TMouseButton;
  ...
  private
    ...
    procedure TranferirTurmaDisciplina( Const disciplina, cd_turma_nova, cd_turma_antiga, cd_curso: String; anosemestre, cd_pessoa, cd_etapa: Integer);
  public
    ...
    procedure Filtrar;
    procedure remover_notas_aluno( cd_pessoa, nr_anosemestre, cd_disciplina: integer; cd_turma : String);
    procedure remover_aulas_aluno( cd_pessoa, nr_anosemestre, cd_disciplina: integer; cd_turma : String);
    { Public declarations }
  end;
...
uses
   uDM, Main, uDataSaida, uMotivos, uDig,
  ...
  uTransferir, uNovaMens, uMensalidades, uAlunoAfrica, uCursos, uTransfDiscTurma,
  ClassDisciplinas, ClassPlanosPagamento, UFMatriculasFiltroData, uUsuario,
  uIUMDataModule, uFTrocaDeTurma, uFMatriculaCursoTurma, Types, uRemessaClass;
...
//         'MATRICULAS_CURSO.*,' +
         ...
         'matriculas_curso.cd_matricula_curso, matriculas_curso.cd_pessoa, matriculas_curso.nr_matricula, matriculas_curso.cd_curso,' +
         'matriculas_curso.cd_grade, matriculas_curso.nr_anosem_grade, matriculas_curso.nr_anosem_ingresso, matriculas_curso.cd_turno,' +
         'matriculas_curso.dt_ingresso, matriculas_curso.cd_ingresso, matriculas_curso.cd_instituicao, matriculas_curso.cd_situacao,' +
         'matriculas_curso.nr_anosem_conclusao, matriculas_curso.dt_conclusao, matriculas_curso.dt_colacao, matriculas_curso.dt_exp_diploma,' +
         'matriculas_curso.dt_saida, matriculas_curso.nr_media_curso, matriculas_curso.nr_cert_folha,' +
         'matriculas_curso.nr_cert_registro, matriculas_curso.cd_cert_livro, matriculas_curso.sn_bloqueio_nr_media_curso,' +
         'matriculas_curso.sn_gerado_manual, matriculas_curso.dt_certificado, matriculas_curso.nr_serie_diploma,' +
         'matriculas_curso.cd_responsavel_registro, matriculas_curso.nr_processo, matriculas_curso.dt_publicacao_dou,' +
         'matriculas_curso.situacao_processo, matriculas_curso.ds_observacao_enade,' +
         'CAST(ds_obs AS CHAR(255)) AS ds_obs, ' +
         ' cm.ds_curso, '      +
         'CASE ' +
         ...
         'MATRICULAS_CURSO.CD_SITUACAO AS CD_SITUACAO_CURSO, ' +
      ...
      'FROM ' +
         'MATRICULAS ' +
            ...
            ' INNER JOIN cursos_coligadas ccl ON ( ccl.cd_curso = MATRICULAS.curso AND ccl.cd_coligada = TURMAS.cd_coligada ) ' +
            ' INNER JOIN cursos_mestre cm ON ( cm.CD_CURSO = ccl.cd_curso )  ' +
            ' INNER JOIN departamentos dp ON ( dp.codigo =  ccl.cd_depto ) ' +
   ...
   SFiltroEmissao = 'DATE(MATRICULAS.DATAEMISSAO) BETWEEN :INICIO AND :FIM';
   SFiltroDisciplina =
      'EXISTS (' +
         'SELECT FI.ID_FICHAINDIVIDUAL ' +
         'FROM FICHAINDIVIDUAL FI ' +
         'WHERE '+
            'MATRICULAS.TURMA = FI.TURMAMATRICULA AND ' +
...
begin
         ...
         case TipoFiltro of
            ...
            begin
               tblMatriculas.SQLList.Values[LProtocol] := Format(SSQLMatriculas,
                  ...
                  '']);
               LParams.CreateParam(ftUnknown, 'INICIO', ptUnknown).AsDateTime :=
                  fMatriculasFiltroData.dtpInicio.SelectedDate;
               LParams.CreateParam(ftUnknown, 'FIM', ptUnknown).AsDateTime :=
                  fMatriculasFiltroData.dtpFim.SelectedDate;
   ...
   begin
      filtro_dt_fim := DateToStr(fMatriculasFiltroData.dtpFim.SelectedDate);
      filtro_dt_inicio := DateToStr(fMatriculasFiltroData.dtpInicio.SelectedDate);
   end
   else
   begin
      filtro_dt_fim := '';
...
procedure TfrmMatriculas.grdKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
   ...
   procedure RemoveFinanciamento;
   const
      SSQLUpdateMensalidades = ''
         + 'UPDATE '
         + '	mensalidades m '
         + '		JOIN fin_financiamento_mensalidade f ON '
         + '			(m.cd_mensalidade = f.CD_MENSALIDADE) '
         + '		JOIN fin_financiamento_pessoa p ON '
         + '			(f.CD_FINANCIAMENTO_PESSOA = p.CD_FINANCIAMENTO_PESSOA) '
         + '		JOIN mensalidades o ON '
         + '			(f.CD_MENSALIDADE_ORIGEM = o.cd_mensalidade) '
         + 'SET '
         + '	m.datavencimento = o.datavencimento, '
         + '	m.databasecorrecao = o.databasecorrecao, '
         + '	m.cd_tipo_titulo = o.cd_tipo_titulo, '
         + '	m.cd_coligada = o.cd_coligada, '
         + '	m.cd_resp = o.cd_resp '
         + 'WHERE '
         + '	p.CD_PESSOA = ? AND '
      ...
      SSQLDeleteMensalidades = ''
         ...
         + '	( '
         + '		SELECT '
         + '			p.CD_FINANCIAMENTO_PESSOA '
         + '		FROM '
         + '			fin_financiamento_pessoa p '
         + '		WHERE '
         + '			m.CD_FINANCIAMENTO_PESSOA = p.CD_FINANCIAMENTO_PESSOA AND '
         + '			p.CD_PESSOA = ? AND '
         + '			p.NR_ANOSEMESTRE = ? AND '
         + '			p.CD_TURMA = ? '
         + '	) ';
      SSQLDeleteFinanciamento = ''
         + 'DELETE '
         + '	p '
         + 'FROM '
         + '	fin_financiamento_pessoa p '
   ...
   begin
      Stmt := PrepareStatement(SSQLUpdateMensalidades);
      try
         Stmt.SetInt(1, tblMatriculasCODIGOALUNO.AsInteger);
         Stmt.SetInt(2, tblMatriculasAnoSemestre.AsInteger);
         Stmt.SetString(3, tblMatriculasTurma.AsString);
      ...
      Stmt := PrepareStatement(SSQLDeleteMensalidades);
      try
         Stmt.SetInt(1, tblMatriculasCODIGOALUNO.AsInteger);
         Stmt.SetInt(2, tblMatriculasAnoSemestre.AsInteger);
         Stmt.SetString(3, tblMatriculasTurma.AsString);
      ...
      Stmt := PrepareStatement(SSQLDeleteFinanciamento);
      try
         Stmt.SetInt(1, tblMatriculasCODIGOALUNO.AsInteger);
         Stmt.SetInt(2, tblMatriculasAnoSemestre.AsInteger);
         Stmt.SetString(3, tblMatriculasTurma.AsString);
...
begin
  ...
  Begin
      ...
      while not DM.qAux1.EOF do begin
         ...
         begin
           ...
           objRemessa.registraAcaoCancelamento(DM.qAux1.FieldByName('nossonumero').AsString);
         end;
      ...
      Dm.qAux1.Close();
      DM.qAux1.SQL.Clear();
      DM.qAux1.SQL.Add( 'DELETE fmc FROM fin_mov_cr fmc ' );
      DM.qAux1.SQL.Add( ' WHERE fmc.cd_mensalidade IN (( ');
      DM.qAux1.SQL.Add( ' SELECT m.cd_mensalidade FROM mensalidades m WHERE  m.codigoaluno = :codigoaluno AND m.turma = :turma AND' );
      ...
      Dm.qAux1.Close();
      DM.qAux1.SQL.Clear();
      DM.qAux1.SQL.Add( ' DELETE m FROM mensalidades m ' );
      ...
      // REMOVE AS NOTAS E AULAS DO ALUNO
      remover_notas_aluno(
  ...
  if ( Key = ord('A') ) and ( Shift = [ssCtrl, ssShift, ssAlt] ) AND (not tblMatriculas.IsEmpty) then  Begin
    ...
    while not DM.qAux1.Eof do begin
       ...
       frmProcessando.lblRegistros.Caption := sErro + 'Registros: ' + IntToStr(i) + ' de ' + IntToStr( frmProcessando.Bar.Max );
...
procedure TfrmMatriculas.grdMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
var
   Right, Bottom: Integer;
   P: TPoint;
begin
   if (Button = mbRight) and (not tblMatriculas.IsEmpty) then
   begin
      P := Mouse.CursorPos;

      if frmMatriculasTotais = nil then
         Application.CreateForm(TfrmMatriculasTotais, frmMatriculasTotais);

      frmMatriculasTotais.Load;

      frmMatriculasTotais.Left := P.X;
      frmMatriculasTotais.Top := P.Y - frmMatriculasTotais.Height;

      Right := frmMatriculasTotais.Left + frmMatriculasTotais.Width;
      Bottom := frmMatriculasTotais.Top + frmMatriculasTotais.Height;

      if Right > Screen.Width then
         frmMatriculasTotais.Left := Screen.Width - frmMatriculasTotais.Width;

      if Bottom > Screen.Height then
         frmMatriculasTotais.Top := Screen.Height - frmMatriculasTotais.Height;

      if frmMatriculasTotais.Top < 0 then
         frmMatriculasTotais.Top := 0;

      frmMatriculasTotais.Show;
   end;
end;
...
begin
  ...
  Application.CreateForm( TfrmTransfDisc, frmTransfDisc );
...
//  grd.Columns[0].Visible :=  (DM.variavel_parametro('matriculas_apresentar_nro_aluno') = 'S');
  ...
  S := Format(SSettingKeyFmt, [Name, tblMatriculas.Name]);
  tblMatriculas.IndexFieldNames := DM.GetUserSetting(S, 'Academico', tblMatriculas.IndexFieldNames);
...
CONST
   ...
   SQL_CURSOS_DOCUMENTOS = ''+
      ...
      '  d.cd_siga ,             '+
      '  da.dt_conferencia,      '+
      '  pec.nm_pessoa as usuario_conferencia, '+
      '	 CASE '+
      '   WHEN da.cd_conferencia = 1 then "Conferido" '+
      '	  WHEN da.cd_conferencia = 2 then "Rejeitado" '+
      '   WHEN da.apresentou = ''S'' and da.sn_digitalizado = ''S'' then "Não Conferido" '+
      '   ELSE "" END AS cd_conferencia '+
      'FROM                      '+
      '  documentos_alunos da    '+
      '  INNER JOIN pessoas pe ON (da.codigoaluno = pe.cd_pessoa) '+
      '  LEFT JOIN pessoas pec ON (da.usuario_conferencia = pec.cd_pessoa) '+
      '  LEFT JOIN documentos_necessarios dd ON (da.cod_documento = dd.cod_documento AND dd.curso = :curso) '+
        ...
        begin
          ...
          if tblGradeAnoSemestre.AsInteger <> tblMatriculasAnoSemestre.AsInteger then begin
              ...
              begin
                 ...
                 // Salva a data da matricula da disciplina na fichaindividual
                 DM.tblFichaIndividualdt_matricula.AsDateTime := DM.CurrDateFromDatabase;

                 // Salva a data de inclusão do registro da fichaindividual
                 DM.tblFichaIndividualdt_inclusao.AsDateTime := Now();
              ...
              begin
                 ...
                 qyAlterarDiscExistente.FieldByName('dt_saida').Clear;
...
begin
  ...
  // REMOVE AS NOTAS E AULAS DA DISCIPLINA QUE O ALUNO ESTA SENDO EXCLUÍDO
   remover_notas_aluno(
      tblFichaIndividualCodigoAluno.AsInteger,
      tblFichaIndividualAnoSemestre.AsInteger,
      tblFichaIndividualDisciplina.AsInteger,
...
procedure TfrmMatriculas.remover_aulas_aluno(cd_pessoa, nr_anosemestre, cd_disciplina: integer; cd_turma : String);
CONST
   DS_REMOVER_AULAS_ALUNOS = ''
      + ' DELETE FROM '
      + '   diario_aulas_alunos '
      + ' WHERE '
      ...
      + '   nr_anosem  = :nr_anosem  '  ;
var
   qry_remover_aulas : TUMZQuery;
begin
   DM.CriarConsulta(qry_remover_aulas);

   qry_remover_aulas.Close;
   qry_remover_aulas.SQL.Clear;

   qry_remover_aulas.SQL.TEXT := DS_REMOVER_AULAS_ALUNOS;

   if ( cd_disciplina <> 0 )  then
   begin
       qry_remover_aulas.SQL.Add(
        ' AND cd_disciplina = :cd_disciplina'
       );
       qry_remover_aulas.ParamByName('cd_disciplina').AsInteger := cd_disciplina;
   end;

   qry_remover_aulas.ParamByName('cd_pessoa').AsInteger := cd_pessoa;
   qry_remover_aulas.ParamByName('cd_turma').AsString := cd_turma;
   qry_remover_aulas.ParamByName('nr_anosem').AsInteger := nr_anosemestre;

   qry_remover_aulas.ExecSQL;
   qry_remover_aulas.Free;
end;

procedure TfrmMatriculas.remover_notas_aluno(cd_pessoa, nr_anosemestre, cd_disciplina: integer; cd_turma : String);
CONST
   DS_REMOVER_NOTAS_ALUNOS = ''
      + ' DELETE FROM '
...
var
   qry_remover_notas : TUMZQuery;
begin
   DM.CriarConsulta(qry_remover_notas);

   qry_remover_notas.Close;
   qry_remover_notas.SQL.Clear;

   qry_remover_notas.SQL.TEXT := DS_REMOVER_NOTAS_ALUNOS;

   if ( cd_disciplina <> 0 )  then
   begin
       qry_remover_notas.SQL.Add(
        ' AND cd_disciplina = :cd_disciplina'
       );
       qry_remover_notas.ParamByName('cd_disciplina').AsInteger := cd_disciplina;
   end;

   qry_remover_notas.ParamByName('cd_pessoa').AsInteger := cd_pessoa;
   qry_remover_notas.ParamByName('cd_turma').AsString := cd_turma;
   qry_remover_notas.ParamByName('nr_anosem').AsInteger := nr_anosemestre;
   
   qry_remover_notas.ExecSQL;

   qry_remover_notas.Free;
end;
...
begin
  ...
  S := Format(SSettingKeyFmt, [Name, tblMatriculas.Name]);
  DM.SetUserSetting(S, tblMatriculas.IndexFieldNames, 'Academico');
...
begin
    ...
    Begin
        ...
        DM.setLog(
           ...
           + ';'           
           + tblDocDocumento.AsString,
           DM.getColigadaByTurma(
...
var
   ...
   AnoSemestre: Integer;
begin
   if tblMatriculas.IsEmpty then
   begin
      TurmaId := sFiltroTurma;
      AnoSemestre := StrToIntDef(sFiltroAnosemestre, DM.GetAnoSemestreAtual);
   end;

   if not tblMatriculas.IsEmpty then
   begin
      TurmaId := tblMatriculasTurma.AsString;
      AnoSemestre := tblMatriculasAnoSemestre.AsInteger;
   end;

   if DM.UsuarioLogado.TemPermissao(1006, npAcesso, True) then
   begin
      if not PrincipalForm.ProcuraForm(frmCursos) then
      begin
         Application.CreateForm(TfrmCursos, frmCursos);
      end;
      frmCursos.LocateTurma(TurmaId, AnoSemestre);
      frmCursos.Show;
   end;
...
procedure TfrmMatriculas.CancelarMatrcula1Click(Sender: TObject);
   ...
   procedure AntecipaFinanciamento;
   const
      SSQLSelectMensalidades = ''
         ...
         + '					(f.CD_FINANCIAMENTO_PESSOA = p.CD_FINANCIAMENTO_PESSOA) '
         + '		WHERE '
         + '			m.cd_mensalidade = f.CD_MENSALIDADE AND '
         + '			p.CD_PESSOA = ? AND '
         + '			p.NR_ANOSEMESTRE = ? AND '
         + '			p.CD_TURMA = ? '
         + '	) '
   ...
   var
      ...
      IncDate, NewDate: TDate;
   begin
      IncDate := Date;
      DecodeDate(IncDate, Year, Month, Day);
      IncDate := EncodeDate(Year, Month, 1);
      ...
      try
         ...
         try
            while Rs.Next do
            begin
               DecodeDate(Rs.GetDateByName('DT_VENCTO'), Year, Month, NewDay);
               IncDate := IncMonth(IncDate, 1);
               DecodeDate(IncDate, Year, Month, Day);
               try
                  NewDate := EncodeDate(Year, Month, NewDay);
               except
                  on E: Exception do
                     NewDate := EndOfAMonth(Year, Month);
               end;
               Rs.UpdateDateByName('DT_VENCTO', NewDate);
               Rs.UpdateRow;
            end;
         finally
            Rs.Close;
   ...
   function VerificaFinanciamento: Boolean;
   const
      SCaption = 'Confirmação';
      SMsg = ''
         + 'A matrícula selecionada está vinculada com um programa de '
         + 'financiamento estudantil. Ao confirmar esta ação, o aluno será '
         + 'excluído do programa de financiamento e a data de vencimento de '
         + 'cada um dos títulos financeiros resultantes desse financiamento '
         + 'será antecipada, iniciando no próximo mês. Deseja realmente '
         + 'continuar?';

      SSQLSelectFinanciamento = ''
         + 'SELECT '
         + '	1 '
         + 'FROM '
         + '	fin_financiamento_pessoa f '
         + 'WHERE '
         + '	f.NR_ANOSEMESTRE = ? AND '
         + '	f.CD_TURMA = ? AND '
         + '	f.CD_PESSOA = ? AND '
         + '	f.SN_ATIVO = 1 ';
   var
   ...
   begin
      ...
      Stmt := PrepareStatement(SSQLSelectFinanciamento);
      try
         Stmt.SetInt(1, tblMatriculasAnoSemestre.AsInteger);
         Stmt.SetString(2, tblMatriculasTurma.AsString);
         Stmt.SetInt(3, tblMatriculasCODIGOALUNO.AsInteger);
         Rs := Stmt.ExecuteQueryPrepared;
         try
            if Rs.Next then
            begin
               Result := MessageBox(Handle, SMsg, SCaption,
   ...
   function GetListaResponsaveisNaoCancelados(const AlunoId,
      AnoSemestre: Integer; const TurmaId: AnsiString): AnsiString;
   const
      SFilterTipoTituloFmt = ' AND m.cd_tipo_titulo IN (%s) ';
      SSQLSelectResp = ''
         + 'SELECT '
         + '	r.nm_pessoa '
         + 'FROM '
         + '	pessoas r '
         + 'WHERE '
         + '	r.sn_nao_bloquear_financeiro = 1 AND '
         + '	EXISTS '
         + '	( '
         + '		SELECT '
         + '			m.cd_mensalidade '
         ...
         + '			m.anosemestre = ? AND '
         + '			m.tipoparcela != 4 AND '
         + '			m.situacao IN (10%s) AND '
         + '			m.cd_resp = r.cd_pessoa %s'
         + '	) '
         + 'ORDER BY '
         + '	r.nm_pessoa ';
   var
   ...
   begin
      ...
      S := DM.variavel_parametro('financeiro_cancelar_pendente');

      if S = 'S' then
      begin
         S := DM.variavel_parametro('financeiro_cancelar_vencida');
      ...
      S := DM.variavel_parametro('financeiro_cancelar_titulos');
      S := Trim(S);

      if S <> '' then
      begin
         S2 := Format(SFilterTipoTituloFmt, [S]);
      end;

      Result := '';

      S := Format(SSQLSelectResp, [S1, S2]);
      ...
      try
         ...
         try
            ...
            begin
               if Result <> '' then
                  Result := Result + #13;
               Result := Result + Rs.GetStringByName('nm_pessoa');
            end;
         finally
            Rs.Close;
            Rs := nil;
...
const
   ...
   SSQLSelectMensalidades = ''
      ...
      + '	mensalidades m '
      + '		JOIN pessoas r ON '
      + '			(m.cd_resp = r.cd_pessoa) '
      + 'WHERE '
      + '	m.codigoaluno = :codigo AND '
      + '	m.turma = :turma AND '
      + '	m.anosemestre = :anosemestre AND '
      + '	m.tipoparcela != 4 AND '
      + '	m.situacao IN (%s) AND '
      + '	r.sn_nao_bloquear_financeiro = 0 ';

   SMsgRespBloqueados = ''
      + 'O Aluno possui mensalidades que não foram canceladas, pois estão '
      + 'relacionadas com os seguintes responsáveis financeiros:'#13'%s';
...
var
  ...
  bCancelar, bCancelamentosImpedidos, bCancelarDDA , continua, BloqueouCancelamento: Boolean;
...
//  Then exit;
   ...
   if TPopupMenu(Sender).Tag in [3, 4] then
   begin
      if not VerificaFinanciamento then
      begin
         Screen.Cursor := crDefault;
         Exit;
      end;

      AntecipaFinanciamento;
      RemoveFinanciamento;
   end;
...
//     frmDataSaida.ShowModal;
     ...
     if ((Dm.variavel_parametro('financeiro_cancelar_pendente') = 'S') or
         ...
         DM.tblMensalidades.SQL.Text := Format(SSQLSelectMensalidades, ['2, 10']);
     end else begin
        // Cancelar somente reservas
        DM.tblMensalidades.SQL.Text := Format(SSQLSelectMensalidades, ['10']);
     end;
     ...
     if sTitulos <> '' then begin
        ...
        sTitulos := ' AND (m.cd_tipo_titulo = ' + sTitulos;
        sTitulos := StringReplace(sTitulos, ',', ' OR m.cd_tipo_titulo = ', [rfReplaceAll]);
        sTitulos := sTitulos + ' )';
     ...
     if not DM.tblMensalidades.EOF then begin
        ...
        Begin
           ...
           if bCancelar then begin
             ...
             bCancelarDDA :=
               NovoProcessoRemessa and
               (TPopupMenu(Sender).Tag in [3, 4, 5, 6, 7]) and

               objRemessa.verificaAcaoEstaFila(Dm.tblMensalidades.FieldByName('nossonumero').AsString, tarBaixarDDA);

             if bCancelarDDA then
             begin
               objRemessa.CancelaDDA(
                  Dm.tblMensalidades.FieldByName('nossonumero').AsString);
             end;

             if NovoProcessoRemessa and (not bCancelarDDA) then
             begin
               // registra a nova ação de cancelamento no novo modelo de remessa
               objRemessa.registraAcaoCancelamento(Dm.tblMensalidades.FieldByName('nossonumero').AsString);
             end;

             // ajuste da remessa quando mensalidade consta no DA
             if TPopupMenu(Sender).Tag IN [3,4,5,6,7]  then
             begin
             ...
             Movimento.MensalidadeOrigem := Dm.tblMensalidadescd_mensalidade.AsInteger;
...
//     Mensagem('Você deverá registrar um motivo para alteração da situação da matrícula do aluno.', 'Informação', MB_OK + MB_ICONINFORMATION);
   ...
   if BloqueouCancelamento then
   begin
      S := GetListaResponsaveisNaoCancelados(
         tblMatriculasCODIGOALUNO.AsInteger,
         tblMatriculasAnoSemestre.AsInteger,
         tblMatriculasTurma.AsString);
      ...
      if S <> '' then
      begin
         S := Format(SMsgRespBloqueados, [S]);
         MessageBox(Handle, PChar(S), SCaptionWarn,
...
begin
   ...
   begin
      ...
      if not(tblFichaIndividualSituacao.AsInteger in [3, 4, 5, 6, 7, 8, x]) and
            ( ClasseDisciplinas.IsUltimaDisciplinaAtiva(
               tblFichaIndividualAnoSemestre.AsInteger,
               tblFichaIndividualTurmaMatricula.AsString,
               tblFichaIndividualDisciplina.AsInteger,
               tblFichaIndividualCodigoAluno.AsInteger,
               tblFichaIndividualTurma.AsString
            )
...
procedure TfrmMatriculas.miHistoricoClick(Sender: TObject);
begin
   ...
   begin
      ...
      frm_historico_anterior.codigoaluno := tblMatriculasCODIGOALUNO.AsInteger;
      frm_historico_anterior.codigoMatriculaCurso := tblMatriculascd_matricula_curso.AsLargeInt;
      frm_historico_anterior.txtAluno.Text := tblMatriculasNome.AsString;
      frm_historico_anterior.txtCurso.Text := tblMatriculasCurso.AsString;
      frm_historico_anterior.Filtra;
   end;
end;
...
begin
   ...
   begin
      HintFiltro := Format(SHintFiltroData, [
         DateToStr(fMatriculasFiltroData.dtpInicio.SelectedDate),
         DateToStr(fMatriculasFiltroData.dtpFim.SelectedDate)]
      );
...
procedure TfrmMatriculas.TranferirTurmaDisciplina(const disciplina, cd_turma_nova, cd_turma_antiga,
  cd_curso: String; anosemestre, cd_pessoa, cd_etapa: Integer);
CONST
   SQL_TRANSFERIR_TURMA =
   ' UPDATE  '
   + '    fichaindividual fi '
   + ' SET  '
   + '    fi.turma = :turma, '
   ...
   + '    fi.curso = :cd_curso '
   + ' AND '
   + '    fi.anosemestre = :anosemestre '
   + ' AND  '
   + '    fi.codigoaluno = :cd_pessoa '
   + ' AND  '
   + '    fi.turma = :cd_turma_antiga ';

   SQL_TRANSFERIR_TURMA_OPT =
   ' UPDATE  '
   + '    fichaindividual_optativas fi '
   + ' SET  '
   + '    fi.turma = :turma '
   + ' WHERE    '
   + '    fi.disciplina = :disciplina '
...
var
   qryTransferirTurma : TUMZQuery;
   teste : TformTransferenciaDeTurma;
begin
   DM.CriarConsulta(qryTransferirTurma);

   qryTransferirTurma.SQL.Clear;
   qryTransferirTurma.Close();
   qryTransferirTurma.SQL.Add(SQL_TRANSFERIR_TURMA);

   qryTransferirTurma.ParamByName('turma').AsString := cd_turma_nova;
   qryTransferirTurma.ParamByName('serie').AsInteger := cd_etapa;
   qryTransferirTurma.ParamByName('cd_turma_antiga').AsString := cd_turma_antiga;
   qryTransferirTurma.ParamByName('disciplina').AsString := disciplina;
   qryTransferirTurma.ParamByName('cd_curso').AsString := cd_curso;
   qryTransferirTurma.ParamByName('anosemestre').AsInteger := anosemestre;
   qryTransferirTurma.ParamByName('cd_pessoa').AsInteger := cd_pessoa;

   qryTransferirTurma.ExecSQL();

   qryTransferirTurma.SQL.Clear;
   qryTransferirTurma.Close();
   qryTransferirTurma.SQL.Add(SQL_TRANSFERIR_TURMA_OPT);
   ...
   qryTransferirTurma.Free;

   if Dm.variavel_parametro('TransferenciaLevarNotasParciais') = 'S' then
   begin

//      qryTransfereFreqs.SQL.Add(' AND daa.cd_disciplina = :CD_DISCIPLINA');
      qryTransfereFreqs.ParamByName('CD_PESSOA').AsString := IntToStr(cd_pessoa);
      qryTransfereFreqs.ParamByName('CD_TURMA').AsString := cd_turma_antiga;
      qryTransfereFreqs.ParamByName('NR_ANOSEM').AsInteger := anosemestre;
      qryTransfereFreqs.ParamByName('CD_TURMA_NOVA').AsString := cd_turma_nova;
      qryTransfereFreqs.ParamByName('NR_ANOSEM_NOVO').AsInteger := anosemestre;
      qryTransfereFreqs.ParamByName('CD_DISCIPLINA').AsString := disciplina;
      qryTransfereFreqs.ExecSQL;

//      qryTransfereNotas.SQL.Text :=  qryTransfereNotas.SQL.Text + ' AND cd_disciplina = :CD_DISCIPLINA';
      qryTransfereNotas.ParamByName('CD_PESSOA').AsString := IntToStr(cd_pessoa);
      qryTransfereNotas.ParamByName('CD_TURMA').AsString := cd_turma_antiga;
      qryTransfereNotas.ParamByName('NR_ANOSEM').AsInteger := anosemestre;
      qryTransfereNotas.ParamByName('CD_TURMA_NOVA').AsString := cd_turma_nova;
      qryTransfereNotas.ParamByName('NR_ANOSEM_NOVO').AsInteger := anosemestre;
      qryTransfereNotas.ParamByName('CD_DISCIPLINA').AsString := disciplina;
...
begin
  ...
  if formTransferenciaDeTurma = nil then
...
begin
   ...
   begin
      ...
      Dm.qAux1.Close;
      Dm.qAux1.SQL.Text := 'SELECT 1 as ID, pb.* FROM pessoas_bolsas pb';
      Dm.qAux1.SQL.Add(' WHERE cd_pessoa = :codigoaluno AND cd_curso = :curso');
      Dm.qAux1.SQL.Add(' ORDER BY nr_ano_censo DESC limit 1');
      ...
      if (Dm.qAux1.FieldByName('ID').Value > 0 ) then
      begin

         SBolsasTipos:=TStringList.Create;
         SBolsasTipos.QuoteChar := ' ';

         if (Dm.qAux1.FieldByName('sn_re_fies').Value = 1 ) then
         begin
            SBolsasTipos.Add('- FIES;');
         end;

         if ((Dm.qAux1.FieldByName('sn_re_governo_estadual').Value = 1) OR (Dm.qAux1.FieldByName('sn_nre_governo_estadual').Value = 1) ) then
         begin
            SBolsasTipos.Add('- Governo Estadual;');
         end;

         if ((Dm.qAux1.FieldByName('sn_re_governo_municipal').Value = 1) OR (Dm.qAux1.FieldByName('sn_nre_governo_municipal').Value = 1) ) then
         begin
            SBolsasTipos.Add('- Governo Municipal;');
         end;

         if ((Dm.qAux1.FieldByName('sn_re_ies').Value = 1) OR (Dm.qAux1.FieldByName('sn_nre_ies').Value = 1) ) then
         begin
            SBolsasTipos.Add('- IES;');
         end;

         if ((Dm.qAux1.FieldByName('sn_re_entidades_externas').Value = 1) OR (Dm.qAux1.FieldByName('sn_nre_entidades_externas').Value = 1) ) then
         begin
            SBolsasTipos.Add('- Entidades Externas;');
         end;

         if ((Dm.qAux1.FieldByName('sn_re_outros').Value = 1) OR (Dm.qAux1.FieldByName('sn_nre_outros').Value = 1) ) then
         begin
            SBolsasTipos.Add('- Outros;');
         end;

         if ((Dm.qAux1.FieldByName('sn_nre_prouni_integral').Value = 1) OR (Dm.qAux1.FieldByName('sn_nre_prouni_parcial').Value = 1) ) then
         begin
            SBolsasTipos.Add('- Prouni;');
         end;
         ...
         if (Dm.qAux1.FieldByName('sn_parfor').Value = 1 ) then
         begin
            SBolsasTipos.Add('- PARFOR;');
         end;

         if (Dm.qAux1.FieldByName('sn_bolsa_educa_mais_brasil').Value = 1 ) then
         begin
            SBolsasTipos.Add('- EDUCAMAIS BRASIL;');
         end;

         SBolsasTipos.Delimiter := #13;

         SMsgConfirm:= 'Este aluno está vinculado aos seguintes tipos de financiamento estudantil no ano de ' + IntToStr(Dm.qAux1.FieldByName('nr_ano_censo').AsInteger) + ':' + #13;
         SMsgConfirm:= SMsgConfirm + SBolsasTipos.DelimitedText;
         SMsgConfirm:= SMsgConfirm + #13#13 + 'Deseja realmente prosseguir?';
      ...
      if ((Self.tblMatriculasSituacao.AsInteger in [0, 9]) OR
         (Self.tblMatriculasCD_SITUACAO_CURSO.AsInteger <> 1)) then
      begin

         Mensagem(
            'Este aluno não pode realizar a transferência de curso, pois o mesmo'+
            ' não está com a situação do curso como Cursando!' +
            #13 + 'Para o caso deste aluno deverá ser feita uma nova matrícula em um outro curso!',
            'Atenção',
...
case  iTipo of
  ...
  1 : begin
      if numerarCurso = False then
        if Mensagem(
            'Tem certeza que deseja renumerar a turma selecionada?',
            'Confirmação',
        ...
        with DM.qAux2 do begin
            ...
            SQL.Add(' INNER JOIN situacao s ON (s.cd_situacao = m.situacao) ');
            SQL.Add(' SET m.nr_aluno = 0 ');
            SQL.Add(' WHERE m.turma = :CdTurma AND m.anosemestre = :NrAnoSem ');
            SQL.Add('   AND s.cd_situacao_pai IN (3,4,5,6,7,8) ');
        ...
        with DM.qAux2 do begin
            ...
            SQL.Clear;
            SQL.Add(' SELECT ');
            SQL.Add('    m.codigoaluno, m.turma, m.anosemestre, p.nm_pessoa ');
            SQL.Add(' FROM matriculas m, pessoas p, situacao s ');
            SQL.Add(' WHERE m.codigoaluno = p.cd_pessoa ');
            SQL.Add('   AND s.cd_situacao =  m.situacao ');
            SQL.Add('   AND m.turma = :CdTurma AND m.anosemestre = :NrAnoSem ');
            SQL.Add('   AND s.cd_situacao_pai NOT IN (3,4,5,6,7,8) ');
...
procedure TfrmMatriculas.ToolButton7Click(Sender: TObject);
begin

   frmTransfDisc.disciplina  := tblFichaIndividualCodigo.AsString;
   frmTransfDisc.anosemestre := tblFichaIndividualAnoSemestre.AsInteger;
   frmTransfDisc.turma       := tblFichaIndividualTurma.AsString;
   frmTransfDisc.cd_pessoa   := tblFichaIndividualCodigoAluno.AsInteger;
   frmTransfDisc.curso       := tblFichaIndividualCurso.AsString;
   frmTransfDisc.etapa       := tblFichaIndividualSerie.AsInteger;
   frmTransfDisc.lblNomeAluno.Caption := tblMatriculasNome.AsString;
   frmTransfDisc.txtAnoSemestre.Text := tblMatriculasAnoSemestre.AsString;


   if frmTransfDisc.ShowModal = mrOk then
   begin
      TranferirTurmaDisciplina(
         tblFichaIndividualCodigo.AsString,
         frmTransfDisc.qryListaTurmaDisccd_turma.AsString,
         tblFichaIndividualTurma.AsString,
         tblFichaIndividualCurso.AsString,
         tblFichaIndividualAnoSemestre.AsInteger,
         tblFichaIndividualCodigoAluno.AsInteger,
         frmTransfDisc.qryListaTurmaDiscetapa.AsInteger
      );

      // Atualiza a listagem de disciplinas
      dtcMatriculasDataChange(nil, nil);
   end;
