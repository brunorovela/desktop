...
uses
  ...
  ClassRegistros, uItemCombo, UMAjuda, ZAbstractRODataset, ZAbstractDataset;
...
type
  ...
  private
    ...
    function GetSaidaProdutos: Boolean;
    procedure SetSaidaProdutos(const Value: Boolean);
    procedure atualizarValores();
    function getQdeAcoesDescontosCondicionais(objParcela: TMBParcela): Integer;
    function getValorMinOperacao() : double;
  ...
  public
   ...
   function verifica_caixa_fechado( cd_admin_cartao : Integer; field : String ): Boolean;
    ...
    function validarMensalidadesBaixadas() : Boolean;
    ...
    property SaidaProdutos: Boolean read GetSaidaProdutos write SetSaidaProdutos;
    { Public declarations }
  end;
...
uses uFSelecionarPessoa, Main, uImpMens, UConfirmacaoBaixa, uMensalidadeBaixarCartao,
     uUsuario, uMensalidadeAplicarDesconto, uMensalidades;
...
begin
   sql_busca_conta_automatica := ''
      +' SELECT  '
      +' fccon.tp_conta tp_conta,  fccon.cd_caixa, '
      +' 	%s '
      +' FROM '
      +' fin_cadastro_admin_cartao fcac '
      +'INNER JOIN fin_cadastro_contas fccon ON ( '
      +'	  $$condicao$$ = fccon.cd_caixa '
      +' ) '
      +' INNER JOIN fin_cadastro_contas_tipos t ON ( '
      +'	 t.cd_tipo = fccon.tp_conta '
      +' ) '
      +' WHERE '
   ...
   bCaixaFechado := false;
   if  ( qyVerificaCaixaFechado.FieldByName('tp_conta').AsInteger = 3 ) then
   begin
      qryControleCaixa.Close;
      qryControleCaixa.ParamByName('cd_conta_banco').AsInteger := qyVerificaCaixaFechado.FieldByNAme('cd_caixa').AsInteger;
      qryControleCaixa.Open();
      qryControleCaixa.Locate('cd_conta_banco', qyVerificaCaixaFechado.FieldByNAme('cd_caixa').AsString , []);

      if (UpperCase( qryControleCaixa.FieldByName('ds_situacao').AsString ) = 'F') then
      begin
         bCaixaFechado := True;
      end;
   end;
...
begin
   ...
   begin
      ...
      begin
         ...
         sgGridParcelas.Cols[4][0]     := 'Resp. Parc.';
         sgGridParcelas.Cols[5][0]     := 'Vl. Nominal';
         sgGridParcelas.Cols[6][0]     := 'Vl. Desc';
         sgGridParcelas.Cols[7][0]     := 'Vl. Juros';
         sgGridParcelas.Cols[8][0]     := 'Vl. Total';
         sgGridParcelas.Cols[9][0]     := 'Vl. Pago';
         ...
         begin
            ...
            sgGridParcelas.Cols[4][i+1]   := objParcela.getValorCampo('resp_mensalidade');
            sgGridParcelas.Cols[5][i+1]   := objParcela.getValorCampo('vl_nominal');
            sgGridParcelas.Cols[6][i+1]   := objParcela.getValorCampo('valordesconto');
            sgGridParcelas.Cols[7][i+1]   := objParcela.getValorCampo('valorjuros');
            sgGridParcelas.Cols[8][i+1]   := objParcela.getValorCampo('valortotal');
            sgGridParcelas.Cols[9][i+1]   := objParcela.getValorCampo('valorpago');
         end;
...
begin
   ...
   if cbAdministradoras.ItemIndex = -1 then Exit;
   ...
   //se a tarifa for menor que o valor minimo e diferente de 0 seta o valor
   //minimo como tarifa (diferente de 0 pois pode configurar tarifa credito 0)
   if (valorTaxa < getValorMinOperacao) AND (valorTaxa <> 0) then
   begin
     ValorTaxa := getValorMinOperacao;
   end;
...
begin
   ...
   begin
      ...
      //Quando vem da baixa manual do retorno, a grid calculava o juros e desconto com a data atual
      //Essa função atualiza a grid com a data que está na txtDataBaixa.Text
      txtDataBaixaExit(nil);
   end;
end;
...
const
   SSQLSaidaProdutos = ''
      ...
      + '      m.situacao, '
      + '      m.depto, '
      + '      COALESCE(m.valorjuros, 0) valorjuros, '
      + '      m.turma, '
      + '      m.anosemestre, '
      + '      m.valorbruto + COALESCE(m.valorextra, 0) - COALESCE(m.descontoextra, 0) vl_nominal, '
      + '      GET_DESCONTOS(m.cd_mensalidade, CURRENT_DATE) desconto_calc, '
      + '      COALESCE(m.valorjuros, 0) + COALESCE(GET_ACRESCIMOS(m.cd_mensalidade, CURRENT_DATE), 0) juros_calc, '
      + '      ( '
      + '            m.valorbruto + '
      ...
      + '            GET_ACRESCIMOS(m.cd_mensalidade, CURRENT_DATE) '
      + '      ) valortotal, '
      + '      t.ds_tipo_titulo, '
      + '      p.nm_pessoa resp_mensalidade '
      + 'FROM '
      + '      mensalidades m '
      + '            JOIN pessoas p ON '
      ...
      + '                        JOIN comp_estoque e ON '
      + '                              (em.CD_COMPRA = e.CD_COMPRA) '
      + '            WHERE '
      + '                  m.cd_mensalidade = em.CD_MENSALIDADE AND '
      + '                  e.TP_ENTRADA_SAIDA = 2 '
      + '      ) '
      + 'ORDER BY '
      + '      m.datavencimento, '
      + '      m.parcela ';
...
begin
   ...
   if SaidaProdutos then
   begin
      qyMensalidades.SQL.Text := SSQLSaidaProdutos;
      qyMensalidades.ParamByName('codigoaluno').AsString := cd_pessoa;
      qyMensalidades.Open;
   end;

   if not SaidaProdutos then
   begin
      ...
      '  tt.ds_tipo_titulo, p.nm_pessoa resp_mensalidade                                       '+
      'FROM mensalidades as me                                                                 '+
      ' INNER JOIN pessoas p ON ( p.cd_pessoa = me.cd_resp ) ' +
...
function TfrmMensalidadesBaixar.getValorMinOperacao: Double;
var
  qyAdminCartao: TUMZQuery;
   vl_min_operacao: Double;
begin
      Result:= 0;
      DM.CriarConsulta( qyAdminCartao );
      qyAdminCartao.Close();
      qyAdminCartao.SQL.Clear();
      qyAdminCartao.SQL.Add(' SELECT vl_min_operacao ');
      qyAdminCartao.SQL.Add(' FROM fin_cadastro_admin_cartao ');
      qyAdminCartao.SQL.Add(' WHERE ds_nome = :ds_nome ');
      qyAdminCartao.ParamByName('ds_nome').AsString := Self.cbAdministradoras.text;
      qyAdminCartao.Open;
      vl_min_operacao := qyAdminCartao.FieldByName('vl_min_operacao').AsFloat;

      Result:= StrToFloat(FormatFloat('###########.0,00',vl_min_operacao));
end;

procedure TfrmMensalidadesBaixar.btnBaixarClick(Sender: TObject);
const
   SMsgErrorFmt = ''
      + 'Ocorreu um erro durante o processo de baixa do título financeiro. Não '
      + 'foi possível concluir o processo.'#13'%s';
   SCaptionError = 'Erro';
var
   S: AnsiString;
   vl_min_operacao: Double;
   vl_min_operacao_formatado: string;
begin

   // Se as mensalidades não estão válidas a serem baixadas não efetua a baixa
   if (not validarMensalidadesBaixadas()) then Exit;
   ...
   if (cbAdministradoras.text <> '') then
   begin

       if (getValorMinOperacao > StrToFloat( StringReplace(txtCartao.text, '.' , '' , [rfReplaceAll] ) ) ) then
       begin
         vl_min_operacao := getValorMinOperacao;
         vl_min_operacao_formatado := FormatFloat('#######,0.000,00',vl_min_operacao);
         Mensagem('Você não pode baixar essa mensalidade, valor mínimo configurado da administradora do cartão é: R$: ' + vl_min_operacao_formatado );
         frmMensalidadesBaixar.Show;
         ativaBotaoBaixar( true );
        Exit
       end;
   end;
         ...
         begin
            ...
            begin
               ...
               try
                  ...
                  begin
                     //DM.db.Rollback;
                     S := Format(SMsgErrorFmt, [E.Message]);
                     MessageBox(Handle, PChar(S), SCaptionError,
...
procedure TfrmMensalidadesBaixar.SetSaidaProdutos(const Value: Boolean);
begin
   FSaidaProdutos := Value;
end;
...
begin
   ...
   if TextoColuna <> '' then
   begin
      if ACol in [5, 6, 7, 8, 9]  then //Coluna a ser alinhada a direita
      begin
         if ARow >= 1 then
         begin
            try
...
begin
   ...
   if ( (ACol in [ 0, 1, 2, 3, 5, 8 ] ) OR (sn_ativo = '0') ) then
   begin
      sgGridParcelas.Options := sgGridParcelas.Options - [goEditing];
   end
   else
   begin
      if ACol = 6 then //coluna valor desconto
      begin
      ...
      else if(ACol = 7) then //coluna valor juros
      begin
      ...
      else if(ACol = 9) then //coluna valor pago
      begin
...
function TfrmMensalidadesBaixar.validarMensalidadesBaixadas(): Boolean;
const
  SQL_CHECAR_MENSALIDADE =
      ' SELECT '+
      '     situacao, cd_mensalidade_origem '+
      ' FROM '+
      '     mensalidades '+
      ' WHERE '+
      '     cd_mensalidade = :cd_mensalidade ';
      
  SQL_ATUALIZAR_MENSALIDADE_ORIGEM =
      ' UPDATE '+
      '     mensalidades '+
      ' SET '+
      ...
      '     cd_mensalidade = :cd_mensalidade ';
var
  i, iPessoa, iParcela: Integer;
  objParcela: TMBParcela;
  ListaParcelas: TObjectList;
  qyChecarMensalidade: TUMZReadOnlyQuery;
  qyAtualizarMensalidadeOrigem: TUMZQuery;
begin

  DM.CriarConsulta(qyChecarMensalidade);
  Dm.CriarConsulta(qyAtualizarMensalidadeOrigem);

  qyChecarMensalidade.SQL.Text := SQL_CHECAR_MENSALIDADE;
  qyAtualizarMensalidadeOrigem.SQL.Text := SQL_ATUALIZAR_MENSALIDADE_ORIGEM;

  // Verifica se existe uma lista de pessoas
  if ListaPessoas = nil then
  begin
    Result := true;
    Exit;
  ...
  // Percorre a lista de pessoas que possuem mensalidades a serem baixadas
  for iPessoa := 0 to ListaPessoas.Count - 1 do
  begin
    // Pega a pessoa
    objPessoa := (ListaPessoas.items[iPessoa] as TMBPessoa);
    ...
    // Se a lista de parcelas NÃO existir, passa para outra pessoa
    if ListaParcelas = nil then continue;
    ...
    begin
      ...
      // Se a parcela estiver com sn_ativo <> 1 é uma parcela NÃO selecionada, passa para a próxima
      if objParcela.getValorCampo('sn_ativo') <> '1' then continue;

      // Busca a situação mais atual da mensalidade no banco de dados
      qyChecarMensalidade.Close();
      qyChecarMensalidade.ParamByName('cd_mensalidade').AsInteger := StrToInt(objParcela.getValorCampo('cd_mensalidade'));
      qyChecarMensalidade.Open();

      // Verifica a situação atual das mensalidades que serão baixadas
      case qyChecarMensalidade.FieldByName('situacao').AsInteger of

        // Se a situação for 0 - OK ou 1 - Parcial
        0..1 :
        begin
          Mensagem( 'Existem mensalidades já baixadas.', Application.Title, MB_OK + MB_ICONEXCLAMATION );
          Result := false;
          Exit;
        end;

        // Se a situação for 4 - Isentada, 5 - Cancelada, 7 - Desc. Financeiro
        4, 5, 7 :
        begin
          Mensagem( 'Existem mensalidades canceladas.', Application.Title, MB_OK + MB_ICONEXCLAMATION );
          Result := false;
          Exit;
        end;

        // Se a situação for 6 - Desconto
        6 :
        begin
          Mensagem( 'Existem mensalidades baixadas por desconto.', Application.Title, MB_OK + MB_ICONEXCLAMATION );
          Result := false;
          Exit;
        end;
      ...
      {
        Foi detectado que em algumas situações o código da mensalidade origem está como -1, esse código é inserido como
        -1 na criação da mensalidade mas na sequência já é alterado para o código da própria mensalidade.
        Não foi descoberto porque continua sendo -1 em alguns casos pontuais, mas isso gera problemas na baixa, portanto,
        foi criado esse procedimento que verifica se o cd_mensalidade_origem é -1 o sistema deve alterar esse
        valor para o cd_mensalidade
      }
      if qyChecarMensalidade.FieldByName('cd_mensalidade_origem').AsInteger = -1 then
      begin
        qyAtualizarMensalidadeOrigem.Close();
        qyAtualizarMensalidadeOrigem.ParamByName('cd_mensalidade').AsInteger := StrToInt(objParcela.getValorCampo('cd_mensalidade'));
  ...
  FreeAndNil(qyAtualizarMensalidadeOrigem);
...
const
   SChave = 'UMFinanceiro.Recebimentos.Recebimentos.Baixar.ReutilizarTD';

   SSemPermissao = 'Você não possui permissão para reutilizar o talão de depósito para realizar baixa de mensalidades.';
...
begin
   ...
   begin
      ...
      TemPermissao := DM.UsuarioLogado.TemPermissao(DM.UsuarioLogado.Pessoa.Codigo, SChave, npAcesso, False);

      if not TemPermissao then
      begin
         MessageDlg(SSemPermissao, mtError, [mbOK], -1);
         qyPesquisa.Close;
         qyPesquisa.Free;
         Result := False;
         Exit;
...
begin
   ...
   begin
      ...
      begin
         ...
         if trim(sgGridParcelas.Cols[ 9 ][ i ]) <> '' then
         begin
            sgGridParcelas.Cols[ 9 ][ i ]:= FloatToStrF(StrToFloat(sgGridParcelas.Cols[ 9 ][ i ]), ffFixed, 12, 2);
         end;
         
         bConvert:= TryStrToFloat(trim(sgGridParcelas.Cells[8, I]), cValor);
         if  ((not bConvert) or ( cValor = 0.0 )) then
         begin
            SetGridFocus( sgGridParcelas, I, 9 );
            Break;
         end;
      end;
   end;
...
begin
   ...
   begin
      ...
      begin
         ...
         sgGridParcelas.Cols[ 9 ][ iLinha ] := objParcela.getValorCampo('valorpago');
         sgGridParcelas.Cols[ 8 ][ iLinha ] := objParcela.getValorCampo('valortotal');
      end
...
const
   SMsgError = ''
      + 'O sistema gerou uma inconsistência na distribuição dos valores. Entre '
...
begin
   ...
   begin
      raise Exception.Create(SMsgError);
   end;
end;
...
const
   SQL_ACOES_MOVIMENTO_CONDICIONAIS =
      ...
      '  (cd_mensalidade_origem = :cd_mensalidade OR cd_mensalidade = :cd_mensalidade) ';
var
...
function TfrmMensalidadesBaixar.GetSaidaProdutos: Boolean;
begin
   Result := FSaidaProdutos;
end;
