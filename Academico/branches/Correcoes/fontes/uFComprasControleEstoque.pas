...
type
  TfrmComprasControleEstoque = class(TForm)
    ...
    qyRegistraEntradacd_compra_troca: TIntegerField;
    qyEstoqueCD_COMPRA_TROCA: TIntegerField;
    qyEstoqueNM_COLIGADA: TStringField;
    procedure qyEstoqueAfterOpen(DataSet: TDataSet);
...
var
   ...
   sn_alterar : Boolean;
begin
  if (DM.UsuarioLogado.TemPermissao(0,'UMFinanceiro.ProdutoAlterarValorUnitario', npAcesso, true) = false) then
     abort;
...
procedure TfrmComprasControleEstoque.dsEstoqueDataChange(Sender: TObject;
  Field: TField);
begin
   btnCancelarMovimentacao.Enabled := (qyEstoqueTP_ENTRADA_SAIDA.AsInteger = 2) AND (qyEstoque.RecordCount > 0);
   btnAlterarValorUnitario.Enabled  := (qyEstoqueCD_COMPRA.AsInteger > 0) AND (qyEstoque.RecordCount > 0);
end;
...
const
   SQL_ESTOQUE =
      ...
      ' 			AND TP_ENTRADA_SAIDA = 1 '+
      '				AND DT_ENTRADA <= DATE_FORMAT( :DT_ENTRADA_FIM, ''%Y-%m-%d 23:59:59'' )   '+
      '           ORDER BY DT_ENTRADA DESC LIMIT 1 ) AS vl_compra,'+
      '       IF(ce.TP_ENTRADA_SAIDA=1, cp.vl_produto, ce.vl_compra ) as vl_venda, ' +
      '       ce.TP_PESSOA, ce.cd_pessoa, ce.nr_anosemestre, ce.cd_compra_troca, IFNULL(co.nm_coligada, "") as nm_coligada ' +
      ...
      '      LEFT JOIN coligadas co ON ( co.cd_coligada = ce.cd_coligada  ) '+
...
procedure TfrmComprasControleEstoque.qyEstoqueAfterOpen(DataSet: TDataSet);
begin
  btnCancelarMovimentacao.Enabled := (qyEstoqueTP_ENTRADA_SAIDA.AsInteger = 2) AND (qyEstoque.RecordCount > 0);
   btnAlterarValorUnitario.Enabled  := (qyEstoqueCD_COMPRA.AsInteger > 0) AND (qyEstoque.RecordCount > 0);
end;
...
const
   ...
   sql_atualiza_financeiro = ' UPDATE comp_estoque_mensalidade SET cd_compra = :compra_nova WHERE cd_compra = :compra_antiga ';

   sql_atualiza_acumulados = ' UPDATE comp_acumulados SET cd_compra = :compra_nova WHERE cd_compra = :compra_antiga ';
var
   ...
   nr_quantidade, cd_compra, cd_compra_nova: Integer;
begin
   ...
   //Bloqueando a troca de produtos quando a movimentação é de ENTRADA
   if qyEstoqueDS_SAIDA.AsString = '' then
   begin
     Mensagem( 'Não é possível trocar movimentações de entrada.', Application.Title, MB_OK, Handle );
     Exit;
   end;

   if qyEstoqueCD_COMPRA_TROCA.AsInteger <> 0 then
   begin
     Mensagem( 'Esta movimentação já foi trocada.', Application.Title, MB_OK, Handle );
     Exit;
   end;
   ...
   begin
      ...
      begin
         ...
         msgNovoProduto :=
            msgNovoProduto
            + '. Existe uma diferença de R$'
            + FloatToStrF(diferencaValor, ffFixed, 9, 2 )
            + ' do produto atual.';

         if ( Mensagem(
            msgNovoProduto + Chr(13)
            + 'Necessário um ajuste manual da parcela na Planilha de Recebimentos. Deseja continuar?',
      ...
      qyRegistraEntradaCD_COMPRA_TROCA.AsInteger := qyEstoqueCD_COMPRA.AsInteger;

      qyRegistraEntrada.Post();

      // Atualizar o dados acumulados para o código da compra  novo
      qryExecutar.SQL.Text := 'SELECT MAX(cd_compra) as cd_compra_nova FROM comp_estoque';
      qryExecutar.Open;

      cd_compra_nova := qryExecutar.FieldByName('cd_compra_nova').AsInteger;
      qryExecutar.Close();
      qryExecutar.SQL.Text := 'UPDATE comp_estoque SET cd_compra_troca = :cd_compra_troca WHERE cd_compra = :cd_compra';
      qryExecutar.ParamByName('cd_compra_troca').AsInteger := cd_compra_nova;
      qryExecutar.ParamByName('cd_compra').AsInteger := qyEstoqueCD_COMPRA.AsInteger;
      qryExecutar.ExecSQL();

      qryExecutar.Close;


      // Atualizar o finacneiro para o código da compra  novo
      qryExecutar.Close();
      qryExecutar.SQL.Text := sql_atualiza_financeiro;
      qryExecutar.ParamByName('compra_nova').AsInteger := cd_compra;
      qryExecutar.ParamByName('compra_antiga').AsInteger := qyEstoqueCD_COMPRA.AsInteger;
      qryExecutar.ExecSQL();

      // Atualizar o dados acumulados para o código da compra  novo
      qryExecutar.Close();
      qryExecutar.SQL.Text := sql_atualiza_acumulados;
      qryExecutar.ParamByName('compra_nova').AsInteger := cd_compra;
      qryExecutar.ParamByName('compra_antiga').AsInteger := qyEstoqueCD_COMPRA.AsInteger;
      qryExecutar.ExecSQL();
