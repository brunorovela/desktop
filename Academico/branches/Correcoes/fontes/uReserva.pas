...
const
   ...
   SSQLAtualizarSituacaoMatriculaCurso =
      'SELECT CD_MATRICULA_CURSO, CD_SITUACAO FROM MATRICULAS_CURSO WHERE CD_MATRICULA_CURSO = ?';
     ...
     begin
        ...
        try
            ...
            begin
               ...
               // altera a situação da matricula curso para cursando
               if (MatriculaCurso > 0) then
               begin
                 Stmt := PrepareStatement(SSQLAtualizarSituacaoMatriculaCurso);
                 try
                    Stmt.SetResultSetConcurrency(rcUpdatable);
                    Stmt.SetInt(1, MatriculaCurso);
            ...
            qryReservar.ParamByName('DataEntrada').AsDateTime := DataHoje;
...
const
SSQLSelectCreditosFromTurmasHorarios =
      '  SELECT ' +
      '      COALESCE(SUM(nr_aulas), 0) AS qtdcreditos ' +
      '  FROM ' +
      '      ( ' +
      '      SELECT ' +
      '          gd.nr_aulas ' +
      '      FROM ' +
      '          turmas AS t ' +
      '          INNER JOIN turmas_horarios AS h ON (t.codigo = h.turma_base AND t.anosemestre = h.anosemestre) ' +
      '          INNER JOIN turmas AS th ON (h.turma = th.codigo AND h.anosemestre = th.anosemestre) ' +
      '          INNER JOIN grades_disciplinas AS gd ON ( ' +
      '              th.cd_grade = gd.cd_grade ' +
      '              AND th.curso = gd.cd_curso ' +
      '              AND gd.cd_disciplina = h.disciplina ' +
      '              AND gd.cd_curso = th.curso ' +
...
begin
 // Total de Créditos
  ...
  begin
     qSel.Close();
     qSel.SQL.Text := SSQLSelectCreditosFromTurmasHorarios;

     qSel.ParamByName('turma').AsString := txtTurmaPara.Text;
     qSel.ParamByName('anosemestre').AsInteger := StrToInt(txtAnoSemestrePara.Text);
