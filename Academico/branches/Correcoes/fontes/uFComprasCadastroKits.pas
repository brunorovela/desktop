...
type
  TformCadKits = class(TForm)
    ...
    qyKitsSN_BAIXAR_TITULO_AUTO: TSmallintField;
...
begin
   ...
   if (qyKitsNR_PARCELAS.AsInteger < 1 ) then
   begin
...
const
   SQL_COD_BARRA_EXISTE =
      ' SELECT COUNT(*) AS registros FROM COMP_CODIGO_BARRAS WHERE CD_CODIGO_BARRAS = :cd_cod_barra and (cd_kit <> :kit or cd_kit is null) and sn_cantina = 0 ';
var
...
const
   SQL_BUSCA_VALOR =
      'SELECT COALESCE(sum((ck.nr_quantidade * cp.VL_PRODUTO)),0) as ValorKit ' +
