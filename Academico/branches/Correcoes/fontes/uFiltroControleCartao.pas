...
begin
   ...
   { Filtra o TDI DA tabela fin_cartao_operacao }
   if ((trim(edTID.text) <> '')) then
   begin
      SQLOtimizado := SQLOtimizado + ' AND (fco.ds_tid_cartao = :ds_tid_cartao ) ';
      SQLOtimizado := StringReplace(
         SQLOtimizado,
         ':ds_tid_cartao',
         QuotedStr(trim(edTID.text)),
         [rfReplaceAll, rfIgnoreCase]
      );
   end;

   if ((trim(filtro_cd_mensalidade.text) <> '')) then
   begin
      SQLOtimizado := SQLOtimizado + ' AND (m.cd_mensalidade = :cd_mensalidade ) ';

      SQLOtimizado := StringReplace(
         SQLOtimizado,
         ':cd_mensalidade',
         QuotedStr(trim(filtro_cd_mensalidade.text)),
         [rfReplaceAll, rfIgnoreCase]
      );
   end;

   SQLOtimizado := SQLOtimizado + 'AND (FCC.CD_COLIGADA IN ('+DM.GetColigadas+') OR FCC.SN_TODAS_COLIGADAS = 1 )';


   Result := SQLOtimizado;
   Result := Result + ' GROUP BY fcp.cd_parcela ';
   Result := Result + ' ) AS parcelas ';
end;

function TfrmFiltroControleCartao.getSQLOtimizadoBusca: String;
{Foi utilizado Group BY nesta situação pois quando várias mensalidades eram pagas em várias parcelas o sistema listava o mesmo registro várias vezes}
const
   SQL_CAMPOS =
      ' fcac.ds_nome, '+
      ' m.datapagamento as dt_baixa, '+
      ...
      ' fco.nr_taxa, '+
      ' IF (ROUND((ROUND((fco.nr_taxa * fcp.vl_valor_parcela), 2) / 100), 2) < fcac.vl_min_operacao AND  ROUND((ROUND((fco.nr_taxa * fcp.vl_valor_parcela), 2) / 100), 2) > 0 ' +
      ' , fcac.vl_min_operacao  , ROUND((ROUND((fco.nr_taxa * fcp.vl_valor_parcela), 2) / 100), 2) ) AS tarifa,' +
      '     IF '+
		  '     ( '+
		  '     (ROUND( ( ROUND( ( COALESCE(fco.nr_taxa,0) * COALESCE(fcp.vl_valor_parcela,0) ), 2 ) /'+
			'     100 ), 2 ) > COALESCE(fcac.vl_min_operacao,0)) OR ( COALESCE(fco.nr_taxa,0) = 0), '+
      '	  	ROUND( '+
      '     fcp.vl_valor_parcela - ROUND( ( ROUND( ( fco.nr_taxa * fcp.vl_valor_parcela ), 2 ) / 100 ), 2 ),2),'+
  		'     ROUND(fcp.vl_valor_parcela - fcac.vl_min_operacao, 2 )) AS valor_liquido, '+
      ' fcc.ds_caixa, '+
      ' fco.cd_operacao, '+
