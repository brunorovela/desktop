...
uses
  ...
  ZDataset, UZDataset, ImgList, ToolWin, Variants, uFSelecionarTurma, ZDbcIntfs,
  UZDbcFuncs;
...
begin
   ...
   try
      Stmt.SetString(1, tblFichaTurma.AsString);
      Stmt.SetInt(2, tblFichaAnoSemestre.AsInteger);

      if (txtDisciplina.Text <> '') and (not popFicha.Items[0].Checked) then
         Stmt.SetInt(3, tblFichadisciplina.AsInteger);

      if (txtDisciplina.Text = '') or popFicha.Items[0].Checked then
...
begin
  ...
  { Faça do primeiro aluno até o último }
   ...
   begin
      ...
      erro := true;
      tentativas := 0;
      // Tenta pelo menos mais 3 vezes
      while (tentativas <= 3) and erro do
      begin
         try
            { Calcular a média Final, individualmente por aluno }
            DM.TotalFaltas(
