...
type
  TfrmComprasRegistroSaidas = class(TForm)
    ...
    Label19: TLabel;
    UpDown1: TUpDown;
    qyRegistraSaidacd_coligada: TIntegerField;
    pnColigadas: TPanel;
    Label3: TLabel;
    ...
    procedure atualizaColigada(CodigoAluno : Integer);
  private
    ...
    function SetColigada(vcd_coligada : Integer) :integer;
...
uses
   ...
   UFComprasAgendamentoSaida, uCamposPlanilhas, uItemCombo;
...
begin
   ...
   atualizaColigada(qyRegistraSaidacd_pessoa.AsInteger);
end;
...
begin
   ...
   qyRegistraSaidacd_coligada.AsInteger := cd_coligada;
...
procedure TfrmComprasRegistroSaidas.CarregarComboColigadas;
begin
   cbColigadas.Clear;
...
function TfrmComprasRegistroSaidas.GetColigada: Integer;
begin
   Result := 0;
...
function TfrmComprasRegistroSaidas.SetColigada(vcd_coligada : Integer): Integer;
Var
  i : integer;
begin
   Result := 0;

   if vcd_coligada <= 0 then begin
      cbColigadas.ItemIndex := -1;
      result := 0;
      exit;
   end;

   for i := 0 to cbColigadas.Items.Count - 1 do  begin

      if ( cbColigadas.Items.Objects[i] <> nil ) then begin
         if (IntToStr(vcd_coligada) = TItemCombo(cbColigadas.Items.Objects[i]).getCodigo() ) then begin
...
Procedure TfrmComprasRegistroSaidas.atualizaColigada(CodigoAluno : Integer);
Begin

  // Buscar a Coligadas
   Dm.qAux2.Close;
   Dm.qAux2.SQL.Text := ' '+
    ' SELECT t.cd_coligada FROM matriculas as m'+
    ' INNER JOIN turmas as t ON (t.anosemestre = m.anosemestre AND t.codigo = m.turma)'+
    ' INNER JOIN cursos_mestre as cm ON (cm.cd_curso = m.curso)'+
    ' WHERE m.situacao not IN (3,4,5,6,7,8,9) AND m.codigoaluno = :codigoaluno AND m.anosemestre = :anosemestre'+
    ' ORDER BY nr_relevancia LIMIT 1';

   Dm.qAux2.ParamByName('codigoaluno').AsString := IntToStr( CodigoAluno );
   Dm.qAux2.ParamByName('anosemestre').AsString := qyRegistraSaidaNR_ANOSEMESTRE.AsString;
   dm.qAux2.Open();

   SetColigada(Dm.qAux2.FieldByName('cd_coligada').AsInteger);

   if (cbColigadas.ItemIndex >=0)  then begin
       cbColigadas.Enabled := false;

   end else begin
       cbColigadas.Enabled := true;
