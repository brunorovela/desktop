...
type
  TfrmMensalidadeAplicarDescontos = class(TForm)
    ...
    lblDescontoCustomizado: TLabel;
    bvlDescontoCustomizado: TBevel;
    lblDescontoCustomizadoValor: TLabel;
...
uses uDM, uUsuario, General, uClassMovimento, uItemCombo, uRemessaClass;
...
const
   SMsgTipoDesconto = 'Não é possível aplicar desconto fixo em parcelas parciais ou negociadas.';
begin

   if (rgTipoDesconto.ItemIndex = 0) and (not rgTipoDesconto.Controls[0].Enabled) then
   begin
      MessageDlg(SMsgTipoDesconto, mtError, [mbOK], -1);
      Exit;
   end;
...
begin
   ...
   DM.UpdateDescontoFixo(iCodigoMensalidade);
...
begin
   ...
   begin
      lblDescontoCustomizado.Visible := False;
      lblDescontoCustomizadoValor.Visible := False;
      bvlDescontoCustomizado.Visible := False;

      if qyValoresTotais.FieldByName('vl_descontos').AsCurrency <> qyValoresTotais.FieldByName('vl_total_descontos').AsCurrency then
      begin
         lblDescontoCustomizado.Visible := True;
         lblDescontoCustomizadoValor.Visible := True;
         bvlDescontoCustomizado.Visible := True;
         lblDescontoCustomizadoValor.Caption := qyValoresTotais.FieldByName('vl_descontos').AsString;
      end;
...
begin
   ...
   begin
      if rgTipoDesconto.ItemIndex = 2 then
      begin
          // Se for desconto financeiro 100%, aplicar sobre o valor extra tb
          valorDesconto := ((qyValoresTotais.FieldByName('vl_liquido_antes_condicionais').AsCurrency  + qyMensalidade.FieldByName('valorextra').AsCurrency )* edPercentualDesconto.GetValor())/100;
      end else begin
...
begin
   ...
   if qyDescontos.FieldByName('situacao').AsInteger = -99 then
   begin
      Mensagem('Não é possível remover o desconto baseado na regra definida pela instituição.', 'Atenção', MB_OK + MB_ICONWARNING);
      Exit;
   end;
...
begin
   ...
   while not qyMensalidadesAplicar.Eof do begin
      ...
      begin
        ...
        objRemessa.registraAcaoCancelamento(qyMensalidadesAplicar.FieldByName('nossonumero').AsString);
      end;
