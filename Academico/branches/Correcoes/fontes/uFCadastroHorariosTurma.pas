...
uses
  ...
  ZAbstractDataset, UMDateTimePicker, uMensagem;
...
type
   TfrmCadastroHorariosTurma = class(TForm)
    ...
    umdtSimulacao: TUMDateTimePicker;
    procedure umdtSimulacaoChangeDate(Sender: TObject);
   ...
   strict private
      ...
      horario_turmas_checar_coincidencias,limpandoGrade : Boolean;
      ...
      function verificaPrimeiroHorarioTurma(): TDateTime;
...
uses uUMNucleo, uDM, uGeneral, uUsuario, General, Main, uFSelDisciplinaProfessor,
  uFDefinicaoNovaDataHorario, uListaExecucao, uUrania, uFSelCategoriaHorario, uFUraniaErros,
   uFReajusteAulasHorarios, uFSelecionarSala, uSala, uTurma, uHorario, uFDefinicaoPrimeiroHorario;
...
procedure TfrmCadastroHorariosTurma.btnLimparClick(Sender: TObject);
var
   i,j:integer;
begin
   if TGeneral.Mensagem('Você esta executando a limpeza da grade de horários para esta turma, deseja continuar? ', '', MB_YESNO + MB_ICONQUESTION + MB_DEFBUTTON2, Handle) = mrYes then
   begin
      ...
      for I := 1 to sgHorarios.RowCount - 1 do
      begin
         for j := 2 to sgHorarios.ColCount - 1 do
         begin
            HorariosTurma.LimparDisciplinasGrade(i, j);
         end;
      end;
      ...
      tbImportar.Enabled  := true;
      limpandoGrade := false;

      TGeneral.Mensagem('Limpeza da grade de horários executada com sucesso!');
   end;
end;
   ...
   try
      ...
      begin
         ...
         begin
            ...
            dataNovoHorario := verificaPrimeiroHorarioTurma();

            // Preenche as datas de inicio/fim dos horários que foram inseridos ou excluidos
            TfrmDefinicaoNovaDataHorario.ajustarDatasInicioFim(ListaExecucao, dataNovoHorario);
      ...
      begin
         ...
         case TGeneral.Mensagem(MSG_PERGUNTA_ALTERACAO_HORARIO, mtConfirmation, [mbYes, mbNo, mbCancel], ['Manutenção do horário atual', 'Novo horário após data específica', 'Cancelar'], 150, mbCancel) of
            mrYes: begin
               ListaExecucao := HorariosTurma.GetListaExecucao( resultado_filtro_turma.nr_anosemestre, 1 );
               ListaDisciplinasHorarios := HorariosTurma.GetListaHorariosDasDisciplinas();

               // Verifica se a manutenção de horário conflita com horários anteriores
               if TfrmDefinicaoNovaDataHorario.verificaConflitoHorario(ListaExecucao, resultado_filtro_turma.nr_anosemestre, resultado_filtro_turma.cd_turma, DM.DataAtual, sMensagemErro, HorariosTurma, ListaDisciplinasHorarios, mostrarConflitoAula) then
               begin
                  if mostrarConflitoAula then
                  begin
                     if not TfrmReajusteAulasTurma.ajustarAulasHoras(ListaExecucao, ListaDisciplinasHorarios, resultado_filtro_turma.nr_anosemestre, resultado_filtro_turma.cd_turma) then
                     begin
                        exit;
                     end;
                  end;
                  ...
                  begin
                     // Preenche as datas de inicio/fim dos horários que foram inseridos ou excluidos
                     TfrmDefinicaoNovaDataHorario.ajustarDatasInicioFim(ListaExecucao, DM.DataAtual);
                     ListaExecucao.ExecuteAll();
                     Filtrar(False);
                  end;

               end else begin
                  // Mostra os erros ocorridos caso o retorno não seja True
                  parametros.titulo := 'Atenção';
                  parametros.opcoes := (BT_OK + ICO_ERROR);
                  parametros.mensagens.msgPrincipal := sMensagemErro;
                  parametros.mensagens.msgTopo := 'Não foi possível realizar essa operação'+#13;
            ...
            mrNo: begin
               ListaExecucao := HorariosTurma.GetListaExecucao( resultado_filtro_turma.nr_anosemestre, 2 );
               ListaDisciplinasHorarios := HorariosTurma.GetListaHorariosDasDisciplinas();

               if TfrmDefinicaoNovaDataHorario.definirNovoHorario(ListaExecucao, ListaDisciplinasHorarios, resultado_filtro_turma.nr_anosemestre, resultado_filtro_turma.cd_turma, sMensagemErro, HorariosTurma, mostrarConflitoAula) then
               begin
                  if TGeneral.Mensagem(MSG_PERGUNTA_SALVAR_ALTERACOES, '', MB_YESNO + MB_ICONQUESTION + MB_DEFBUTTON2, Handle) = mrYes then
                  begin
                     ListaExecucao.ExecuteAll();
               ...
               end else if Length(sMensagemErro) > 0 then
               begin
                  // Mostra os erros ocorridos caso o retorno não seja True
                  parametros.titulo := 'Atenção';
                  parametros.opcoes := (BT_OK + ICO_ERROR);
...
begin
   ...
   begin
      ...
      disciplina.adicionais.cd_turma := qyGetDisciplinas.FieldByName('turma_optativa').AsString;
      disciplina.adicionais.cd_turma_base := resultado_filtro_turma.cd_turma;
      disciplina.adicionais.cd_professor := qyGetDisciplinas.FieldByName('cd_professor').AsInteger;
      disciplina.adicionais.sala := TTurma.Create(resultado_filtro_turma).GetSalaAutomatico(disciplina, False);
...
const
   SQL_TURMAS_HORARIOS =
      ...
      '        d.codigo, d.sigla, d.descricao, d.curso, us.cd_sala, ' +
      ...
      '       %s ' +
      ' ORDER BY th.cd_turmas_horarios, codigo, dia_semana ';
var
...
begin
   ...
   if cbSimularHorario.Checked then
   begin
      qyGetTurmasHorarios.ParamByName('dt_simulacao').AsDateTime := umdtSimulacao.Date;
   end;
...
begin
   ...
   HorariosTurma.Subject.RegisterObserver(TComponentHorariosTurmasObserver.Create(umdtSimulacao, HorariosTurma, False));
...
const
   SQL_CARREGAR_HORARIOS =
      ...
      + '    h.codigo, '
      + '    h.ds_horario, '
      + '    CONCAT( DATE_FORMAT(h.hr_inicio,"%H:%i") ," - ", DATE_FORMAT( h.hr_fim,"%H:%i") ) hr_aula,'
      + '    h.ds_categoria  '
      + ' FROM '
      + '    horarios h ' 
      + ' WHERE '
...
begin
   ...
   qyGetHorarios.Close();
   qyGetHorarios.SQL.Clear();
   qyGetHorarios.SQL.Text := SQL_CARREGAR_HORARIOS;

   if not (Trim(ASFiltro) = '') then
   begin
      qyGetHorarios.SQL.Add(
         ' AND ds_categoria = ' + ASFiltro
      );
   end;
                    
   qyGetHorarios.SQL.Add( ' ORDER BY h.ds_horario ' );
   qyGetHorarios.Open();
   ...
   begin
      HorariosTurma.AddHorario(THorarioDiaSemana.Create(
         ...
         qyGetHorarios.FieldByName('hr_aula').AsString,
         qyGetHorarios.FieldByName('ds_categoria').AsString
      ));
...
begin
   ...
   if limpandoGrade then
   begin
      TGeneral.Mensagem('Aguarde o fim da limpeza da grade de horários para continuar');
      Exit;
   end;
...
procedure TfrmCadastroHorariosTurma.umdtSimulacaoChangeDate(
  Sender: TObject);
begin
   Filtrar(False);
end;

function TfrmCadastroHorariosTurma.verificaPrimeiroHorarioTurma(): TDateTime;
var
   temHorario : Boolean;
   dataInicial: TDateTime;
begin
   dataInicial := dm.DataAtual;

   Application.CreateForm( TfrmDefinicaoPrimeiroHorario, frmDefinicaoPrimeiroHorario );

   frmDefinicaoPrimeiroHorario.setDataInicioHorario(dataInicial);
   frmDefinicaoPrimeiroHorario.ShowModal;

   dataInicial := frmDefinicaoPrimeiroHorario.getDataInicioHorario;

   FreeAndNil(frmDefinicaoPrimeiroHorario);
...
begin
   ...
   begin
      ...
      begin
         ...
         MontarGrade(QuotedStr(CategoriaFiltrada));
      end;
