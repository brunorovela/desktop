...
type
  Tfrm_CadContas = class(TForm)
    ...
    UMAjuda_juros_mensal: TUMAjuda;
    UMAjuda_juros_mes: TUMAjuda;
    UMAjuda3: TUMAjuda;
  ...
  private
    ...
    procedure removerMovimentoSaldoInicial(cd_caixa, cd_coligada: Integer);

    function VerificaUnidadeEnsino(): Boolean;
    function isContaUsada(cd_caixa, cd_coligada: Integer): Boolean;
  public
  end;
...
begin
   ...
   qyCentros.Close();
   qyCentros.ParamByName('cd_coligada').AsInteger := DM.GetUsuarioLogado.GetColigadaLogada.Codigo;
   sn_ativo.ItemIndex := 2;
   qyCentros.Open();
...
function Tfrm_CadContas.isContaUsada(cd_caixa, cd_coligada: Integer): Boolean;
const
   SQL_IS_CONTA_USADA =
      ' SELECT '+
      '	COUNT(1) AS qtd '+
...
var
   qyIsContaUsada: TUMZQuery;
begin
   DM.CriarConsulta(qyIsContaUsada);

   qyIsContaUsada.SQL.Text := SQL_IS_CONTA_USADA;
   qyIsContaUsada.ParamByName('cd_caixa').AsInteger := cd_caixa;
   qyIsContaUsada.ParamByName('cd_coligada').AsInteger := cd_coligada;
   qyIsContaUsada.Open;

   // Maior que 1 porque o sistema lança um movimento na tesouraria
   Result := qyIsContaUsada.FieldByName('qtd').AsInteger > 1;
end;
...
const
   SQL_CONTAS_FILTRADAS = 'SELECT * FROM FIN_CADASTRO_CONTAS '+
      ' WHERE CD_COLIGADA = %d AND sn_ativa IN (%s) '+
      ' ORDER BY TP_CONTA, DS_CAIXA';

    SQL_CENTROS_FILTRADOS = 'SELECT * FROM fin_centro_custos ' +
                'WHERE cd_coligada = %d';
var
   ds_condicao : String;
begin
   ...
   begin
      if coligada = 0 then
        Begin
          ...
          qyCentros.SQL.Text := Format(SQL_CENTROS_FILTRADOS, [
             0
           ])
        end
      else
...
Var
   ...
   isUsada,
   isRemoverMovimentoSaldoInicial: Boolean;
begin
  ...
  // Verificar se existe lançamento da tesouraria
   ...
   // Se for diferente do tipo de conta 3 o sistema deve validar diferente,
   // pois para esses tipos de contas o sistema ja cria automáticamente um
   // movimento na de saldo inicial.
   if qyContastp_conta.AsInteger <> 3 then
   begin
      isUsada := Self.isContaUsada(qyContascd_caixa.AsInteger, qyContascd_coligada.AsInteger);
      isRemoverMovimentoSaldoInicial := True;
   end;

   if qyContastp_conta.AsInteger = 3 then
   begin
      isUsada := DM.ExisteRegistro(
         'fin_mov_tesouraria',
         ['cd_caixa', 'cd_coligada'],
         [qyContas.FieldByName('cd_caixa').AsString,
         qyContascd_coligada.AsString]);
         
      isRemoverMovimentoSaldoInicial := False;
   end;
  ...
  begin
      ...
      // Remove o movimento de Saldo Inicial que é criado automáticamente pelo
      // sistema quando o tipo de conta é diferente de 3.
      if isRemoverMovimentoSaldoInicial then
      begin
         removerMovimentoSaldoInicial(qyContascd_caixa.AsInteger, qyContascd_coligada.AsInteger);
      end;
...
begin
   ...
   begin
      ...
      begin
         Mensagem(
           'Informe um valor Percentual para multa!',
            Application.Title,
   ...
   begin
     qyContasvl_multa_percent.Clear;
   end;
   ...
   begin
      ...
      begin
         Mensagem(
           'Informe um valor Percentual para os juros mensais!',
            Application.Title,
            MB_OK + MB_ICONWARNING
         );
         DBEdit11.SetFocus();
   ...
   begin
      qyContasvl_juros_percent.Clear;
   end;
   ...
   begin
      ...
      begin
         Mensagem(
           'Informe um valor Percentual para os juros ao mês!',
            Application.Title,
            MB_OK + MB_ICONWARNING
         );
         DBEdit15.SetFocus();
...
procedure Tfrm_CadContas.sn_ativoClick(Sender: TObject);
begin
   Filtra();
end;
...
begin
   ...
   // Habilita, depois irá desabilitar se necessário
   DBEdit3.Enabled := True;
   DBEdit11.Enabled := True;
   DBEdit15.Enabled := True;
   
   if ( qyContassn_multa.AsString = 'N' ) then
   begin
      DBEdit3.Enabled := False;
   end;

   if ( qyContassn_juros.AsString = 'N' ) then
   begin
      DBEdit11.Enabled := False;
   end;

   if ( qyContassn_juros_mensal.AsString = 'N' ) then
   begin
...
begin
    ...
    sn_ativo.Enabled          := qyContas.State in [dsBrowse, dsInactive];
end;
...
procedure Tfrm_CadContas.removerMovimentoSaldoInicial(cd_caixa,
  cd_coligada: Integer);
const
   SQL_REMOVER_SALDO_INICIAL =
      'DELETE '+
      'FROM '+
      '	fin_mov_tesouraria '+
      'WHERE '+
      '	cd_caixa = :cd_caixa '+
      '  AND cd_acao = 5 AND cd_origem = 3 '+
      '	AND cd_coligada = :cd_coligada ';
var
   qyRemoverMovimentoSaldoInicial: TUMZQuery;
begin
   DM.CriarConsulta(qyRemoverMovimentoSaldoInicial);

   qyRemoverMovimentoSaldoInicial.SQL.Text := SQL_REMOVER_SALDO_INICIAL;
   qyRemoverMovimentoSaldoInicial.ParamByName('cd_caixa').AsInteger := cd_caixa;
   qyRemoverMovimentoSaldoInicial.ParamByName('cd_coligada').AsInteger := cd_coligada;
   qyRemoverMovimentoSaldoInicial.ExecSQL;
end;
...
procedure Tfrm_CadContas.DBCheckBox2Click(Sender: TObject);
begin
  if not (qyContas.State in [dsInsert, dsEdit]) then
   begin
      Exit;
   ...
   begin
      DBEdit3.Enabled := False;
      qyContasvl_multa_percent.AsString := '';
      Exit;
   end;
...
procedure Tfrm_CadContas.DBCheckBox3Click(Sender: TObject);
begin
   if not (qyContas.State in [dsInsert, dsEdit]) then
   begin
      Exit;
   ...
   begin
      DBEdit11.Enabled := False;
      qyContasvl_juros_percent.AsString := '';
      Exit;
   end;
...
procedure Tfrm_CadContas.DBCheckBox5Click(Sender: TObject);
begin
   if not (qyContas.State in [dsInsert, dsEdit]) then
   begin
      Exit;
   ...
   begin
      DBEdit15.Enabled := False;
      qyContasvl_juros_mensal.AsString := '';
      Exit;
   end;
