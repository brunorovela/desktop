...
begin
   ...
   else if (edValorCredito.getValorDouble() > Self.objMensalidade.getValorTotalPendente()) then
   begin
      TGeneral.Mensagem('Valor inválido. Informe um valor inferior ou igual ao valor da mensalidade. Valor: ' +
      FormatFloat( '###,###,##0.00',  Self.objMensalidade.getValorTotalPendente()), '', MB_OK + MB_ICONEXCLAMATION, Handle);
      Self.ModalResult := mrNone;
      Exit;
   end;
...
begin
   ...
   begin
      ...
      Devolvido := TCredito(listaCreditos[I]).GetValorCreditoDevolvido;
   ...
   lbInformativoValorMaximo.Caption := Format( lbInformativoValorMaximo.Caption,
      [Self.objMensalidade.getValorTotalPendente], TGeneral.GetFormatSettings() );
end;
