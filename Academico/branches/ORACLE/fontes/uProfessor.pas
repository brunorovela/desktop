...
function Tfrm_Professores.isProfessor: Boolean;
const
   SQL_IS_PROFESSOR =
      ' SELECT '+
      '      count(ngp.cd_grupo) AS qtd '+
      '   FROM '+
      '      nu_grupos_pessoas ngp '+
      '   INNER JOIN nu_grupos ng ON (ng.cd_grupo = ngp.cd_grupo) '+
      '   WHERE '+
      '      ng.ds_papel = ''PROFESSOR'' '+
      '      AND ngp.cd_pessoa = :cd_pessoa ';
var
   qryIsProfessor: TUMZReadOnlyQuery;
begin
   DM.CriarConsulta(qryIsProfessor);

   qryIsProfessor.SQL.Text := SQL_IS_PROFESSOR;
   qryIsProfessor.ParamByName('cd_pessoa').AsInteger := tblPessoacd_pessoa.AsInteger;
   qryIsProfessor.Open;

   Result := qryIsProfessor.FieldByName('qtd').AsInteger > 0;
end;
