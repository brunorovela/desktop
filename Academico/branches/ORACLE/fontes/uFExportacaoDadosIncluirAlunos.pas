...
uses
  ...
  ZAbstractDataset, UZDataset, UMDateTimePicker;
...
type
  TfExportacaoDadosIncluirAlunos = class(TForm)
    ...
    umdtMatriculasAtivas: TUMDateTimePicker;
    qyAlunoscd_situacao_curso: TIntegerField;
...
begin
   ...
   begin
      ...
      begin
         if isEnadeSemAC() then
         begin
            SQL.Add(Format(SQL_ALTERAR_ALUNO, ['GET_PERCCH_CURSADA_ENADE(cd_pessoa, cd_turma, nr_anosemestre)']));
         end
         else
         begin
            SQL.Add(Format(SQL_ALTERAR_ALUNO, ['GET_PERCCH_CURSADA(cd_pessoa, cd_turma, nr_anosemestre)']));
         end;
      end
      ELSE
      begin
...
procedure TfExportacaoDadosIncluirAlunos.ckbMatriculasAtivasClick(
  Sender: TObject);
begin
   umdtMatriculasAtivas.Enabled := ckbMatriculasAtivas.Checked;
end;
...
const
   SQL_FILTRO_ALUNOS =
      ...
      '        GET_SITUACAO_CURSO_ALUNO(:cd_processo, m.codigoaluno, m.turma, m.anosemestre) AS cd_situacao_curso ' +
      ' FROM matriculas m ' +
...
begin
   ...
   begin
      ...
      begin
         if isEnadeSemAC() then
         begin
            SQL.Add(Format(SQL_FILTRO_ALUNOS, ['GET_PERCCH_CURSADA_ENADE(p.cd_pessoa, m.turma, m.anosemestre) AS nr_carga_horaria']));
         end
         else
         begin
            SQL.Add(Format(SQL_FILTRO_ALUNOS, ['GET_PERCCH_CURSADA(p.cd_pessoa, m.turma, m.anosemestre) AS nr_carga_horaria']));
         end;
      end
      else
      begin
      ...
      begin
         SQL.Add('AND m.dataemissao <= ' + QuotedStr(formatDateTime('yyyy-mm-dd', umdtMatriculasAtivas.Date)));
      end;
...
begin
   ...
   umdtMatriculasAtivas.Visible := IsProcessoENADE;
...
const
   SQL_INSERIR_ALUNO = ' INSERT INTO expo_processo_pessoas ' +
                       ...
                       ' VALUES ' +
                       '    (:cd_processo, :cd_pessoa, :cd_turma, :nr_anosemestre, :cd_situacao, :sn_selecionado, GET_SITUACAO_CURSO_ALUNO(:cd_processo, :cd_pessoa, :cd_turma, :nr_anosemestre), %s ) ';
...
begin
   ...
   begin
      ...
      begin
         if isEnadeSemAC() then
         begin
            SQL.Add(Format(SQL_INSERIR_ALUNO, [' GET_PERCCH_CURSADA_ENADE(cd_pessoa, cd_turma, nr_anosemestre)']));
         end
         else
         begin
            SQL.Add(Format(SQL_INSERIR_ALUNO, [' GET_PERCCH_CURSADA(cd_pessoa, cd_turma, nr_anosemestre) ']));
         end;
      end
      else
      begin
...
function TfExportacaoDadosIncluirAlunos.isEnadeSemAC: Boolean;
begin
   Result := DM.variavel_parametro('matricula_exportardados_enade_semac') = 'S';
end;
