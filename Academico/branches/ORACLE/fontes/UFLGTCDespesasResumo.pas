...
type
   TfrmLGTCDespesasResumo = class(TForm)
    ...
    qryFormaPgto: TUMZReadOnlyQuery;
    qryFormaPgtoDS_CONTEUDO: TStringField;
   ...
   private
      ...
      function GetFormaPgto(const PessoaID, ProfessorID: Integer;
...
begin
   ...
   begin
      ...
      begin
         ...
         if FCaixaPadraoID = 0 then
            cdsContaPagarCD_CAIXA.Clear;
         if FCaixaPadraoID <> 0 then
            cdsContaPagarCD_CAIXA.AsLargeInt := FCaixaPadraoID;
...
function TfrmLGTCDespesasResumo.GetFormaPgto(const PessoaID,
   ProfessorID: Integer; const Default: Cardinal): Cardinal;
var
   CampoID: Integer;
   UsarPadrao: Boolean;
begin
   UsarPadrao := FUsarFormaPgtoPadrao or
      ((PessoaID <> ProfessorID) and (FCampoFormaPgtoID = -1)) or
      ((PessoaID = ProfessorID) and (FCampoFormaPgtoProfessorID = -1));
   ...
   begin
      ...
      if PessoaID = ProfessorID then
         CampoID := FCampoFormaPgtoProfessorID;

      qryFormaPgto.Close;
      qryFormaPgto.ParamByName('CD_PESSOA').AsInteger := PessoaID;
      qryFormaPgto.ParamByName('CD_CAMPO').AsInteger := CampoID;
      qryFormaPgto.Open;

      if qryFormaPgto.IsEmpty then
         Result := Default;

      if not qryFormaPgto.IsEmpty then
         Result := StrToIntDef(qryFormaPgtoDS_CONTEUDO.AsString, Default);
   end;
end;
...
begin
   ...
   S := DM.variavel_parametro('LogisticaAulas.Compromisso.Gerar.FormaPgto.Caixa');
