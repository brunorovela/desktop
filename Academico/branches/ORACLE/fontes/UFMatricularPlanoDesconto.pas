...
uses
  ...
  OleServer, OutlookXP, ImgList, ComCtrls, ToolWin, uUsuario;
...
type
   TfMatricularPlanoDesconto = class(TForm)
    ...
    cdsParcelasPlanoDescontoNR_CREDITOS: TFloatField;
    cdsParcelasPlanoDescontoBackupNR_CREDITOS: TFloatField;
    procedure FormDestroy(Sender: TObject);
    procedure dbgDescontosAplicadosKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
...
procedure TfMatricularPlanoDesconto.dbgDescontosAplicadosKeyDown(
...
begin
   ...
   FPermissoes := DM.UsuarioLogado.GetPermissoes(DM.iCdPessoaLogado, 'UMFinanceiro.Alunos.Matricula.Matricular.PlanoDesconto');
   rbDescontoCondicional.Enabled := npAlterar in FPermissoes;
   rbDescontoFixo.Enabled := npAlterar in FPermissoes;
   dbgPlanosDesconto.Columns[1].ReadOnly := not (npAlterar in FPermissoes);
   dbgPlanosDesconto.Columns[2].ReadOnly := not (npAlterar in FPermissoes);
end;

procedure TfMatricularPlanoDesconto.FormDestroy(Sender: TObject);
var
   I: Integer;
begin
...
begin
   ...
   begin
      ...
      cdsParcelasPlanoDescontoNR_CREDITOS.AsFloat := cdsParcelasPlanoDescontoBackupNR_CREDITOS.AsFloat;
      cdsParcelasPlanoDescontoBackup.Next;
   end;
...
begin
   ...
   begin
      ...
      cdsParcelasPlanoDescontoBackupNR_CREDITOS.AsFloat := cdsParcelasPlanoDescontoNR_CREDITOS.AsFloat;
      cdsParcelasPlanoDesconto.Next;
   end;
...
var
   idxPercentualPlano, valorTotal, diffDesconto, Descontos: Currency;
begin
   ...
   diffDesconto := valorTotal - Descontos;
   ...
   if diffDesconto < valorDescontoCondicional then
   begin
      valorDescontoCondicional := diffDesconto;
   end;
