...
const
   SQL_FILTRAR =
      ' SELECT DISTINCT CP.CD_PRODUTO, CP.DS_PRODUTO, CP.SN_IMPRIMIR_CUPOM, ' +
      ...
      '  , (SELECT ce.VL_COMPRA FROM comp_estoque ce WHERE ce.CD_PRODUTO = cp.CD_PRODUTO AND ce.TP_ENTRADA_SAIDA = 1 ORDER BY ce.cd_compra DESC LIMIT 1) vl_compra ' +
      ' FROM COMP_PRODUTOS CP ' +
      '      LEFT JOIN  COMP_CODIGO_BARRAS CB on (CP.cd_produto = CB.cd_produto AND (CB.sn_cantina = 0 OR CP.SN_CANTINA_ESTOQUE = 1) )     ' +
      '      INNER JOIN COMP_CATEGORIAS CC ON ( CC.CD_CATEGORIA = CP.CD_CATEGORIA ) ' +
      '      INNER JOIN comp_categorias_coligadas ccc ON ( ccc.cd_categoria = CC.CD_CATEGORIA ) ' +
      ' WHERE (CC.sn_cantina = 0 OR CP.SN_CANTINA_ESTOQUE = 1)  ';
