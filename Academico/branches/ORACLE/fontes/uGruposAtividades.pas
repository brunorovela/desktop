...
type
  TfrmGruposAtividades = class(TForm)
    ...
    qyGruposcd_disciplina: TIntegerField;
    qyGruposdescDisc: TStringField;
    ToolButton1: TToolButton;
    qyGruposcd_professor: TIntegerField;
    qyGruposnr_anosemestre: TIntegerField;
    qyGruposnr_max_alunos: TIntegerField;
...
begin
   ...
   qyGrupos.SQL.Text := ' SELECT '+
                        ...
                        '   dg.cd_disciplina, '+
                        '   dg.nr_max_alunos,  '+
                        '   d.descricao as descDisc, '+
                        '   prof.cd_pessoa as cd_professor, '+
                        ...
                        '   LEFT JOIN disciplinas D on (d.codigo = dg.cd_disciplina and dg.cd_curso = d.curso) '+
                        ' WHERE ';
...
procedure TfrmGruposAtividades.ToolButton1Click(Sender: TObject);
var
   frmCadGruposAtividades: TfrmCadGruposAtividades;
begin
   // Verifica permissão
   ...
   if qyGrupos.RecNo = 0 then
   begin
      Mensagem('Você deve selecionar um registro para duplicar.','Aviso',MB_OK);
      exit;
   end;

   // Verifica informações de Anosemestre/Curso/Turma/Disciplina/Professor a serem repassadas para o cadastro
   frmCadGruposAtividades.setAnoSemestre(qyGruposnr_anosemestre.AsInteger);
   frmCadGruposAtividades.setCurso(qyGruposcd_curso.AsString);
   frmCadGruposAtividades.setTurma(qyGruposcd_turma.AsString);
   frmCadGruposAtividades.setDisciplina(qyGruposcd_disciplina.AsString);
   frmCadGruposAtividades.setDisciplinaDesc(qyGruposdescDisc.AsString);
   frmCadGruposAtividades.setProfessor(qyGruposcd_professor.AsString);
   frmCadGruposAtividades.edMaxVagasGrupo.Text := qyGruposnr_max_alunos.AsString;
