...
uses
  ...
  Provider, UMComboBox, UMAjuda, UMLookupComboBox, uPessoas_Descontos, uLkJSON;
...
type
  TfPessoasDescontoCad = class(TForm)
    ...
    cbManterDescontos: TDBCheckBox;
    UMAjuda1: TUMAjuda;
    qryPessoaDescontosn_manter_desconto_fixo: TSmallintField;
    rbAplicaBruto: TRadioButton;
    rbAplicaLiquido: TRadioButton;
    qryPessoaDescontonr_forma_aplicar_divisao: TSmallintField;
    lcbDepartamentos: TUMLookupComboBox;
    lcbCursos: TUMLookupComboBox;
    cbDescPlano: TUMLookupComboBox;
    qryPessoaDescontoVL_FIXO: TFloatField;
    lblValorFixo: TLabel;
    dbeValorFixo: TDBEdit;
    lblLegenda: TLabel;
    qryPessoaDescontoCD_TIPO_TITULO: TIntegerField;
    qryPessoaDescontoCD_COLIGADA_MATRIZ: TIntegerField;
    qryPessoaDescontoSN_ALTERAR_VENCTO: TSmallintField;
    qryPessoaDescontoDT_VENCTO_INICIAL: TDateField;
    qryPessoaDescontoSN_ALTERAR_TIPO_TITULO: TSmallintField;
    dbchTipoTitulo: TDBCheckBox;
    dbchVencto: TDBCheckBox;
    dbeVenctoInicial: TDBEdit;
    qryTiposTituloCD_COLIGADA_MATRIZ: TIntegerField;
    qryPessoaDescontoDS_TIPO_TITULO: TStringField;
    dblcTipoTitulo: TDBLookupComboBox;
    Panel1: TPanel;
    bvlTipoTitulo: TBevel;
    bvlVencto: TBevel;
    Bevel1: TBevel;
    dbchDividirCondicional: TDBCheckBox;
    qryPessoaDescontoSN_DIVIDIR_DESCONTO_CONDICIONAL: TSmallintField;
    procedure rbAplicaBrutoClick(Sender: TObject);
    procedure rbAplicaLiquidoClick(Sender: TObject);
    procedure qryPessoaDescontoBeforePost(DataSet: TDataSet);
    procedure qryPessoaDescontoCD_TIPO_TITULOChange(Sender: TField);
    procedure dbchVenctoClick(Sender: TObject);
    procedure dbchTipoTituloClick(Sender: TObject);
    procedure cdsTposTituloDescAfterOpen(DataSet: TDataSet);
    procedure qryPessoaDescontoBeforeEdit(DataSet: TDataSet);
    procedure bbtnCancelarClick(Sender: TObject);
    procedure bbtnConfirmarClick(Sender: TObject);
    procedure cbManterDescontosClick(Sender: TObject);
...
uses
   uFSelecionarPessoa, uDM, UItemCombo, uIUMDataModule;
...
procedure TfPessoasDescontoCad.bbtnCancelarClick(Sender: TObject);
begin
   ModalResult:= mrCancel;
end;

procedure TfPessoasDescontoCad.bbtnConfirmarClick(Sender: TObject);
begin
   ModalResult := mrOk;
end;
...
procedure TfPessoasDescontoCad.cbManterDescontosClick(Sender: TObject);
begin
   if cbManterDescontos.Enabled then
   begin
      rbAplicaBruto.Enabled := cbManterDescontos.Checked;
      rbAplicaLiquido.Enabled := cbManterDescontos.Checked;
   end;

   if rbAplicaLiquido.Checked and rbAplicaLiquido.Enabled then
   begin
      dbchDividirCondicional.Enabled := True;
   end;

   if (not rbAplicaLiquido.Checked) or (not rbAplicaLiquido.Enabled) then
   begin
      if qryPessoaDesconto.State in [dsEdit, dsInsert] then
      begin
         qryPessoaDescontoSN_DIVIDIR_DESCONTO_CONDICIONAL.AsInteger := 0;
      end;
      dbchDividirCondicional.Enabled := False;
   end;
end;

procedure TfPessoasDescontoCad.cdsTposTituloDescAfterOpen(DataSet: TDataSet);
begin
   if qryPessoaDesconto.State = dsEdit then
      FJSONRecord.Add('TIPOS_TITULO', DM.DataSetToJson(cdsTposTituloDesc));
end;
...
procedure TfPessoasDescontoCad.dbchTipoTituloClick(Sender: TObject);
begin
   dblcTipoTitulo.Enabled := dbchTipoTitulo.Checked;
end;

procedure TfPessoasDescontoCad.dbchVenctoClick(Sender: TObject);
begin
   dbeVenctoInicial.Enabled := dbchVencto.Checked;
end;
...
begin
   ...
   if dbrgTipoDesconto.ItemIndex = 1 then
   begin
      dbchTipoTitulo.Enabled := True;
      dblcTipoTitulo.Enabled := True;
      dbchVencto.Enabled := True;
      dbeVenctoInicial.Enabled := True;
   end;

   if dbrgTipoDesconto.ItemIndex <> 1 then
   begin
      if qryPessoaDesconto.State in [dsInsert, dsEdit] then
      begin
         qryPessoaDescontoSN_ALTERAR_TIPO_TITULO.AsInteger := 0;
         qryPessoaDescontoCD_TIPO_TITULO.Clear;
         qryPessoaDescontoSN_ALTERAR_VENCTO.AsInteger := 0;
         qryPessoaDescontoDT_VENCTO_INICIAL.Clear;
      end;
      dbchTipoTitulo.Enabled := False;
      dblcTipoTitulo.Enabled := False;
      dbchVencto.Enabled := False;
      dbeVenctoInicial.Enabled := False;
   end;

   cbManterDescontos.Checked := False;
   cbManterDescontos.Enabled := (dbrgTipoDesconto.ItemIndex = 1);
   cbManterDescontos.Checked := (qryPessoaDescontosn_manter_desconto_fixo.AsInteger = 1);

   rbAplicaBruto.Enabled := cbManterDescontos.Enabled and cbManterDescontos.Checked;
   rbAplicaLiquido.Enabled := cbManterDescontos.Enabled and cbManterDescontos.Checked;

   rbAplicaBruto.Checked := ((qryPessoaDescontonr_forma_aplicar_divisao.AsInteger = 1) or qryPessoaDescontonr_forma_aplicar_divisao.IsNull);
   rbAplicaLiquido.Checked := (qryPessoaDescontonr_forma_aplicar_divisao.AsInteger = 2);
end;
...
var
   JSONRecord, JSONNew: TlkJSONobject;
   Operacao, Chave, Obs: AnsiString;
begin
   if ModalResult = mrOk then
   begin
      qryPessoaDescontonr_forma_aplicar_divisao.AsInteger := 1;

      if rbAplicaLiquido.Checked then
      begin
         qryPessoaDescontonr_forma_aplicar_divisao.AsInteger := 2;
      end;

      if qryPessoaDesconto.State = dsInsert then
         Operacao := 'Inclusao';
      ...
      Chave := qryPessoaDescontoCD_DESCONTO_PESSOA.AsString;

      if Assigned(FJSONRecord) then
      begin
         JSONNew := DM.RecordToJSON(qryPessoaDesconto);
         JSONNew.Add('TIPOS_TITULO', DM.DataSetToJson(cdsTposTituloDesc));

         JSONRecord := TlkJSONobject.Create(False);
         JSONRecord.Add('antigo', FJSONRecord);
         JSONRecord.Add('novo', JSONNew);
      end
      else
      begin
         JSONRecord := DM.RecordToJSON(qryPessoaDesconto);
         JSONRecord.Add('TIPOS_TITULO', DM.DataSetToJson(cdsTposTituloDesc));
      end;
         
      try
         Obs := UTF8Decode(TlkJSON.GenerateText(JSONRecord));
         DM.setLog(2021, Operacao, Chave, DM.UsuarioLogado.ColigadaLogada, Obs);
      finally
         FreeAndNil(JSONRecord);
      end;
      
      if frm_Pessoas_Descontos.qryPessoasDesconto.State = dsBrowse then
      begin

         frm_Pessoas_Descontos.qryPessoasDesconto.Refresh;

         if not(frm_Pessoas_Descontos.qryPessoasDesconto.Locate('CD_DESCONTO_PESSOA', dm.LastInsert, [])) then
         begin

            frm_Pessoas_Descontos.qryPessoasDesconto.SQL.Text := Format(frm_Pessoas_Descontos.SSQL, [DM.UsuarioLogado.GetColigadasFilhaSelecionada(), '']);
            frm_Pessoas_Descontos.qryPessoasDesconto.SQL.Add( ' AND CD_DESCONTO_PESSOA = ' + IntToStr(dm.LastInsert) );

            frm_Pessoas_Descontos.qryPessoasDesconto.Open;

         end;
         
      end;

      if frm_Pessoas_Descontos.qryPessoasDesconto.State = dsInactive then
      begin
         frm_Pessoas_Descontos.qryPessoasDesconto.SQL.Text := Format(frm_Pessoas_Descontos.SSQL, [DM.UsuarioLogado.GetColigadasFilhaSelecionada(), '']);
         frm_Pessoas_Descontos.qryPessoasDesconto.SQL.Add( ' AND CD_DESCONTO_PESSOA = ' + IntToStr(dm.LastInsert) );
...
procedure TfPessoasDescontoCad.qryPessoaDescontoBeforeEdit(DataSet: TDataSet);
begin
   FJSONRecord := DM.RecordToJSON(DataSet);
end;

procedure TfPessoasDescontoCad.qryPessoaDescontoBeforePost(DataSet: TDataSet);
begin
   if not dbchTipoTitulo.Checked then
   begin
      qryPessoaDescontoCD_TIPO_TITULO.Clear;
      qryPessoaDescontoCD_COLIGADA_MATRIZ.Clear;
   end;

   if not dbchVencto.Checked then
   begin
      qryPessoaDescontoDT_VENCTO_INICIAL.Clear;
   end;
end;
...
begin
   if (not qryPessoaDescontoCD_DEPTO.IsNull) and
      (lcbDepartamentos.Color = clInfoBk) then
   begin
      lcbDepartamentos.Color := clWindow;
   end;
end;
...
begin
   if ((not qryPessoaDescontoCD_DESCONTO.IsNull) or
      ...
      (cbDescPlano.Color = clInfoBk) then
   begin
      cbDescPlano.Color := clWindow;
   end;
   ...
   if not qryPlanosDescontoVALORDESCONTO.IsNull then
      qryPessoaDescontoVL_FIXO.AsCurrency := qryPlanosDescontoVALORDESCONTO.AsCurrency
   else
      qryPessoaDescontoVL_FIXO.Clear;
end;
...
procedure TfPessoasDescontoCad.qryPessoaDescontoCD_TIPO_TITULOChange(
  Sender: TField);
begin
   qryPessoaDescontoCD_COLIGADA_MATRIZ.AsInteger := DM.UsuarioLogado.ColigadaLogada;
end;
...
procedure TfPessoasDescontoCad.rbAplicaBrutoClick(Sender: TObject);
begin
   if rbAplicaLiquido.Checked then
   begin
      dbchDividirCondicional.Enabled := True;
   ...
   if not rbAplicaLiquido.Checked then
   begin
      if qryPessoaDesconto.State in dsEditModes then
      begin
         qryPessoaDescontoSN_DIVIDIR_DESCONTO_CONDICIONAL.AsInteger := 0;
      end;
      dbchDividirCondicional.Enabled := False;
...
procedure TfPessoasDescontoCad.rbAplicaLiquidoClick(Sender: TObject);
begin
   if rbAplicaLiquido.Checked then
   begin
      dbchDividirCondicional.Enabled := True;
...
begin
   ...
   begin
      Result := False;
       lcbDepartamentos.Color := clInfoBk;
   end;
   ...
   if qryPessoaDescontoDS_CURSO.IsNull then
   begin
      Result := False;
      lcbCursos.Color := clInfoBk;
   end;
