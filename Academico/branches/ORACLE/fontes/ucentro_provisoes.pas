...
type
  ...
  private
    ...
    procedure filtrar;
    procedure ativarInclusaoAlteracaoExclusao();
...
procedure Tfrm_centro_provisoes.ativarInclusaoAlteracaoExclusao;
var
  condicao : Boolean;
begin
  condicao := PageControl1.TabIndex = 1;

  btnInserir.Enabled := condicao AND not (tblProvisoes.State in [dsInsert, dsEdit]);
  btnAlterar.Enabled := condicao AND not (tblProvisoes.State in [dsInsert, dsEdit]);
  btnExcluir.Enabled := condicao AND not (tblProvisoes.State in [dsInsert, dsEdit]);
...
begin
    ...
    ativarInclusaoAlteracaoExclusao();
end;
...
procedure Tfrm_centro_provisoes.PageControl1Change(Sender: TObject);
begin
  ativarInclusaoAlteracaoExclusao;
end;
