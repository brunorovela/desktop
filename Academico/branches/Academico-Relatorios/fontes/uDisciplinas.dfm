object frmDisciplinas: TfrmDisciplinas
  ...
  object pgcTabs: TPageControl
    ...
    object tbsDisciplinas: TTabSheet
      ...
      object pnlDiscListagem: TPanel
        ...
        object dbgDisciplinas: TZSortedGrid
          Left = 0
          Top = 71
          Width = 404
          Height = 503
  ...
  object qry_verifica_cursos_disciplinas: TUMZQuery
    Connection = DM.db
    SQL.Strings = (
      'SELECT'
      #9'COUNT(D.CD_DISCIPLINA_PAI) has_disciplinas_curso'
      'FROM'
      #9'CURSOS_MESTRE C'
      'INNER JOIN cursos_coligadas cc ON (cc.CD_CURSO = c.CD_CURSO)'
      'LEFT JOIN DISCIPLINAS D ON (C.CD_CURSO = D.CURSO)'
      'AND ('
      #9'D.CD_DISCIPLINA_PAI = :CD_DISCIPLINA_PAI'
      ')'
      'WHERE'
      #9'C.SN_ATIVO = '#39'S'#39
      #9'AND D.CD_DISCIPLINA_PAI IS NOT NULL'
      #9'AND CC.CD_COLIGADA IN (%s)')
    Params = <
      item
        DataType = ftUnknown
        Name = 'CD_DISCIPLINA_PAI'
