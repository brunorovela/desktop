...
uses
  ...
  UZDataset, Math, Grids, DBGrids, StrUtils,
  DBTables, ZStoredProcedure, ClassRegistros, Spin, UMDateTimePicker,
  UMEditNumerico, UMEditMonetario, uItemCombo, UMComboBox, UMAjuda, DateUtils,
...
type
  TfrmNegociacao = class(TForm)
    ...
    lblPercentualEntrada: TLabel;
    edtPercentualEntrada: TEdit;
    lblValorEntrada: TLabel;
    qyParcNovasnossonumero: TStringField;
    procedure edtPercentualEntradaExit(Sender: TObject);
    procedure qyNegociavl_entradaChange(Sender: TField);
    procedure qyNegociavl_bruto_negociaChange(Sender: TField);
    procedure edtPercentualEntradaKeyPress(Sender: TObject; var Key: Char);
  ...
  private
    ...
    procedure copiaArray(arrBase:Array of Currency;var arrResultado:Array of Currency);
      procedure CalcValorEntrada;
      procedure CalcPercentualEntrada;
      procedure TransfereDescontos;
  public
    BrutoTotal : Currency;
...
uses uDM, Main, uUsuario, uMotivos, uClassMovimento,
  uNegociacaoSimula, uMensalidades, uCobranca, uGeneral, uRemessaClass;
...
begin
   ...
   begin
      ...
      if qyNegociavl_desconto_negocia.AsCurrency > 0 then begin

         ArrayDescontosNovas[n] := RoundFloat(( qyNegociavl_desconto_negocia.AsCurrency / qyNegociavl_bruto_negocia.AsCurrency * qyNegociavl_entrada.AsCurrency ), 2)
   ...
   begin
      ...
      if MonthOf(MyDay) > wMonth then
      begin
         repeat
            MyDay := MyDay - 1;
         until (MonthOf(MyDay) = wMonth) and (not (DayOfWeek(MyDay) in [1, 7]));
      end;

      ArrayCalculados[n] := RoundFloat( ( cTotalBrutoCadaParcela ), 2 );
      ArrayDatas[n] := MyDay;
      ArrayJurosNovas[n] := valorLiquidoCadaParcela - ArrayCalculados[n];
      // Verificar se tem desconto e se teve entrada, pois o desconto foi calculado proporcionalmente na entrada.
      if qyNegociavl_desconto_negocia.AsCurrency > 0 then begin

         if qyNegociavl_entrada.AsCurrency > 0 then begin

            ArrayDescontosNovas[n] := (qyNegociavl_desconto_negocia.AsCurrency - ArrayDescontosNovas[0]) / txtParcelas.Value ;
   ...
   begin
      ArrayJurosNovas[n - 1] := ArrayJurosNovas[n - 1] + (((qyNegociavl_bruto_negocia.AsCurrency - qyNegociavl_entrada.AsCurrency + qyNegociavl_juros_negocia.AsCurrency) * jurosNovasParcelas) - cJurosSoma) + qyNegociavl_juros_negocia.AsCurrency;
   ...
   if (cJurosSoma <> qyNegociavl_juros_negocia.AsCurrency) AND (jurosNovasParcelas <= 0) then
   begin
...
begin
   ...
   //Se for 0 marca vencidas(também vale pra manual), caso não marca todas
...
procedure TfrmNegociacao.CalcPercentualEntrada;
var
   Multa, Entrada, Percentual: Double;
begin
   Multa := qyNegociavl_bruto_negocia.AsFloat;
   Entrada := qyNegociavl_entrada.AsFloat;

   if Entrada <> 0 then
   begin
      Percentual := Entrada * 100 / Multa;
      edtPercentualEntrada.Text := Format('%f', [Percentual]);
   end;

   if Entrada = 0 then
   begin
      edtPercentualEntrada.Text := '0';
   end;
end;
...
procedure TfrmNegociacao.CalcValorEntrada;
var
   FmtSettings: TFormatSettings;
   Percentual, Entrada, Multa: Double;
begin
   GetLocaleFormatSettings(LOCALE_SYSTEM_DEFAULT, FmtSettings);
   FmtSettings.ThousandSeparator := #32;
   FmtSettings.DecimalSeparator := #44;
   Percentual := StrToFloatDef(edtPercentualEntrada.Text, 0, FmtSettings);

   if Percentual <> 0 then
   begin
      Multa := qyNegociavl_bruto_negocia.AsFloat;
      Entrada := Multa * Percentual / 100;
      qyNegociavl_entrada.AsFloat := Entrada;
   end;
end;
...
begin
  ...
  begin
    ...
    Self.ModalResult := mrOk;
    if PrincipalForm.ProcuraForm( TForm(frmMensalidades) ) then begin
      frmMensalidades.Filtra();
    end;
    if PrincipalForm.ProcuraForm( TForm(frmCobranca) ) then begin
      frmCobranca.PreencherTitulos();
   end;
    CloseModal;
  end;
end;
...
procedure TfrmNegociacao.TransfereDescontos;
const
   SSQLUpdateMovimentos = ''
      + 'UPDATE '
      + '    fin_mov_cr m '
      + 'SET '
      + '    m.cd_mensalidade_origem = m.cd_mensalidade '
      + 'WHERE '
      + '    m.cd_mensalidade_origem IS NULL AND '
      + '    EXISTS ( '
      + '        SELECT '
      + '            n.cd_negocia '
      + '        FROM '
   ...
   SSQLSelectParcelas = ''
      ...
      + '    m.valordesconto '
      + 'FROM '
      + '    fin_negocia_parc_nova p '
      + '        JOIN mensalidades m ON '
      + '            (p.cd_mensalidade = m.cd_mensalidade) '
      + 'WHERE '
      + '    COALESCE(m.valordesconto, 0) > 0 AND '
      + '    p.cd_negocia = ? ';
   SSQLSelectDescontos = ''
      + 'SELECT '
      + '    m.cd_acao, '
      + '    SUM(COALESCE(m.vl_desconto, 0)) vl_desconto '
      + 'FROM '
      + '    fin_mov_cr m '
      + 'WHERE '
      + '    m.sn_desc_condicional = 1 AND '
      + '    EXISTS ( '
      + '        SELECT '
      + '            n.cd_negocia '
      + '        FROM '
      + '            fin_negocia_parc_atual n '
      + '        WHERE '
      + '            m.cd_mensalidade_origem = n.cd_mensalidade AND '
      + '            n.cd_negocia = ? '
      + '    ) '
      + 'GROUP BY '
      + '    m.cd_acao ';
   SSQLSelectDescontoTotal = ''
      + 'SELECT '
      + '    SUM(COALESCE(m.vl_desconto, 0)) '
      + 'FROM '
      + '    fin_mov_cr m '
      + 'WHERE '
      + '    m.sn_desc_condicional = 1 AND '
      ...
      + '            n.cd_negocia = ? '
      + '    ) ';
   SObservacao = 'Transferência de desconto condicional após negociação';
var
   NegociacaoId: Integer;
   StmtUpdate, StmtParcelas, StmtDescontos, StmtDescontoTotal: IZPreparedStatement;
   RsParcelas, RsDescontos, RsDescontoTotal: IZResultSet;
   DescontoTotal, DescontoNovo: Double;
   Movimento: TMovimento;
begin
   NegociacaoId := qyNegociacd_negocia.AsInteger;
   
   StmtUpdate := PrepareStatement(SSQLUpdateMovimentos);
   try
      StmtUpdate.SetInt(1, NegociacaoId);
   ...
   StmtDescontoTotal := PrepareStatement(SSQLSelectDescontoTotal);
   try
      StmtDescontoTotal.SetInt(1, NegociacaoId);
      RsDescontoTotal := StmtDescontoTotal.ExecuteQueryPrepared;
      try
         if RsDescontoTotal.Next then
            DescontoTotal := RsDescontoTotal.GetFloat(1);
      finally
         RsDescontoTotal.Close;
         RsDescontoTotal := nil;
      end;
   finally
      StmtDescontoTotal.Close;
      StmtDescontoTotal := nil;
   end;
   ...
   try
      ...
      Movimento.Observacao := SObservacao;
      Movimento.DescCondicional := 1;

      StmtParcelas := PrepareStatement(SSQLSelectParcelas);
      try

         StmtParcelas.SetInt(1, NegociacaoId);
         RsParcelas := StmtParcelas.ExecuteQueryPrepared;
         try

            StmtDescontos := PrepareStatement(SSQLSelectDescontos);
            try

               StmtDescontos.SetResultSetConcurrency(rcUpdatable);
               StmtDescontos.SetInt(1, NegociacaoId);
               RsDescontos := StmtDescontos.ExecuteQueryPrepared;
               try

                  while RsParcelas.Next do
                  begin

                     if not RsDescontos.IsBeforeFirst then
                        RsDescontos.BeforeFirst;

                     while RsDescontos.Next do
                     begin
                        DescontoNovo :=
                           RsParcelas.GetFloatByName('valordesconto') *
                           RsDescontos.GetFloatByName('vl_desconto') /
                           DescontoTotal;

                        Movimento.CodigoTitulo := RsParcelas.GetIntByName('cd_mensalidade_origem');
                        Movimento.MensalidadeOrigem := RsParcelas.GetIntByName('cd_mensalidade');
                        Movimento.Coligada := RsParcelas.GetIntByName('cd_coligada');
                        Movimento.CodigoMovimento := RsParcelas.GetIntByName('cd_mensalidade');
                        Movimento.CodAcao := RsDescontos.GetIntByName('cd_acao');
                        Movimento.ValorDesconto := DescontoNovo;
...
var
   situacoesMensalidade:String;
   objRemessa: TRemessa;
begin
   situacoesMensalidade := '';
   if not DM.UsuarioLogado.TemPermissao( 2017, npExcluir, True ) then Exit;

   // Verificar se alguma parcela Nova já foi PAGA ou NEGOCIADA, então neste caso, não deixar desfazer
   qyAux.SQL.Text :=
     ' SELECT sf.ds_situacao FROM mensalidades INNER JOIN fin_negocia_parc_nova ON (mensalidades.cd_mensalidade = fin_negocia_parc_nova.cd_mensalidade) ' +
     ' INNER JOIN situacoes_financeiras sf ON ( sf.cd_situacao = mensalidades.situacao) '+
     ' WHERE  fin_negocia_parc_nova.cd_negocia = ' + qyNegociaAnteriorcd_negocia.AsString +
     ' AND mensalidades.situacao NOT IN (2) AND mensalidades.tipoparcela = 4 GROUP BY sf.cd_situacao';
   qyAux.Open();
   ...
   if not qyAux.Eof then begin
      ...
      while not qyAux.Eof do
      begin
         situacoesMensalidade := situacoesMensalidade + chr(13) + ' -'+ qyAux.FieldByName('ds_situacao').AsString;
         qyAux.Next;
      end;

     Mensagem('Atenção' + chr(13) + 'Não é possível desfazer esta negociação. Existem parcelas negociadas nas seguintes situações:'+ situacoesMensalidade, 'UNIMESTRE', MB_OK + MB_ICONWARNING);
     Exit;
   end;
   ...
   // Antes de excluir as parcelas novas, se estiver utilizando remessa nova, enviar pedido de baixa

   if DM.variavel_parametro('sn_processo_remessa_novo') = '1' then begin

      objRemessa := TRemessa.Create;

      qyParcNovas.First;
      while not qyParcNovas.Eof do begin

          objRemessa.registraAcaoBaixa(qyParcNovasNossonumero.AsString);
...
begin
   ...
   sNroDiasVenc := '';
    ...
    if not DM.UsuarioLogado.TemPermissao(
            DM.iCdPessoaLogado, 'UMFinanceiro.SalvaNegociacao', npIncluir, True
    ) then
    begin
       Exit;
    end;
...
procedure TfrmNegociacao.edtPercentualEntradaExit(Sender: TObject);
begin
   CalcValorEntrada;
end;

procedure TfrmNegociacao.edtPercentualEntradaKeyPress(Sender: TObject;
  var Key: Char);
begin
   if Key = #44 then
   begin
      if AnsiContainsStr(edtPercentualEntrada.Text, #44) then
      begin
         Key := #0;
      end;
   end;
...
procedure TfrmNegociacao.qyNegociavl_bruto_negociaChange(Sender: TField);
begin
   qyNegocia.DisableControls;
   CalcValorEntrada;
   qyNegocia.EnableControls;
end;

procedure TfrmNegociacao.qyNegociavl_entradaChange(Sender: TField);
begin
   qyNegocia.DisableControls;
   CalcPercentualEntrada;
   qyNegocia.EnableControls;
...
begin
  ...
  tblMensalidades.SQL.Clear();
   ...
   tblMensalidades.SQL.Text :=
     ...
     '   INNER JOIN Coligadas AS c ON (c.cd_coligada = men.cd_coligada )  ' +
     ...
     '   LEFT JOIN matriculas mat ON (mat.codigoaluno = men.codigoaluno AND mat.turma = men.turma AND mat.anosemestre = men.anosemestre) '+
     '  WHERE men.codigoAluno = ' + Format('%d', [Self.FICodigoAluno]) +
     '        AND c.cd_coligada_matriz = ' + IntToStr(DM.GetUsuarioLogado.GetColigadaLogada.Codigo) +
   ...
   begin
    ...
    if tblMensalidadesSituacao.AsInteger in [0, 1] then begin
       ...
       ArrayValores[tblMensalidades.RecNo-1]  := tblMensalidadesValorTotalCalc.Value - tblMensalidadesValorJurosCalc.Value  // ValorTotal Reajustado até aqui
        ...
        if (tblMensalidadesDataVencimento.AsDateTime < IncDay(DataHoje, -1) ) then
           sSit := 'VENC'
        else
           sSit := 'PEND';
...
begin
  ...
  btnSimular.Enabled   :=  DM.UsuarioLogado.TemPermissao( DM.iCdPessoaLogado, 'UMFinanceiro.SalvaNegociacao', npIncluir, false);
