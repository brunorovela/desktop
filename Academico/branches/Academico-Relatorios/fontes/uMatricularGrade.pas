...
procedure TfrmMatricularGrade.carregaCodigoGrade;
const
   SQL_BUSCA_GRADE =
   'SELECT CD_GRADE FROM turmas WHERE codigo = :codigo AND anosemestre = :anosem AND curso = :curso';
var
   qryGrade : TUMZQuery;
begin
   dm.CriarConsulta(qryGrade);
   qryGrade.SQL.Add(SQL_BUSCA_GRADE);
   qryGrade.ParamByName('codigo').AsString := turmaFiltro;
   qryGrade.ParamByName('curso').AsString := cursoFiltro;
   qryGrade.ParamByName('anosem').AsInteger := anoSemestre;

   qryGrade.Open;

   if(qryGrade.IsEmpty) then
   begin
      exit;
   end;

   gradeFiltro := qryGrade.FieldByName('CD_GRADE').AsString;
end;
