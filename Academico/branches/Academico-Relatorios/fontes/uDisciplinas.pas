...
begin
   CarregaCursos;
   textoDisciplina := dbeDiscDesc.Text;
end;
...
var
   ...
   S: AnsiString;
begin
   if dbeDiscDesc.Text <> textoDisciplina then
   begin
      Stmt := PrepareStatement(SSQLSelectDescDup);
      try
         Stmt.SetString(1, dbeDiscDesc.Text);
         Stmt.SetString(2, dbeDiscCodPai.Text);
         Rs := Stmt.ExecuteQueryPrepared;
         if Rs.Next then
         begin
   ...
   if dbeDiscSigla.Text = '' then
   begin
      Result := False;
      MessageDlg('É obrigatório preencher a sigla da disciplina', mtInformation, [mbOK], -1);
   end
   else if dbeDiscCodPai.Text = '' then
   begin
      Result := False;
      MessageDlg('É obrigatório preencher o código mestre da disciplina', mtInformation, [mbOK], -1);
   end
   else if dbeDiscDesc.Text = '' then
   begin
      Result := False;
