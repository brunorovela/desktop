...
type                  
  TfrmMensalidades = class(TForm)
    ...
    tblMensalidadesresp: TStringField;
    N4: TMenuItem;
    ConfirmarNegociao1: TMenuItem;
    procedure ConfirmarNegociao1Click(Sender: TObject);
...
begin
   ...
   if tblMensalidadessn_nfe_gerada.AsInteger = 1 then begin
     Mensagem('Não é possível aplicar créditos em títulos que já tenham a Nota Fiscal emitida. '+
                  'Você pode tentar cancelar a nota para poder aplicar o crédito.',
                  'Aviso',
...
begin
   ...
   begin
      Mensagem(
         ...
         'utilize a opção de cancelar movimentação, no Controle de Cartões.',
         'Aviso',
...
begin
  ...
  if tblMensalidadesPercDescExtra.AsCurrency > 100 then begin
     tblMensalidadesPercDescExtra.AsCurrency := 100;
  end;
  ...
  if tblMensalidadesPercDescCond.AsCurrency > 100 then begin
      tblMensalidadesPercDescCond.AsCurrency := 100;
  end;
...
begin
  ...
  begin
     tblMensalidades.SQL.Text :=
      ...
      '  LEFT JOIN pessoas AS pfc ON (pfc.cd_pessoa = c.CD_RESPONSAVEL ) '+
     ...
     tblMensalidades.SQL.Text :=
      ...
      + '        rem_envios.cd_envio '
      + '      FROM rem_envios '
      + '      JOIN rem_layouts ON (rem_envios.cd_layout = rem_layouts.cd_layout) '
      + '      JOIN rem_ocorrencias ON (rem_envios.cd_ocorrencia = rem_ocorrencias.cd_ocorrencia) '
      + '      WHERE rem_envios.nr_nossonumero = me.nossonumero '
      + '      AND rem_layouts.cd_origem = 3 '
      + '      AND rem_ocorrencias.cd_acao = 101 '
      + '    ) '
      + '  THEN '
      + '    IF(EXISTS  (SELECT cd_pessoa FROM pessoas_debito_automatico as dx where dx.cd_pessoa = me.codigoaluno AND dx.sn_autoriza_debito = 1 )     '
      + '        , ''*'', ''NÃO'') '
      + '  WHEN '
      + '    EXISTS ( '
      + '      SELECT '
      ...
      + '      WHERE rem_envios.nr_nossonumero = me.nossonumero'
      + '      AND rem_layouts.cd_origem = 3 '
      + '        AND IFNULL(rem_envios.cd_arquivo,0) >= 0  '
      + '      AND rem_envios.sn_ignorado = 0   '
      + '      AND rem_ocorrencias.cd_acao != 101 '
      + '    ) '
      + '  THEN '
      + '    ''SIM'' '
      ...
      + '          FROM               '
      + '            fin_cheques      '
      + '            JOIN situacoes ON (situacoes.cd_situacao = fin_cheques.cd_situacao AND situacoes.cd_modulo = 2050) '
      + '              JOIN fin_cheques_mensalidades ON                                                                 '
      + '                (fin_cheques.CD_CHEQUE = fin_cheques_mensalidades.CD_CHEQUE)                                   '
      + '          WHERE                                                                                                '
      + '            situacoes.cd_auxiliar = 2 AND                                                                      '
      + '            fin_cheques_mensalidades.cd_mensalidade = me.cd_mensalidade                                        '
      + '        )                                                                                                      '
      + '      THEN                                                                                                     '
      + '        CONCAT(sf.ds_sigla_situacao, ''(CH-DEV)'')                                                               '
      + '      ELSE '
      ...
      ' INNER JOIN fin_moedas mo ON (me.cd_moeda = mo.cd_moeda)                     ' +
      ' INNER JOIN situacoes_financeiras sf ON (IF(me.situacao = 2 AND me.datavencimento < current_date(), 200, me.situacao)  =  sf.cd_situacao)' +
      ' INNER JOIN Pessoas pe ON (me.CodigoAluno = pe.cd_pessoa ) LEFT JOIN pessoas Resp ON (COALESCE(me.cd_resp, pe.cd_resp_finan) = Resp.cd_pessoa) '+
      '  LEFT JOIN departamentos dp ON (me.depto = dp.codigo) ' +
      ...
      '  LEFT JOIN fin_cadastro_contas tc ON (tc.cd_caixa = te.cd_caixa) ' +
      '  LEFT JOIN fin_cobranca c ON (me.codigoaluno = c.cd_pessoa) '+
      '  LEFT JOIN pessoas pfc ON (pfc.cd_pessoa = c.CD_RESPONSAVEL )	 '+
      '  LEFT JOIN situacoes sit ON (c.cd_situacao = sit.cd_situacao) AND (sit.cd_modulo = 2053) '+
...
begin
  ...
  if tblMensalidadessn_nfe_gerada.AsInteger = 1 then begin
     Mensagem('Não é possível alterar os descontos desta mensalidade pois a Nota Fiscal já foi emitida, '+
                  'você pode utilizar o recurso de Negociação sobre essa parcela.',
                  'Aviso',
                  MB_OK + MB_ICONWARNING);
     Exit;
  end;
...
procedure TfrmMensalidades.ConfirmarNegociao1Click(Sender: TObject);
begin
     // Colocado temporariamente para confirmar a negociação simulada e paga a primeira parcela
     if ((tblMensalidadesSituacao.AsInteger = 0) OR (tblmensalidadesSituacao.AsInteger = 1)) then  begin
         dm.ComfirmarNegociacaoBaixa(tblMensalidadescd_mensalidade.AsInteger);
         DM.LimpaMensaNegociacaoPaga(tblMensalidadescd_mensalidade.AsInteger, tblMensalidadesCodigoAluno.AsInteger);
         tblMensalidades.CLose();
         tblMensalidades.Open();
     end else begin
         MessageBox(0, 'A parcela precisa estar paga para confirmar a negociação', 'Atenção', MB_OK);
     end;
end;
