...
uses
  ...
  UMComboBox, uListaColigadas, uColigada, UMDateTimePicker;
...
begin
   ...
   qyAux.SQL.Text :=
      ...
      ' WHERE ct_tipo_titulo = 1 AND sn_ativo = 1 AND cd_coligada_matriz = :cd_coligada_matriz ORDER BY ds_tipo_titulo ';
...
begin
   ...
   begin
         ...
         begin
            umdtVencto.Enabled := false;
            rgVencto.Enabled := false;
            rgVencto.ItemIndex := 0;
         end;
         if AICodigoAluno <> 0 then
         begin
            umdtVencto.Date := Self.getDataVencimentoAluno(AICodigoAluno);
         end;
   end;
end;
...
begin
   ...
   try
      dtDataVencimento := umdtVencto.Date;
   except
