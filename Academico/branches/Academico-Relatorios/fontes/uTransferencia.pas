...
uses
   Windows, Dialogs, SysUtils, Contnrs, DB, ZConnection, uDM, uClassMovimento, Classes, General,
   ZAbstractRODataset, ZAbstractDataset, ZDataset, UZDataset, Math, ZDbcIntfs, UZDbcFuncs;
...
uses ClassPlanosPagamento, ClassRegistros, uMatriculas, uRemessaClass;
...
const
   ...
   SQL_GET_CENTROCUSTO_DEPTO =
      ...
      'FROM '+
         'turmas t '+
            'INNER JOIN cursos_coligadas as c ON (t.cd_coligada = c.cd_coligada AND t.curso = c.cd_curso) '+
      'WHERE '+
         't.anosemestre = :anosemestre AND '+
         't.codigo = :codigo';
   ...
   SQL_ATUALIZA_TURMA_PARCELAS =
      ...
      'WHERE '+
         ...
         '(sn_nfe_gerada = 0 OR cd_coligada = :coligada_nova) %s';
   ...
   SQL_ATUALIZA_PARCELAS =
      ...
      'WHERE '+
         ...
         'curso = :curso AND '+
         'anosemestre = :anosemestre AND ' +
         '(sn_nfe_gerada = 0 OR cd_coligada = :coligada_nova)';
   ...
   SQL_INSERE_MATRICULA_TURMA =
      'INSERT INTO matriculas '+
         '(anosemestre, turma, codigoaluno, curso, dataemissao, usuario, planopagamento, situacao, dataentrada, cd_ingresso, cd_matricula_curso, cd_turma_anterior, cd_pessoa_matriculou)'+
      'VALUES '+
         '(:anosemestre, :turma, :codigoaluno, :curso, NOW(), :usuario, :planopagamento, :situacao, NOW(), :cd_ingresso, :cd_matricula_curso, :cd_turma_anterior, :cd_pessoa_matriculou)';

   SQL_INSERE_FICHAINDIVIDUAL =
      'INSERT INTO fichaindividual '+
         '(anosemestre, turma, codigograde, codigoaluno, curso, disciplina, serie, situacao, turmamatricula, dt_inclusao) '+
      'VALUES '+
...
const
   SQL_VERIFICA_NFE =
      'SELECT * from Mensalidades '+
         ' WHERE codigoaluno = ? AND turma = ? AND anosemestre = ? AND ' +
         ' sn_nfe_gerada = 1 AND cd_coligada <> ? ';
var
...
begin
   ...
   {
      Verifica se há nfes vinculadas a outras unidades e avisa o usuário
   }
   Stmt := PrepareStatement(SQL_VERIFICA_NFE);
   try
      Stmt.SetInt(1, AUMMatriculaTurmaAtual.CodAluno);
      Stmt.SetString(2, AUMMatriculaTurmaAtual.CodTurma);
      Stmt.SetInt(3, AUMMatriculaTurmaAtual.AnoSemestre);
      Stmt.SetInt(4, AUMatriculaTurmaNovo.CodColigada);
...
var
   LSMatSitPadrao, LSFinParcAnt, LSTitulosCancelar, Param, Filter: string;
begin
   ...
   Param := DM.variavel_parametro('financeiro_parcelas');

   Filter := 'AND situacao = 2 and DATE(datavencimento) >= CURRENT_DATE';

   if Param = '[PAGAS]' then
   begin
      Filter := 'AND situacao IN (0, 3, 5)';
   end;
   ...
   FQyAtualizaTurmaParcelas.SQL.Text := Format(SQL_ATUALIZA_TURMA_PARCELAS,
      [Filter]);
      ...
      try
         ...
         begin
            ...
            begin
               ...
               if (AUMMatriculaTurmaNovo.PlanoPagamento.CreditosBase > 0) then
               begin
               LDValorCredito :=
                  Self.FQyGetItensPlanoPgto.FieldByName('valorbruto').AsCurrency /
               ...
               begin
                  ...
                  if ((AUMMatriculaTurmaNovo.PlanoPagamento.CreditosBase * LDCreditos) > 0) then
                  begin
                     LDValorBruto :=
                     Self.FQyGetItensPlanoPgto.FieldByName('valorbruto').Value /
                     AUMMatriculaTurmaNovo.PlanoPagamento.CreditosBase * LDCreditos;
               ...
               if ( Self.FQyGetItensPlanoPgto.FieldByName('valorbruto').Value > 0) AND
               (Self.FQyGetItensPlanoPgto.FieldByName('descontoextra').Value > 0) then
               begin
                LDDescExtra :=
         ...
         begin
            ...
            begin
               ...
               begin
                 ...
                 objRemessa.registraAcaoCancelamento(Self.FQyGetMensCancelar.FieldByName('nossonumero').AsString);
               end;
...
begin
   ...
   Self.FQyDesativaFichaIndividual.SQL.Text := Self.FQyDesativaFichaIndividual.SQL.Text + ' AND not fi.situacao in (0,12)';
end;
...
begin
   ...
   Self.FQyCriaMatriculaTurma.ParamByName('cd_pessoa_matriculou').AsInteger := umatriculas.frmMatriculas.tblMatriculasCD_PESSOA_MATRICULOU.AsInteger;
   try
