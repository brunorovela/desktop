...
begin
  ...
  if not snPermissaoRestrita then  begin
     frm_Tesouraria_gerar.qyPlano_Contas.SQL.Add(' AND IFNULL(sn_restringir_permissao,0) = 0  ');
   end;
  frm_Tesouraria_gerar.qyPlano_Contas.SQL.Add(
  'ORDER BY                      '+
  '   ds_conta                   ');
...
begin
   ...
   '    fin_acoes_movimento                        ' +
   ' WHERE                                         ' +
   '    NOT(ISNULL(cd_movimento_estorno))  AND     ' +
...
begin
  if not DM.UsuarioLogado.TemPermissao( iModuloPadrao, npAlterar, True ) then Exit;
   ...
   begin
      ...
      if (qryTesourariatp_entrada_saida.AsInteger = 1) then
          frm_Tesouraria_gerar.recebendo := true
      else
          frm_Tesouraria_gerar.recebendo := false;
