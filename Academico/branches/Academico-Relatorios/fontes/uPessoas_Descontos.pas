...
type
  ...
  Tfrm_Pessoas_Descontos = class(TForm)
    ...
    qryTiposTituloCD_DESCONTO_PESSOA: TIntegerField;
    qryTiposTituloCD_TIPO_TITULO: TSmallintField;
    qryTiposTituloCD_COLIGADA: TSmallintField;
...
uses
   ...
   UFPessoasDescontoCad, UFObservacoes, uLkJSON;
...
begin
   ...
   begin
      ...
      fPessoasDescontoCad.dblcTipoTitulo.Enabled := fPessoasDescontoCad.qryPessoaDescontoSN_ALTERAR_TIPO_TITULO.AsInteger = 1;
...
begin
   ...
   begin
      ...
      begin
         JSONRecord := DM.RecordToJSON(qryPessoasDesconto);
         try

            qryTiposTitulo.ParamByName('CD_DESCONTO_PESSOA').AsInteger := qryPessoasDescontoCD_DESCONTO_PESSOA.AsInteger;
            qryTiposTitulo.Open;
            JSONRecord.Add('TIPOS_TITULO', DM.DataSetToJson(qryTiposTitulo));
            qryTiposTitulo.Close;

            Obs := UTF8Decode(TlkJSON.GenerateText(JSONRecord));
            DM.setLog(2021, SOperacao,
               qryPessoasDescontoCD_DESCONTO_PESSOA.AsString,
               DM.UsuarioLogado.ColigadaLogada, Obs);
