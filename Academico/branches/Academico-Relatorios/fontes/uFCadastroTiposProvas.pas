...
begin
     ...
     if ( hasAvaliacoesVinculadas( qyTiposAvaliacao.FieldByName('cd_avaliacao_tipo').AsInteger ) ) then begin
        ...
        if qyTiposAvaliacao.FieldByName('cd_tipo').AsInteger IN [1, 2] then begin
           ...
           dbrgTipoAvaliacao.Controls[4].Enabled := False;

        // 3-PS, 4-RE ou 5-PI, não permite alterar o tipo
        end else if qyTiposAvaliacao.FieldByName('cd_tipo').AsInteger IN [3, 4, 5] then begin
...
begin
   ...
   if (tipo = 1) then begin
      ...
      qyTiposAvaliacao.FieldByName('ds_tipo').AsString := 'PI';
