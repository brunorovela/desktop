...
type
  TfrmAtivComplementares = class(TForm)
    ...
    Label11: TLabel;
    dbAtividadesHrSemestre: TDBEdit;
    Label12: TLabel;
    qyAtividadesnr_horas_etapa: TFloatField;
    DBCheckBox1: TDBCheckBox;
    qyGrupossn_estagio: TSmallintField;
    procedure dbAtividadesHrSemestreKeyPress(Sender: TObject; var Key: Char);
    procedure dsGruposStateChange(Sender: TObject);
    procedure dsAreasStateChange(Sender: TObject);
...
procedure TfrmAtivComplementares.dbAtividadesHrSemestreKeyPress(Sender: TObject;
  var Key: Char);
begin
   Self.ProximoCampo( Key );
end;
...
procedure TfrmAtivComplementares.dsAreasStateChange(Sender: TObject);
begin
   if qyAreas.State = dsInsert then
      qyAreascd_ge_area.AsInteger := SugereCodigo(pcGeral.ActivePageIndex);
end;
...
procedure TfrmAtivComplementares.dsAtividadesStateChange(Sender: TObject);
begin
   if qyAtividades.State = dsInsert then
      qyAtividadescd_ge_atividade.AsInteger := SugereCodigo(pcGeral.ActivePageIndex);
end;

procedure TfrmAtivComplementares.dsGruposStateChange(Sender: TObject);
begin
   if qyGrupos.State = dsInsert then
      qyGruposcd_ge_grupo.AsInteger := SugereCodigo(pcGeral.ActivePageIndex);
end;
