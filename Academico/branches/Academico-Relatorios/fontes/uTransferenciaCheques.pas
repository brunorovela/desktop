...
type
  TfTransferenciaCheques = class(TForm)
    ...
    edValorTotal: TEdit;
    lbMsg: TLabel;
    umdtTransferencia: TUMDateTimePicker;
    Label3: TLabel;
  ...
  public
    ListaContas, ListaSituacao, ListaAuxiliar : Array of Integer;
    iCodCaixa, iCodSituacao, iCodColigada: Integer;
    bAtivarDebito: Boolean;
    iContaPadrao : Integer;
    { Public declarations }
  end;
...
begin
   ...
   if Dm.EstaBloqueado(umdtTransferencia.Date, true) then begin
      umdtTransferencia.SetFocus();
      Exit;
   end;
...
begin
   ...
   begin
      ...
      // retiorado para manter a seleção na conta que está o cheque.
...
begin
   ...
   qySituacao.SQL.Text := '' +
     ' SELECT cd_situacao, ds_valor, cd_auxiliar               ' +
     ' FROM                                       ' + 
     '      situacoes                             ' +
   ...
   begin
      ...
      SetLength(ListaAuxiliar, length(ListaAuxiliar) + 1);
      ListaSituacao[length(ListaSituacao) - 1] := qySituacao.FieldByName('cd_situacao').AsInteger;
      ListaAuxiliar[length(ListaAuxiliar) - 1] := qySituacao.FieldByName('cd_auxiliar').AsInteger;
...
procedure TfTransferenciaCheques.cbSituacaoChange(Sender: TObject);
begin
   ckbAtivaDebito.Checked := ListaAuxiliar[cbSituacao.ItemIndex] = 2;
   ckbAtivaDebito.Enabled := ListaAuxiliar[cbSituacao.ItemIndex] <> 2;
end;

procedure TfTransferenciaCheques.FormHide(Sender: TObject);
begin
   ckbAtivaDebito.Enabled := true;
end;
...
var
   permissao : Boolean;
begin
   carregaContas(iContaPadrao);
   carregaSituacao();
   umdtTransferencia.Date:= Now();

   ckbAtivaDebito.Visible := DM.variavel_parametro('financeiro_tipo_titulo_cheque_devolvido') <> '';

   permissao := DM.UsuarioLogado.TemPermissao(DM.iCdPessoaLogado, 'UMFinanceiro.Tesouraria.ControleCheques.Transferir', npAcesso, false);
   ckbAtivaDebito.Enabled := (permissao AND ckbAtivaDebito.Enabled);
end;
