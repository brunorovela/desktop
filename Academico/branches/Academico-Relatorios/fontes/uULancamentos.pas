...
type
   ...
   TExecucaoCompromisso = class(AExecucao)
      strict private
         ...
         FValorTotal: Double;
         FTitulosVinculados: TStringList;
      ...
      public
         ...
         { Essa função atualiza e resga o valor do parâmetro financeiro_cd_titulo_cp
           que contém o próximo cd_titulo da tabela fin_contas_pagar }
         function GetParametroUltimoTitulo: Integer;
         ...
         { Retorna os títulos vinculados a um título informado (pai e filhos) }
         function GetTitulosVinculados(cd_titulo: Integer): TStringList;

         { Apagar a linha da movimentação financeira do contas a pagar que foi removido }
         procedure ApagarFinMovCP();
         ...
         { títulos vinculados a um determinado título (título pai e filhos) }
         property TitulosVinculados: TStringList read FTitulosVinculados write FTitulosVinculados;
   end;
   ...
   TExecucaoRateioLC = class(AExecucao)
      strict private
         ...
         FLancamento: Integer;
         FChave: Integer;
         FSnLancamentoEspecifico : Integer;
      ...
      public
         ...
         property SnLancamentoEspecifico: Integer read FSnLancamentoEspecifico write FSnLancamentoEspecifico;
...
begin
   ...
   if Self.TipoExecucao = teDelete then
   begin
      Self.FOnAfterSQLExecute := Self.ApagarFinMovCP;
   end;
...
procedure TExecucaoCompromisso.ApagarFinMovCP();
const
  SQL_DELETE_FIN_MOV_CP = 'DELETE FROM fin_mov_cp WHERE cd_titulo IN (:cd_titulo)';
var
  qyExecucao: TUMZQuery;
begin
  if Self.FTitulosVinculados.Count = 0 then
  begin
    Exit;
  end;

  DM.CriarConsulta( qyExecucao );

  qyExecucao.Close();
  qyExecucao.SQL.Clear();

  Self.FTitulosVinculados.Delimiter := ',';

  qyExecucao.SQL.Text := ReplaceStr(SQL_DELETE_FIN_MOV_CP, ':cd_titulo', Self.FTitulosVinculados.DelimitedText);

  qyExecucao.ExecSQL;

  FreeAndNil(qyExecucao);
end;

function TExecucaoCompromisso.GetParametroUltimoTitulo: Integer;
const
  SQL_ATUALIZAR_PARAMETRO = ''+
    ' UPDATE '+
    '   parametros '+
    ' SET '+
    ' 	ds_valor = ds_valor + 1 '+
    ' WHERE '+
    ' 	ds_variavel = "financeiro_cd_titulo_cp"';
var
...
begin
   ...
   qyParametroTitulo.SQL.Text := SQL_ATUALIZAR_PARAMETRO;

   qyParametroTitulo.ExecSQL;

   Result := StrToInt(DM.variavel_parametro( 'financeiro_cd_titulo_cp' ));

   FreeAndNil(qyParametroTitulo);
end;

function TExecucaoCompromisso.GetTitulosVinculados(cd_titulo: Integer): TStringList;
const
  SQL_TITULOS_VINCULADOS = ''+
    ' SELECT '+
    '   cd_titulo '+
    ' FROM '+
    ' 	fin_contas_pagar '+
    ' WHERE '+
    ' 	cd_titulo = :cd_titulo OR '+
    '   cd_titulo_principal = :cd_titulo ';
var
   qyTitulos : TUMZQuery;
begin
  DM.CriarConsulta( qyTitulos );

  qyTitulos.Close();
  qyTitulos.SQL.Clear();

  qyTitulos.SQL.Text := SQL_TITULOS_VINCULADOS;

  qyTitulos.ParamByName('cd_titulo').AsInteger := cd_titulo;

  qyTitulos.Open;

  Result := TStringList.Create;

  // Varre os títulos encontados e adiciona no retorno
  while (not qyTitulos.Eof) do
  begin
    Result.Add(qyTitulos.FieldByName('cd_titulo').AsString);

    qyTitulos.Next;
  end;

  FreeAndNil(qyTitulos);
end;
...
begin
   if Self.TipoExecucao = teDelete then
   begin
      Self.TitulosVinculados := Self.GetTitulosVinculados(Self.Titulo);
      Parametros.ParamByName('cd_titulo').AsInteger := Self.Titulo;
      Exit;
   end;

   Self.LastInsertID := Self.GetParametroUltimoTitulo();

   Parametros.ParamByName('cd_titulo').AsInteger := Self.LastInsertID;
...
begin
   ...
   Parametros.ParamByName('vl_movimento').AsFloat := Self.Valor;
   Parametros.ParamByName('sn_lancamento_especifico').AsInteger := Self.SnLancamentoEspecifico;
end;
