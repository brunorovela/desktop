...
type
  ...
  private
      ...
      function RegistraEquivalencia: Boolean;
...
uses uEquivalencias, uDM, uFSelecionarPessoa, Variants, uUsuario, uUMNucleo,
  UFCadInstituicao, Main;

{$R *.DFM}

procedure TfrmCadEquiv.BitBtn1Click(Sender: TObject);
const
   SMsgSucesso = 'Equivalência registrada com sucesso. Deseja limpar os campos para iniciar um novo registro?';
var
   UserChoice: Integer;
begin
   if RegistraEquivalencia then
   begin
      frmEquivalencias.AtualizaEquiv;
      UserChoice := MessageDlg(SMsgSucesso, mtConfirmation, [mbYes, mbNo], -1);
...
procedure TfrmCadEquiv.BitBtn3Click(Sender: TObject);
begin
   if RegistraEquivalencia then
   begin
      Close;
...
function TfrmCadEquiv.RegistraEquivalencia: Boolean;
const
   FORMATO_LOG =
...
procedure TfrmCadEquiv.SpeedButton2Click(Sender: TObject);
begin
   ...
   begin
      eInstituicao.Text := fCadInstituicao.qryInstituicoesNM_INSTITUICAO.AsString;
   end;
...
procedure TfrmCadEquiv.LimpaCampos;
begin
   eDisciplina.Clear;
   eProfessor.Clear;
   eAnoSemestre.Clear;
   ...
   eConceito.Clear;
   eFrequencia.Clear;
   eConteudoEquivalente.Clear;
   ckbHistorico.Checked := False;
   if dbSituacao.Enabled then
      dbSituacao.ItemIndex := -1;
end;
