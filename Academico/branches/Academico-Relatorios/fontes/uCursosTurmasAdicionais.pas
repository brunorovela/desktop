...
type
  Tfrm_cursos_turmas_adicional = class(TForm)
    ...
    qyArquivocd_coligada: TSmallintField;
      ...
      begin
        ...
        Begin
            ...
            begin
               ...
               DM.qAux1.ParamByName('curso').AsString := codigo_curso_turma;
            end else if(tela = 'UNIDADES') then
            begin
               Dm.qAux1.SQL.Add('select * from cursos_turmas_cadastro_arq where cd_campo = :campo AND cd_coligada = :coligada');
               DM.qAux1.ParamByName('coligada').AsString := codigo_curso_turma;
            end else begin
   ...
   Begin
       ...
       begin
         ...
         begin
            if telaAtual = 'CURSOS' then
            begin
               qyArquivocd_coligada.AsInteger := 0;
               qyArquivocd_curso.AsString := cursoTurma;
               qyArquivocd_campo.AsInteger := sxDados.Components[i].Tag;
               qyArquivo.Post;
            end
            else
            if( telaAtual = 'UNIDADES') then
            begin
               qyArquivocd_coligada.AsString := cursoTurma;
               qyArquivocd_campo.AsInteger := sxDados.Components[i].Tag;
               qyArquivo.Post;
            end
            else
...
begin
   ...
   if( telaAtual = 'UNIDADES') then
   begin
      qyArquivo.SQL.Add(' cd_coligada = "'+cursoTurma+'"');
   end;
