...
begin
   ...
   with qyListaAlunos do begin
      ...
      begin
         ...
         '  INNER JOIN cursos_coligadas cc ON (cc.CD_CURSO = m.curso)                   '+
         '  INNER JOIN turmas t ON (t.CODIGO = m.TURMA)                                 '+
         '  LEFT JOIN disciplinas d ON (m.curso = d.curso)                             '+
         '  $$SQL_DISCIPLINA$$                                                          '+
         ' WHERE                                                                         '+
         ...
         SQL.Text := StringReplace(
            ...
            '$$SQL_DISCIPLINA$$',
               ...
               +' ON( m.turma = fi.turmamatricula AND '
                  ...
                  +' fi.disciplina = ( '
                  +'     SELECT DISTINCT '
                  +'       d2.codigo FROM disciplinas d2 '
                  +'     WHERE '
                  +'       d2.codigo = :disciplina) AND fi.curso = :curso )',
      ...
      begin
         ...
         begin
            situacaoesIN :=
               situacaoesIN
               + ', '
               + IntToStr(pmSituacao.Items[I].Tag);
...
var
   ...
   procedure TentaGerarPDF;
   begin
    if LeftStr(qyExp.FieldByName('im_documento').AsString, 4) = '%PDF' then
    begin
      FS := TFileStream.Create(
        sDir +
        '\' +
        Self.LimpaString(qyExp.FieldByName('documento').AsString) +
        '_' +
        Self.LimpaString(Self.getSiglaParte(qyExp.FieldByName('cd_tipo').AsString)) +
        '.pdf'
      , fmCreate or fmOpenWrite);
      try
        TBlobField(qyExp.FieldByName( 'im_documento' )).SaveToStream(FS);
      finally
        FS.Free;
      end;
    end else
    if Copy(qyExp.FieldByName('im_documento').AsString, 2, 3) = 'PNG' then
    begin
      FS := TFileStream.Create(
        sDir +
        '\' +
...
begin
   ...
   if not qyListaAlunos.IsEmpty then begin
      ...
      while not qyListaAlunos.Eof do begin
         Inc(i);
         frmSpl.Label1.Caption := ''
            +'Processando Registro: '
            + IntToStr(i)
            + ' de '
            + IntToStr(qyListaAlunos.RecordCount);
            
         Application.ProcessMessages();
         
         GerarFotosAluno(
            qyListaAlunoscodigoaluno.AsString,
            qyListaAlunosnome_pessoa.AsString,
            qyListaAlunoscurso.AsString,
            qyListaAlunosturma.AsString
         );
