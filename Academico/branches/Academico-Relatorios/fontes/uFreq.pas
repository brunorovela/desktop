...
begin
   ...
   // Inserido codigo abaixo para calcular a frequencia ao atualizar faltas nesta tela.
   qyCalcularFrequenciaAluno.SQL.Clear();
   qyCalcularFrequenciaAluno.SQL.Add('select a.ds_frequencia_tipo, a.nr_casas_decimais_frequencia from turmas as t ');
   qyCalcularFrequenciaAluno.SQL.Add('INNER JOIN avaliacoes_parametros_matriz as a ON (a.cd_avaliacao = t.cd_avaliacao) ');
   qyCalcularFrequenciaAluno.SQL.Add('WHERE t.anosemestre = :nr_anosemestre ');
   qyCalcularFrequenciaAluno.SQL.Add('AND t.codigo = :cd_turma ');
   qyCalcularFrequenciaAluno.ParamByName('nr_anosemestre').AsInteger := nr_anosemestre;
   qyCalcularFrequenciaAluno.ParamByName('cd_turma').AsString := cd_turma;
   qyCalcularFrequenciaAluno.Open();

   ds_freq_tipo :=  qyCalcularFrequenciaAluno.FieldByName('ds_frequencia_tipo').AsString;
   nr_decimais_freq :=  qyCalcularFrequenciaAluno.FieldByName('nr_casas_decimais_frequencia').AsInteger;

   qyCalcularFrequenciaAluno.close();
   qyCalcularFrequenciaAluno.SQL.Clear();
   qyCalcularFrequenciaAluno.SQL.Add('CALL DIA_CALCULO_FREQUENCIA_ALUNO(:nr_anosemestre, :cd_turma, :cd_turma_matricula, :cd_pessoa, :cd_frequencia_tipo, :nr_decimais_freq, 0)');

   qyCalcularFrequenciaAluno.ParamByName('nr_anosemestre').AsInteger := nr_anosemestre;
   qyCalcularFrequenciaAluno.ParamByName('cd_turma').AsString := cd_turma;
   qyCalcularFrequenciaAluno.ParamByName('cd_turma_matricula').AsString := cd_turma;
   qyCalcularFrequenciaAluno.ParamByName('cd_pessoa').AsInteger := cd_pessoa;
   qyCalcularFrequenciaAluno.ParamByName('cd_frequencia_tipo').AsString := ds_freq_tipo;
   qyCalcularFrequenciaAluno.ParamByName('nr_decimais_freq').AsInteger := nr_decimais_freq;
   qyCalcularFrequenciaAluno.ExecSQL();
