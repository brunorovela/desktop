...
begin
   ...
   // Se o contador voltou menor que 1 é porque nenhum título foi selecionado
   if (ContaTitulos < 1) then
   begin
      Mensagem( 'Nenhum título selecionado', '', MB_ICONWARNING + MB_OK, 0 );
      Exit;
   end;
...
const
   ...
   SQL_FILTRAR =
      ...
      ' WHERE ca.SN_PAGO = 0 AND '+
      ' (SELECT COUNT(ccc.cd_coligada) FROM comp_categorias_coligadas ccc WHERE ccc.cd_categoria = cc.CD_CATEGORIA AND ccc.cd_coligada IN ([:CD_COLIGADA:]) > 0) ';
...
begin
   ...
   begin
      ...
      qyFiltrar.ParamByName('DT_INICIO').AsString := DateTimeToStr(umdtInicial.Date, fs);
   end;
   { Data Final }
   ...
   begin
      ...
      qyFiltrar.ParamByName('DT_FIM').AsString := DateTimeToStr(umdtFinal.Date, fs);
   end;
   { Código da Pessoa }   
   if ( cd_pessoa > 0 ) then
   begin
