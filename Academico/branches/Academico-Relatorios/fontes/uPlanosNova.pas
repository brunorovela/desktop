...
uses
  ...
  UZSortedGrid, Spin, DBClient, UZDbcFuncs, ZDbcIntfs;
...
type
  TfrmPlanosNova = class(TForm)
    ...
    qryTurmasCursoCD_CURSO: TStringField;
    qryTurmasCursoCD_TURMA: TStringField;
    qyPlanosSN_USAR_MATRICULA_ONLINE: TSmallintField;
    dbchMatriculaOnline: TDBCheckBox;
    Label12: TLabel;
    edTurma: TEdit;
    ...
    DBEdit2: TDBEdit;
    LblDtFim: TLabel;
    dbrgTipoVencto: TDBRadioGroup;
    qyPlanosNR_TIPO_VENCTO: TSmallintField;
    pgcVencto: TPageControl;
    tbsFixo: TTabSheet;
    tbsDinamico: TTabSheet;
    lblFormulaDias: TLabel;
    lblFormulaVencto: TLabel;
    edAPartirDe: TDBEdit;
    lblAPartirDe: TLabel;
    qyPlanosItensSN_BLOQUEADO: TSmallintField;
    qyPlanosNR_FORMULA_VENCTO: TSmallintField;
    qyPlanosNR_FORMULA_OPERADOR: TSmallintField;
    qyPlanosNR_FORMULA_DIAS: TIntegerField;
    dbeFormulaDias: TDBEdit;
    qyPlanosItensNR_FORMULA_VENCTO: TSmallintField;
    qyPlanosItensNR_FORMULA_OPERADOR: TSmallintField;
    qyPlanosItensNR_FORMULA_DIAS: TIntegerField;
    qyPlanosNM_FORMULA: TStringField;
    dblcFormulaTipo: TDBLookupComboBox;
    cdsOperadores: TClientDataSet;
    cdsOperadoresCD_OPERADOR: TIntegerField;
    cdsOperadoresNM_OPERADOR: TStringField;
    qyPlanosNM_OPERADOR: TStringField;
    dblcFormulaOperador: TDBLookupComboBox;
    qyPlanosItensDS_FORMULA_VENCTO: TStringField;
    cdsFormulas: TClientDataSet;
    cdsFormulasCD_FORMULA: TIntegerField;
    cdsFormulasNM_FORMULA: TStringField;
    qyPlanosItensNM_FORMULA: TStringField;
    qyPlanosItensNM_OPERADOR: TStringField;
    procedure speDiasChange(Sender: TObject);
    procedure cbOperadorChange(Sender: TObject);
    procedure cbFormulaChange(Sender: TObject);
    procedure grdParcelasColEnter(Sender: TObject);
    procedure grdParcelasCellClick(Column: TColumn);
    procedure grdParcelasEditButtonClick(Sender: TObject);
    procedure dbrgTipoVenctoChange(Sender: TObject);
    procedure qyPlanosItensCalcFields(DataSet: TDataSet);
...
uses
   uDM, Main, General, uFSelecionarCurso, uFSelecionarTurma, uItemCombo,
   uPlanosCopiar, ClassPlanosPagamento, Types, uFSelecionarColigada, uUsuario,
   uLkJSON, UFFinPlanosFormulaVencto;
...
procedure TfrmPlanosNova.apagarTurmasPlano;
var
...
var
   ...
   JSONCurso: TlkJSONobject;
   JSONTurma: TlkJSONstring;
   JSONTurmas: TlkJSONlist;
   Chave: AnsiString;
begin
   ...
   Begin
      ...
      qryTurmasCurso.ParamByName('CD_COLIGADA').AsInteger := resultado_filtro.cd_coligada;
      qryTurmasCurso.ParamByName('NR_ANOSEMESTRE').AsInteger := qyPlanosNR_ANOSEMESTRE.AsInteger;
      qryTurmasCurso.ParamByName('CD_CURSO').AsString := filtro_curso;
      qryTurmasCurso.Open;

      if not qryTurmasCurso.IsEmpty then
      begin
         JSONCurso := TlkJSONobject.Create(False);
         try
            JSONCurso.Add('CD_PLANO', qyPlanosCD_PLANO.AsInteger);
            JSONCurso.Add('CD_CURSO', filtro_curso);

            JSONTurmas := TlkJSONlist.Create;
            while not qryTurmasCurso.Eof do
            begin
               JSONTurma := TlkJSONstring.Create;
               JSONTurma.Value := qryTurmasCursoCD_TURMA.AsString;
               JSONTurmas.Add(JSONTurma);
               qryTurmasCurso.Next;
            end;

            JSONCurso.Add('turmas', JSONTurmas);

            Chave := Format(SFmtChave, [qyPlanosCD_PLANO.AsInteger, filtro_curso]);

            DM.setLog(2006, 'Inclusao', Chave, DM.UsuarioLogado.ColigadaLogada,
               UTF8Decode(TlkJSON.GenerateText(JSONCurso)));
         finally
            FreeAndNil(JSONCurso);
         end;
      end;
      ...
      // FiltrarPlanosTurmas();
      // Filtrar tudo para atualizar o numero de turmas.
      btnFiltrarClick(nil);
...
begin
   ...
   resultado_filtro := TfrmSelecionarTurma.Filtrar([bftAnosemestre],
      nr_anosemestre, -1, -1, '', -1, False, True);

   if not resultado_filtro.filtrado then
      Exit;

   for I := 0 to TfrmSelecionarTurma.GetInstancia.grdTurmas.SelectedRows.Count - 1 do
   begin
      Bookmark := TBookmark(TfrmSelecionarTurma.GetInstancia.grdTurmas.SelectedRows[I]);
      TfrmSelecionarTurma.GetInstancia.qyTurmas.GotoBookmark(Bookmark);

      filtro_curso := TfrmSelecionarTurma.GetInstancia.qyTurmas.FieldByName('curso').AsString;
      filtro_turma := TfrmSelecionarTurma.GetInstancia.qyTurmas.FieldByName('codigo').AsString;

      qyInsertPlanosTurma.ParamByName('CD_PLANO').AsInteger := qyPlanosCD_PLANO.AsInteger;
      qyInsertPlanosTurma.ParamByName('CD_COLIGADA').AsInteger := TfrmSelecionarTurma.GetInstancia.qyTurmas.FieldByName('cd_coligada').AsInteger;
      ...
      try
         ...
         begin
            JSONTurma := TlkJSONobject.Create(False);
            try
               JSONTurma.Add('CD_PLANO', qyPlanosCD_PLANO.AsInteger);
               JSONTurma.Add('CD_CURSO', filtro_curso);
               JSONTurma.Add('CD_TURMA', filtro_turma);

               Chave := Format(SFmtChave, [qyPlanosCD_PLANO.AsInteger, filtro_curso, filtro_turma]);

               DM.setLog(2006, 'Inclusao', Chave,
                  DM.UsuarioLogado.ColigadaLogada,
                  UTF8Decode(TlkJSON.GenerateText(JSONTurma)));
            finally
               FreeAndNil(JSONTurma);
            end;
         end;
      except on E: Exception do
          MessageDlg('Turma ' + filtro_turma + ' já foi incluída neste plano.', mtWarning, [mbOK], -1);
      end;
   ...
   pcInformacoes.ActivePage := tsTurmas;
   FiltrarPlanosTurmas();
   // Filtrar tudo, para atualizar o número de turmas.
   // FiltrarPlanosTurmas();
...
var
   ...
   TipoTitulo : Integer;
   S: string;
   UltimoDia, DiaValido, RetrocederMes: Boolean;
   I, ParcIni, NumParcelas, Dia, MesOriginal, MesCalculado, AnoOriginal, AnoCalculado: Integer;
...
begin
   ...
   // Se estive copiando o plano
   if (btnCalcularParcelas.Tag = 0 ) AND
      (Mensagem(
         'Deseja calcular o valor das parcelas?',
         Application.Title, MB_YESNO + MB_ICONQUESTION ) = ID_NO )
   then
      Exit;
   ...
   begin
      ...
      if qyPlanosNR_TIPO_VENCTO.AsInteger = 1 then
      begin
         qyPlanosItensNR_FORMULA_VENCTO.AsInteger := qyPlanosNR_FORMULA_VENCTO.AsInteger;
         qyPlanosItensNR_FORMULA_OPERADOR.AsInteger := qyPlanosNR_FORMULA_OPERADOR.AsInteger;
         qyPlanosItensNR_FORMULA_DIAS.AsInteger := qyPlanosNR_FORMULA_DIAS.AsInteger;
      end;

      if qyPlanosNR_TIPO_VENCTO.AsInteger = 0 then
      begin
   ...
   begin
      ...
      begin
         ...
         begin
            ...
            begin
               Dia := DayOf(APartirDe);
               MesOriginal := MonthOf(Vencto);
               AnoOriginal := YearOf(Vencto);
               ...
               begin
                  ...
                  MesCalculado := MonthOf(Temp);
                  AnoCalculado := YearOf(Temp);

                  if MesCalculado > MesOriginal then
                     Break;
               ...
               RetrocederMes :=
                  (MesCalculado > MesOriginal) or
                  (AnoCalculado > AnoOriginal);

               while (MesCalculado > MesOriginal) or (AnoCalculado > AnoOriginal) do
               begin
                  Temp := IncDay(Temp, -1);
                  MesCalculado := MonthOf(Temp);
                  AnoCalculado := YearOf(Temp);
               ...
               if RetrocederMes then
               begin
                  repeat
                     DiaValido := True;

                     if qyPlanosSN_PULAR_FERIADOS.AsInteger = 1 then
                     begin
                        DiaValido := not DM.getFeriado(Temp);
                     end;

                     if DiaValido then
                     begin
                        if qyPlanosSN_PULAR_SABADOS.AsInteger = 1 then
                        begin
                           DiaValido := DayOfWeek(Temp) <> 7;
                        end;
                     end;

                     if DiaValido then
                     begin
                        if qyPlanosSN_PULAR_DOMINGOS.AsInteger = 1 then
                        begin
                           DiaValido := DayOfWeek(Temp) <> 1;
                        end;
                     end;

                     if not DiaValido then
                        Temp := IncDay(Temp, -1);
      ...
      begin
         ...
         if I > 0 then
         begin
            qyPlanosItensNR_FORMULA_VENCTO.AsInteger := 2;
            qyPlanosItensNR_FORMULA_OPERADOR.AsInteger := 1;
            qyPlanosItensNR_FORMULA_DIAS.AsInteger := I * 30;
         end;
      end;
...
begin
   ...
   if qyPlanosNR_TIPO_VENCTO.AsInteger = 1 then
   begin
      frmPlanosCopiar.umdtApartirDe.Enabled := False;
      frmPlanosCopiar.Label4.Enabled := False;
   end;

   if qyPlanosNR_TIPO_VENCTO.AsInteger = 0 then
   begin
      frmPlanosCopiar.umdtApartirDe.Enabled := True;
      frmPlanosCopiar.Label4.Enabled := True;
   end;
   ...
   cd_plano_novo := ClassePlanosPagamento.CopiaPlanoPagamento(
      qyPlanosCD_PLANO.AsInteger,
      Dm.GetUsuarioLogado.GetColigadaLogada.Codigo,
      frmPlanosCopiar.nr_anosemestre,
      frmPlanosCopiar.umdtApartirDe.Date,
      frmPlanosCopiar.sn_copiar_turmas,
      frmPlanosCopiar.sn_calcular_parcelas,
   ...
   if ( frmPlanosCopiar.sn_calcular_parcelas )  OR
      (qyPlanosSN_PULAR_FERIADOS.AsInteger = 1) OR
      (qyPlanosSN_PULAR_SABADOS.AsInteger = 1)  OR
      (qyPlanosSN_PULAR_DOMINGOS.AsInteger = 1)
   then
   begin
...
procedure TfrmPlanosNova.cbFormulaChange(Sender: TObject);
begin
   qyPlanos.Edit;
end;

procedure TfrmPlanosNova.cbOperadorChange(Sender: TObject);
begin
   qyPlanos.Edit;
end;
...
procedure TfrmPlanosNova.dbrgTipoVenctoChange(Sender: TObject);
begin
   pgcVencto.ActivePageIndex := qyPlanosNR_TIPO_VENCTO.AsInteger;
end;
...
begin
   ...
   grdParcelas.Columns[0].Visible := qyPlanosNR_TIPO_VENCTO.AsInteger = 0;
   grdParcelas.Columns[1].Visible := qyPlanosNR_TIPO_VENCTO.AsInteger = 0;
   grdParcelas.Columns[2].Visible := qyPlanosNR_TIPO_VENCTO.AsInteger = 0;
   grdParcelas.Columns[3].Visible := qyPlanosNR_TIPO_VENCTO.AsInteger = 1;
   ...
   begin
      ...
      grdParcelas.Columns[3].ButtonStyle := cbsEllipsis;
      ...
      lblFormulaVencto.Enabled := True;
      lblFormulaDias.Enabled := True;
      dblcFormulaTipo.Enabled := True;
      dblcFormulaOperador.Enabled := True;
      dbeFormulaDias.Enabled := True;
   end
   else
   begin
      pnNaoPodeExcluirItens.Visible := true;
      grdParcelas.ReadOnly := true;
      grdParcelas.Columns[3].ButtonStyle := cbsNone;
      ...
      lblFormulaVencto.Enabled := False;
      lblFormulaDias.Enabled := False;
      dblcFormulaTipo.Enabled := False;
      dblcFormulaOperador.Enabled := False;
      dbeFormulaDias.Enabled := False;
   end;
...
begin
   ...
   dbrgTipoVencto.Enabled := qyPlanos.State in dsEditModes;
   dblcFormulaTipo.Enabled := (qyPlanos.State in dsEditModes) and (qyPlanosNR_TIPO_VENCTO.AsInteger = 1);
   dblcFormulaOperador.Enabled := (qyPlanos.State in dsEditModes) and (qyPlanosNR_TIPO_VENCTO.AsInteger = 1);
   dbeFormulaDias.Enabled := (qyPlanos.State in dsEditModes) and (qyPlanosNR_TIPO_VENCTO.AsInteger = 1);
   edAPartirDe.Enabled := (qyPlanos.State in dsEditModes) and (qyPlanosNR_TIPO_VENCTO.AsInteger = 0);
...
// ATENÇÃO: PRESTAR ATENÇÃO QUE O SELECT ACIMA, TERMINA EM "," POIS O CAMPO DA CONSTANTE ABAIXO SERÁ ADICIONADO
   SQL_PADRAO_FROM_WHERE = ' FROM FIN_PLANOS fp [$filtro_tabela_curso_turma$] ' +
                           ' WHERE fp.sn_ativo = 1 ';
   SQL_PADRAO_CAMPO_NR_TOTAL_TURMAS =
      '(SELECT COUNT(DISTINCT T.CODIGO, T.ANOSEMESTRE) ' +
      'FROM TURMAS T ' +
      'JOIN FIN_PLANOS_TURMAS FPT ON (T.CODIGO = FPT.CD_TURMA) AND (T.CURSO = FPT.CD_CURSO) ' +
      'WHERE FPT.CD_PLANO = FP.CD_PLANO AND T.ANOSEMESTRE = FP.NR_ANOSEMESTRE)';
   ...
   SFiltroVigenciaDe = ' AND fp.dt_vigencia_inicio >= :dt_vigencia_inicio';
   SFiltroVigenciaAte = ' AND fp.dt_vigencia_fim <= :dt_vigencia_fim';
   SFiltroVigencia = ' AND fp.sn_vigencia = 1';
Var
   bPosicaoAtual : Pointer;
begin
   bPosicaoAtual := qyPlanos.GetBookmark();
   qyPlanos.Close();
   ...
   begin
      qyPlanos.SQL.Add(SFiltroVigenciaDe);
      qyPlanos.ParamByName('dt_vigencia_inicio').AsDate := dtpFiltroVigenciaDe.Date;
   end;

   if dtpFiltroVigenciaAte.Checked then
   begin
      qyPlanos.SQL.Add(SFiltroVigenciaAte);
      qyPlanos.ParamByName('dt_vigencia_fim').AsDate := dtpFiltroVigenciaAte.Date;
   end;

   if dtpFiltroVigenciaDe.Checked or dtpFiltroVigenciaAte.Checked then
   begin
      qyPlanos.SQL.Add(SFiltroVigencia);
   end;
   ...
   if qyPlanos.BookmarkValid(bPosicaoAtual) then begin
      qyPlanos.GotoBookmark(bPosicaoAtual);
   end else begin
      qyPlanos.First();
   end;
...
begin
   ...
   cdsFormulas.Insert;
   cdsFormulasCD_FORMULA.AsInteger := 0;
   cdsFormulasNM_FORMULA.AsString := 'Data de início da turma';

   cdsFormulas.Insert;
   cdsFormulasCD_FORMULA.AsInteger := 1;
   cdsFormulasNM_FORMULA.AsString := 'Data de matrícula';

   cdsFormulas.Insert;
   cdsFormulasCD_FORMULA.AsInteger := 2;
   cdsFormulasNM_FORMULA.AsString := 'Primeiro vencimento';

   cdsFormulas.Post;

   cdsOperadores.Insert;
   cdsOperadoresCD_OPERADOR.AsInteger := 0;
   cdsOperadoresNM_OPERADOR.AsString := '-';

   cdsOperadores.Insert;
   cdsOperadoresCD_OPERADOR.AsInteger := 1;
   cdsOperadoresNM_OPERADOR.AsString := '+';
...
procedure TfrmPlanosNova.grdParcelasCellClick(Column: TColumn);
begin
   if (Column.FieldName = 'SN_BLOQUEADO') and (not grdParcelas.ReadOnly) then
   begin
...
procedure TfrmPlanosNova.grdParcelasColEnter(Sender: TObject);
var
   GridOptions: TDBGridOptions;
begin
   GridOptions := grdParcelas.Options;
   Include(GridOptions, dgEditing);

   if grdParcelas.SelectedField.FieldName = 'SN_BLOQUEADO' then
      Exclude(GridOptions, dgEditing);

   grdParcelas.Options := GridOptions;
end;
...
begin
   ...
   if Column.FieldName = 'SN_BLOQUEADO' then
   begin
      DrawRect := Rect;
...
procedure TfrmPlanosNova.grdParcelasEditButtonClick(Sender: TObject);
begin
   if qyPlanosItensNR_PARCELA.AsInteger <> 0 then
      cdsFormulas.Filtered := False;
      
   if TfrmFinPlanosFormulaVencto.Edit and (qyPlanosItens.State = dsEdit) then
      qyPlanosItens.Post
   else
      qyPlanosItens.Cancel;
      
   cdsFormulas.Filtered := True;
end;
...
begin
   ...
   begin
      ...
      begin
         SQL.Text := 'SELECT COUNT(DISTINCT cd_mensalidade) qtd FROM mensalidades m JOIN fin_planos_itens i ON (m.cd_item_plano = i.CD_PLANO_ITEM) WHERE i.CD_PLANO = :cd_plano';
...
procedure TfrmPlanosNova.qyPlanosItensCalcFields(DataSet: TDataSet);
const
   FormulaFmt = '%s %s %d dias';
   IndexToFormula: array[0..2] of AnsiString = (
      'Data de início da turma',
      'Data de matrícula',
      'Primeiro vencimento'
   );
   IndexToOperador: array[0..1] of AnsiString = ('-', '+');
var
   S, Formula, Operador: AnsiString;
begin
   Formula := IndexToFormula[DataSet.FieldByName('NR_FORMULA_VENCTO').AsInteger];
   Operador := IndexToOperador[DataSet.FieldByName('NR_FORMULA_OPERADOR').AsInteger];
   S := Format(FormulaFmt, [Formula, Operador, DataSet.FieldByName('NR_FORMULA_DIAS').AsInteger]);
   DataSet.FieldByName('DS_FORMULA_VENCTO').AsString := S;
end;
...
const
   SSQLSelectMaxOrdem = 'SELECT COALESCE(MAX(NR_ORDEM), 0) + 1 NR_ORDEM FROM FIN_PLANOS';
var
   Rs: IZResultSet;
begin
   dsPlanos.OnDataChange := nil;

   Rs := ExecuteQuery(SSQLSelectMaxOrdem);
   try
      if Rs.Next then
      begin
         DataSet.FieldByName('NR_ORDEM').AsInteger := Rs.GetIntByName('NR_ORDEM');
      end;
   finally
      Rs.Close;
      Rs := nil;
...
procedure TfrmPlanosNova.speDiasChange(Sender: TObject);
begin
   qyPlanos.Edit;
end;
...
begin
   ...
   JSONPlano := TlkJSONobject.Create(False);
   try
      JSONTurmas := TlkJSONlist.Create;
      ...
      begin
         ...
         JSONTurma := TlkJSONobject.Create(False);
         JSONTurma.Add('CD_CURSO', qyPlanosTurmasCD_CURSO.AsString);
         JSONTurma.Add('CD_TURMA', qyPlanosTurmasCD_TURMA.AsString);

         JSONTurmas.Add(JSONTurma);
      end;

      JSONPlano.Add('CD_PLANO', qyPlanosCD_PLANO.AsInteger);
      JSONPlano.Add('turmas', JSONTurmas);

      DM.setLog(2006, 'Exclusao', qyPlanosCD_PLANO.AsString,
         DM.UsuarioLogado.ColigadaLogada,
         UTF8Decode(TlkJSON.GenerateText(JSONPlano)));
   finally
      FreeAndNil(JSONPlano);
   end;
