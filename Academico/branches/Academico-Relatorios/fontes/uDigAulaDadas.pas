...
type
  TfrmDigAulasDadas = class(TForm)
    ...
    pmQtd: TPopupMenu;
    qryAulasDadasDiario: TUMZQuery;
    lblMensagemNr_Aulas: TLabel;
    qryAtualizaNr_Aulas: TUMZQuery;
    tblAulasnumeroaulas: TLargeintField;
    procedure tblAulasAfterOpen(DataSet: TDataSet);
  ...
  private
    ...
    sn_atualiza_qtd_aulas : Boolean;
    procedure aulas_dadas_diario();
    procedure atualiza_aulas_dadas_diario();
...
begin
 ...
 if (txtTurma.Text = '') and (txtDisciplina.Text = '') and (txtProfessor.Text='') then Exit;
  ...
  tblAulas.SQL.Add(
      ...
      +'       DA.TURMA = tp.turma '
      +'      AND'
      +'       DA.DISCIPLINA = tp.disciplina '
      +'      AND'
      +'       DA.ANOSEMESTRE = tp.anosemestre '
      +'   ),tp.numeroaulas '
      +' ) AS numeroaulas, '
      + ' d.ordem, '
      + ' tp.professor, '
  ...
  tblAulas.SQL.Add(
      ' WHERE '
         ...
         + ' AND tp.anosemestre = :anosemestre '
  );
...
procedure TfrmDigAulasDadas.atualiza_aulas_dadas_diario;
begin
  tblAulas.DisableControls;
  tblAulas.First;
  while not tblAulas.Eof do
  begin
      qryAulasDadasDiario.Close;
      qryAulasDadasDiario.ParamByName('anosemestre').AsInteger := tblAulasanosemestre.AsInteger;
      qryAulasDadasDiario.ParamByName('turma').AsString        := tblAulasturma.AsString;
      qryAulasDadasDiario.ParamByName('disciplina').AsInteger  := tblAulasdisciplina.AsInteger;

      qryAulasDadasDiario.Open;
      if( qryAulasDadasDiario.FieldByName('qtd_aulas').AsInteger <= 0 ) OR
      ( tblAulasnumeroaulas.AsInteger = qryAulasDadasDiario.FieldByName('qtd_aulas').AsInteger )
      then
      begin
         tblAulas.Next;
         continue;
      ...
      qryAtualizaNr_Aulas.ParamByName('qtd_aulas').AsInteger := qryAulasDadasDiario.FieldByName('qtd_aulas').AsInteger;

      qryAtualizaNr_Aulas.ParamByName('anosemestre').AsInteger := tblAulasanosemestre.AsInteger;
      qryAtualizaNr_Aulas.ParamByName('turma').AsString        := tblAulasturma.AsString;
      qryAtualizaNr_Aulas.ParamByName('disciplina').AsInteger  := tblAulasdisciplina.AsInteger;
      qryAtualizaNr_Aulas.ExecSQL;

      tblAulas.Next;
   end;
   tblAulas.EnableControls;
end;

procedure TfrmDigAulasDadas.aulas_dadas_diario;
begin
   qryAulasDadasDiario.Close;
   qryAulasDadasDiario.ParamByName('anosemestre').AsInteger := tblAulasanosemestre.AsInteger;
   qryAulasDadasDiario.ParamByName('turma').AsString        := tblAulasturma.AsString;
   ...
   dbeN1.Enabled := True;
   lblMensagemNr_Aulas.Visible := False;
   if ( qryAulasDadasDiario.FieldByName('qtd_aulas').AsInteger <= 0 ) then
   begin
      tblAulas.EnableControls;
      Exit;
   end;
   ...
   dbeN1.Enabled := False;
   dbeN1.Text    := qryAulasDadasDiario.FieldByName('qtd_aulas').AsString;
   lblMensagemNr_Aulas.Visible := True;
...
procedure TfrmDigAulasDadas.tblAulasAfterOpen(DataSet: TDataSet);
begin
	atualiza_aulas_dadas_diario();
end;
