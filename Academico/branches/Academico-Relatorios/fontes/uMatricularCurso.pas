...
uses
  ...
  UMLookupComboBox, uItemCombo, UMDateTimePicker;
...
type
  TFrmMatricularCurso = class(TForm)
    ...
    dtpDtIngresso: TDateTimePicker;
    dtpDtSaida: TDateTimePicker;
    dtpDtConclusao: TDateTimePicker;
    dtpDtColacao: TDateTimePicker;
    dtpDtExpDiploma: TDateTimePicker;
    Label17: TLabel;
    DBEdit11: TDBEdit;
    dtpDtCertificado: TDateTimePicker;
    qyMatriculaCursodt_certificado: TDateTimeField;
    qryDataMatricula: TUMZQuery;
    btnEscolaFundamental: TSpeedButton;
    qyMatriculaCursoNR_CERT_FOLHA: TIntegerField;
    qyMatriculaCursoNR_CERT_REGISTRO: TIntegerField;
    qyMatriculaCursods_obs: TMemoField;
    DBMemo1: TDBMemo;
    procedure btnEscolaFundamentalClick(Sender: TObject);
    procedure dtpDtCertificadoChange(Sender: TObject);
    procedure DBEdit11Exit(Sender: TObject);
    procedure dtpDtExpDiplomaChange(Sender: TObject);
    procedure dtpDtColacaoChange(Sender: TObject);
    procedure dtpDtConclusaoChange(Sender: TObject);
    procedure DBEdit8Exit(Sender: TObject);
    procedure DBEdit7Exit(Sender: TObject);
    procedure DBEdit5Exit(Sender: TObject);
    procedure dtpDtSaidaChange(Sender: TObject);
    procedure DbDataSaidaExit(Sender: TObject);
    procedure DBEdit4Exit(Sender: TObject);
    procedure dtpDtIngressoChange(Sender: TObject);
...
uses uFSelecionarCurso, uSelConcurso, uCursoColigada, UFCadInstituicao;
...
var
   dataCurso,dataTurma : TDate;
begin
   dataCurso := StrToDate(DateToStr(qyMatriculaCursodt_saida.AsDateTime));
   dataTurma := StrToDate(DateToStr(qryDataMatricula.FieldByName('dataSaida').AsDateTime));

   if (dataTurma > 0) then
   begin

      //Não preencheu data de saida do curso, apenas avisa e sai se o usuário selecionar
      if (dataCurso = 0) then
      begin
         if (Mensagem('Existe uma data de saída informada para a matricula turma do aluno, deseja sair sem informar a saída do curso?', '', MB_YESNO + MB_ICONWARNING, Handle) = mrNo) then
         begin
            exit;
         end;
      end;

      //Preencheu do curso, verifica se é maior que a da turma
      if (dataCurso > 0) then
      begin
         if (dataCurso < dataTurma) then
         begin
            Mensagem('Você deve selecionar uma data de saída do curso maior ou igual a data de saída da turma!', '', MB_OK + MB_ICONERROR, Handle);
            exit;
         end;
      end;
   end;
   ...
   //   if qyMatriculaCursoSN_GERADO_MANUAL.asString = '' then
     // begin
      ...
      //Força a atribuição
      qyMatriculaCursonr_matricula.AsString := DBEdit3.Text;
      qyMatriculaCursods_obs.AsString := DBMemo1.Text;
...
procedure TFrmMatricularCurso.btnEscolaFundamentalClick(Sender: TObject);
begin
   ...
   begin
      ...
      begin
         ...
         if (dm.tblInstituicoescd_instituicao.AsString = fCadInstituicao.qryInstituicoescd_instituicao.AsString) then
         begin
...
procedure TFrmMatricularCurso.DbDataSaidaExit(Sender: TObject);
begin
   if not qyMatriculaCursodt_saida.IsNull then
   begin
      dtpDtSaida.Date := qyMatriculaCursodt_saida.AsDateTime;
   end;
end;

procedure TFrmMatricularCurso.DBEdit11Exit(Sender: TObject);
begin
   if not qyMatriculaCursodt_certificado.IsNull then
   begin
      dtpDtCertificado.Date := qyMatriculaCursodt_certificado.AsDateTime;
   end;
end;

procedure TFrmMatricularCurso.DBEdit4Exit(Sender: TObject);
begin
   if not qyMatriculaCursodt_ingresso.IsNull then
   begin
      dtpDtIngresso.Date := qyMatriculaCursodt_ingresso.AsDateTime;
   end;
end;

procedure TFrmMatricularCurso.DBEdit5Exit(Sender: TObject);
begin
   if not qyMatriculaCursodt_conclusao.IsNull then
   begin
      dtpDtConclusao.Date := qyMatriculaCursodt_conclusao.AsDateTime;
   end;
end;

procedure TFrmMatricularCurso.DBEdit7Exit(Sender: TObject);
begin
   if not qyMatriculaCursodt_colacao.IsNull then
   begin
      dtpDtColacao.Date := qyMatriculaCursodt_colacao.AsDateTime;
   end;
end;

procedure TFrmMatricularCurso.DBEdit8Exit(Sender: TObject);
begin
   if not qyMatriculaCursodt_exp_diploma.IsNull then
   begin
      dtpDtExpDiploma.Date := qyMatriculaCursodt_exp_diploma.AsDateTime;
   end;
end;
...
procedure TFrmMatricularCurso.dtpDtSaidaChange(Sender: TObject);
begin
   qyMatriculaCursodt_saida.AsString := DateToStr(dtpDtSaida.Date);
end;

procedure TFrmMatricularCurso.dtpDtCertificadoChange(Sender: TObject);
begin
   qyMatriculaCursodt_certificado.AsString := DateToStr(dtpDtCertificado.Date);
end;

procedure TFrmMatricularCurso.dtpDtColacaoChange(Sender: TObject);
begin
   qyMatriculaCursodt_colacao.AsString := DateToStr(dtpDtColacao.Date);
end;

procedure TFrmMatricularCurso.dtpDtConclusaoChange(Sender: TObject);
begin
   qyMatriculaCursodt_conclusao.AsString := DateToStr(dtpDtConclusao.Date);
end;

procedure TFrmMatricularCurso.dtpDtExpDiplomaChange(Sender: TObject);
begin
   qyMatriculaCursodt_exp_diploma.AsString := DateToStr(dtpDtExpDiploma.Date);
end;

procedure TFrmMatricularCurso.dtpDtIngressoChange(Sender: TObject);
begin
   qyMatriculaCursodt_ingresso.AsString := DateToStr(dtpDtIngresso.Date);
end;
...
begin
   ...
   begin
      ...
      begin
        ...
        begin
           cbCertificado.ItemIndex := cbCertificado.Items.Count-1;
         end;
   ...
   qryDataMatricula.Close;
   qryDataMatricula.ParamByName('cd_pessoa').AsString := qyMatriculaCursocd_pessoa.AsString;
   qryDataMatricula.ParamByName('cd_matricula_curso').AsString := qyMatriculaCursoCD_MATRICULA_CURSO.AsString;
   qryDataMatricula.Open;
end;

procedure TFrmMatricularCurso.gravaLog;
var
   chaves, msgLog, logInicial, valorNovo, valorAntigo : String;
begin
   chaves := FillString(qyMatriculaCursocd_pessoa.AsString, alRight, '0', 10, true ) + ';' +
                qyMatriculaCursonr_anosem_ingresso.AsString + ';' +
                qyMatriculaCursocd_curso.AsString + ';';
   logInicial := 'Alterados dados da matrícula curso do aluno ' + qyMatriculaCursocd_pessoa.AsString + ' no curso ' + qyMatriculaCursocd_curso.AsString + ', ';
   msgLog := dm.geraLogForm(FrmMatricularCurso);

   // Verificação do campo de ceritificado
   valorNovo := '';
   valorAntigo := '';
   if (qyMatriculaCursoCD_CERT_LIVRO.AsString <> '') then
   begin
      valorNovo := qyMatriculaCursoCD_CERT_LIVRO.AsString;
   end;
   if (qyMatriculaCursoCD_CERT_LIVRO.OldValue <> null) then
   begin
       valorAntigo := qyMatriculaCursoCD_CERT_LIVRO.OldValue;
   end;

   if (valorNovo <> valorAntigo) then
   begin
      msgLog := msgLog + ', Certificado: ' + valorAntigo + ' para ' + valorNovo;
   end;

   if(msgLog <> '') then
   begin
      msgLog := logInicial + msgLog;
      DM.setLog(1009, 'Alteração', chaves, strtoint(DM.getColigadaByCurso(qyMatriculaCursocd_curso.AsString)), msgLog);
   end;
end;
...
begin
   ...
   dtpDtIngresso.DateTime := Now();
   if not qyMatriculaCursodt_ingresso.IsNull then
   begin
      dtpDtIngresso.DateTime := qyMatriculaCursodt_ingresso.AsDateTime;
   end;

   dtpDtSaida.DateTime := Now();
   if not qyMatriculaCursodt_saida.IsNull then
   begin
      dtpDtSaida.DateTime := qyMatriculaCursodt_saida.AsDateTime;
   end;

   dtpDtConclusao.DateTime := Now();
   if not qyMatriculaCursodt_conclusao.IsNull then
   begin
      dtpDtConclusao.DateTime := qyMatriculaCursodt_conclusao.AsDateTime;
   end;

   dtpDtColacao.DateTime := Now();
   if not qyMatriculaCursodt_colacao.IsNull then
   begin
      dtpDtColacao.DateTime := qyMatriculaCursodt_colacao.AsDateTime;
   end;

   dtpDtExpDiploma.DateTime := Now();
   if not qyMatriculaCursodt_exp_diploma.IsNull then
   begin
      dtpDtExpDiploma.DateTime := qyMatriculaCursodt_exp_diploma.AsDateTime;
   end;

   dtpDtCertificado.DateTime := Now();
   if not qyMatriculaCursodt_certificado.IsNull then
   begin
      dtpDtCertificado.DateTime := qyMatriculaCursodt_certificado.AsDateTime;
   end;
end;
