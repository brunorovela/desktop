...
AS INFORMAÇÕES DE UMA INSCRIÇÃO
   ...
   public
      ...
      function getCodigoGePessoa:Integer;
...
uses DB, uDM,General, ZAbstractRODataset, ZAbstractDataset, ZDataset, UZDataset;
...
begin
   ...
   DM.CriarConsulta(qryAddInscricao);
...
begin
   DM.CriarConsulta(qryFechaAtividade);
   try
...
begin
   DM.CriarConsulta(qryFechaEntradas);
   try
      //Com esta query faça
...
begin
   oInscricao:=nil;
   DM.CriarConsulta(tblInscricaoAux);
...
function TUMInscricao.getCodigoGePessoa: Integer;
const
  SQL_GET_GE_PESSOA =
    'SELECT cd_ge_aluno FROM ge_alunos WHERE cd_pessoa = :cd_pessoa AND cd_curso = :cd_curso AND cd_ge_atividade = :cd_ge_atividade;';
begin
  //
end;
...
begin
   ...
   //Cria query
   DM.CriarConsulta(tblTotalInscricoes);
...
begin
   //nova query
   DM.CriarConsulta(tblTotalPresentes);
   iNrPresentes :=0;
   try
...
begin
   DM.CriarConsulta(qryInscricaoAtividade);
...
begin
   ...
   DM.CriarConsulta(qrNovaEntradaAtividade);
   try
...
var
   qrEntradaAtividade: TUMZQuery;
   dtEntrada:String;
   dtSaida:String;

begin
   DM.CriarConsulta(qrEntradaAtividade);
   ...
   if(oInscricao.SnPagouInscricao) then
   begin
     //Self.addEntrada;
     Result := taEntrada;
   end else
