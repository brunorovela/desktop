...
uses
  ...
  UZSortedGrid, ZSqlUpdate, UMComboBox;
...
type
  TfrmCursosMoodle = class(TForm)
    ...
    qryCursoKonvivaDS_METODO_IMPORTACAO: TStringField;
    qryCursoKonvivaCD_AVALIACAO_TIPO: TIntegerField;
    cbSistemaIntegra: TUMComboBox;
    Label1: TLabel;
    qryNuIntegracaoExterna: TUMZQuery;
    qryNuIntegracaoExternacd_sistema: TIntegerField;
    qryNuIntegracaoExternads_sistema: TStringField;
    qyMoodleCursosDS_SISTEMA: TStringField;
    qyMoodleCursosCD_SISTEMA: TIntegerField;
    qryAcoesMoodleCurso: TUMZQuery;
    ...
    procedure pcMoodleTurmasDiscSelect();
...
uses
 uIncluirCursosMoodle, uIncluirTurmasDisciplinasMoodle, uUsuario, uItemCombo;
{$R *.dfm}
...
const
   ...
   S_SQL_CURSOS =
      ...
      + '   NIE.DS_SISTEMA '
      + ' FROM '
      + '   UNIM_MOODLE_CURSOS UMC '
      + ' LEFT JOIN NU_INTEGRACAO_EXTERNA NIE ON ( '
      + '   NIE.CD_SISTEMA = UMC.CD_INTEGRACAO_EXTERNA  )';

   S_SQL_CURSOS_FILTRO = S_SQL_CURSOS + ' WHERE %s';
   S_SQL_FILTRO_DESC = 'UMC.DS_DESCRICAO LIKE :DS_DESCRICAO';
   S_SQL_FILTRO_SIGLA = 'UMC.DS_SIGLA LIKE :DS_SIGLA';
   S_SQL_FILTRO_INTEGRACAO = 'NIE.CD_SISTEMA = :CD_SISTEMA';
   S_SQL_FILTRO_LIKE = '%%%s%%';
var
   ...
   cd_sistema : String;
begin
   cd_sistema := TItemCombo(cbSistemaIntegra.Items.Objects[cbSistemaIntegra.ItemIndex]).getCodigo;

   if ( TRIM(cd_sistema) = '0' ) then
   begin
     cd_sistema := '';
   end;

   S := lbleDescricao.Text + lbleSigla.Text + cd_sistema;
   ...
   begin
      ...
      try
         ...
         if Trim(cd_sistema) <> EmptyStr  then
         begin
            if Filters <> EmptyStr then
               Filters := Filters + S_SQL_OP;
            Filters := Filters + S_SQL_FILTRO_INTEGRACAO;
...
begin
   ...
   qryNuIntegracaoExterna.Close;
   qryNuIntegracaoExterna.Open;

   qryNuIntegracaoExterna.First();

   cbSistemaIntegra.Clear();

   cbSistemaIntegra.Items.AddObject(
      'Selecione...',
      TItemCombo.Create(
         '0',
         'Selecione'
   ...
   while not qryNuIntegracaoExterna.Eof do
   begin
      if qryNuIntegracaoExternacd_sistema.AsInteger > 0 then
      begin
        cbSistemaIntegra.Items.AddObject(
            qryNuIntegracaoExternads_sistema.AsString,
            TItemCombo.Create(
                  qryNuIntegracaoExternacd_sistema.AsString,
                  qryNuIntegracaoExternads_sistema.AsString
               )
            );
      end;

      qryNuIntegracaoExterna.Next();
   end;
...
procedure TfrmCursosMoodle.pcMoodleTurmasDiscSelect();
var
   sAux, descricaoCursoSel, siglaCursoSel : String;
begin
   ...
   begin
      ...
      begin
         ...
         gridTurmasDiscMoodle.SelectedRows.CurrentRowSelected := true;
...
const
   ...
   SQL_UPDATE = '' + 
          ' UPDATE unim_moodle_cursos SET '
         + ' ds_descricao = :ds_descricao, '
         + ' ds_sigla = :ds_sigla, '
         + ' cd_integracao_externa = :cd_integracao_externa '
         + ' WHERE CD_MOODLE_CURSO = :CD_MOODLE_CURSO ';
var
   S: AnsiString;
begin
   ...
   frmIncluirCursosMoodle.lbleCodigoKonviva.ReadOnly := True;
   frmIncluirCursosMoodle.lbleCodigoKonviva.Clear;
   frmIncluirCursosMoodle.rgImportar.Enabled := False;
   frmIncluirCursosMoodle.rgImportar.ItemIndex := -1;
   frmIncluirCursosMoodle.lblTipoProva.Enabled := False;
   frmIncluirCursosMoodle.dblcTipoAvaliacao.Enabled := False;

   frmIncluirCursosMoodle.CarregaComboIntegracaoExterna( qyMoodleCursosCD_SISTEMA.asInteger );

   if not qryCursoKonviva.IsEmpty then
   begin
      frmIncluirCursosMoodle.chkKonviva.Checked := True;
      frmIncluirCursosMoodle.rgImportar.Enabled := True;
      frmIncluirCursosMoodle.lblTipoProva.Enabled := True;
      frmIncluirCursosMoodle.dblcTipoAvaliacao.Enabled := True;
      
      frmIncluirCursosMoodle.lbleCodigoKonviva.Text := qryCursoKonvivaCD_KONVIVA_CURSO.AsString;

      if qryCursoKonvivaDS_METODO_IMPORTACAO.AsString = 'final' then
         frmIncluirCursosMoodle.rgImportar.ItemIndex := 0;

      if qryCursoKonvivaDS_METODO_IMPORTACAO.AsString = 'parcial' then
      begin
         frmIncluirCursosMoodle.rgImportar.ItemIndex := 1;
         frmIncluirCursosMoodle.dblcTipoAvaliacao.KeyValue := qryCursoKonvivaCD_AVALIACAO_TIPO.AsInteger;
      end;

      if qryCursoKonvivaDS_METODO_IMPORTACAO.AsString = 'nao_importar' then
         frmIncluirCursosMoodle.rgImportar.ItemIndex := 2;
   end;
   ...
   begin
      ...
      qryAcoesMoodleCurso.SQL.Clear();
      
      qryAcoesMoodleCurso.SQL.Add( SQL_UPDATE );

      qryAcoesMoodleCurso.ParamByName('ds_sigla').AsString := frmIncluirCursosMoodle.lbleSigla.Text;
      qryAcoesMoodleCurso.ParamByName('ds_descricao').AsString := frmIncluirCursosMoodle.lbleDescricao.Text;
      qryAcoesMoodleCurso.ParamByName('cd_integracao_externa').AsInteger :=
         StrToInt(TItemCombo(
            frmIncluirCursosMoodle.cbSistemaIntegraIncluir.Items.Objects[
               frmIncluirCursosMoodle.cbSistemaIntegraIncluir.ItemIndex
            ]
         ).getCodigo);

      qryAcoesMoodleCurso.ParamByName('CD_MOODLE_CURSO').AsInteger :=
         qyMoodleCursosCD_MOODLE_CURSO.AsInteger;

      qryAcoesMoodleCurso.ExecSQL();

      if not qryCursoKonviva.IsEmpty then
      begin
         qryCursoKonviva.Edit;
         qryCursoKonvivaCD_AVALIACAO_TIPO.Clear;
         case frmIncluirCursosMoodle.rgImportar.ItemIndex of
            0:
               qryCursoKonvivaDS_METODO_IMPORTACAO.AsString := 'final';
            1:
               begin
                  qryCursoKonvivaDS_METODO_IMPORTACAO.AsString := 'parcial';
                  qryCursoKonvivaCD_AVALIACAO_TIPO.AsInteger :=
                     frmIncluirCursosMoodle.qryTipoAvaliacaoCD_AVALIACAO_TIPO.AsInteger;
               end;
            2:
               qryCursoKonvivaDS_METODO_IMPORTACAO.AsString := 'nao_importar';
         end;
...
const
   ...
   SQL_INSERT = ''
         + ' INSERT INTO unim_moodle_cursos ('
         + '   ds_descricao, '
         + '   ds_sigla, '
...
begin
   ...
   frmIncluirCursosMoodle.lbleCodigoKonviva.ReadOnly := False;
   frmIncluirCursosMoodle.lbleCodigoKonviva.Clear;
   frmIncluirCursosMoodle.chkKonviva.Checked := False;
   frmIncluirCursosMoodle.lbleCodigoKonviva.Enabled := False;

   frmIncluirCursosMoodle.CarregaComboIntegracaoExterna(0);

   if frmIncluirCursosMoodle.ShowModal = mrOk then
   begin
      qryAcoesMoodleCurso.SQL.Clear();
      
      qryAcoesMoodleCurso.SQL.Add(SQL_INSERT);

      qryAcoesMoodleCurso.ParamByName('ds_sigla').AsString := frmIncluirCursosMoodle.lbleSigla.Text;
      qryAcoesMoodleCurso.ParamByName('ds_descricao').AsString := frmIncluirCursosMoodle.lbleDescricao.Text;
      qryAcoesMoodleCurso.ParamByName('cd_integracao_externa').AsString :=
         TItemCombo(
            frmIncluirCursosMoodle.cbSistemaIntegraIncluir.Items.Objects[
               frmIncluirCursosMoodle.cbSistemaIntegraIncluir.ItemIndex
            ]
      ...
      begin
         ...
         qryCursoKonvivaCD_MOODLE_CURSO.AsLargeInt := DM.LastInsert();
