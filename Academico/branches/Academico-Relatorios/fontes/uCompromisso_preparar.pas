...
uses
  ...
  ZAbstractDataset, ZDataset, UZDataset, General, Buttons;
...
type
  Tfrm_Compromisso_Preparar = class(TForm)
    ...
    btnMarcaTodos: TToolButton;
    btnDesmarcaTodos: TToolButton;
    procedure btnMarcaTodosClick(Sender: TObject);
    procedure btnDesmarcaTodosClick(Sender: TObject);
...
begin
   ...
   begin
      ...
      begin
         if (DataSet.FieldByName('vl_pendente').AsCurrency > 0) AND (DataSet.FieldByName('sn_previsao').AsString <> 'S') then
         begin
            ...
            begin
              ...
              Cells[5,linha] := FloatToStrF(DataSet.FieldByName('vl_multa').AsCurrency, ffFixed, 12, 2);
              Cells[6,linha] := FloatToStrF(DataSet.FieldByName('vl_mora').AsCurrency, ffFixed, 12, 2);;
              Cells[7,linha] := FloatToStrF(DataSet.FieldByName('vl_desconto').AsCurrency, ffFixed, 12, 2);
...
begin
   if ACol = 0 then
   begin
      NovoCheckBox := (sgCompromissos.Objects[ACol, ARow] as TCheckBox);

      if NovoCheckBox <> nil then
      begin
         Rect := sgCompromissos.CellRect(0, ARow);
   ...
   if ACol = 8 then
   begin
      if (not (gdFixed in State)) AND (podeEditarGrid) then
      begin
         AlinharCheck;
      end;
   end;
...
procedure Tfrm_Compromisso_Preparar.ToolButton3Click(Sender: TObject);
const
   SQL_REMOVER_ITEM =
     'DELETE FROM fin_cp_preparacao_itens WHERE cd_preparacao = :preparacao AND cd_titulo = :titulo';
var
   qryRemoveItem : TUMZQuery;
   ultimoItem : Boolean;
begin
   if not DM.UsuarioLogado.TemPermissao( 2041, npAlterar, True ) then Exit;
   ultimoItem := (qyItens.RecordCount = 1);

   if qyPreparacaosn_pgto_autorizado.AsString = 'S' then
   begin
      Mensagem('Não é possível excluir um título desta preparação pois a mesma já foi autorizada. Caso necessite remover este item acesse Compromissos > Pagamentos Preparados > Autorizar Pagamento e remova a autorização da mesma.', 'Alerta', MB_OK + MB_ICONWARNING);
      Exit;
   end;

   if (ultimoItem) then
   begin
      if(Mensagem('Este é o ultimo compromisso vinculado a esta preparação, ao excluir ele será excluida também a preparação, deseja continuar?', 'Alerta', MB_YESNO + MB_ICONWARNING) = mrNo) then
      begin
         Exit;
      end;
   end;

   dm.CriarConsulta(qryRemoveItem);
   qryRemoveItem.SQL.Add(SQL_REMOVER_ITEM);
   qryRemoveItem.ParamByName('preparacao').AsString := qyItenscd_preparacao.AsString;
   qryRemoveItem.ParamByName('titulo').AsString := qyItenscd_titulo.AsString;
   qryRemoveItem.ExecSQL;
...
procedure Tfrm_Compromisso_Preparar.btnMarcaTodosClick(Sender: TObject);
var
   I: Integer;
   CheckBox: TCheckBox;
begin
   for I := 1 to sgCompromissos.RowCount - 1 do
   begin
      if not (sgCompromissos.Objects[0, I] is TCheckBox) then
         Continue;
      CheckBox := TCheckBox(sgCompromissos.Objects[0, I]);
...
begin
   if qyPreparacaodsCaixa.Value = '' then
   begin
     Mensagem('Não é possível preparar um pagamento sem uma conta selecionada', 'Alerta', MB_OK + MB_ICONWARNING);
...
Begin
   ...
   qyAux.SQL.Clear();
   qyAux.SQL.Add(
      'INSERT INTO fin_cp_preparacao_itens( '
         + ' cd_preparacao, cd_titulo, '
         + ' cd_coligada, vl_baixa, '
         + ' vl_multa, vl_juros, '
         + ' vl_desconto, sn_baixado '
      + ') VALUES ');
   qyAux.SQL.Add('( '
      + ' :cd_preparacao, :cd_titulo,'
      + ' :cd_coligada, :vl_baixa, '
...
procedure Tfrm_Compromisso_Preparar.btnDesmarcaTodosClick(Sender: TObject);
var
   I: Integer;
   CheckBox: TCheckBox;
begin
...
begin
   ...
   begin
      if TCheckBox(sgCompromissos.Objects[0, n]).Checked then
      begin
         if (sgCompromissos.cells[8,n] = '') then
         begin
            continue;
         end;
