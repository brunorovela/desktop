...
begin
   ...
   if cmTipoCampo.ItemIndex = 0 then
      ...
      qyPessoasCamposds_tipo.AsString := 'X'  // Seleção Multipla - CheckListBox
   else
...
begin
   ...
   if (cmTipoCampo.ItemIndex = 1) and (qyOpcoes.RecordCount >0) then begin
...
begin
   ...
   else if (qyPessoasCamposds_tipo.AsString ='X')   then
...
var
   i: integer;
begin
   if (cmTipoCampo.ItemIndex = 1) and
...
begin
   ...
   if (qyPessoasCamposds_tipo.AsString = 'S') and (qyOpcoes.RecordCount >0) then
   begin
      dblNomeOpcao.Visible := true;
      lblOpcao.Visible := true;
   end;
