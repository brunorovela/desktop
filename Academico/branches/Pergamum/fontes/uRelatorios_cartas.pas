...
begin
   ...
   umdtInicial.Enabled := contrario;

   lblDtFinal.Enabled := contrario;
   umdtFinal.Enabled := contrario;
...
begin
   umdtInicial.Date := DM.DataAtual;
   umdtFinal.Date := DM.DataAtual;
   ...
   try
      ...
      begin
         ...
         if ckDtFiltrar.Checked then begin
            qyOcorrencias.SQL.Add(' AND (o.dt_registro >= "'+formatDateTime('yyyy-mm-dd',umdtInicial.Date)+'" AND o.dt_registro <= "'+formatDateTime('yyyy-mm-dd',umdtFinal.Date)+'")');
         end;
         ...
         begin
            if not qyOcorrencias.IsEmpty then begin
               while not qyOcorrencias.Eof do begin
                  ...
                  if ckDtFiltrar.Checked then begin
                     qyAtualizar.SQL.Add(' AND (o.dt_registro >= "'+formatDateTime('yyyy-mm-dd',umdtInicial.Date)+'" AND o.dt_registro <= "'+formatDateTime('yyyy-mm-dd',umdtFinal.Date)+'")');
                  end;
      ...
      begin
         ...
         if ckDtFiltrar.Checked then begin
            ...
            qyAtualizar.ParamByName('pInicio').AsString := formatDateTime('yyyy-mm-dd',umdtInicial.Date);
            qyAtualizar.ParamByName('pFim').AsString := formatDateTime('yyyy-mm-dd',umdtFinal.Date);
         end;
      end else begin
   ...
   if length(sCartas) > 0 then begin
     ...
     begin
        ...
        if(ckDtFiltrar.Checked)then
            sFSelecao := sFSelecao + ' AND {ocorrencias.dt_registro} >= Date('+FormatDateTime('yyyy,mm,dd', umdtInicial.Date)+') AND {ocorrencias.dt_registro} <= Date('+FormatDateTime('yyyy,mm,dd', umdtFinal.Date)+')';
     end
     else
     ...
     begin
        slFormulas.Add('data_inicio=Date('+FormatDateTime('yyyy,mm,dd', umdtInicial.Date)+') ');
        slFormulas.Add('data_fim=Date('+FormatDateTime('yyyy,mm,dd', umdtFinal.Date)+') ');
     end;
