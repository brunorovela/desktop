...
//    procedure getListaCentroCustos();
    ...
    function validaContaCentroIguais(cd_centro, cd_conta : String; bInserir: Boolean) : Boolean;
    { Public declarations }
  end;
...
begin
   ...
   bContaCentroIguais := validaContaCentroIguais(cd_centro, cd_conta, false);

   if ( bContaCentroIguais ) then
   begin
...
begin
   ...
   bContaCentroIguais := validaContaCentroIguais(cd_centro, cd_conta, true);
...
function TfrmApropriaAlteraCaixa.validaContaCentroIguais(cd_centro,
  cd_conta : String; bInserir: Boolean): Boolean;
  var i   : integer;
      msg : String;
begin
   msg := 'Você não pode selecionar uma combinação de conta e centro de custo iguais nas apropriações.';
   Result := True;

   for i := 0 to ListaApropriacoes.Count -1 do
   begin
      if ((TFinApropriaTesouraria(ListaApropriacoes.Items[i]).getValorCampo('cd_conta')  = cd_conta) AND
         (TFinApropriaTesouraria(ListaApropriacoes.Items[i]).getValorCampo('cd_centro')  = cd_centro))
      Then
      begin
         if bInserir then
         begin
          ...
          begin
           if ( i <> Self.getCdApropriacaoSelecionada ) then
            begin
               Result := False;
                  Mensagem(
                  msg,
