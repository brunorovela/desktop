object fCancelTaxasRecorrencia: TfCancelTaxasRecorrencia
  ...
  object qrySelectParcelas: TUMZQuery
    ...
    SQLList.Strings = (
        ...
        'juros,'#13#10'   m.dt_competencia,'#13#10'   m.cd_coligada,'#13#10'   (SUM(mc.vl_e' +
        'ntrada) - SUM(mc.vl_saida)) vl_pendente'#13#10'FROM'#13#10'   fichaindividua' +
        'l fi'#13#10'      JOIN mensalidades m ON'#13#10'         (fi.cd_mensalidade_' +
        'exame = m.cd_mensalidade)'#13#10'      JOIN pessoas p ON'#13#10'         (fi' +
        '.codigoaluno = p.cd_pessoa)'#13#10'      LEFT JOIN fin_mov_cr mc ON'#13#10' ' +
        '        (m.cd_mensalidade_origem = mc.cd_mensalidade)'#13#10'WHERE'#13#10'  ' +
        ' m.situacao = 10'#13#10'GROUP BY'#13#10'   m.cd_mensalidade_origem,'#13#10'   m.cd' +
        '_coligada'
        ...
        'rjuros,'#13#10'   m.dt_competencia,'#13#10'   m.cd_coligada,'#13#10#9' AGRUPADO.vl_' +
        'pendente'#13#10'FROM'#13#10'(SELECT '#13#10#9' M.cd_mensalidade_origem, '#13#10#9' M.cd_co' +
        'ligada, '#13#10#9' FIRST_OCCURRENCE(M.CD_MENSALIDADE) AS CD_MENSALIDADE' +
        ','#13#10#9' (SUM(mc.vl_entrada) - SUM(mc.vl_saida)) AS vl_pendente'#13#10' FR' +
        'OM '#13#10#9' MENSALIDADES M'#13#10#9' LEFT JOIN fin_mov_cr mc ON'#13#10'         (M' +
        '.cd_mensalidade_origem = mc.cd_mensalidade)'#13#10' WHERE'#13#10#9' M.SITUACA' +
        'O = 10'#13#10' GROUP BY'#13#10#9' M.cd_mensalidade_origem, M.cd_coligada) AGR' +
        'UPADO'#13#10'INNER JOIN mensalidades m ON (AGRUPADO.CD_MENSALIDADE = m' +
        '.CD_MENSALIDADE)'#13#10'INNER JOIN fichaindividual fi ON (fi.cd_mensal' +
        'idade_exame = m.cd_mensalidade)'#13#10'INNER JOIN pessoas p ON (fi.cod' +
        'igoaluno = p.cd_pessoa)')
    Left = 168
    Top = 120
