...
type
  ...
  public
    ...
    lst_situacao_fin_desc : array of String[100];
    lst_situacao_fin_prot : array of Integer;
    lst_situacao_fin_sigla : array of String[100];
    ...
    procedure DuplicarProvaDisciplinaFrente(iAnosemestre: Integer; sCurso: string; sTurma: string; iDisciplina: Integer; iDisciplinaFrente: Integer; iBimestre: Integer;iNrProvaFilha : Integer);
    ...
    Function GerarMensalidade(
         ...
         liberarDescontos    : Boolean = false;
         criarAcaoCondicionalPadrao : Boolean = true) : Integer;
    ...
    function MatricularGrade( lngCodigoAluno : Longint;
                               ...
                               forcarMatriculaPreRequisito: Boolean = False) : String;
    ...
    procedure recalcularDescCondicionalAcoesMovimento(iCodigoMensalidade: Integer; cDescontoCondicionalOriginal, cNovoDescontoCondicionalParcela: Currency; atualizarDescontoPrevisto: Boolean = True; restauraDesconto:Boolean = false);
    function getValorEntradaAcaoMensalidade(cd_mensalidade:Integer): Currency;
...
uses
   ...
   uFinApropriaTesouraria, UMLookupComboBox, uFSplash, md5;
...
procedure TDM.DuplicarProvaDisciplinaFrente(iAnosemestre: Integer; sCurso,
  sTurma: string; iDisciplina, iDisciplinaFrente, iBimestre: integer; iNrProvaFilha : Integer);
var
...
begin
   ...
   try
      Stmt := db.DbcConnection.CreateStatement;
      Stmt.SetResultSetConcurrency(rcUpdatable);
      Rs := Stmt.ExecuteQuery('SELECT CD_SITUACAO, DS_SITUACAO, DS_SIGLA_SITUACAO, SN_PROTESTO FROM SITUACOES_FINANCEIRAS ORDER BY CD_SITUACAO');
      try
         Rs.Last;
...
begin
  ...
  qryGetDate.SQL.Text := 'SELECT md5(concat(now(), concat('''+IntToStr(Parcela)+''','''+turma+'''))) as chave';
  qryGetDate.Open;
...
function TDM.MatricularGrade(lngCodigoAluno : Longint; strTurma : String; intAnoSemestre: Integer; sGrade : String = '01'; forcarMatriculaPreRequisito: Boolean = False) : String;
const
   SSQLInsertDisc =
      'INSERT INTO FICHAINDIVIDUAL ' +
         '(ANOSEMESTRE, CODIGOALUNO, TURMA, TURMAMATRICULA, DISCIPLINA, SITUACAO, CURSO, SERIE, GRAU)' +
      'SELECT ' +
         'M.ANOSEMESTRE,' +
         'M.CODIGOALUNO,' +
         'M.TURMA,' +
...
begin
   ...
   if DM.variavel_parametro('reserva_matric_disciplinas_horario') = 'S' then
   begin
   ...
   if DM.variavel_parametro('reserva_matric_disciplinas_horario') = 'N' then
   begin
...
begin
   ...
   Result := False;
   for i := Low(AISituacoes) to High(AISituacoes) - 1 do
   begin
...
begin
  ...
  begin // Marcar cada parcela como negociada
    ...
    try
      qryUpdate.FieldByName('ValorDesconto').Value := StrToCurr(ArrayDescontosParc[n]);
    except on E: Exception do
      qryUpdate.FieldByName('ValorDesconto').Value := StrToCurr('0');
    end;
    ...
    ValorTotal := qryUpdate.FieldByName('ValorBruto').AsCurrency
                  ...
                  - StrToCurr(ArrayDescontosParc[n]);
  ...
  begin
     ...
     iCodMensa := DM.GerarMensalidade(
       ...
       StrToDateTime(ArrayDatas[intIndice]), 2,
       ...
       cd_tipo_titulo,
       cd_tipo_conta,
       DM.tblDeptoCursoTurmacd_centro.AsInteger, 0
       ,0, 'N', 0, 0, 10, 0, 0, Dm.tblDeptoCursoTurmacd_coligada.AsInteger,
        0,100,0,'',0,liberarDescontos
     );
...
Var
  ...
  bAjustou, descCond, descExtra : Boolean;
  sFiltro1, sFiltro2,
  sLog : String;
  dtBloqueio : TDateTime;
  dt_hoje : String;
  porcDescFisico,porcDescCondicional,cValorEntrada : Currency;
begin
 // Essa função é disparada sempre que houve mudança na grade curricular do aluno - matríuclas em disciplinas
  ...
  begin
    Close();
    SQL.Text := '' +
      ' SELECT SUM(ROUND(m.vl_credito * IF(COALESCE(i.NR_CREDITOS_MINIMOS,0) > :Creditos, COALESCE(i.NR_CREDITOS_MINIMOS,0), :Creditos ), 2)) VlCompleto ' +
      ' FROM mensalidades m' +
      ...
      with qSel do begin
        ...
        begin
           ...
           begin
              ...
              qAtualiza.SQL.Text := '' +
               ' UPDATE mensalidades ' +
               '    SET valorbruto = :NovoValor, nr_creditos = (:NovoValor / vl_credito), ' +
              ...
              begin
                 NovoValor := RoundFloat(FieldByName('ValorMinimo').AsFloat, 2);
                 NovoCredito := FieldByName('nr_creditos_minimos').AsFloat;
              end else begin
                 NovoValor := RoundFloat(FieldByName('NovoValor').AsFloat, 2);
              ...
              if FieldByName('valorbruto').AsFloat > 0 then
              begin
                 NovoDesconto := RoundFloat(RoundFloat(FieldByName('valordesconto').AsFloat / FieldByName('valorbruto').AsFloat, 4)
                                                               * qAtualiza.ParamByName('NovoValor').AsFloat, 2);

                 NovoDescExtra := RoundFloat(RoundFloat(FieldByName('descontoextra').AsFloat / FieldByName('valorbruto').AsFloat, 4)
                                                               * qAtualiza.ParamByName('NovoValor').AsFloat, 2);
              end else begin
                 NovoDesconto := 0;
                 NovoDescExtra := 0;
                 if FieldByName('ind_desconto').AsFloat > 0 then
                 begin
              ...
              porcDescFisico      := FieldByName('vl_perc_desc_fixo').AsFloat;
              porcDescCondicional := FieldByName('vl_perc_desc_cond').AsFloat;

              descCond := false;
              descExtra := false;
              //Se o desconto está zerado mas a porcentagem não temos de restaurar o desconto
              if((NovoDescExtra = 0) AND (porcDescFisico > 0) AND (NovoValor > 0) ) then
              begin
                 NovoDescExtra := RoundFloat(((porcDescFisico * NovoValor) / 100),2);
                 descExtra := true;
              end;

              //Se o desconto Condicional está zerado mas a porcentagem não temos de restaurar o desconto
              if((NovoDesconto = 0) AND (porcDescCondicional > 0) AND (NovoValor > 0) ) then
              begin
                 NovoDesconto := RoundFloat(((porcDescCondicional * NovoValor) / 100),2);
                 descCond := true;
              end;

              //Se tiver valor para ser feito o calculo
              if((FieldByName('valortotal').AsFloat > 0) AND (FieldByName('valorbruto').AsFloat > 0) AND (not descCond)) then
              begin
                 porcDescCondicional := RoundFloat(((100 * FieldByName('valordesconto').AsFloat) / FieldByName('valorbruto').AsFloat),2);
              end;

              //Se tiver valor para ser feito o calculo
              if((FieldByName('valortotal').AsFloat > 0) AND (FieldByName('valorbruto').AsFloat > 0) AND (not descExtra)) then
              begin
                 porcDescFisico := RoundFloat(((100 * FieldByName('descontoextra').AsFloat) / FieldByName('valorbruto').AsFloat),2);
              end;
              ...
              qAtualiza.ParamByName('porcDescFisico').AsCurrency := porcDescFisico;
              qAtualiza.ParamByName('porcDescCondicional').AsCurrency := porcDescCondicional;
              qAtualiza.ExecSQL();
              ...
              qAtualiza.ParamByName('NovoValor').AsFloat    := RoundFloat(NovoValor + FieldByName('valorextra').AsFloat,2);
              qAtualiza.ExecSQL();

              // Atualizar Movimento de Desconto Condicional
              DM.recalcularDescCondicionalAcoesMovimento(FieldByName('cd_mensalidade').AsInteger, RoundFloat(FieldByName('valordesconto').AsFloat,2), RoundFloat(NovoDesconto, 2), false, true);
              ...
              // Atualizar Movimento de Desconto Extra, parte do valor
              qAtualiza.Close();
              qAtualiza.SQL.Clear();
              qAtualiza.SQL.Text := ' UPDATE fin_mov_cr f, fin_acoes_movimento a ' +
                   ' SET f.vl_saida = round((CASE WHEN f.vl_saida > 0 THEN ((f.vl_saida * :NovoDescExtra) / :descontoextra) ' +
                   '                         ELSE (f.vl_perc_desc_fixo * :NovoDescExtra) / 100 END),2) ' +
                   ' WHERE f.cd_acao = a.cd_acao AND            ' +
                   '       f.cd_mensalidade = :CdMensalidade AND' +
                   '       a.cd_tipo_acao = 3 AND f.sn_desc_condicional = 0 ';
              // Ação Tipo = 3 => BAIXA POR DESCONTO;
              qAtualiza.ParamByName('CdMensalidade').AsInteger := FieldByName('cd_mensalidade').AsInteger;
              qAtualiza.ParamByName('NovoDescExtra').AsFloat    := NovoDescExtra;
              qAtualiza.ParamByName('descontoextra').AsFloat    := RoundFloat(FieldByName('descontoextra').AsFloat,2);
              qAtualiza.ExecSQL();

              //Atualiza primeiro o valor da fin_mov_cr, depois usa o valor de saida atualizado para o calculo do campo de vl_perc_desc_fixo
              //                   '     '+
              qAtualiza.Close();
              qAtualiza.SQL.Clear();
              qAtualiza.SQL.Text := ' UPDATE fin_mov_cr f, fin_acoes_movimento a ' +
                   ' SET f.vl_perc_desc_fixo = CASE WHEN :cValorEntrada = 0 THEN f.vl_perc_desc_fixo ELSE round(((100 * f.vl_saida) / :NovoDescExtra),2) END  ' +
                   ' WHERE f.cd_acao = a.cd_acao AND            ' +
                   '       f.cd_mensalidade = :CdMensalidade AND' +
                   '       a.cd_tipo_acao = 3 AND f.sn_desc_condicional = 0 ';
              qAtualiza.ParamByName('CdMensalidade').AsInteger := FieldByName('cd_mensalidade').AsInteger;
              qAtualiza.ParamByName('cValorEntrada').AsCurrency := cValorEntrada;
              qAtualiza.ParamByName('NovoDescExtra').AsFloat    := NovoDescExtra;
              qAtualiza.ExecSQL();
           end;
...
var
   ...
   ResultSet: IZResultSet;
   HasRecords: Boolean;
   qryInsereConclusao, qryConclusaoCurso, qryConcluido, qryAprovado, qryExcluiConclusao: TUMZQuery;
begin
   ...
   if (qryConclusaoCurso.RecordCount > 0) AND ((qryConclusaoCurso.FieldByName('GRAU').AsInteger = 1) OR (qryConclusaoCurso.FieldByName('GRAU').AsInteger = 2)) then
   begin
      ...
      begin
         ...
         qryInsereConclusao.SQL.Text := 'INSERT INTO historico_escolas_grau(CODIGOALUNO, GRAU, ANO, DS_ESCOLA, DS_ESCOLA_CIDADE, DS_ESCOLA_ESTADO, DS_CURSO, CD_INSTITUICAO, SN_AUTOMATICO)'+
            'VALUES(:cd_aluno, :grau, YEAR(NOW()), :ds_escola, :ds_escola_cidade, :ds_escola_estado, :ds_curso, :cd_instituicao, 1)';
            ...
            begin
               ...
               try
                  ...
                  begin
                     ...
                     Statement.SetInt(4, ResultSet.GetInt(8));
                     Statement.SetInt(5, ResultSet.GetInt(5));
                     Statement.SetInt(6, ResultSet.GetInt(7));
                     Statement.SetInt(7, ResultSet.GetInt(6));
                     try
...
function TDM.getValorEntradaAcaoMensalidade(cd_mensalidade: Integer): Currency;
const
...
begin
   ...
   cValorEntrada := qyGetEntrada.FieldByName('entrada').AsCurrency;
  ...
  while not qyMatriculas.EOF do begin
      ...
      if (bFinanceiro OR bDocumento) then begin
         ...
         // Exit;
          ...
          if variavel_parametro('data_emissao_matricula_ativacao') <> 'S' then
          begin
             qyVerifica.Close;
             qyVerifica.SQL.Clear;
             qyVerifica.SQL.Add( 'UPDATE matriculas SET situacao = 1' );
             qyVerifica.SQL.Add( 'WHERE anoSemestre = ' + IntToStr(intanosemestre) + ' and ');
             qyVerifica.SQL.Add( 'turma = ' + QuotedStr(strTurma) + ' and ');
             qyVerifica.SQL.Add( 'codigoaluno = ' + IntToStr(intCodigoAluno) + ' and ');
             qyVerifica.SQL.Add( 'situacao = 10');
             qyVerifica.ExecSQL;
          end
...
var
   ResultSet: IZResultSet;
begin
   ResultSet := ExecuteQuery('SELECT CURDATE() FROM DUAL');
   try
      if ResultSet.Next then
...
begin
  ...
  qAux1.SQL.Add( '	be.cd_pessoa = :cd_pessoa                                 ');
...
begin
   ...
   begin
      ...
      begin
         ...
         S := variavel_parametro('data_emissao_matricula_ativacao');
         ...
         begin
            Stmt := PrepareStatement(SSQLAtivaMatricula);
         end;
         try
            Stmt.SetInt(1, AnoSemestre);
            Stmt.SetString(2, Turma);
...
begin
    if DM.variavel_parametro('docs_digitalizados_base') <> '' then
    Begin
      dbDocs.Database := DM.variavel_parametro('docs_digitalizados_base');
    end;
end;
...
begin
   ...
   if not (Acao in [arGerarMensalidade, arImpBoleto]) then begin
      ...
      qyEx.Close();
      qyEx.SQL.Text :=
        ' SELECT ir.cd_mensalidade, rm.cd_remessa, oc.*  from mensalidades as m1  '+
        ' INNER JOIN mensalidades as m2 ON (m2.cd_boleto = m1.cd_boleto AND m2.cd_resp = m1.cd_resp)          '+
        ' INNER JOIN itensretorno as ir ON (ir.cd_mensalidade = m2.cd_mensalidade) '+
        ' INNER JOIN retorno as rt ON (rt.codigo = ir.codigoretorno)               '+
        ' INNER JOIN rem_dados as rm ON (rm.cd_mensalidade = m2.cd_mensalidade)    '+
        ' INNER JOIN banco_codigo_retorno as oc ON (ir.cd_ocorrencia = oc.cd_retorno AND rt.cd_banco = oc.cd_banco AND oc.sn_aceito = 1) '+
        ' INNER JOIN rem_layouts rl ON (rm.cd_layout = rl.cd_layout AND rl.sn_sempre_reenviar = 0)                                       '+
        ' WHERE m1.cd_mensalidade = ' + IntToStr(iCodMensalidade);
      qyEx.Open();
...
begin
   ...
   // Existem provas criadas, verifica qual é o numero da próxima prova
   while not qyNroProvas.Eof do
   begin

      // Verifica se a prova que será criada já existe, nesse caso incrementa o número da proxima prova
      if (qyNroProvas.FieldByName('nro_nota').AsInteger = iNroProxProva) then
      begin
         iNroProxProva := iNroProxProva + 1;
      end else begin
         break; // Encontrou uma número vago para a próxima prova
      end;
...
procedure TDM.recalcularDescCondicionalAcoesMovimento(iCodigoMensalidade: Integer; cDescontoCondicionalOriginal, cNovoDescontoCondicionalParcela: Currency; atualizarDescontoPrevisto: Boolean; restauraDesconto:Boolean);
const
...
begin
   ...
   begin
      cIndicePercentual := ((cNovoDescontoCondicionalParcela * 100) / cDescontoCondicionalOriginal)/100;
   end;
   ...
   begin
      ...
      begin
         cNovoDesconto := RoundFloat(cNovoDescontoCondicionalParcela - cValorMovimentacoes, 2);
      end else begin
         // Cálcula o proporcional das ações de movimento para incluir o novo valor na ação de movimento
         cNovoDesconto := RoundFloat(qyAcao.FieldByName('vl_desconto').AsCurrency * cIndicePercentual, 2);;
      end;
      ...
      qyAtualizarAcao.ParamByName('vl_desconto').AsCurrency := cNovoDesconto;
...
const
   SQL_DADOS_MENSALIDADE = '' +
      ' SELECT ' +
      '   DISTINCT m.cd_mensalidade_origem, m.cd_coligada, tt.ds_tipo_titulo ' +
      ' FROM ' +
      '   mensalidades m ' +
      '   INNER JOIN fin_tipos_titulo tt ON (tt.cd_tipo_titulo = m.cd_tipo_titulo) ' +
...
begin
   ...
   movimento := TMovimento.create;
   try
      movimento.Observacao := qyDadosMensalidade.FieldByName('ds_tipo_titulo').AsString + ' (Ação automática de desconto condicional)';
...
begin
   ...
   // Aplica dos descontos condicionais nas ações de movimento da mensalidade
   qyAcoes.Close;
