...
const
   SQL_COMBO_CATEGORIAS =
      ' SELECT CD_CATEGORIA, DS_CATEGORIA FROM COMP_CATEGORIAS ';
...
begin
   ...
   begin
      qyListaCategorias.SQL.Add('WHERE CD_COLIGADA = :CD_COLIGADA');
      qyListaCategorias.ParamByName('CD_COLIGADA').AsInteger := cd_coligada;
   end else begin
      qyListaCategorias.SQL.Add(Format('WHERE CD_COLIGADA IN ( %s )', [DM.GetColigadas()]));
   end;
...
const
   SQL_FILTRAR =
      ' SELECT CP.CD_PRODUTO, CP.DS_PRODUTO, CP.SN_IMPRIMIR_CUPOM, ' +
      ...
      '      LEFT JOIN  COMP_CODIGO_BARRAS CB on (CP.cd_produto = CB.cd_produto)     ' +
      '      INNER JOIN COMP_CATEGORIAS CC ON ( CC.CD_CATEGORIA = CP.CD_CATEGORIA ) ' +
      ' WHERE 1=1 ';
   SQL_FILTRAR_ORACLE =
      ' SELECT CP.CD_PRODUTO, CP.DS_PRODUTO, CP.SN_IMPRIMIR_CUPOM, ' +
      '        CAST( GET_COMPRAS_QTD_ESTOQUE( CP.CD_PRODUTO ) AS NUMBER(19) ) AS QTD_PRODUTOS, ' +
      '        CB.cd_codigo_barras as codBarras' +
      ' FROM COMP_PRODUTOS CP ' +
      '      LEFT JOIN COMP_CODIGO_BARRAS CB on (CP.cd_produto = CB.cd_produto)     ' +
      '      INNER JOIN COMP_CATEGORIAS CC ON ( CC.CD_CATEGORIA = CP.CD_CATEGORIA ) ' +
      ' WHERE 1=1 ';
   ...
   SQL_FILTRAR_WHERE_COLIGADA =
      ' AND CC.CD_COLIGADA =  :CD_COLIGADA ';
...
begin
   ...
   if DM.db.Protocol = DB_PROTOCOL_ORACLE then
      qyListaProdutos.SQL.Text := SQL_FILTRAR_ORACLE
   else
