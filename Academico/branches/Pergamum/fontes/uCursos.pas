...
type
  TfrmCursos = class(TForm)
    ...
    procedure qyCursosMestreAfterScroll(DataSet: TDataSet);
...
uses
   ...
   uSala, uTurma, uFSelecionarTurma,uFSelecionarPessoa, uProcessando,uFSplash;
...
begin
   ...
   begin
      ...
      TfSplash.GetInstancia().SetSplashText('Processando '+IntToStr(contador)+' de '+IntToStr(numTabelas)+'.');
      inc(contador);
                     ...
                     try
                        ...
                        begin
                           DM.setLog(
                              ...
                              Format(SLogErro, [CurrCode, NewCode, E.Message, SQLUpdate])
                           );
...
begin
   ...
   tblDisc.Close;
   tblDisc.SQL.Clear;
   tblDisc.SQL.Add( 'Select * From Disciplinas where Curso = :Codigo AND sn_ativo = 1');
...
begin
   ...
   if qyCursosMestre.State in [dsbrowse] then
   begin
      ...
      begin
         ...
         if(TItemCombo(cbSituacoes.Items.Objects[cbSituacoes.ItemIndex]).getCodigo = qyCursosMestreNR_GRAU.AsString) and not(entrou) then
         begin
            entrou := true;
         end;
      end;
...
//   carregarDisciplinasPai();
  ...
  if (TitemCombo(cbColigadas.Items.Objects[cbColigadas.ItemIndex]).getCodigo() <> qyColigadacd_coligada.AsString) then
  begin

     for i := 0 to cbColigadas.Items.Count do
     begin
...
procedure TfrmCursos.qyCursosMestreAfterScroll(DataSet: TDataSet);
var
   qyVerificaCursoColigada: TUMZQuery;
   sChave, sValor : String;
   i: Integer;
begin
   ...
   begin
      ...
      sChave := qyCursosMestreCD_CURSO.AsString + ';' + TItemCombo(cbColigadas.Items.Objects[i]).getCodigo();
      if arrCursosColigadas.Values[sChave] = '' then
      begin
         DM.CriarConsulta(qyVerificaCursoColigada);
         ...
         qyVerificaCursoColigada.SQL.Text := ' SELECT COUNT(CD_CURSO) AS registros FROM cursos_coligadas WHERE cd_curso = :cd_curso AND cd_coligada = :cd_coligada AND sn_ativo = 1 ';
         qyVerificaCursoColigada.ParamByName('cd_curso').AsString := qyCursosMestreCD_CURSO.AsString;
         qyVerificaCursoColigada.ParamByName('cd_coligada').AsString := TItemCombo(cbColigadas.Items.Objects[i]).getCodigo();
         qyVerificaCursoColigada.Open();
      ...
      begin
         cbColigadas.ItemIndex := i;
         try
            cbColigadasChange(Self);
         except
            //Não executa abrir a tela         
   		 end;
