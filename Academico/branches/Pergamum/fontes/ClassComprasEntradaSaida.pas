...
Type
   ...
   TClasseComprasEntradaSaida = class(TObject)
      private
         ...
         function getAnoSemSaidaProduto(codigoaluno: integer):Integer;
      ...
      public
         ...
         function GerarTituloVarios( vl_bruto : Currency; dt_vencto: TDateTime; const cd_compra, cd_tipo_titulo, nr_parcela: Integer; const sn_gerar_mensalidade: Boolean = False; const sn_verifica_titulo: Boolean = true): Integer;
...
function TClasseComprasEntradaSaida.GerarTituloVarios(vl_bruto : Currency; dt_vencto: TDateTime;
   ...
   const sn_gerar_mensalidade: Boolean = False; const sn_verifica_titulo: Boolean = true): Integer;
const
   SQL_GERAR_TITULO =
      ' SELECT cp.SN_ACUMULAR_VALORES, cp.SN_GERAR_TITULO, ce.CD_PESSOA, ce.VL_COMPRA ' +
      ' FROM comp_estoque ce ' +
...
begin
   ...
   begin
      // Vamos gerar um título a ser pago
      GerarMensalidadeVarios(qyGerarTitulo.FieldByName('cd_pessoa').AsInteger,0,cd_tipo_titulo,nr_parcela,vl_bruto,dt_vencto,cd_compra);
   end else begin
      // Vamos acumular os valores
      AdicionaAcumulado( cd_compra, qyGerarTitulo.FieldByName('cd_pessoa').AsInteger );
   end;
...
begin
   ...
   while not qryBuscaAnosem.Eof do
   begin
      anoSemReturn := qryBuscaAnosem.FieldByName('anosemestre').AsInteger;
      break;
   end;
...
const
   SQL_ZERA_IMPRESSAO =
      ' UPDATE COMP_ESTOQUE ce SET ce.SN_PRIMEIRA_IMPRESSAO = 0 ' +
      ' INNER JOIN comp_estoque_mensalidade cee ON (cee.cd_compra = ce.cd_compra) ' +
