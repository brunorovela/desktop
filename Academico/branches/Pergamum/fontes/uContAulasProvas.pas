...
uses
  ...
  uColigada, UMDateTimePicker, Menus;
...
begin
   ...
   begin
      ...
      with qyAulas do begin
         ...
         '    da.cd_professor, p.nm_pessoa, da.conteudo, ' +
         '    dg.cd_grupo, dg.nm_grupo, da.sn_aula_compartilhada, ati.ds_aula_tipo, da.cd_aula_tipo, S.`DS_VALOR` `DS_SITUACAO`, ' +
         ...
         ' LEFT JOIN `SITUACOES` S ON (DA.`CD_SITUACAO` = S.`CD_SITUACAO`) AND (S.`CD_MODULO` = 1029) ' +
         ' WHERE                                                                                               ' +
         '');
         sAux := '';
   ...
   //aba Provas
      ...
      with qyProvas do begin
         ...
         '    dp.nr_nota_maxima, dp.nr_nota_minima, dg.cd_grupo, dg.nm_grupo, avt.cd_tipo, dt.sn_diario_fechado, '+
         ...
         ' INNER JOIN coligadas c ON (c.cd_coligada = t.cd_coligada) ' +
         ' LEFT JOIN diario_turmas dt ON (dt.curso = t.curso AND dt.anosemestre = t.anosemestre AND dt.turma = t.codigo AND dt.disciplina = d.codigo AND dt.bimestre = dp.bimestre AND dt.professor = dp.cd_professor)' +
