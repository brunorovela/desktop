...
begin
   ...
   begin
      ...
      '     SELECT '+
	   ...
	   '     FROM '+
      '        fin_controle_caixa) '+
      '  AND cd_conta_banco = ' + IntToStr(iContaDe);
     qyCalculaSaldo.Open();
   ...
   begin
      ...
      qyCalculaSaldo.ParamByName('DT_BASE').AsDate := umdtAjuste.Date;
      qyCalculaSaldo.Open;
   ...
   //MOVER PRA CIMA PRA PEGAR O TP_CONTA E REALIZAR A VERIFICAÇÃO ACIMA!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   qyMovimentos.Close;
   qyMovimentos.SQL.Text := ' '+
   'SELECT                                   '+
   '  fmt.cd_movimento_te as cd_mov_te, '+
...
begin
   ...
   if (StrToFloat(edValor.Text) > saldoTotal) then
   begin
