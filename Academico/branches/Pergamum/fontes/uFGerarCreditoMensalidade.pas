unit uFGerarCreditoMensalidade;

interface

uses
  Windows, Messages, SysUtils, Variants, Classes, Graphics, Controls, Forms,
  Dialogs, ExtCtrls, StdCtrls, Buttons, UMEditMonetario, uMensalidade, uCredito;

type
   TfrmGerarCreditoMensalidade = class(TForm)
      paCabecalho: TPanel;
      edMensalidade: TEdit;
      Label1: TLabel;
      Label2: TLabel;
...
procedure TfrmGerarCreditoMensalidade.Button2Click(Sender: TObject);
var
   inserir : Boolean;
begin
   if (edValorCredito.getValorDouble() > objMensalidade.ValorPago) OR
   ...
   if inserir then
   begin
      objCredito := TCredito.Create();
      objCredito.Mensalidade := objMensalidade;
   end else begin
      { Caso não seja uma inclusão, então devemos testar se o novo valor digitado, está dentro do valor permitido }
      if edValorCredito.getValorDouble() < objCredito.GetValorCreditoUsado() then
      begin
   ...
   objCredito.ValorCredito := edValorCredito.getValorDouble();
   objCredito.Motivo := mmMotivo.Lines.Text;
   objCredito.Usuario := TLoginManager.GetInstancia().GetDM().GetUsuarioLogado().Pessoa;

   if inserir then
   begin
      TCredito.Inserir(objCredito);
   end else begin
      TCredito.Alterar(objCredito);
   end;
end;

procedure TfrmGerarCreditoMensalidade.edValorCreditoExit(Sender: TObject);
begin
   if (edValorCredito.getValorDouble() > objMensalidade.ValorPago) OR
      (edValorCredito.getValorDouble() = 0) OR
      (
         (objCredito <> nil) AND
         (edValorCredito.getValorDouble() < objCredito.GetValorCreditoUsado())
      ) then
   begin
...
procedure TfrmGerarCreditoMensalidade.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
   Action := caFree;
end;

procedure TfrmGerarCreditoMensalidade.FormCloseQuery(Sender: TObject;
...
procedure TfrmGerarCreditoMensalidade.FormCreate(Sender: TObject);
begin
   Self.objMensalidade := nil;
   Self.objCredito := nil;
end;

procedure TfrmGerarCreditoMensalidade.FormKeyDown(Sender: TObject;
...
procedure TfrmGerarCreditoMensalidade.FormShow(Sender: TObject);
var
   mensalidade: TMensalidade;
   valorMinimo: Double;
begin
   if (objMensalidade = nil) OR NOT(objMensalidade is TMensalidade) then
   begin
   ...
   edMensalidade.Text := IntToStr(objMensalidade.Codigo);
   valorMinimo := 0;

   try
      Self.objCredito := TCredito.GetCreditoFromMensalidade(objMensalidade);

      edValorCredito.setValor(objCredito.ValorCredito);
      mmMotivo.Lines.Text := objCredito.Motivo;

      valorMinimo := Self.objCredito.GetValorCreditoUsado();
   except
      on E:TExceptionDatabase do
      begin
         edValorCredito.setValor(objMensalidade.ValorPago);
   ...
   begin
      lbInformativoValorMaximo.Caption := Format(
         ...
         [valorMinimo, objMensalidade.ValorPago],
         TGeneral.GetFormatSettings()
      );
   end else begin
      lbInformativoValorMaximo.Caption := Format(
         '(valor máximo: %m)',
         [objMensalidade.ValorPago],
         TGeneral.GetFormatSettings()
      );
   end;
...
procedure TfrmGerarCreditoMensalidade.Panel1Resize(Sender: TObject);
begin
   pnResize.Left := Trunc(TPanel(Sender).Width / 2) - Trunc(pnResize.Width/2);
end;

procedure TfrmGerarCreditoMensalidade.Panel2Resize(Sender: TObject);
begin
