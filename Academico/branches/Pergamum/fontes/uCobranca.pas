...
uses
  ...
  RegExpr, UMDateTimePicker, uColigada;
...
uses
   ...
   uFSelecionarDisciplina, uFSelecionarColigada;
...
begin
   ...
   cd_pessoa := qyCobrancaCD_PESSOA.AsInteger;
   ...
   begin
      ...
      qyCobranca.Locate('cd_pessoa', cd_pessoa, []);
...
begin
   ...
   try
      ...
      try
         ...
         try
            DecodeDate(qyCobranca.ParamByName('DT_SPC').asDate, myYear, myMonth, myDay);
         ...
         try
            DecodeDate(qyCobranca.ParamByName('DT_SPCATE').asDate, myYear, myMonth, myDay);
         ...
         try
            DecodeDate(qyCobranca.ParamByName('DT_JURIDICO').asDate, myYear, myMonth, myDay);
         ...
         try
            DecodeDate(qyCobranca.ParamByName('DT_JURIDICOATE').asDate, myYear, myMonth, myDay);
         ...
         try
            DecodeDate(qyCobranca.ParamByName('DT_SERASA').asDate, myYear, myMonth, myDay);
         ...
         try
            DecodeDate(qyCobranca.ParamByName('DT_SERASAATE').asDate, myYear, myMonth, myDay);
         ...
         try
            DecodeDate(qyCobranca.ParamByName('DT_CARTORIO').asDate, myYear, myMonth, myDay);
         ...
         try
            DecodeDate(qyCobranca.ParamByName('DT_CARTORIOATE').asDate, myYear, myMonth, myDay);
         ...
         try
            DecodeDate(qyCobranca.ParamByName('DT_PAGAMENTO_INFERIOR').asDate, myYear, myMonth, myDay);
         ...
         try
            DecodeDate(qyCobranca.ParamByName('DT_PAGAMENTO_SUPERIOR').asDate, myYear, myMonth, myDay);
         ...
         try
            DecodeDate(qyCobranca.ParamByName('DT_RETORNO_INFERIOR').asDate, myYear, myMonth, myDay);
         ...
         try
            DecodeDate(qyCobranca.ParamByName('DT_RETORNO_SUPERIOR').asDate, myYear, myMonth, myDay);
         ...
         try
            DecodeDate(qyCobranca.ParamByName('DT_VENCTO_SUPERIOR').asDate, myYear, myMonth, myDay);
         ...
         try
            DecodeDate(qyCobranca.ParamByName('DT_VENCTO_INFERIOR').asDate, myYear, myMonth, myDay);
...
const
   ...
   S_SQL_MYSQL =
      'SELECT '+
         ...
         'SUM(if(ME.SITUACAO IN (2 $situacao_ligacao$), COALESCE(ME.VALORBRUTO, 0) + COALESCE(ME.VALOREXTRA, 0) ,0)) VL_BRUTO,'+
         'SUM(if(ME.SITUACAO IN (2 $situacao_ligacao$), COALESCE(ME.DESCONTOEXTRA, 0),0)) VL_DESC_FIXO,'+
         'SUM(if(ME.SITUACAO IN (2 $situacao_ligacao$), COALESCE(ME.VALORBRUTO, 0) + COALESCE(ME.VALOREXTRA, 0) + COALESCE(ME.VALORJUROS, 0) - COALESCE(ME.DESCONTOEXTRA, 0) ,0)) VL_LIQUIDO,'+
         'SUM(if(ME.SITUACAO IN (2 $situacao_ligacao$), COALESCE(ME.VALORBRUTO, 0) + COALESCE(ME.VALOREXTRA, 0) + COALESCE(ME.VALORJUROS, 0) ' +
         ...
         'SI.DS_SIGLA, '+
         'CAST(DATEDIFF(NOW(), CAST(MIN(if(ME.SITUACAO IN (2 $situacao_ligacao$),DATAVENCIMENTO, NULL)) AS DATETIME)) AS SIGNED) DIAS_ATRASO, ' +
         'CAST(MAX(if(ME.SITUACAO IN (2 $situacao_ligacao$),ME.ANOSEMESTRE, NULL) ) AS CHAR(5))  ULTIMOANOSEMESTRE,'+
         'CAST(MIN(if(ME.SITUACAO IN (2 $situacao_ligacao$),ME.ANOSEMESTRE, NULL)) AS CHAR(5)) PRIMEIROANOSEMESTRE,'+
         'CAST(MAX(if(ME.SITUACAO IN (2 $situacao_ligacao$),DATAVENCIMENTO, NULL)) AS DATETIME) ULTIMOVENCIMENTO,'+
         'CAST(MIN(if(ME.SITUACAO IN (2 $situacao_ligacao$),DATAVENCIMENTO, NULL)) AS DATETIME) PRIMEIROVENCIMENTO,'+
         'COUNT(if(ME.SITUACAO IN (2 $situacao_ligacao$), ME.CD_MENSALIDADE, NULL) ) QTDTITULOS, '+
         'T.CURSO ' +
      'FROM '+
   ...
   S_SQL_ORACLE =
      'SELECT '+
         ...
         'CAST(MAX(if(ME.SITUACAO IN (2 $situacao_ligacao$), ANOSEMESTRE, NULL) ) AS CHAR(5)) ULTIMOANOSEMESTRE,'+
         'CAST(MIN(if(ME.SITUACAO IN (2 $situacao_ligacao$),ANOSEMESTRE, NULL) ) AS CHAR(5)) PRIMEIROANOSEMESTRE,'+
         'MAX(if(ME.SITUACAO IN (2 $situacao_ligacao$), DATAVENCIMENTO, NULL) ) ULTIMOVENCIMENTO,'+
         'MIN(if(ME.SITUACAO IN (2 $situacao_ligacao$), DATAVENCIMENTO, NULL) ) PRIMEIROVENCIMENTO,'+
         'CAST(COUNT(DISTINCT if(ME.SITUACAO IN (2 $situacao_ligacao$), CD_MENSALIDADE, NULL)) AS NUMBER(19)) QTDTITULOS '+
      'FROM '+
         ' [$ordem_tabelas$] '+ { ESTA LOUCURA FOI REALIZADA PARA OTIMIZAR O SQL JA DE CARA QUANDO FOR FILTRADO ALGUM CAMPO DE MENSALIDADE }
            'LEFT JOIN PESSOAS RM ON (RM.CD_PESSOA = ME.CD_RESP) '+
            'LEFT JOIN PESSOAS R ON '+
   ...
   try
      ...
      qyCobranca.SQLList.Values[DM.db.Protocol] :=
         StringReplace(
            qyCobranca.SQLList.Values[DM.db.Protocol],
            '$situacao_ligacao$',
            getMensalidadesProtesto( ckbDataPagamento.Checked ),
            [rfReplaceAll, rfIgnoreCase]
         );
      ...
      begin
         ...
         qyCobranca.SQLList.Values[DM.db.Protocol] := StringReplace(
               qyCobranca.SQLList.Values[DM.db.Protocol],
               '$cobranca_em$',
               '',
               [rfReplaceAll, rfIgnoreCase]
      ...
      begin
         ...
         qyCobranca.ParamByName('DIASATRASO_DE').AsString := edDiasAtrasoDe.Text;
         qyCobranca.ParamByName('DIASATRASO_ATE').AsString := edDiasAtrasoAte.Text;
      end else begin
         qyCobranca.SQLList.Values[DM.db.Protocol] := StringReplace(
               qyCobranca.SQLList.Values[DM.db.Protocol],
               '$HAVING_DIAS_ATRASO$',
...
begin
   ...
   if pgCobranca.ActivePage = tsDadosDevedor then begin
      ...
      if ( qyCobranca.RecNo > 0 ) then begin
...
begin
   ...
   if qyCobrancaDT_RETORNO.AsString = '' then begin
     ...
     umdtRetorno.setDate(qyCobrancaDT_RETORNO.AsDateTime);
...
begin
   ...
   lbTotais.Caption := 'Total de registros: ' + IntToStr(qyCobranca.RecordCount);
end;
