...
begin
   ...
   if ((cbDataFiltro.ItemIndex > 0) and (cbDataFiltro.Enabled = True)) then
   begin
      qryContasPagar.ParamByName('DT_INICIAL').AsDate := umdtInicial.Date;
      qryContasPagar.ParamByName('DT_FINAL').AsDate := umdtFinal.Date;
   end;
   qryContasPagar.Open;
   
   AtualizaResumo(LSFiltros, umdtInicial.Date, umdtFinal.Date);
...
procedure Tfrm_Compromisso_Planilha.umdtInicialChangeDate(
  Sender: TObject);
begin
   Filtra();
end;
...
begin
   ...
   umdtInicial.Enabled := cbDataFiltro.ItemIndex <> 0;
   umdtFinal.Enabled := cbDataFiltro.ItemIndex <> 0;
...
begin
   ...
   if (Self.tituloEmPreparacao(qryContasPagarCD_TITULO.AsInteger)) then
   begin
      Mensagem('Não é possível excluir este título pois ele já foi preparado para pagamento. Caso necessite remover o mesmo, acesse Compromissos > Planilha de compromissos > Preparar e remova este título da preparação.', 'Informação', MB_OK + MB_ICONWARNING);
      Exit;
   end;
...
begin
  ...
  case cbDataFiltro.ItemIndex  of
    1 : begin // Provável Pagamento
          sFiltro := sFiltro + 'ToText({fin_contas_pagar.dt_provavel_pgto}, "yyyyMMdd") >= "'+FormatDateTime('yyyyMMdd', umdtInicial.Date)+'" AND ToText({fin_contas_pagar.dt_provavel_pgto}, "yyyyMMdd") <= "'+FormatDateTime('yyyyMMdd', umdtFinal.Date)+'" AND ';
          sFiltroShow := sFiltroShow + '"PROVÁVEL PAGTO = DE ' + FormatDateTime( 'dd/mm/yyyy', umdtInicial.Date ) + ' ATE ' + FormatDateTime( 'dd/mm/yyyy', umdtFinal.Date ) + '" + CHR(13) + ';
        end;
    2 : begin // Vencimento
          sFiltro := sFiltro + 'ToText({fin_contas_pagar.dt_vencimento}, "yyyyMMdd") >= "'+FormatDateTime('yyyyMMdd', umdtInicial.Date)+'" AND ToText({fin_contas_pagar.dt_vencimento}, "yyyyMMdd") <= "'+FormatDateTime('yyyyMMdd', umdtFinal.Date)+'" AND ';
    ...
    3 : begin // Competência
          sFiltro := sFiltro + 'ToText({fin_contas_pagar.dt_competencia}, "yyyyMMdd") >= "'+FormatDateTime('yyyyMMdd', umdtInicial.Date)+'" AND ToText({fin_contas_pagar.dt_competencia}, "yyyyMMdd") <= "'+FormatDateTime('yyyyMMdd', umdtFinal.Date)+'" AND ';
          sFiltroShow := sFiltroShow + '"COMPETÊNCIA = DE ' + FormatDateTime( 'dd/mm/yyyy', umdtInicial.Date ) + ' ATE ' + FormatDateTime( 'dd/mm/yyyy', umdtFinal.Date ) + '" + CHR(13) + ';
        end;
    4 : begin // Lançamento
          sFiltro := sFiltro + 'ToText({fin_contas_pagar.dt_lancamento}, "yyyyMMdd") >= "'+FormatDateTime('yyyyMMdd', umdtInicial.Date)+'" AND ToText({fin_contas_pagar.dt_lancamento}, "yyyyMMdd") <= "'+FormatDateTime('yyyyMMdd', umdtFinal.Date)+'" AND ';
          sFiltroShow := sFiltroShow + '"DATA DE LANÇAMENTO = DE ' + FormatDateTime( 'dd/mm/yyyy', umdtInicial.Date ) + ' ATE ' + FormatDateTime( 'dd/mm/yyyy', umdtFinal.Date ) + '" + CHR(13) + ';
        end;
    5 : begin // Emissão da Nota
          sFiltro := sFiltro + 'ToText({fin_contas_pagar.dt_emissao_nota}, "yyyyMMdd") >= "'+FormatDateTime('yyyyMMdd', umdtInicial.Date)+'" AND ToText({fin_contas_pagar.dt_emissao_nota}, "yyyyMMdd") <= "'+FormatDateTime('yyyyMMdd', umdtFinal.Date)+'" AND ';
    ...
    6 : begin // Data de Pagamento
          sFiltro := sFiltro + 'ToText({fin_contas_pagar.dt_pagamento}, "yyyyMMdd") >= "'+FormatDateTime('yyyyMMdd', umdtInicial.Date)+'" AND ToText({fin_contas_pagar.dt_pagamento}, "yyyyMMdd") <= "'+FormatDateTime('yyyyMMdd', umdtFinal.Date)+'" AND ';
          sFiltroShow := sFiltroShow + '"DATA DE PAGAMENTO = DE ' + FormatDateTime( 'dd/mm/yyyy', umdtInicial.Date ) + ' ATE ' + FormatDateTime( 'dd/mm/yyyy', umdtFinal.Date ) + '" + CHR(13) + ';
        end;
