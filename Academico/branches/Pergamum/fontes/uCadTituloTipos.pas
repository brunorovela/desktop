...
type
  Tfrm_CadTituloTipos = class(TForm)
    ...
    procedure meDataPadraoKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    procedure meDataPadrao2KeyPress(Sender: TObject; var Key: Char);
...
procedure Tfrm_CadTituloTipos.grdTitleClick(Column: TColumn);
var
   n: Integer;
begin
...
procedure Tfrm_CadTituloTipos.meDataPadrao2KeyPress(Sender: TObject;
  var Key: Char);
begin
  if (key = #127) or (key = #08)  then begin
    ShowMessage('aaaa');
  end;
end;
...
procedure Tfrm_CadTituloTipos.FiltraTitulos;
const
   SQL_FILTRA_TITULOS =
      ' SELECT ' +
      ' * ' +
...
begin
   qyTituloTipos.Close();
   qyTituloTipos.SQL.Text := Format(SQL_FILTRA_TITULOS, [ordem]);
...
begin
   case cbFiltroTipoConta.ItemIndex of
      0: qyTituloTipos.Filtered := False;
      1:
         begin
            qyTituloTipos.Filter := ' ct_tipo_titulo = 1 ';
            qyTituloTipos.Filtered := True;
         end;
      2:
         begin
            qyTituloTipos.Filter := ' ct_tipo_titulo = 2 ';
            qyTituloTipos.Filtered := True;
         end;   
   end;
end;
