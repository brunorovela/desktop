...
type
  ...
  private
    ...
    function inserirRecibo(sn_estorno_africa: Boolean = false): Integer;
  ...
  public
    ...
    function imprimirRecibo(sn_estorno_africa: Boolean = false): Boolean;
...
function TFinReciboManager.imprimirRecibo(sn_estorno_africa: Boolean): Boolean;
var
  AQuery: TUMZQuery;
begin
  try
  if ((AMensalidade.FieldByName('cd_recibo').AsString = '') OR EhEstorno()) then begin
    ...
    inserirRecibo(sn_estorno_africa);
  end
  else
  begin
    //atualiza data
...
function TFinReciboManager.inserirRecibo(sn_estorno_africa: Boolean): Integer;
var
  AQuery: TUMZQuery;
  iRecibo: Integer;
begin
  ...
  AQuery.SQL.Text := 'INSERT INTO recibos '+
    '(cd_mensalidade, cd_aluno, nr_parcela, cd_turma, dt_vencimento, dt_impressao, dt_recibo, ds_extenso, nr_recibo, cd_pessoa, sn_estorno ) '+
    ' VALUES '                                                                                              +
    '(:cd_mensalidade, :cd_aluno, :nr_parcela, :cd_turma, :dt_vencimento, NOW(), NOW(), :ds_extenso, :nr_recibo, :cd_pessoa, :sn_estorno ) ';
  ...
  AQuery.ParamByName('sn_estorno').AsInteger := 0;

  if sn_estorno_africa then
  begin
     AQuery.ParamByName('sn_estorno').AsInteger := 1;
  end;
