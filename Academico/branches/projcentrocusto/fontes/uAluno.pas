...
uses
  ...
  VirtualTrees, uEditorContatosPessoa, UMAjuda;
...
uses
   ...
   uImpAlunos, uUsuario, uMatriculas, Math, uPessoas_adicional, uSelCep,
...
begin
   ...
   qyColigadasMatriculas.SQL.Text := 'SELECT DISTINCT t.cd_coligada as cd_coligada FROM '+
                                     '    matriculas m INNER JOIN turmas t ON (m.turma = t.codigo AND m.anosemestre = t.anosemestre) WHERE m.codigoaluno = :cd_pessoa';
...
var
   hasHierarquia : Boolean;
   permissaoAlterarSenhaLogin, permissaoIncluirSenhaLogin: Boolean;
begin
  inherited;
  ...
  End
      ...
      begin
         edLoginAluno.Enabled := permissaoAlterarSenhaLogin and edLoginAluno.Enabled;
         edSenhaAluno.Enabled := permissaoAlterarSenhaLogin and edSenhaAluno.Enabled;
         edRedgAluno.Enabled := permissaoAlterarSenhaLogin and edRedgAluno.Enabled;
         edLoginPai.Enabled := permissaoAlterarSenhaLogin and edLoginPai.Enabled;
         edSenhaPai.Enabled := permissaoAlterarSenhaLogin and edSenhaPai.Enabled;
         edRedgPai.Enabled := permissaoAlterarSenhaLogin and edRedgPai.Enabled;
         edLoginMae.Enabled := permissaoAlterarSenhaLogin and edLoginMae.Enabled;
         edSenhaMae.Enabled := permissaoAlterarSenhaLogin and edSenhaMae.Enabled;
         edRedgMae.Enabled := permissaoAlterarSenhaLogin and edRedgMae.Enabled;
         edLoginRespAcad.Enabled := permissaoAlterarSenhaLogin and edLoginRespAcad.Enabled;
         edSenhaRespAcad.Enabled := permissaoAlterarSenhaLogin and edSenhaRespAcad.Enabled;
         edRedgRespAcad.Enabled := permissaoAlterarSenhaLogin and  edRedgRespAcad.Enabled;
         edLoginRespFinan.Enabled := permissaoAlterarSenhaLogin and edLoginRespFinan.Enabled;
         edSenhaRespFinan.Enabled  := permissaoAlterarSenhaLogin and edSenhaRespFinan.Enabled;
         edRedgRespFinan.Enabled := permissaoAlterarSenhaLogin and edRedgRespFinan.Enabled;
      end;
   ...
   if (qyPaiResp.FieldByName('ds_senha').AsString = '') AND (edSenha.Text = '*****') then begin
    ...
    begin
       ...
       try
          ...
          if((qyPaiResp.FieldByName('ds_senha').AsString = MD5Print(MD5String(edSenha.Text))) and (qyPaiResp.FieldByName('ds_senha').AsString = MD5Print(MD5String(edRedgSenha.Text))))then
             qyUpdateProvisoria.ParamByName('provisoria').AsString := 'N'
          else
             qyUpdateProvisoria.ParamByName('provisoria').AsString := 'S';
...
//   with qyPessoas do begin
//     Close();
//     SQL.Text := 'UPDATE pessoas SET sn_senha_provisoria = :pSenhaProvisoria WHERE cd_pessoa = :cd_pessoa';
//     if(cbProvisoria.Checked) then
//     begin
//        ParamByName('pSenhaProvisoria').AsString := 'S';
//     end
//     else
//     begin
//        ParamByName('pSenhaProvisoria').AsString := 'N';
//     end;
//
//     ParamByName('cd_pessoa').AsInteger := qyPaiResp.FieldByName('cd_pessoa').AsInteger;
//     ExecSQL();
...
begin
   ...
   begin
      if not PrincipalForm.ProcuraForm(frmFinanceiro) then
...
begin
   ...
   if pgPessoa.ActivePage = tsComplementares then begin
      Self.alteraLogin(edLoginAluno,edSenhaAluno,edRedgAluno,cbProvAluno,loginAntigo,tblPessoa,tblPessoacd_pessoa.AsInteger);
      ...
      try
         ...
         begin
            ...
            qryResp.Open();
            Self.alteraLogin(edLoginPai,edSenhaPai,edRedgPai,cbProvPai,loginPaiAntigo,qryResp,tblPessoacd_pai.AsInteger);
            qryResp.Close();
         end;
         ...
         begin
            ...
            qryResp.Open();
            Self.alteraLogin(edLoginMae,edSenhaMae,edRedgMae,cbProvMae, loginMaeAntigo,qryResp,tblPessoacd_mae.AsInteger);
            qryResp.Close();
         end;
         ...
         begin
            ...
            qryResp.Open();
            Self.alteraLogin(edLoginRespAcad,edSenhaRespAcad,edRedgRespAcad, cbProvRespAcad,loginRespAcadAntigo,qryResp,tblPessoacd_resp_acad.AsInteger);
            qryResp.Close();
         end;
         ...
         begin
            ...
            qryResp.Open();
            Self.alteraLogin(edLoginRespFinan,edSenhaRespFinan,edRedgRespFinan,cbProvRespFinan,loginRespFinanAntigo,qryResp,tblPessoacd_resp_finan.AsInteger);
            qryResp.Close();
         end;
      finally
         FreeAndNil(qryResp);
...
begin
   ...
   { inicia permissao }
   iPermissao := 1001;
   msgIdade := 'Você está prestes a vincular uma pessoa menor de idade como resposável financeira pelo acadêmico selecionado. Tem certeza que deseja prosseguir?';
...
procedure Tfrm_Alunos.sbMaisInformacoesFinanceiroClick(Sender: TObject);
begin
  { Apresentar a tela de campos adicionais }
  Application.CreateForm(Tfrm_pessoas_adicional, frm_pessoas_adicional);
