...
uses
  ...
  RegExpr, ZSqlUpdate, VirtualTrees, uEditorContatosPessoa, UMAjuda;
...
uses
   uDM, General, Main, uUsuario, Math, uCadEmpresas, MaskUtils, Variants,
   ...
   uGeneral, uContatoPessoa, uContatoTipo;
...
begin
  ...
  if tblPessoadescPais.AsString = 'BRASIL' then
...
{**
   ...
   e logins.
*}
procedure Tfrm_Pessoas.alteraLogin(edLogin, edSenha, edRedgSenha: TEdit; sLoginAntigo: string; qyPaiResp: TUMZQuery);
var
   bLoginOk:Boolean;
begin
   bLoginOk := false;
...
begin
   ...
   begin
      ...
      qyExcluirVinculo.ParamByName('cd_pessoa_externa').AsInteger := qyVinculosExternocd_pessoa_externa.AsInteger;
...
begin
   ...
   begin
    ...
    if ( DM.variavel_parametro('email_obrigatorio') = 'S' ) AND
       (
...
begin
   ...
   if ( DM.tblEstados.FieldByName('cd_pais').AsString = '' ) then
   begin
...
begin
   ...
   qyInserirVinculo.ParamByName('cd_pessoa_externa').AsInteger := codigoPessoaExterna;
...
const
   ...
   SQL_CONSULTA_MENSALIDADES =
      ' SELECT COUNT(*) registros FROM mensalidades WHERE codigoaluno = :cd_pessoa ';
...
{Função criada para realizar a inserçãos do log de alterações de cadastro da pessoa}
procedure Tfrm_Pessoas.setLogAlteracoesCadastro;
var
   LICodigoPessoa : Integer;
   ds_Mensagem_Log: String;
begin
          
   ds_Mensagem_Log := 'Alterado os seguintes campos abaixo:';

   if (nm_Pessoa <> tblPessoa.FieldByName('nm_pessoa').AsString) then
   begin
      ds_Mensagem_Log :=
         ds_Mensagem_Log + #10+#13 + 'Alterado Nome: ' + nm_Pessoa + ' ==> ' + tblPessoanm_pessoa.AsString;
   end;

   if (nm_pessoa_oficial <> tblPessoa.FieldByName('nm_pessoa_oficial').AsString) then
   begin
      ds_Mensagem_Log :=
         ds_Mensagem_Log + #10+#13 + 'Alterado Nome Oficial: ' + nm_pessoa_oficial + ' ==> ' + tblPessoanm_pessoa_oficial.AsString;
   end;

   if (dt_Nascimento <> tblPessoadt_nascimento.AsString) then
   begin
      ds_Mensagem_Log :=
         ds_Mensagem_Log + #10+#13 + 'Alterado a Data de nascimento: ' + dt_Nascimento + ' ==> ' + tblPessoadt_nascimento.AsString;
   end;

   if (pais_Nasc <> tblPessoads_pais_nascimento.AsString) then
   begin
      ds_Mensagem_Log :=
         ds_Mensagem_Log + #10+#13 + 'Alterado o País de nascimento: ' + pais_Nasc + ' ==> ' + tblPessoads_pais_nascimento.AsString;
   end;

   if (estado_Nasc <> tblPessoads_estado_nascimento.AsString) then
   begin
      ds_Mensagem_Log :=
         ds_Mensagem_Log + #10+#13 + 'Alterado o Estado de nascimento: ' + estado_Nasc + ' ==> ' + tblPessoads_estado_nascimento.AsString;
   end;

   if (descMunicipio <> tblPessoadescMunicipio.AsString) then
   begin
      ds_Mensagem_Log :=
         ds_Mensagem_Log + #10+#13 + 'Alterado a Cidade de Nascimento: ' + descMunicipio + ' ==> ' + tblPessoadescMunicipio.AsString;
   end;

   if (ds_cpf <> tblPessoads_cpf.AsString) then
   begin
      ds_Mensagem_Log :=
         ds_Mensagem_Log + #10+#13 + 'Alterado o CPF: ' + ds_cpf + ' ==> ' + tblPessoads_cpf.AsString;
   end;

   if (ds_rg <> tblPessoads_identidade.AsString) then
   begin
      ds_Mensagem_Log :=
         ds_Mensagem_Log + #10+#13 + 'Alterado o RG: ' + ds_rg + ' ==> ' + tblPessoads_identidade.AsString;
   end;

   if (nm_pai <> tblPessoanm_pai.AsString) then
   begin
      ds_Mensagem_Log :=
         ds_Mensagem_Log + #10+#13 + 'Alterado o Nome do pai: ' + nm_pai + ' ==> ' + tblPessoanm_pai.AsString;
   end;

   if (nm_mae <> tblPessoanm_mae.AsString) then
   begin
      ds_Mensagem_Log :=
         ds_Mensagem_Log + #10+#13 + 'Alterado o Nome da mãe: ' + nm_mae + ' ==> ' + tblPessoanm_mae.AsString;
   end;

   if (ds_cep <> tblPessoads_cep.AsString) then
   begin
      ds_Mensagem_Log :=
         ds_Mensagem_Log + #10+#13 + 'Alterado o CEP: ' + ds_cep + ' ==> ' + tblPessoads_cep.AsString;
   end;

   if (descPais <> tblPessoadescPais.AsString) then
   begin
      ds_Mensagem_Log :=
         ds_Mensagem_Log + #10+#13 + 'Alterado o País: ' + descPais + ' ==> ' + tblPessoadescPais.AsString;
   end;

   if (descEstado <> tblPessoadescEstado.AsString) then
   begin
      ds_Mensagem_Log :=
         ds_Mensagem_Log + #10+#13 + 'Alterado o Estado: ' + descEstado + ' ==> ' + tblPessoadescEstado.AsString;
   end;

   if (descMunicipioAtual <> tblPessoadescMunicipioAtual.AsString) then
   begin
      ds_Mensagem_Log :=
         ds_Mensagem_Log + #10+#13 + 'Alterado a Cidade: ' + descMunicipioAtual + ' ==> ' + tblPessoadescMunicipioAtual.AsString;
   end;

   if (ds_bairro <> tblPessoads_bairro.AsString) then
   begin
      ds_Mensagem_Log :=
         ds_Mensagem_Log + #10+#13 + 'Alterado o Bairro: ' + ds_bairro + ' ==> ' + tblPessoads_bairro.AsString;
   end;

   if (descLogradouro <> tblPessoadesLogradouro.AsString) then
   begin
      ds_Mensagem_Log :=
         ds_Mensagem_Log + #10+#13 + 'Alterado o Tipo do logradouro: ' + descLogradouro + ' ==> ' + tblPessoadesLogradouro.AsString;
   end;

   if (ds_logradouro <> tblPessoads_logradouro.AsString) then
   begin
      ds_Mensagem_Log :=
         ds_Mensagem_Log + #10+#13 + 'Alterado o Logradouro: ' + ds_logradouro + ' ==> ' + tblPessoads_logradouro.AsString;
   end;

   if (ds_logradouro_nro <> tblPessoads_logradouro_nro.AsString) then
   begin
      ds_Mensagem_Log :=
         ds_Mensagem_Log + #10+#13 + 'Alterado o Número : ' + ds_logradouro_nro + ' ==> ' + tblPessoads_logradouro_nro.AsString;
   end;

   if (ds_complemento <> tblPessoads_complemento.AsString) then
   begin
      ds_Mensagem_Log :=
         ds_Mensagem_Log + #10+#13 + 'Alterado o Complemento: ' + ds_complemento + ' ==> ' + tblPessoads_complemento.AsString;
   end;


   if (nm_Pessoa = '') then
   begin
      DM.setLog(1001, 'Inclusao', tblPessoacd_pessoa.AsString, DM.UsuarioLogado.ColigadaLogada);
   end
   else if (ds_Mensagem_Log <> 'Alterado os seguintes campos abaixo:') then
   begin
      DM.setLog(1001, 'Alteracao', tblPessoacd_pessoa.AsString, 0, ds_Mensagem_Log);
   end;
...
begin
   ...
   if ((tblPessoa.FieldByName('ds_forma_conheceu').asString <> '')
      AND (cbFormaConhece.ItemIndex = 0) AND (tblPessoa.State <> dsInsert)) then
   begin

      cbFormaConhece.ItemIndex := (i + 1);
