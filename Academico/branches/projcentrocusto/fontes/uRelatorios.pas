...
begin
   ...
   begin
      S := Format('%d,%d,%d %d:%d:%d', [
         YearOf(txtDtInicial.Date),
         MonthOf(txtDtInicial.Date),
         DayOf(txtDtInicial.Date),
         HourOf(txtDtInicial.DateTime),
         MinuteOf(txtDtInicial.DateTime),
         SecondOf(txtDtInicial.DateTime)
      ]);
      SLParametros.Values['.dataInicio'] := S;

      S := Format('%d,%d,%d %d:%d:%d', [
         YearOf(txtDtFinal.Date),
         MonthOf(txtDtFinal.Date),
         DayOf(txtDtFinal.Date),
         HourOf(txtDtFinal.DateTime),
         MinuteOf(txtDtFinal.DateTime),
         SecondOf(txtDtFinal.DateTime)
      ]);
   ...
   while (Length(Trim(sParametros)) > 1) do begin
      ...
      if (Pos('.depto', pAtual) > 0) then begin
         if (txtDepartamento.Text <> '') then begin
            ...
            sFiltro := sFiltro + sOp + pAtual + ' = ' + txtDepartamento.Text;
            sOp := ' and ';
      ...
      for i := 0 to (slSQLExp.Count - 1) do begin
         ...
         sAux := slSQLExp.ValueFromIndex[i];
          ...
          if (Pos('[data_inicio]', sAux) > 0 ) and (ckDtFiltrar.Checked) then begin
             sAux := ReplaceStr(sAux, '[data_inicio]', ''''+ FormatDateTime('yyyy-mm-dd', txtDtInicial.Date) + '''');
          end;
          if (Pos('[data_fim]', sAux) > 0 ) and (ckDtFiltrar.Checked) then begin
             sAux := ReplaceStr(sAux, '[data_fim]', ''''+ FormatDateTime('yyyy-mm-dd', txtDtFinal.Date) + '''');
          end;
