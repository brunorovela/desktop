...
uses
  ...
  ComCtrls, Buttons, ImgList, ToolWin;
...
type
  TfrmComprasControleEstoque = class(TForm)
    ...
    qyEstoqueCD_MENSALIDADE: TIntegerField;
...
begin
   ...
   if not ( qyEstoqueCD_MENSALIDADE.IsNull ) AND
      ( ClasseComprasEntradaSaida.SituacaoMensalidade( qyEstoqueCD_MENSALIDADE.AsInteger ) <> msPendente ) then
   begin
      mensalidade := ClasseMensalidade.GetMensalidade(qyEstoqueCD_MENSALIDADE.AsInteger);
...
begin
   ...
   qyEstoque.ParamByName('DT_ENTRADA_INICIO').AsDateTime := dtpDataInicial.DateTime;
   qyEstoque.ParamByName('DT_ENTRADA_FIM').AsDateTime := dtpDataFinal.DateTime;
   ...
   if rbPesquisaPorProduto.Checked then
   begin                                                     
      lbMsgTopo.Caption := 'Saldo inicial em ' + FormatDateTime('dd/mm/yyyy', dtpDataInicial.DateTime) + ': ' +
                           IntToStr(GetQtdEstoqueData( edCodigo.Text, dtpDataInicial.DateTime, 1 ));
      lbMsgRodape.Caption := 'Saldo final em ' + FormatDateTime('dd/mm/yyyy', dtpDataFinal.DateTime) + ': ' +
                             IntToStr(GetQtdEstoqueData( edCodigo.Text, dtpDataFinal.DateTime ));
   end else if rbPesquisaPorPessoa.Checked then
   begin
      lbMsgTopo.Caption := '';
      lbMsgRodape.Caption := 'Quantidade total de Produtos: ' +
                             IntToStr(GetTotalProdutosPessoa( StrToInt(edCodigo.Text), dtpDataInicial.DateTime, dtpDataFinal.DateTime ));
   end;
end;
