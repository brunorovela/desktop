...
uses
  ...
  Db, ZConnection, ComCtrls, StdCtrls, ExtCtrls, Buttons, General, ZAbstractRODataset, ZAbstractDataset, ZDataset, UZDataset, Grids,
...
type
  Tfrm_Retorno_Banco = class(TForm)
    ...
    tblItenssn_baixado: TSmallintField;
    ...
    tblItenscd_mensalidade: TLargeintField;
...
begin
   ...
   begin
      ...
      qRetorno.ParamByName('CODIGORETORNO').AsInteger := tblRetornoCodigo.AsInteger + 1;
      qRetorno.ParamByName('OCORRENCIA').AsString := 'BAIXA PENDENTE';
      qRetorno.ParamByName('CD_OCORRENCIA').AsString := strOcorrencia;
      qRetorno.ParamByName('VENCIMENTO').AsDateTime := DM.tblMensalidadesDataVencimento.AsDateTime;
      qRetorno.ParamByName('VALORTITULO').AsFloat := cValorPendente;
      qRetorno.ParamByName('NOSSONUMERO').AsString := DM.tblMensalidadesNossoNumero.AsString;
      qRetorno.ParamByName('VALORPAGO').AsFloat := cValorPendente;
      ...
      qRetorno.ParamByName('PARCELA').AsString := '';
      qRetorno.ParamByName('SEQUENCIANOARQUIVO').AsInteger := StrToInt(sColSequencia);
      qRetorno.ParamByName('DATAPAGAMENTO').AsDateTime := StrToDate(pega_data_pgtO());
      qRetorno.ParamByName('DT_CREDITO').AsDateTime := StrToDate(pega_data_credito());
      qRetorno.ExecSQL;
      FreeAndNil(qRetorno);
   end;
...
begin
   ...
   sColOrdem := 'i.sequencianoarquivo';
end;
...
begin
  ...
  qAcao.SQL.Text :=
    ' INSERT INTO itensretorno (codigoretorno, ocorrencia, vencimento, valortitulo, nossonumero, valorpago, valoracrescimo,' +
    ' codigoaluno, parcela, cd_mensalidade, cd_ocorrencia, cd_motivo, sequencianoarquivo, datapagamento, dt_credito, ' +
    ' vl_tarifa, sn_baixado) VALUES ' +
    ' (:codigoretorno, :ocorrencia, :vencimento, :valortitulo, :nossonumero, :valorpago, :valoracrescimo,' +
    ' :codigoaluno, :parcela, :cd_mensalidade, :cd_ocorrencia, :cd_motivo, :sequencianoarquivo, :datapagamento, :dt_credito, ' +
    ' :vl_tarifa, :sn_baixado) ';   

  qAcao.ParamByName('codigoretorno').AsInteger := lngCodigo;

  qAcao.ParamByName('ocorrencia').AsString := ocorrencia;

  qAcao.ParamByName('valortitulo').AsFloat := ValTitulo;

  if strNossoNumero = '' then begin
     qAcao.ParamByName('nossonumero').AsString := sColCPF + ';' + sColVencimento;
  end else begin
     qAcao.ParamByName('nossonumero').AsString := strNossoNumero;
  end;

  if tblBancovl_acresc_inicio.AsInteger > 0 then begin
     qAcao.ParamByName('valoracrescimo').AsFloat := ValAcrescimo
  end else begin
     qAcao.ParamByName('valoracrescimo').AsFloat := 0;
  end;

  if (tblBancotarifa_tam.AsInteger > 0) AND (bTarifaPorOcorrencia) AND (DM.qAux2.FieldByName('sn_baixar').AsString = 'T')
  then begin
       qAcao.ParamByName('valorpago').AsFloat := 0;
  end else begin

         qAcao.ParamByName('valorpago').AsFloat := ValPago;
  ...
  if achou then begin
     ...
     qAcao.ParamByName('codigoaluno').AsString    := DM.tblMensalidadesCodigoAluno.AsString;
     qAcao.ParamByName('parcela').AsString        := Dm.tblMensalidadesParcela.AsString;
     qAcao.ParamByName('cd_mensalidade').AsString := DM.tblMensalidadescd_mensalidade.AsString;
     qAcao.ParamByName('vencimento').AsDateTime     := DM.tblMensalidadesDataVencimento.AsDateTime;

  End else begin

     qAcao.ParamByName('codigoaluno').AsString    := '';
     qAcao.ParamByName('parcela').AsString        := '';
     qAcao.ParamByName('cd_mensalidade').AsString := '';
     qAcao.ParamByName('vencimento').AsString     := '';

  end;  

  qAcao.ParamByName('cd_ocorrencia').AsString := cd_ocorrencia;
  qAcao.ParamByName('cd_motivo').AsString := strMotivo;

  qAcao.ParamByName('sequencianoarquivo').AsInteger := StrToInt( sColSequencia );

  qAcao.ParamByName('datapagamento').AsDateTime :=  StrToDate(pega_data_pgto());

  qAcao.ParamByName('dt_credito').AsDateTime :=  StrToDate(pega_data_credito());

  if (tblBancotarifa_tam.AsInteger > 0) then begin

     qAcao.ParamByName('vl_tarifa').AsFloat := Tarifa;

  end
  else begin
     qAcao.ParamByName('vl_tarifa').AsFloat := 0;
  end;

  qAcao.ParamByName('sn_baixado').AsInteger := Baixado;
...
begin
    ...
    if tcItens.TabIndex = 0 then begin

      tblItens.Close();

      tblItens.SQL.Text := ' select i.*, c.sn_baixar, m.turma, m.ds_historico, m.valorbruto, m.valordesconto, m.descontoextra, m.valorextra, p.nm_pessoa, p.ds_cpf, b.ds_motivo, resp.nm_pessoa AS responsavel ' +
       '  from itensretorno i INNER JOIN retorno r ON (i.codigoretorno = r.codigo)  ' +
       '  LEFT JOIN mensalidades m ON (i.cd_mensalidade = m.cd_mensalidade AND i.codigoaluno = m.codigoaluno) ' +
       '  LEFT JOIN pessoas p ON (i.codigoaluno = p.cd_pessoa) ' +
       '  LEFT JOIN pessoas resp ON (m.cd_resp = resp.cd_pessoa) '+
       '  LEFT JOIN banco_codigo_retorno c ON (c.cd_retorno = i.cd_ocorrencia and c.cd_banco = r.cd_banco AND c.cd_origem = 1) ' +
       '  LEFT JOIN banco_retorno_motivos b ON (b.cd_banco = r.cd_banco AND b.cd_motivo = i.cd_motivo AND c.cd_grupo_motivos = b.cd_grupo_motivos) ' +
       '  WHERE codigoretorno = :codigoretorno ' +
       '  ORDER BY ' + sColOrdem;
      ...
      tblItens.SQL.Text := ' select i.*, c.sn_baixar, m.turma, m.ds_historico, m.valorbruto, m.valordesconto, m.descontoextra, m.valorextra, p.nm_pessoa, p.ds_cpf, b.ds_motivo, resp.nm_pessoa AS responsavel ' +
       ...
       '  LEFT JOIN banco_codigo_retorno c ON (c.cd_retorno = i.cd_ocorrencia and c.cd_banco = r.cd_banco AND c.cd_origem = 1) ' +
       '  LEFT JOIN banco_retorno_motivos b ON (b.cd_banco = r.cd_banco AND b.cd_motivo = i.cd_motivo AND c.cd_grupo_motivos = b.cd_grupo_motivos) ' +
       '  WHERE codigoretorno = :codigoretorno AND ocorrencia = :ocorrencia ' +
       '  ORDER BY ' + sColOrdem;

      tblItens.ParamByName('CodigoRetorno').AsInteger := tblRetornoCodigo.AsInteger;
      tblItens.ParamByName('ocorrencia').AsString     := tcItens.Tabs.Strings[tcItens.TabIndex];
      tblItens.Open();
...
begin
   ...
   { Eliminar os itens de retorno da linha selecionada } 
   Dm.qAux1.Close;
   Dm.qAux1.SQL.Clear;
   Dm.qAux1.SQL.Add(' delete from itensretorno ');
   Dm.qAux1.SQL.Add(' where codigoretorno = :codigo ');
   Dm.qAux1.ParamByName('codigo').AsInteger := tblRetornoCodigo.AsInteger;
   Dm.qAux1.ExecSQL;

   { Apagar a linha de retorno selecionada }
   Dm.qAux1.Close;
   Dm.qAux1.SQL.Clear;
   Dm.qAux1.SQL.Add(' delete from retorno ');
