...
begin
   ...
   if qryTesouraria.Params.FindParam('CD_ABERTURA_CAIXA') <> nil then
      ...
      qryTesouraria.ParamByName('DT_MOVIMENTO').AsString := DateToStr(dtpDataInicio.Date, DM.fsDateTimeFormatPadrao);
      qryTesouraria.ParamByName('DT_MOVIMENTO_FIM').AsString := DateToStr(dtpDataFinal.Date, DM.fsDateTimeFormatPadrao);
   end;
   qryTesouraria.Open;
...
begin
  ...
  DecodeDate(dtpDataInicio.Date, ano, mes, dia);
  DecodeDate(dtpDataFinal.Date, anof, mesf, diaf);
  ...
  if (anof <> ano) OR (mesf <> mes) then begin
      ...
      dtpDataFinal.Date := DataFim;
  end else begin
      if dtpDataInicio.Date > dtpDataFinal.Date then begin
         dtpDataFinal.Date := dtpDataInicio.Date;
      end;
...
begin
   ...
   begin
      ...
      qrySelectSaldo.ParamByName('DT_BASE').AsDate := dtpDataInicio.Date;
      qrySelectSaldo.Open;
      ...
      qrySelectSaldoComp.ParamByName('DT_BASE').AsDate := dtpDataInicio.Date;
...
//   qryTesouraria.OnCalcFields := qryTesourariaCalcFields;
   ...
   if qyTotais.Params.FindParam('CD_ABERTURA_CAIXA') <> nil then
      ...
      qyTotais.ParamByName('DT_MOVIMENTO').AsDate := dtpDataInicio.Date;
      qyTotais.ParamByName('DT_MOVIMENTO_FIM').AsDate := dtpDataFinal.Date;
   end;
   ...
   for n := 0 to grd.SelectedRows.Count - 1 do begin
      ...
      try
         ...
         if TSpeedButton(Sender).Tag = 1 then begin
            ...
            if n <= 0 then
               frmTesourariaCompensar.dtpCompensar.DateTime := DM.DataAtual;

            if frmTesourariaCompensar.ShowModal = mrOk then begin

               if frmTesourariaCompensar.dtpCompensar.DateTime <= DataAtualizaSaldo then
                  DataAtualizaSaldo := frmTesourariaCompensar.dtpCompensar.DateTime;

               qyCompensar.ParamByName('DtCompensa').AsDate := frmTesourariaCompensar.dtpCompensar.Date;
...
begin
   ...
   frmTesourariaEstorno.lblDescricao.Caption  := frmTesourariaEstorno.qryEstornods_estorno.AsString;
   frmTesourariaEstorno.nmCaixa.Caption       := frmTesourariaEstorno.qryEstornods_caixa.AsString;
