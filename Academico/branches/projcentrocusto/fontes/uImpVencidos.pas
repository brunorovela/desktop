...
type
  TfrmImpVencidos = class(TForm)
    ...
    procedure datDeKeyPress(Sender: TObject; var Key: Char);
...
procedure TfrmImpVencidos.datDeKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
  begin
...
begin
  ...
  if flgAND then SFormula := SFormula + ' AND ';
  SFormula := SFormula + '{Mensalidades.DataVencimento} <= ' + 'Date(' + FormatDateTime('yyyy,mm,dd', datAte.Date) + ')' ;
  flgAND := True;
