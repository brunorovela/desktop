...
uses
  ...
  DB, UZDataset, Buttons, Contnrs, uMensalidadesBaixarParcelaClass;
...
begin
  ...
  begin
     ...
     if not frmRepBloquetos.PrepararBloquetosParaImpressao(sFiltroAux, bAgrupado, ListaContas[cbContas.ItemIndex]) then begin
        Exit;
     end;
...
//     DM.FazAcaoRemessa(arImpBoleto, frmRepBloquetos.tblBloquetos.FieldByName('cd_mensalidade').AsInteger, true);
  ...
  begin
     ...
     end else if (Format('%d', [FUMRecMensalidade.CodigoRecibo]) <> '') AND (Dm.variavel_parametro('fin_recibo_estorno') <> '') then begin
        // Se tem recibo, a parcela está aberta e tem recibo de estorno

        infoRelatorio := PrincipalForm.GetInfoRpt('fin_recibo_estorno');
        sNomeRelatorio := infoRelatorio.Campo['nm_arquivo']+'.rpt';
  ...
  begin
   ...
   if (radFiscal.Checked) AND (radFiscal.tag = 1) then
   begin
...
begin
   ...
   if (DM.variavel_parametro('financeiro_qtd_autenticacao_baixa') = '-1') then begin
      radSelecionado2.Caption := '&Selecionado';
   ...
   if not (FUMRecMensalidade.CodigoSituacao in [2, 10]) then begin
      // Mensalidade está baixada - habilitar impressão do recibo
   ...
   radAgrupado.Enabled := not(reciboFiscalAgrupado);
   ...
   begin
      radFiscal.Caption := 'Fiscal agrupado';
   end;

   if frmMensalidades <> nil then begin
   ...
   end else if frmCobranca <> nil then begin
