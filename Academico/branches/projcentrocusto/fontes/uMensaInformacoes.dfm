object frmMensaInformacoes: TfrmMensaInformacoes
  ...
  object qyMovimento: TUMZQuery
    ...
    SQL.Strings = (
      'SELECT'
      '   a.ds_acao, a.cd_tipo_acao , m.*,  p.nm_pessoa USUARIO'
      'FROM'
      '  fin_mov_cr m INNER JOIN fin_acoes_movimento a'
      '     ON (m.cd_acao = a.cd_acao)'
      '  LEFT JOIN pessoas p ON (p.cd_pessoa = m.cd_usuario)'
      'WHERE'
