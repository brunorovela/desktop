...
type
  Tfrm_Professores = class(Tfrm_Pessoas)
    ...
    qryProfessoresUnidadescd_professor: TLargeintField;
    ...
    qyDocumentosCurso: TStringField;
    ...
    qyDocumentossn_obrigatorio: TStringField;
    ...
    qyDocumentosnr_grupo: TIntegerField;
    ...
    procedure tblProfessorBeforePost(DataSet: TDataSet);
    ...
    procedure tblProfTitulos2NewRecord(DataSet: TDataSet);
...
begin
  ...
  Begin
      ...
      tblSituacoes.Open();
  End
  Else if pgPessoa.ActivePageIndex = 6 Then
  Begin
      if not DM.UsuarioLogado.TemPermissao( iPermissao, npAcesso, True ) then
      begin
         toolDocumentos.Visible  := False;
...
procedure Tfrm_Professores.dsPessoaDataChange(Sender: TObject; Field: TField);
begin
   inherited;
...
procedure Tfrm_Professores.tblProfessorBeforePost(DataSet: TDataSet);
begin
  inherited;

  // SE a opção sn_professor foi desativada retirar do grupo de pessoas
  ...
  begin
     ...
     begin
       ...
       Dm.qAux1.SQL.Text :=
         ...
         '    nu_grupos gu '+
         '                 '+
         '    INNER JOIN nu_grupos_pessoas as gp ON (gu.cd_grupo = gp.cd_grupo) ' +
         ' WHERE ' +
         '    gu.ds_papel IN ("PROFESSOR", "COORDENADOR") AND ' +
         '    gp.cd_pessoa = ' + tblProfessorcd_pessoa.AsString;
         Dm.qAux1.ExecSQL();
     end
     else
     begin
  ...
  // SE a opção sn_funcionario foi desativada retirar do grupo de pessoas
  // FUNCIONARIO e ADMIN
  if tblProfessorsn_funcionario.AsString <> 'S' then begin
     ...
     begin
        ...
        ' DELETE gp.* FROM ' +
        '    nu_grupos  gu ' +
        '    INNER JOIN nu_grupos_pessoas  gp ON (gu.cd_grupo = gp.cd_grupo) ' +
        ' WHERE ' +
        '    gu.ds_papel IN ("ADMIN", "FUNCIONARIO") AND ' +
        '    gp.cd_pessoa = ' + tblProfessorcd_pessoa.AsString;
     	Dm.qAux1.ExecSQL();
...
procedure Tfrm_Professores.tblProfTitulos2NewRecord(DataSet: TDataSet);
begin
  { Faz a auto-numeração no primeiro campo da tabela }
...
begin
   ...
   //Diferença de Ano
   diferencaDeAnos := YearsBetween(tblExperienciadt_inicial_periodo.AsDateTime,tblExperienciadt_final_periodo.AsDateTime);
...
begin
  ...
  if tblProfessorsn_funcionario.AsString <> 'S' then begin
     ...
     qryFuncionarioUpdateColigada.ParamByName('CD_COLIGADA').Clear;
