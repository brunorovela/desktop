...
type
  TfrmFichaIndividual = class(TForm)
    ...
    tblFichacd_grupo: TLargeintField;
...
begin
   ...
   begin
      ...
      qyDisciplinasPI.SQL.Text :=
		   ...
		   '		pint_pt.cd_disciplina, uni_fi.nota9   ' +
		   '	FROM  ' +
      ...
      //nota9 = -1 considera disciplina dispensada da PI, mesmo que o aluno tenha ela.
      if not qyDisciplinasPI.Eof AND (qyDisciplinasPI.FieldByName('nota9').AsInteger <> -1) then
      begin
...
begin
  ...
  if tcFicha.TabIndex = 0 then begin
      ...
      tblFicha.SQL.Add( '       FichaIndividual.*, Disciplinas.Sigla, Disciplinas.descricao, ROUND( Provainstitucional.mediainstitucional, ava.nr_casas_decimais) As nota_pi, COALESCE(dg.cd_grupo, 0) cd_grupo ');
      ...
      tblFicha.SQL.Add( '     LEFT JOIN diario_grupos_pessoas dgp ON ( ');
      tblFicha.SQL.Add( '        dgp.cd_pessoa = fichaindividual.codigoaluno AND ' );
      tblFicha.SQL.Add( '        dgp.dt_saida IS NULL ' );
      tblFicha.SQL.Add( '     ) ');
      tblFicha.SQL.Add( '     LEFT JOIN diario_grupos dg ON ( ');
      tblFicha.SQL.Add( '        dg.nr_anosemestre = fichaindividual.anosemestre AND ' );
      tblFicha.SQL.Add( '        dg.cd_curso = fichaindividual.curso AND ' );
      tblFicha.SQL.Add( '        dg.cd_turma = fichaindividual.turma AND ' );
      tblFicha.SQL.Add( '        dg.cd_disciplina = fichaindividual.disciplina AND ' );
      tblFicha.SQL.Add( '        dg.cd_grupo = dgp.cd_grupo ' );
      tblFicha.SQL.Add( '     ) ');
...
begin
  ...
  if not Dm.IsTrue(Dm.tblFichaIndividualAproveitamento.AsString) then
  Begin

  DM.TotalFaltas( tblFichaCodigoAluno.AsInteger,
                         tblFichaTurma.AsString,
                         tblFichaAnoSemestre.AsInteger);

// Retirado provisoriamente para verificar uma solução para os clientes que calculam individualmente por aluno/disciplina
{
  DM.CalcularMediaFinal( tblFichaCodigoAluno.AsInteger,
                         tblFichaTurma.AsString,
                         tblFichaAnoSemestre.AsInteger);

  DM.SetaSituacaoFinal( tblFichaCodigoAluno.AsInteger,
                        tblFichaTurmaMatricula.AsString,
                        tblFichaAnoSemestre.AsInteger );

  DM.Gerar_Series_Conclusao(tblFichaCodigoAluno.AsInteger, tblFichaCurso.AsString, tblFichaAnoSemestre.AsInteger);
}
  End;
...
begin
   ...
   qyGetLog.SQL.Add('WHERE l.cd_tipo = 9 AND l.cd_chave = :cd_chave');
