...
type                  
  TfrmMensalidades = class(TForm)
    ...
    tblMensalidadesValorJurosCalc: TCurrencyField;
  ...
  public
    ...
    lstTurmas : TStringList;
    lstAnoSemestre : TStringList;
...
uses
   ...
   uBiblioMultaEmprestimo, uClassMovimento, uMensa_Outras_Baixa, uNegociacao,
...
begin
   ...
   if frmComprasRegistroSaidas = nil then
...
begin
  ...
  DM.MontarPlanilha(grd, 'planilha_mensalidades_campos');
  ...
  lstTurmas := TStringList.Create;
  lstAnoSemestre := TStringList.Create;

  lstTurmas.Clear;
  lstAnoSemestre.Clear;
...
begin
  ...
  begin
    ...
    if ( qryDDA.RecordCount > 0)  then
    begin
      tblMensalidadesDDA.AsString := 'SIM'
    end
    else
    begin
      tblMensalidadesDDA.AsString := 'NÃO'
    end;

    if (tblMensalidadesDDA.AsString = 'SIM') then
    begin
  ...
  case tblMensalidadesSituacao.AsInteger of
    0..1 : Begin
              if TemChequePendente(tblMensalidadescd_mensalidade.AsInteger) then
                  tblMensalidadesDescricaoSituacao.AsString := Dm.lst_situacao_fin_sigla[tblMensalidadesSituacao.AsInteger] + '(C)'
              else
                  tblMensalidadesDescricaoSituacao.AsString := Dm.lst_situacao_fin_sigla[tblMensalidadesSituacao.AsInteger];
           End;
    2 :    Begin
              if CompareDate(tblMensalidadesDataVencimento.AsDateTime, DataHoje) = -1 then
    ...
    if tblMensalidadesParcela.AsInteger <> 0 then begin
       ...
       // Parcela ZERO - Não cobrar multa
   	   tblMensalidadesValorTotalCalc.Value := RoundFloat(tblMensalidadesValorBruto.Value +
               tblMensalidadesValorExtra.Value -
               tblMensalidadesdesconto_calc.Value -
       ...
       tblMensalidadesValorDescontoCalc.Value := RoundFloat( tblMensalidadesdesconto_calc.Value , 2 );
    End;
    Exit;
...
begin
  ...
  if popMensalidades.Items[0].Checked then begin
     ...
     if cbTurma.ItemIndex > 0 then begin
         ...
         tblMensalidades.SQL.Add(' and (me.turma = :Turma ) ');
         tblMensalidades.ParamByName('Turma').AsString :=       lstTurmas[cbTurma.ItemIndex-1];
         strSubSelectTblMensalidades := strSubSelectTblMensalidades + ' and (me.turma = '+QuotedStr(lstTurmas[cbTurma.ItemIndex-1])+' )';

         if DM.UsaAnosemestre then begin
            tblMensalidades.SQL.Add(' and (me.anosemestre = :Anosemestre ) ');
            tblMensalidades.ParamByName('AnoSemestre').AsString := lstAnoSemestre[cbTurma.ItemIndex-1];
            strSubSelectTblMensalidades := strSubSelectTblMensalidades + ' and (me.anosemestre = '+QuotedStr(lstAnoSemestre[cbTurma.ItemIndex-1])+' ) ';
         end;
  ...
  if not tblMensalidades.Active then begin
     ...
     if Length( sOrdem ) > 0 then
        tblMensalidades.SQL.Add( ' ORDER BY ' + sOrdem + ', me.datavencimento ' )
     else
        tblMensalidades.SQL.Add( ' ORDER BY NomeAluno, me.datavencimento ' );
...
begin
   ...
   begin
      ...
      or (tblMensalidadessn_juridico.AsInteger = 1) then
      begin
  ...
  if frm_mensa_outras_baixas.ShowModal = mrOk then begin
     ...
     begin
        ...
        begin
           ...
           Mensagem( 'Reposição efetuada!', Application.Title, MB_OK + MB_ICONINFORMATION, Handle );
        end;        
     end;     
...
begin
   ...
   Begin
      ...
      qyTemp1.SQL.Text := Format(
         ' SELECT DISTINCT turma, anosemestre FROM mensalidades ' +
         ' WHERE codigoaluno = ' + IntToStr(lngCodigoAluno) + ' AND cd_coligada IN ( %s ) ',
         [DM.UsuarioLogado.GetColigadasFilhaSelecionada()] );

      qyTemp1.SQL.Add('ORDER BY anosemestre desc, turma desc');
      qyTemp1.Open;
      ...
      Begin
         lstTurmas.Add(qyTemp1.FieldByName('turma').AsString);
         lstAnoSemestre.Add(qyTemp1.FieldByName('anosemestre').AsString);

         if Dm.UsaAnosemestre then
         begin
            cbTurma.Items.Add(qyTemp1.FieldByName('turma').AsString + ' - ' + Copy(qyTemp1.FieldByName('anosemestre').AsString,1,4) + '/' +Copy(qyTemp1.FieldByName('anosemestre').AsString,5,1) ) ;
         end else begin
            cbTurma.Items.Add(qyTemp1.FieldByName('turma').AsString ) ;
         end;
...
begin
  ...
  lstTurmas.Add(filtro_turma.cd_turma);
  lstAnoSemestre.Add(IntToStr(filtro_turma.nr_anosemestre));
...
begin
  ...
  Application.CreateForm(Tfrm_Mensa_Outras_Baixas, frm_Mensa_Outras_Baixas);
  frm_Mensa_Outras_Baixas.tipoBaixa := Desconto;
  frm_mensa_outras_baixas.iCodigoMensalidade := tblMensalidadescd_mensalidade.AsInteger;
...
begin
   ...
   begin
       ...
       qyTemp1.SQL.Text := Format(
         'SELECT DISTINCT anosemestre, turma FROM mensalidades WHERE cd_resp = ' + IntToStr(lngCodigoAluno) + ' AND cd_coligada IN ( %s ) ',
         [ DM.UsuarioLogado.GetColigadasFilhaSelecionada() ] );
       ...
       if Dm.UsaAnosemestre then begin 
           ...
           Begin
               ...
               cbTurma.Items.Add(Copy(qyTemp1.FieldByName('anosemestre').AsString,1,4) + '/' +Copy(qyTemp1.FieldByName('anosemestre').AsString,5,1) ) ;
               qyTemp1.Next;
           End;
...
procedure TfrmMensalidades.grdTitleClick(Column: TColumn);
var
   n : Integer;
begin
...
begin
  ...
  Begin
      ...
      if Mensagem('O cheque ja foi para depósito. Deseja colocá-lo novamente EM CAIXA?', 'Confirmação', MB_YESNO + MB_ICONQUESTION)
      = mrYes
      Then
      Begin
 ...
 if qyTemp2.Eof then begin
  ...
  while not qyTemp2.Eof do begin
      ...
      qyAltera.SQL.Text := ' UPDATE itensretorno SET sn_baixado = 0 WHERE cd_mensalidade = ' + tblMensalidadescd_mensalidade.AsString;
      qyAltera.ExecSQL();
...
//  DM.tblMensalidadesValorJuros.Value := ;
  ...
  if (ClasseComprasEntradaSaida.hasCompraFromMensalidade( tblMensalidadescd_mensalidade.AsInteger ) > 0) then
  begin
...
Var
  ...
  sMsg : String;
begin
  if not DM.UsuarioLogado.TemPermissao( 2013, npExcluir, True ) then Exit ;

  if (tblMensalidadesSituacao.AsInteger in [0,1]) then begin

     sMsg := 'Atenção:' + CHR(13) + CHR(13);
     sMsg := sMsg + 'Não é possível excluir uma parcela baixada.' +CHR(13); 
     sMsg := sMsg + 'Você deverá efetuar o ESTORNO antes de excluí-la';

     Mensagem( sMsg, Application.Title, MB_OK + MB_ICONWARNING );
  ...
  if tblMensalidadessn_nfe_gerada.AsInteger = 1 then begin

     sMsg := 'Atenção:' + CHR(13) + CHR(13);
     sMsg := sMsg + 'Não é possível excluir esta parcela, pois já foi emitida a NF.' +CHR(13);
  ...
  // Não permitir excluir parcelas de negociação, caso tenha bloqueio por competência
  if (tblMensalidadestipoparcela.AsInteger = 4) AND (DM.variavel_parametro('financeiro_bloquear_competencia') = 'S') then begin
     sMsg := 'Atenção:' + CHR(13) + CHR(13);
     sMsg := sMsg + 'Não é possível excluir uma parcela de negociação.' +CHR(13);
  ...
  if tblMensalidadesbloqueto.AsString = 'S' then begin

     sMsg := 'Confirmação:' + CHR(13) + CHR(13);
     sMsg := sMsg + 'A parcela que você está tentando excluir já possui boleto impresso.' +CHR(13) + CHR(13);
     sMsg := sMsg + 'Deseja realmente confirmar a exclusão?';

     if Mensagem( sMsg, Application.Title, MB_YESNO + MB_ICONQUESTION + MB_DEFBUTTON2 ) = ID_NO
     then
        Exit;
  end;

  if (tblMensalidadescd_mensalidade.AsInteger = tblMensalidadescd_mensalidade_origem.AsInteger) or
      tblMensalidadescd_mensalidade_origem.IsNull
  then
  begin
     // Está apagando uma mensalidade origem

     sMsg := 'Confirmação:' + CHR(13) + CHR(13);
     sMsg := sMsg + 'Deseja excluir definitivamente o título selecionado?';

     if Mensagem( sMsg, Application.Title, MB_YESNO + MB_ICONQUESTION + MB_DEFBUTTON2 ) = ID_NO then Exit;
     ...
     with qryDelete do begin
         ...
         DM.setLog(2013, 'Exclusao', chaves, Dm.getColigadaByTurma(tblMensalidadesTurma.AsString, tblMensalidadesAnoSemestre.AsInteger), 'Parcela Excluída (NN = ' + tblMensalidadesNossoNumero.AsString +  ')');
     ...
     Filtra;
          ...
          if tblMensalidadesValorBruto.AsCurrency <= 0 then begin
              ...
              sMsg := 'Confirmação:' + CHR(13) + CHR(13);
              sMsg := sMsg + 'Você está tentando excluir uma parcela PARCIAL. ' + CHR(13);
              sMsg := sMsg + 'Aconselhamos o CANCELAMENTO desta parcela no lugar da exclusão.' + CHR(13) + CHR(13);
              sMsg := sMsg + 'Deseja excluir mesmo assim? ';
              ...
              sMsg := 'Atenção:' + CHR(13) + CHR(13);
              sMsg := sMsg + 'Não é possível excluir um título parcial com valor bruto ' + CHR(13);
              sMsg := sMsg + 'Utilize a opção de CANCELAMENTO no lugar da exclusão.';
...
begin
   ...
   begin
      TGeneral.AlterarCorCelula(grd, Rect, DataCol, Column, State, TGeneral.ColorFromHTML(tblMensalidadesds_cor_background.AsString));
      TGeneral.AlterarCorFonteCelula(grd, Rect, DataCol, Column, State, TGeneral.ColorFromHTML(tblMensalidadesds_cor_fonte.AsString));
   end;
end;
...
begin
   ...
   DM.setLog(2001, 'Alteração', tblMensalidadescd_mensalidade.AsString,
      tblMensalidadescd_coligada.AsInteger, ObsLog);
...
begin
   ...
   qyRoda.SQL.Text := 'SELECT codigoaluno, turma, anosemestre FROM mensalidades ' +
...
begin
   ...
   frmSelColunas.SelecionaColunas(grd, 'planilha_mensalidades_campos');
end;
...
begin
   ...
   if frmNegociacao.tblMensalidades.RecordCount <> 0 then // Verifica se possui parcelas em aberto
   begin
     frmNegociacao.ShowModal;
     Filtra();
   end else if frmNegociacao.qyNegociaAnterior.RecordCount <> 0 then // Verifica se  pessoa possui alguma negociação já realizada no passado
   begin
   ...
   begin
     Mensagem('Não há títulos em aberto para esta pessoa.', 'Atenção', MB_OK+MB_ICONWARNING, Handle )
   end;
end;
