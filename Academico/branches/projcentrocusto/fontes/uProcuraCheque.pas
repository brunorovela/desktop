...
type
  TfrmProcuraCheque = class(TForm)
    ...
    Label1: TLabel;
    edIValor: TEdit;
    dtICompensacao: TDateTimePicker;
    Label2: TLabel;
    ...
    dtEmissaoAte: TDateTimePicker;
    lblEmissaoAte: TLabel;
    dtEmissaoDe: TDateTimePicker;
...
uses uDM, uFSelecionarPessoa, General, uClassMovimento;
...
begin
   ...
   if pcBuscar.ActivePage = tsBuscarAvancado then begin
      ...
      if (ckbEmissao.Checked) then begin
         if (dtEmissaoDe.Date > dtEmissaoAte.Date) then begin
            Mensagem('A Data Inicial deve ser menor que a data Final!!');
            dtEmissaoDe.SetFocus();
            CanClose := False;
            Exit;
         end;
      end;   
...
begin
   dtEmissaoAte.Enabled := ckbEmissao.Checked;
   dtEmissaoDe.Enabled := ckbEmissao.Checked;
