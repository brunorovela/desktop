...
begin
   ...
   qyAux.SQL.Add(' WHERE ct_tipo_titulo = 1 AND cd_coligada_matriz = :cd_coligada AND cd_padrao = 1 ');
...
begin
   ...
   frmPlanosCopiar.dtpAPartirDe.DateTime := qyPlanosDT_APARTIR.AsDateTime;
   ...
   cd_plano_novo := ClassePlanosPagamento.CopiaPlanoPagamento(
      ...
      frmPlanosCopiar.nr_anosemestre, frmPlanosCopiar.dtpAPartirDe.DateTime,
   ...
   DM.setLog( 2006, 'Inclusao',
           ...
           'A Partir: ' + FormatDateTime('dd/mm/yyyy', frmPlanosCopiar.dtpAPartirDe.DateTime) );
...
begin
   ...
   qyListaTiposTitulos.SQL.Add('SELECT  cd_tipo_titulo, ds_tipo_titulo FROM fin_config_tipos_titulo WHERE cd_coligada_matriz = :cd_coligada ORDER BY ds_tipo_titulo');
...
begin
   ...
   begin
      ...
      begin
         SQL.Text := 'SELECT';
         SQL.Add(' count(*) AS qtd');
         SQL.Add(' FROM');
         SQL.Add(' mensalidades');
         SQL.Add(' WHERE');
         SQL.Add(' cd_item_plano IN (:condicao)');

         qyPlanosItens.First;

         condicao := qyPlanosItensCD_PLANO_ITEM.AsString;

         qyPlanosItens.Next;

         while not qyPlanosItens.Eof do
         begin
            condicao := condicao +', '+ qyPlanosItensCD_PLANO_ITEM.AsString;
            qyPlanosItens.Next;
         end;

         ParamByName('condicao').AsString := condicao;
...
begin
   ...
   if qyPlanos.State = dsInsert then begin
