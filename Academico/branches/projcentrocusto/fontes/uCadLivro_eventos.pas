unit uCadLivro_eventos;

interface

uses
   Windows, Messages, SysUtils, Variants, Classes, Graphics, Controls, Forms,
   Dialogs, ExtCtrls, ComCtrls, ImgList, ToolWin, Spin, StdCtrls, DB, ZConnection,
   General, ZAbstractRODataset, ZAbstractDataset, ZDataset, UZDataset, uDM;

type
   TfCadLivroEventos = class(TForm)
...
var
   fCadLivroEventos: TfCadLivroEventos;
...
class function TfCadLivroEventos.AdicionaLivro(AOwner: TComponent): string;
begin
   if not Assigned(fCadLivroEventos) then
      fCadLivroEventos := TfCadLivroEventos.Create(AOwner);
   try
      case fCadLivroEventos.ShowModal of
         mrOk: Result := fCadLivroEventos.ledNome.Text;
         mrCancel: Result := '';
      end;
   finally
      FreeAndNil(fCadLivroEventos);
   end;
end;

procedure TfCadLivroEventos.Salva(Sender: TObject);
const
...
constructor TfCadLivroEventos.Create(AOwner: TComponent);
begin
   inherited;
end;

procedure TfCadLivroEventos.FechaFormulario(Sender: TObject; var Action: TCloseAction);
begin
   Action := caFree;
end;

procedure TfCadLivroEventos.ProcessaTeclasAtalho(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
   case Key of
...
procedure TfCadLivroEventos.Cancela(Sender: TObject);
const
