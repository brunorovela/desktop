...
begin
  ...
  lstSQLExp.Add('saldo=GET_SALDO(' + IntToStr(CaixaDepto) + ', ' + IntToStr(Coligada) + ', ''' + FormatDateTime( 'yyyy-mm-dd', datDe.Date) + ''', NULL)');
  lstSQLExp.Add('saldo_compensado=GET_SALDO_COMP(' + IntToStr(CaixaDepto) + ', ' + IntToStr(Coligada) + ', ''' + FormatDateTime( 'yyyy-mm-dd', datDe.Date) + ''', NULL)');
  ...
  if TipoCaixa = 3 then begin
     ...
     sShowFiltro := sShowFiltro + FormatDateTime( 'dd/mm/yyyy', datDe.Date);
     sShowFiltro := sShowFiltro + ' Até: ' + FormatDateTime( 'dd/mm/yyyy', datAte.Date );

     lstFormulas.Add('ds_filtro="' + sShowFiltro+'"');

     DecodeDate(datDe.Date, wAno, wMes, wDia);

     filtro := sData+' >= ' + 'Date(' + intToStr(wano) + ', ' + intToStr(wmes) + ', ' + intToStr(wDia) + ') ';

     DecodeDate(datAte.Date, wAno, wMes, wDia);
