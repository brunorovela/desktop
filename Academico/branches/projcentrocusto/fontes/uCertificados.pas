...
type
   ...
   TfrmCertificados = class(TForm)
    ...
    qCertificadosnr_cert_folha: TLargeintField;
    qCertificadosnr_cert_registro: TLargeintField;
...
const
   ...
   SQL_PROX_REGISTRO_CERT =
      'SELECT '+
         '(COALESCE(MAX(COALESCE(nr_cert_registro, 0)),0) + 1) AS nr_cert_registro '+
      'FROM '+
         'matriculas_curso '+
      'WHERE '+
         'cd_cert_livro = :cd_cert_livro'+
...
begin
   ...
   Self.FQyProxRegCert.SQL.Text := SQL_PROX_REGISTRO_CERT;
...
begin
   ...
   // Caso não utilize anosemestre, irá retornar "1"
   if not TUMNucleo.GetParametroUsaAnosemestre then Exit;
...
begin
   ...
   Self.FQyProxRegCert.ParamByName('nr_cert_folha').AsInteger := nrCodFolha;
   Self.FQyProxRegCert.Open;
   if Self.FQyProxRegCert.IsEmpty then
      Result := 1
   else
      Result := Self.FQyProxRegCert.FieldByName('nr_cert_registro').AsInteger;
end;
...
begin
   ...
   Self.FQyRegistraCert.ParamByName('nr_cert_registro').AsInteger := Self.getProximaRegistroCert(AICodLivro,AINumPagina);
