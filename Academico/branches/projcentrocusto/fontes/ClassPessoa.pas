...
const
   SQL_LOAD_PESSOA =
      ' SELECT nm_pessoa, cd_pai, cd_mae, cd_resp_acad, cd_resp_finan, im_pessoa, ds_login, ds_senha, ds_cnpj, ' +
      '        COALESCE( YEAR(NOW()) - YEAR(dt_nascimento) - ( CASE WHEN (MONTH(NOW()) - MONTH(dt_nascimento) <= 0) AND ( DAY(NOW()) - DAY(dt_nascimento) <=0 ) THEN 1 ELSE 0 END ), 0 ) AS idade ' +
      ' FROM pessoas ' +
