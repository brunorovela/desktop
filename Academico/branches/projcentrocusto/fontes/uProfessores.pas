...
uses
  ...
  UMComboBox, uColigada, uUMNucleo;
...
type
  ...
  TfrmProfessores = class(TForm)
    ...
    tblTurmasProfessorescreditosAcademicos: TFloatField;
...
Uses uDM, Main, uTableFields, uFSelecionarPessoa, ZDbcIntfs, uGeneral,
  ...
  uFSelecionarColigada, uFSelecionarSala, Types, uSala, uTurma, uTurmaProfessorAceite;
...
begin
   ...
   tblTurmasProfessores.SQL.Add(' SELECT tp.*, (SELECT gd.VL_VALOR FROM grades_disciplinas GD WHERE gd.CD_GRADE = t.cd_grade AND gd.CD_CURSO = t.curso AND gd.CD_DISCIPLINA = tp.disciplina AND gd.NR_SERIE = t.serie) as cargaHoraria, ');
   tblTurmasProfessores.SQL.Add(' (SELECT nr_aulas FROM grades_disciplinas GD WHERE gd.CD_GRADE = t.cd_grade AND gd.CD_CURSO = t.curso AND gd.CD_DISCIPLINA = tp.disciplina AND gd.NR_SERIE = t.serie) as creditosAcademicos ');
   ...
   tblTurmasProfessores.SQL.Add(' WHERE tp.professor = :cd_pessoa AND tp.anosemestre = :AnoSemestre AND t.cd_coligada in ('+DM.GetTodasColigadas+')');
...
var
   DataRect : TRect;
begin
   // Se estivermos na coluna ds_sala, vamos colocar os botões nela
   if (Column.FieldName = 'ds_sala') then
   begin
...
begin
   ...
   tblObsProfessords_obs_sistema.AsString := tblObsProfessords_obs_sistema.AsString +
	 'TURMA: ' + tblTurmasProfessoresTurma.AsString  + ' - DISCIPLINA: ' + tblTurmasProfessoresDisciplina.AsString +
	 ' - ANO/SEM: ' + tblTurmasProfessoresAnoSemestre.DisplayText  + CHR(13) + 'SAÍDA: ' + dateToStr(DataHoje);
...
begin
   ...
   begin
      ...
      if not unicoProfessorTurmaDisciplina() then
      begin
         // Existe outro(s) professor(es) para a turma/disciplina então permite ele remover essa turma/disciplina do professor
         if Mensagem('ATENÇÃO !'+chr(13)+'Caso queira apenas trocar o professor da turma nesta disciplina, não utilize esta opção, utilize o botão "Trocar Prof. da Turma".'+chr(13)+'Deseja realmente retirar esta turma deste professor ?',Application.Title,
         ...
         // Somente esse professor leciona para a turma/disciplina então pergunta se realmente deseja continuar e

     if Mensagem('ATENÇÃO !'+chr(13)+'Você está removendo o único professor desta turma, ao realizar esta ação você poderá perder todos os registros de diário de classe desta disciplina. '+chr(13)+'Deseja Prosseguir?".',Application.Title, MB_YESNO ) = ID_YES then
     begin
        excluirProfessorHorarios (
           tblTurmasProfessoresProfessor.AsInteger,
           tblTurmasProfessoresTurma.AsString,
