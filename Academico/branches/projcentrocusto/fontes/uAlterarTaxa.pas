...
uses
  ...
  UMEditMonetario, General, UMAjuda;
...
begin
   ...
   begin
      ...
      percTaxa := RoundFloat((ValorTaxaed * 100) / valorTotal, 2);
...
begin
   if (key in ['0'..'9']) or
      ((key = ',') and (Pos(',', edValorTaxaPerc.Text) = 0)) or
      (key = #8) then
   begin
      Exit;
   end;
...
begin
   if (key in ['0'..'9']) or
      ((key = ',') and (Pos(',', edValorTaxa.Text) = 0)) or
      (key = #8) then
   begin
      Exit;
   end;
