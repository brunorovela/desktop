...
begin
   ...
   try
      ...
      begin
         LSFiltro := LSFiltro + S_OP + S_FIL_CHEQUE;
      ...
      begin
         LSFiltro := LSFiltro + S_OP + S_FIL_DTEMI;
      ...
      begin
         LSFiltro := LSFiltro + S_OP + S_FIL_VALOR_ENTRE;
      ...
      begin
         LSFiltro := LSFiltro + S_OP + S_FIL_VALOR_MAIOR_QUE;
         LParams.CreateParam(ftUnknown, 'NR_VALOR_MENOR', ptUnknown).AsString := sFValorMenor;
      end
      else if sFValorMaior <> '' then
      begin
         LSFiltro := LSFiltro + S_OP + S_FIL_VALOR_MENOR_QUE;
         LParams.CreateParam(ftUnknown, 'NR_VALOR_MAIOR', ptUnknown).AsString := sFValorMaior;
      end;
      if sOrdem = '' then
         sOrdem := 'C.DT_COMPENSACAO, C.CD_SITUACAO';
      qryCheques.Close;
      qryCheques.SQLList.Values[DM.db.Protocol] := Format(FSSQLOriginal, [DM.UsuarioLogado.GetColigadasFilhaSelecionada(), LSFiltro, sOrdem]);
...
begin
   ...
   if ((sFDtInicio > 0) and (sFDtFim > 0)) then begin
      ...
      frmProcuraCheque.dtVencInicial.Date := sFDtInicio;
      frmProcuraCheque.dtVencFinal.Date := sFDtFim;
   end
   else begin
      frmProcuraCheque.ckbData.Checked := False;
      frmProcuraCheque.dtVencInicial.Date := Now();
      frmProcuraCheque.dtVencFinal.Date := Now();
   end;

   if ((sFDtEmissaoInicio > 0) and (sFDtEmissaoFim > 0)) then begin
      frmProcuraCheque.ckbEmissao.Checked := True;
      frmProcuraCheque.dtEmissaoDe.Date := sFDtEmissaoInicio;
      frmProcuraCheque.dtEmissaoAte.Date := sFDtEmissaoFim;
   end
   else begin
      frmProcuraCheque.ckbEmissao.Checked := False;
      frmProcuraCheque.dtEmissaoDe.Date := Now();
      frmProcuraCheque.dtEmissaoAte.Date := Now();
   end;
   ...
   if not frmProcuraCheque.flagCancel then begin
      ...
      if frmProcuraCheque.ckbData.Checked then begin
         sFDtInicio := frmProcuraCheque.dtVencInicial.Date;
         sFDtFim := frmProcuraCheque.dtVencFinal.Date;
      end
      else begin
         sFDtInicio := 0;
         sFDtFim := 0;
      ...
      if frmProcuraCheque.ckbEmissao.Checked then begin
         sFDtEmissaoInicio := frmProcuraCheque.dtEmissaoDe.Date;
         sFDtEmissaoFim := frmProcuraCheque.dtEmissaoAte.Date;
      end
      else begin
         sFDtEmissaoInicio := 0;
         sFDtEmissaoFim := 0;
      ...
      for i := 0 to ASelList.Count - 1 do begin
         if (ASelList.ValueFromIndex[i] = '1') then begin
            ...
            DM.transfereCheque(
               ...
               fTransferenciaCheques.dtTransferencia.date,
               fTransferenciaCheques.iCodColigada
            );

            // Verificar se foi ativado a opção para gerar um título

            if fTransferenciaCheques.ckbAtivaDebito.Checked then begin
...
begin
   ...
   if ((qryChequesdt_compensacao.AsDateTime > 0)) then begin
      frmProcuraCheque.dtICompensacao.Date := qryChequesdt_compensacao.AsDateTime;
   end
   else begin
      frmProcuraCheque.dtICompensacao.Date := Now();
   end;
   ...
   if not frmProcuraCheque.flagCancel then begin
      ...
      qyChequesActdt_compensacao.AsDateTime := frmProcuraCheque.dtICompensacao.Date;
...
begin
   ...
   // 2. Selecionar o aluno para gerar o cheque - deverá pegar os dados da primeira
   //    mensalidade ligada ao cheque;
   ...
   DM.GerarMensalidade
      ...
      , qryGetAlunoCheque.FieldByName('nr_valor').AsCurrency
      , 0
      ...
      , 0
      , 0
      , qryGetAlunoCheque.FieldByName('historico').AsString
