...
begin
   ...
   if qyAux.FieldByName('dt_bloqueio_financeiro').IsNull then begin
      dtBloqueio.Date := Dm.DataAtual();
   end else begin
      dtBloqueio.Date := qyAux.FieldByName('dt_bloqueio_financeiro').AsDateTime;
   end;
   ...
   if qyAux.FieldByName('dt_bloqueio_boleto').IsNull then begin
      dtBoleto.Date := Dm.DataAtual();
   end else begin
      dtBoleto.Date := qyAux.FieldByName('dt_bloqueio_boleto').AsDateTime;
   end;
...
begin
  ...
  dtBloqueio.Enabled := (cbOpcoes.ItemIndex = 1);

  lbBoleto.Enabled     := (cbBloqueto.ItemIndex = 1);
  dtBoleto.Enabled     := (cbBloqueto.ItemIndex = 1);
...
begin
   ...
   qryBloqueiaFin.ParamByName('DT_BLOQUEIO_FINANCEIRO').AsDate := dtBloqueio.Date;
   qryBloqueiaFin.ParamByName('SN_BLOQUEAR_BOLETO').AsInteger := cbBloqueto.ItemIndex;
   qryBloqueiaFin.ParamByName('DT_BLOQUEIO_BOLETO').AsDate := dtBoleto.Date;
   ...
   sMsgLog := 'Bloqueio Financeiro --> Data: ' +  FormatDateTime('dd/mm/yyyy', dtBloqueio.Date);
   ...
   sMsgLog := sMsgLog + 'Bloqueio Boleto --> Data: ' +  FormatDateTime('dd/mm/yyyy', dtBoleto.Date);
