...
uses
  ...
  Mask, DB, ZConnection, uDM, uRepBloquetos;
...
type
  TfGerarRemessa = class(TForm)
    ...
    Label3: TLabel;
    Label8: TLabel;
    dtGeraFinal: TDateTimePicker;
    ckbGeracao: TCheckBox;
    dtGeraInicial: TDateTimePicker;
...
begin
   dtGeraInicial.Enabled := ckbGeracao.Checked;
   dtGeraFinal.Enabled := ckbGeracao.Checked;
...
begin
   ...
   try
      ...
      begin
         ...
         dtVencInicial.Time :=  StrToTime('00:00:00');
         dtVencFinal.Time   :=  StrToTime('23:59:59');
         AddParam('DT_VENCTO_MENOR', dtVencInicial.DateTime);
         AddParam('DT_VENCTO_MAIOR', dtVencFinal.DateTime);
      end;

      if ckbGeracao.Checked then
      begin
        LSFiltro := LSFiltro + S_OP + S_FIL_DTGERACAO_ENTRE;
        dtGeraInicial.Time := StrToTime('00:00:00');
        dtGeraFinal.Time := StrToTime('23:59:59');
        AddParam('NR_GERACAO_MENOR', dtGeraInicial.DateTime);
        AddParam('NR_GERACAO_MAIOR', dtGeraFinal.DateTime);
      end;
...
procedure TfGerarRemessa.dtVencInicialChange(Sender: TObject);
begin
   if (dtVencInicial.Date > dtVencFinal.Date) then begin
