...
type
  ...
  private
    ...
    codigo_titulo, filtro, filtro2, fornecedor, plano_contas, plano_centro_custos: integer;
    Autoriza : Boolean;
...
begin
   ...
   dtpDataInicial.DateTime := DataHoje;
   dtpDataFinal.DateTime := DataHoje;
...
begin
  ...
  case Key of
    ...
    VK_F5 : Filtra;
    VK_F6 : if sbBaixarCancelar.Enabled then sbBaixarCancelarClick( nil );
...
begin
  ...
  qryContasPagar.FieldByName('vl_pendente').AsFloat := qryContasPagar.FieldByName('vl_entradas').AsFloat - qryContasPagar.FieldByName('vl_saidas').AsFloat;
...
begin
   ...
   frm_Compromisso_Planilha.Cursor := crHourGlass;
   ...
   if cbDataFiltro.ItemIndex > 0 then
   ...
   if cbDataFiltro.ItemIndex > 0 then
   begin
      qryContasPagar.ParamByName('DT_INICIAL').AsDate := dtpDataInicial.Date;
      qryContasPagar.ParamByName('DT_FINAL').AsDate := dtpDataFinal.Date;
   end;
   qryContasPagar.Open;
   
   AtualizaResumo(LSFiltros, dtpDataInicial.Date, dtpDataFinal.Date);
   ...
   grd.SetFocus;
   //qryContasPagar.Locate('CD_TITULO', LICodTitulo, []);
   frm_Compromisso_Planilha.Cursor := crDefault;
End;
...
procedure Tfrm_Compromisso_Planilha.dtpDataInicialChange(Sender: TObject);
begin
  // Alterou a data do filtro
  Filtra() ;
end;

procedure Tfrm_Compromisso_Planilha.dtpDataFinalChange(Sender: TObject);
begin
  // Alterou a data do filtro
  Filtra;
end;
...
begin
   ...
   dtpDataInicial.Enabled := cbDataFiltro.ItemIndex <> 0;
   dtpDataFinal.Enabled := cbDataFiltro.ItemIndex <> 0;
...
begin
   ...
   qryContasPagar.GotoBookmark(bookmarkTitulo);
end;
...
begin
   ...
   // Bloqueio de Data;
   if DM.EstaBloqueado(qryContasPagardt_vencimento.AsDateTime, true) then Exit;

   if Mensagem('Deseja realmente excluir o compromisso selecionado?',  'Confirmação', MB_YESNO + MB_ICONQUESTION ) <> mrYes then Exit;

   // Excluir tabelas relacionadas - MOV CP
   qAuxCP.Close();
   qAuxCP.SQL.Text :=  ' DELETE FROM fin_mov_cp    ' +
    ' WHERE cd_titulo = ' + qryContasPagarcd_titulo.AsString  +
    ' AND cd_coligada = ' + qryContasPagarcd_coligada.AsString;
   qAuxCP.ExecSQL();

   // Excluir tabelas relacionadas - APROPRIA CP
   qAuxCP.Close();
   qAuxCP.SQL.Text :=  ' DELETE FROM fin_apropria_cp    ' +
    ' WHERE cd_titulo = ' + qryContasPagarcd_titulo.AsString  +
    ' AND cd_coligada = ' + qryContasPagarcd_coligada.AsString;
   qAuxCP.ExecSQL();

   // Atualizar os títulos principais no caso de duplicatas
   qAuxCP.Close();
   qAuxCP.SQL.Text :=  ' UPDATE fin_contas_pagar SET cd_titulo_origem = 0  ' +
    ' WHERE cd_titulo_origem <> 0 AND cd_titulo_origem = ' + qryContasPagarcd_titulo.AsString  +
    ' AND cd_coligada = ' + qryContasPagarcd_coligada.AsString;
   qAuxCP.ExecSQL();
   ...
   qAuxCP.Close();
   qAuxCP.SQL.Text :=  ' DELETE FROM fin_contas_pagar ' +
    ' WHERE cd_titulo = ' + qryContasPagarcd_titulo.AsString  +
    ' AND cd_coligada = ' + qryContasPagarcd_coligada.AsString;
   qAuxCP.ExecSQL();
...
begin
  ...
  sFiltro := sFiltro +'{fin_apropria_cp.cd_coligada} = ' + qryContasPagarCD_COLIGADA.AsString +' AND ';
  sFiltroShow := '';

  case cbDataFiltro.ItemIndex  of
    1 : begin // Provável Pagamento
          sFiltro := sFiltro + 'ToText({fin_contas_pagar.dt_provavel_pgto}, "yyyyMMdd") >= "'+FormatDateTime('yyyyMMdd', dtpDataInicial.DateTime)+'" AND ToText({fin_contas_pagar.dt_provavel_pgto}, "yyyyMMdd") <= "'+FormatDateTime('yyyyMMdd', dtpDataFinal.DateTime)+'" AND ';
          sFiltroShow := sFiltroShow + '"PROVÁVEL PAGTO = DE ' + FormatDateTime( 'dd/mm/yyyy', dtpDataInicial.Date ) + ' ATE ' + FormatDateTime( 'dd/mm/yyyy', dtpDataFinal.Date ) + '" + CHR(13) + ';
        end;
    2 : begin // Vencimento
          sFiltro := sFiltro + 'ToText({fin_contas_pagar.dt_vencimento}, "yyyyMMdd") >= "'+FormatDateTime('yyyyMMdd', dtpDataInicial.DateTime)+'" AND ToText({fin_contas_pagar.dt_vencimento}, "yyyyMMdd") <= "'+FormatDateTime('yyyyMMdd', dtpDataFinal.DateTime)+'" AND ';
          sFiltroShow := sFiltroShow + '"VENCIMENTO = DE ' + FormatDateTime( 'dd/mm/yyyy', dtpDataInicial.Date ) + ' ATE ' + FormatDateTime( 'dd/mm/yyyy', dtpDataFinal.Date ) + '" + CHR(13) + ';
        end;
    3 : begin // Competência
          sFiltro := sFiltro + 'ToText({fin_contas_pagar.dt_competencia}, "yyyyMMdd") >= "'+FormatDateTime('yyyyMMdd', dtpDataInicial.DateTime)+'" AND ToText({fin_contas_pagar.dt_competencia}, "yyyyMMdd") <= "'+FormatDateTime('yyyyMMdd', dtpDataFinal.DateTime)+'" AND ';
          sFiltroShow := sFiltroShow + '"COMPETÊNCIA = DE ' + FormatDateTime( 'dd/mm/yyyy', dtpDataInicial.Date ) + ' ATE ' + FormatDateTime( 'dd/mm/yyyy', dtpDataFinal.Date ) + '" + CHR(13) + ';
        end;
    4 : begin // Lançamento
          sFiltro := sFiltro + 'ToText({fin_contas_pagar.dt_lancamento}, "yyyyMMdd") >= "'+FormatDateTime('yyyyMMdd', dtpDataInicial.DateTime)+'" AND ToText({fin_contas_pagar.dt_lancamento}, "yyyyMMdd") <= "'+FormatDateTime('yyyyMMdd', dtpDataFinal.DateTime)+'" AND ';
          sFiltroShow := sFiltroShow + '"DATA DE LANÇAMENTO = DE ' + FormatDateTime( 'dd/mm/yyyy', dtpDataInicial.Date ) + ' ATE ' + FormatDateTime( 'dd/mm/yyyy', dtpDataFinal.Date ) + '" + CHR(13) + ';
        end;
    5 : begin // Emissão da Nota
          sFiltro := sFiltro + 'ToText({fin_contas_pagar.dt_emissao_nota}, "yyyyMMdd") >= "'+FormatDateTime('yyyyMMdd', dtpDataInicial.DateTime)+'" AND ToText({fin_contas_pagar.dt_emissao_nota}, "yyyyMMdd") <= "'+FormatDateTime('yyyyMMdd', dtpDataFinal.DateTime)+'" AND ';
          sFiltroShow := sFiltroShow + '"EMISSÃO DA NOTA = DE ' + FormatDateTime( 'dd/mm/yyyy', dtpDataInicial.Date ) + ' ATE ' + FormatDateTime( 'dd/mm/yyyy', dtpDataFinal.Date ) + '" + CHR(13) + ';
        end;
    6 : begin // Data de Pagamento
          sFiltro := sFiltro + 'ToText({fin_contas_pagar.dt_pagamento}, "yyyyMMdd") >= "'+FormatDateTime('yyyyMMdd', dtpDataInicial.DateTime)+'" AND ToText({fin_contas_pagar.dt_pagamento}, "yyyyMMdd") <= "'+FormatDateTime('yyyyMMdd', dtpDataFinal.DateTime)+'" AND ';
          sFiltroShow := sFiltroShow + '"DATA DE PAGAMENTO = DE ' + FormatDateTime( 'dd/mm/yyyy', dtpDataInicial.Date ) + ' ATE ' + FormatDateTime( 'dd/mm/yyyy', dtpDataFinal.Date ) + '" + CHR(13) + ';
        end;
...
begin
  ...
  begin
    ...
    qAuxCP02.SQL.Add( 'SET cd_situacao = :Situacao, dt_pagamento = NULL ' );
...
begin
   ...
   qFatura.SQL.Text :=
     ' INSERT INTO fin_contas_pagar (cd_titulo, cd_autoriza, cd_titulo_principal, cd_pessoa, cd_coligada, ds_despesa, ds_historico, nr_documento, nr_parcela, dt_emissao_nota, dt_lancamento, ' +
     '    dt_competencia, dt_vencimento, dt_provavel_pgto, vl_despesa, cd_situacao, cd_tipo_titulo,  cd_forma_pgto, cd_caixa, vl_total_despesa) ' +
     ' SELECT :cd_titulo, :cd_autoriza, :cd_titulo_principal, cp.cd_pessoa, cp.cd_coligada, CONCAT("Agrupamento dos títulos """,GROUP_CONCAT(DISTINCT cp.nr_documento SEPARATOR "/"), """ de ", p.nm_pessoa), ' +
     ' CONCAT("Agrupamento dos títulos """,GROUP_CONCAT(DISTINCT cp.nr_documento SEPARATOR "/"), """ de ", p.nm_pessoa) , CONCAT("(A) ",GROUP_CONCAT(DISTINCT cp.nr_documento SEPARATOR "/")), 1, ' +
     ' cp.dt_emissao_nota, CURRENT_DATE(), cp.dt_competencia, cp.dt_vencimento, ' +
     '    cp.dt_provavel_pgto, SUM(cp.vl_despesa), 1, cp.cd_tipo_titulo, cp.cd_forma_pgto, cp.cd_caixa, SUM(cp.vl_despesa) ' +
     ' FROM fin_contas_pagar as cp INNER JOIN pessoas as p ON (p.cd_pessoa = cp.cd_pessoa) WHERE cd_titulo in ' + sCodFaturas +
   ...
   qFatura.SQL.Text :=
     ' INSERT INTO fin_apropria_cp (cd_titulo, cd_coligada, cd_conta, cd_centro, vl_movimento) ' +
     ' SELECT ' + sTitulo + ', ap.cd_coligada, ap.cd_conta, ap.cd_centro, sum((cp.vl_despesa / cp.vl_total_despesa) * ap.vl_movimento) ' +
     '   FROM fin_contas_pagar as cp INNER JOIN fin_apropria_cp as ap ON (cp.cd_titulo_principal = ap.cd_titulo AND cp.cd_coligada = ap.cd_coligada) ' +
     ' WHERE cp.cd_coligada = ' + IntToStr(cd_coligada) + ' AND cp.cd_titulo in ' + sCodFaturas +
     ' GROUP BY ap.cd_coligada, ap.cd_conta, ap.cd_centro ';
