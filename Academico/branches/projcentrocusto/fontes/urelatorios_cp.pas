...
uses
  ...
  UZDataset, DBGrids, UMComboBox, uFSelecionarPessoa;
...
begin
  ...
  txtDataInicial.Enabled := (Pos('data', xField) > 0) or (Pos('dt_', xField) > 0) or (Pos('dt_', xformula) > 0) or (Pos('data', xFormula) > 0);
   ...
   while (Length(Trim(sParametros)) > 1) do begin
      ...
      if (Pos('depto', pCampo) > 0) then begin
         if (txtDepartamento.Text <> '') then begin
            ...
            sFiltroShow := sFiltroShow + '"DEPARTAMENTO = ' + txtDepartamento.Text + '" + CHR(13) + ';
            sAux := ' and ';
         end;
      end;
      ...
      if ckDtFiltrar.Checked then begin
        ...
        if (Pos('provavel', pCampo) > 0) then
              sFiltroShow := sFiltroShow + '"PROVÁVEL PGTO = DE ' + FormatDateTime( 'dd/mm/yyyy', txtDataInicial.Date ) + ' ATE ' + FormatDateTime( 'dd/mm/yyyy', txtDataFinal.Date ) + '" + CHR(13) + ';
        if (Pos('emissao', pCampo) > 0) then
              sFiltroShow := sFiltroShow + '"EMISSÃO DA NOTA = DE ' + FormatDateTime( 'dd/mm/yyyy', txtDataInicial.Date ) + ' ATE ' + FormatDateTime( 'dd/mm/yyyy', txtDataFinal.Date ) + '" + CHR(13) + ';
