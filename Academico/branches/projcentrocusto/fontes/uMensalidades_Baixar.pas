...
type
  TfrmMensalidadesBaixar = class(TForm)
    ...
    popMnuMenuGridPessoa: TPopupMenu;
    popmnuRemovePessoa: TMenuItem;
    pnCheques: TPanel;
    lbNrBanco: TLabel;
    lbNrAgencia: TLabel;
    lbNrCC: TLabel;
...
begin
   ...
   qyCampoAutomatico.ParamByName('cd_admin_cartao').AsInteger :=
...
begin
   ...
   begin
      ...
      begin
         ...
         sgGridCheques.Objects[0, i+1]   := objCheque;

         sgGridCheques.Cols[0][i+1]      := objCheque.getValorCampo('nr_banco');
         sgGridCheques.Cols[1][i+1]      := objCheque.getValorCampo('nr_agencia');
         sgGridCheques.Cols[2][i+1]      := objCheque.getValorCampo('nr_conta');
         sgGridCheques.Cols[3][i+1]      := objCheque.getValorCampo('nr_cheque');
         sgGridCheques.Cols[4][i+1]      := substituiCaracter(objCheque.getValorCampo('vl_cheque'),'.',',');
         sgGridCheques.Cols[5][i+1]      := objCheque.getValorCampo('dt_compensacao');
         sgGridCheques.Cols[6][i+1]      := objCheque.getValorCampo('nm_titular');
      end;
   end;
end;
...
begin
   ...
   codigoAdministradora := StrToInt(TItemCombo(cbAdministradoras.Items.Objects[cbAdministradoras.ItemIndex]).getCodigo);

   taxaOriginal := getValorTaxa(codigoAdministradora, rbCredito.Checked);
...
begin
   ...
   udQtdParcelas.Max := carregaMaximoIntervalo(
      StrToInt(TItemCombo(cbAdministradoras.Items.Objects[cbAdministradoras.ItemIndex]).getCodigo)
   );
  ...
  case key of
    ...
    VK_F6 : begin
       if chkCheque.Checked then
          sgGridCheques.SetFocus();
    end;
    VK_F9 : begin
       if btnBaixar.Enabled then 
...
begin
   ...
   if (trim(Self.getPessoaSelecionada()) <> '') and (trim(Self.getNmPessoaSelecionada()) <> '') then
   begin
   ...
   sgGridParcelas.SetFocus;
   SelecionaParcela(getMensalidadeSelecionada);
end;
...
const
   SQL_BUSCA_DATAS =
      '   SELECT '+
      '      DATE_FORMAT(ADDDATE(now(),INTERVAL %s day), %s) AS data1, '+
      '      DATE_FORMAT(ADDDATE(now(),INTERVAL %s day), %s) AS dataFormatada1, '+
      '      DAYOFWEEK( ADDDATE(now(),INTERVAL %s day) ) AS diaSemana1 '+
      '   FROM '+
      '      fin_cadastro_admin_cartao '+
      '   WHERE '+
      '      cd_admin_cartao = :cd_admin_cartao';
...
procedure TfrmMensalidadesBaixar.getListaContasCaixa;
var
   qyContas    : TUMZQuery;
...
begin
   ...
   begin
      ...
      begin
        ...
        begin
            objItem := TItemComboMCaixa.Create( cd_caixa, ds_caixa, sn_padrao, qyContas.FieldByName('cd_coligada').AsInteger );
...
//      if qyTaxa.RecordCount = 0 then
//      begin
//         Mensagem(
//            'Não existe taxa configurada para crédito nesta administradora de cartão.',
//            'Aviso',
//            MB_OK + MB_ICONWARNING
//         );
...
begin
   ...
   btnBaixar.Enabled := (chkDinheiro.Checked or chkCheque.Checked or chkCartao.Checked);
end;
...
begin
   ...
   begin
      if btnBaixar.Enabled = true then
      begin
         btnBaixar.SetFocus();
      end
      else
...
begin
   ...
   begin
      ...
      if (data_alterada <> data_baixa) or (chkCartao.Checked) then
      begin
...
var
   ...
   objMensalidadeBaixar : TMBMensalidadeBaixa;
   qryGetPessoa         : TUMZQuery;
   cd_caixa             : string;
   //cd_coligada          : integer;
   valor_desconto       : Double;
   MensalidadeDaParcela : Boolean;
begin
   DM.CriarConsulta(qryGetPessoa);
   ...
   begin
      ...
      begin
         ...
         qryGetPessoa.Close;
         qryGetPessoa.SQL.Text := '      '+
            ' SELECT                                   '+
            '    codigoaluno as cd_pessoa, cd_coligada '+
            ' FROM                                     '+
            '    mensalidades                          '+
            ' WHERE                                    '+
            '    cd_mensalidade = :cd_mensalidade      ';
         qryGetPessoa.ParamByName('cd_mensalidade').AsInteger := StrToInt(objMensalidadeBaixar.getValorCampo('cd_mensalidade'));
         qryGetPessoa.Open;

         if MensalidadeDaParcela then
            cd_coligada := qryGetPessoa.FieldByName('cd_coligada').AsInteger;
        ...
        begin
           ...
           DM.LimpaMensaNegociacaoPaga( StrToInt( objMensalidadeBaixar.getValorCampo('cd_mensalidade') ), qryGetPessoa.FieldByName('cd_pessoa').AsInteger )
        end;
      end;

   end;
   FreeAndNil(qryGetPessoa);
end;
