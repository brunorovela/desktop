...
uses
  ...
  uLoginManager, UMComboBox, ExtCtrls;
...
type
  ...
  TDM = class(TDataModule)
   ...
   private
      ...
      function PreRequisitoEspecial(CodAluno: Integer; Curso: String; Disciplina, Grade, Anosemestre: Integer; var PreRequisitoEspecialList: TStringList): Integer;
  ...
  public
    ...
    Function GerarMensalidade(
         ...
         ds_obs_desc         : string = '';
         DataEmissao : TDateTime = 0) : Integer;
    ...
    function MatricularGrade( lngCodigoAluno : Longint;
                               strTurma : String;
                               intAnoSemestre: Integer; sGrade : String = '01') : String;
    ...
    function CamposObrigatoriosPreenchidos(formulario: TForm; cadastro_especifico : String = ''; mostraMsg: Boolean = True): Boolean;
...
uses
   ...
   uMensalidade, uCredito, uPessoa, Contnrs, UFinApropriaCPClass, uFinApropriaTesouraria;
...
function TDM.CamposObrigatoriosPreenchidos(formulario: TForm; cadastro_especifico : String = ''; mostraMsg: Boolean = True): Boolean;
var
...
begin
   ...
   begin
      ...
      begin
         ...
         parametros.mensagens.msgPrincipal := erro;
...
begin
   ...
   if ((not LBContaAtiva) or (LWTipoConta = 3)) and (LICodAbertura <= 0) then
   begin
      Mensagem(
...
function TDM.BaixarMensalidade(conta : Integer; Documento : String; coligada : integer = 0; AlterouJuros : Boolean = false) : Boolean;
...
begin
  ...
  begin
    ...
    if (Documento = '') then begin // Baixa manual, assume os valores digitados
...
{ Movimento no Caixa }
    ...
    if Dm.variavel_parametro('financeiro_retorno_data_movimento') = 'CREDITO' then
    begin
       movimento.DataMovimento := DM.tblMensalidadesdt_credito.AsDateTime;
    end else if Dm.variavel_parametro('financeiro_retorno_data_movimento') <> 'ATUAL' then
    begin
       movimento.DataMovimento := DM.tblMensalidadesDataPagamento.AsDateTime;
    end
    else
 ...
 // Fim da Baixa de Mensalidade, Registrar a Movimentação
  ...
  begin
   ...
   begin
      ...
      GerarMensalidade(         tblMensalidadesCodigoAluno.AsInteger,
                                tblMensalidadesParcela.AsInteger,
                                tblMensalidadesTurma.AsString,
                                RoundFloat( NovoBruto, 2 ),
                                RoundFloat( NovoJuros, 2 ),
...
function TDM.MatricularGrade( lngCodigoAluno : Longint; strTurma : String; intAnoSemestre: Integer; sGrade : String = '01') : String;
const
   SSQLInsertDisc =
...
function TDM.PreRequisitoEspecial(CodAluno: Integer; Curso: String; Disciplina: Integer; Grade : Integer; Anosemestre: Integer; var PreRequisitoEspecialList: TStringList): Integer;
var
   sSql: String;
...
// Pegar a Quantidade de Créditos que o aluno tem matriculado
  ...
  begin
     ...
     qSel.SQL.Text :=
       ...
       '      , me.cd_tipo_titulo, me.datavencimento, me.curso, me.depto, me.cd_tipo_titulo, me.cd_centro_custo, me.cd_coligada, me.vl_credito ' +
     ...
     GerarMensalidade( intCodigoAluno
      ...
      with qSel do begin
        ...
        SQL.Text := '' +
         ...
         ' CASE WHEN COALESCE(ip.valorbruto,0) = 0 THEN 0 ELSE (ip.valordesconto / ip.valorbruto) END as ind_desconto,  ' +
        ...
        begin
           ...
           if FieldByName('valorbruto').AsFloat <> FieldByName('NovoValor').AsFloat then
           begin
...
// O valor do crédito NUNCA deve ser dividido,
// este processo somente deve ocorrer durante uma divisão de títulos               
//               ' VL_CREDITO = VL_CREDITO * :VL_PERCENTUAL_DIVISAO / 100 ' +
               ' WHERE cd_mensalidade = ' + FieldByName('cd_mensalidade').AsString;

              if FieldByName('NovoValor').AsFloat < FieldByName('ValorMinimo').AsFloat then begin
              ...
              begin
                 ...
                 // Cálcula o valor da parcela com base no percentual de divisão,
                 // caso a parcela tenha sido dividida para outro responsável
                 // JOSIMAR NovoValor := NovoValor * FieldByName('VL_PERCENTUAL_DIVISAO').AsFloat / 100;

                 if (FieldByName('NovoValor').AsFloat = 0) AND (FieldByName('Vl_Credito').AsFloat = 0) then
                    NovoCredito := 0
                 else
...
//              qAtualiza.ParamByName('NovoCreditos').AsFloat := NovoCredito;
//              qAtualiza.ParamByName('VL_PERCENTUAL_DIVISAO').AsFloat := FieldByName('VL_PERCENTUAL_DIVISAO').AsFloat;
              ...
              begin
                 qAtualiza.SQL.Text := ' MERGE INTO fin_mov_cr f USING ' +
                                        '    (SELECT f.CD_MENSALIDADE, f.CD_COLIGADA, ' +
                                        '            f.NR_SEQUENCIA, round((f.vl_saida * :NovoDescExtra) / :descontoextra,2) as vl_saida ' +
                                        '     FROM fin_mov_cr f, fin_acoes_movimento a ' +
                                        '     WHERE f.cd_acao = a.cd_acao AND f.cd_mensalidade = :CdMensalidade ' +
                                        '           AND a.cd_tipo_acao = 3) f2 ON ' +
                                        '     (f.CD_MENSALIDADE = f2.CD_MENSALIDADE AND ' +
                                        '      f.CD_COLIGADA = f2.CD_COLIGADA AND ' +
                                        '      f.NR_SEQUENCIA = f2.NR_SEQUENCIA) ' +
                                        ' WHEN MATCHED THEN ' +
                                        '    UPDATE SET f.vl_saida = f2.vl_saida ';
              end
              else
              begin
                 qAtualiza.SQL.Text := '' +
...
const
   SQL_BUSCA_CENTRO =
      ...
      ' WHERE '+
      ' 	fccc.cd_centro_pai = :cd_centro AND fccc.sn_ativo = 1';
...
begin
  ...
  qyMatriculas.Close();
  qyMatriculas.SQL.Text := 'SELECT codigoaluno, turma, anosemestre  FROM matriculas ' +
...
begin
 ...
 begin
   ...
   begin
      ...
      DM.qAux2.SQL.Text := 'REPLACE INTO usuarios_parametros(cd_usuario,ds_parametro,ds_valor) VALUES('+intTostr(dm.iCdPessoaLogado)+','''+nomeFormulario+'.'+TMaskEdit(componente).Name+''','''+TMaskEdit(componente).Text+''')';
   end;
   DM.qAux2.ExecSQL;
 ...
 begin
   ...
   begin
      ...
      DM.qAux2.SQL.Text := 'REPLACE INTO usuarios_parametros(cd_usuario,ds_parametro,ds_valor) VALUES('+intTostr(dm.iCdPessoaLogado)+','''+nomeFormulario+'.'+TComboBox(componente).Name+''','''+TComboBox(componente).Text+''')';
   end;
   DM.qAux2.ExecSQL;
