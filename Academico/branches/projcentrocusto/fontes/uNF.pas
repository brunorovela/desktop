...
type
  TFrmNF = class(TForm)
    ...
    Label1: TLabel;
    txtParcelaFim: TEdit;
    dtpVencimentoInicio: TDateTimePicker;
    Label2: TLabel;
    dtpVencimentoFim: TDateTimePicker;
    dtpPagamentoInicio: TDateTimePicker;
    dtpPagamentoFim: TDateTimePicker;
    Label3: TLabel;
    ...
    dtpCompetenciaInicio: TDateTimePicker;
    Label12: TLabel;
    dtpCompetenciaFim: TDateTimePicker;
...
begin
   // Atualizar o filtro;
   { TODO 5 -oJosimar Zimermann -cAjuste : Corrigir a programação do botão "GerarNota". }
   ...
   while not qyFiltroTitulos.Eof do begin
      ...
      qNota.ParamByName('dt_nf').AsDateTime := dtpDataEmissao.DateTime;
      ...
      if ckbVencimento.Checked then begin
         qNota.ParamByName('filtro_vencimento_inicio').AsDateTime := dtpVencimentoInicio.DateTime;
         qNota.ParamByName('filtro_vencimento_fim').AsDateTime := dtpVencimentoFim.DateTime;
      end else begin
      ...
      if ckbPagamento.Checked then begin
         qNota.ParamByName('filtro_pagamento_inicio').AsDateTime := dtpPagamentoInicio.DateTime;
         qNota.ParamByName('filtro_pagamento_fim').AsDateTime := dtpPagamentoFim.DateTime;
      end else begin
...
begin
   ...
   dtpVencimentoInicio.Enabled := ckbVencimento.Checked;
   dtpVencimentoFim.Enabled := ckbVencimento.Checked;

   dtpPagamentoInicio.Enabled := ckbPagamento.Checked;
   dtpPagamentoFim.Enabled := ckbPagamento.Checked;

   dtpCompetenciaInicio.Enabled := ckCompetencia.Checked;
   dtpCompetenciaFim.Enabled := ckCompetencia.Checked;

   dtpInicio.Enabled := ckbData.Checked;
   dtpFim.Enabled := ckbData.Checked;
...
begin
   ...
   try
      ...
      begin
         ...
         AddParam('DT_NF_MENOR', dtpInicio.DateTime);
         AddParam('DT_NF_MAIOR', dtpFim.DateTime);
      end;
...
begin
   ...
   begin
      AddParam('DT_VENCTO_MENOR', dtpVencimentoInicio.DateTime);
      AddParam('DT_VENCTO_MAIOR', dtpVencimentoFim.DateTime);
      if ckbPagamento.Checked then
      begin
         LSFiltro := LSFiltro + S_OP_AND + Format(S_FIL_DTVENCTO, [S_OP_OR + S_FIL_DTPAGTO]);
         AddParam('DT_PGTO_MENOR', dtpPagamentoInicio.DateTime);
         AddParam('DT_PGTO_MAIOR', dtpPagamentoFim.DateTime);
      end
      else
   ...
   begin
      ...
      AddParam('DT_COMP_MENOR', dtpCompetenciaInicio.DateTime);
      AddParam('DT_COMP_MAIOR', dtpCompetenciaFim.DateTime);
   end;
...
begin
   ...
   dtpVencimentoInicio.Date := Date;
   dtpVencimentoFim.Date := Date;
   dtpPagamentoInicio.Date := Date;
   dtpPagamentoFim.Date := Date;
   dtpCompetenciaInicio.Date := Date;
   dtpCompetenciaFim.Date := Date;
...
begin
   ...
   if TPopupMenu( Sender ).Tag = 1 then begin
      ...
      if ckbData.Checked then begin
          sFiltro := sFiltro + sOp + 'ToText({fin_nf.dt_nf}, "yyyyMMdd") >= "' + FormatDateTime('yyyymmdd', dtpInicio.Date) + '" ';
          sOp := ' AND ';
          sFiltro := sFiltro + sOp + 'ToText({fin_nf.dt_nf}, "yyyyMMdd") <= "' + FormatDateTime('yyyymmdd', dtpFim.Date) + '" ';

          sFiltroShow := sFiltroShow + '"DATA = DE ' + FormatDateTime( 'dd/mm/yyyy', dtpInicio.Date ) + ' ATE ' + FormatDateTime( 'dd/mm/yyyy', dtpFim.Date ) + '" + CHR(13) + ';
      end;
