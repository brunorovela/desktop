...
uses
  ...
  ZAbstractRODataset, ZAbstractDataset, ZDataset, ImgList, ToolWin;
...
begin
   ...
   Else if pgPrincipal.ActivePage = tsDiarioNotas then Begin
      ...
      begin
         btnCalcularTurma.Caption := 'Calcular média do grupo atual';
      end else begin
         btnCalcularTurma.Caption := 'Calcular média de toda a turma';
      end;
...
begin
      ...
      begin
         ...
         begin
            UMGridAulas2.ListaCampos.AdicionarCampo('coluna'+FieldByName('nro_aula').AsString)
               .SetTitulo(FieldByName('data').AsString).SetTipo(tcString).SetTamanho(FieldByName('qtd_aulas').AsInteger).setBloqueado(True);
         end else begin
            UMGridAulas2.ListaCampos.AdicionarCampo('coluna'+FieldByName('nro_aula').AsString)
               .SetTitulo(FieldByName('data').AsString).SetTipo(tcString).SetTamanho(FieldByName('qtd_aulas').AsInteger);
         end;
   ...
   begin
      ...
      begin
         ...
         SQL.Add( '        fi.situacao, da.qtd_aulas, ' );
         ...
         SQL.Add( ' HAVING ((SUM(CASE WHEN dgp.dt_saida IS NULL THEN 1 ELSE 0 END) > 0) AND fi.situacao NOT IN (3,4,5,6,7,8,10)) OR :MOSTRA_TODOS = 1 ' );
...
begin
   ...
   qNotas.Close;
   qNotas.SQL.Text := '';
   qNotas.SQL.Add(' SELECT dp.vl_peso, dp.id_atividade_moodle, ap.sn_conceitos_parciais AS sn_conceitos_parciais, ap.sn_notas AS sn_notas, dp.sn_especial AS sn_especial, dp.nro_nota nro_nota, COALESCE(dp.nr_tipo_digitacao, 2) AS nr_tipo_digitacao ');
   ...
   qNotaRE.Close;
   qNotaRE.SQL.Text := '';
   qNotaRE.SQL.Add(' SELECT ap.sn_conceitos_parciais sn_conceitos_parciais, ap.sn_notas sn_notas, dp.sn_especial sn_especial, dp.nro_nota nro_nota, COALESCE(dp.nr_tipo_digitacao, 2) nr_tipo_digitacao FROM diario_provas dp ');
   ...
   qNotas2.SQL.Add( ' LEFT JOIN avaliacoes_conceitos ac_notas ON (((ap.sn_conceitos_parciais = ''S'' AND ap.sn_notas = ''S'' AND dp.nr_tipo_digitacao = 1) OR (ap.sn_conceitos_parciais = ''S'' AND ap.sn_notas = ''N'')) ');
   ...
   if (snNumero) then
      qNotas2.SQL.Add( ' ORDER BY ma.nr_aluno, p.nm_pessoa ' )
   else
      qNotas2.SQL.Add( ' ORDER BY p.nm_pessoa ' );
   ...
   UMGridNotas2.AdicionaSQLPadrao(
      ...
      ' SET dpa.vl_nota = ' +
      '        CASE COALESCE(dp.nr_tipo_digitacao, 2) ' +
...
procedure TDiarioView.UMGridNotas2AfterUpdateColumn(Sender: TObject;
  ...
  var NotUpdateRow: Boolean);
var
   strAltera, nro_nota, chaves, logDesc, vl_antigo, vl_novo, sn_faltou: String;
...
begin
   ...
   '        WHEN PROF_ANTIGO.NM_PESSOA IS NOT NULL THEN ' +
	'	         CONCAT(GROUP_CONCAT(P.NM_PESSOA ORDER BY P.NM_PESSOA SEPARATOR ", "), CONCAT(" (Anterior: ", PROF_ANTIGO.NM_PESSOA, ")"))' +
	'        ELSE ' +
	'	         GROUP_CONCAT(P.NM_PESSOA ORDER BY P.NM_PESSOA SEPARATOR ", ") ' +
   ...
   '      LEFT JOIN PESSOAS PROF_ANTIGO ON (PROF_ANTIGO.CD_PESSOA = DT.PROFESSOR AND PROF_ANTIGO.CD_PESSOA != TP.PROFESSOR) ' +
...
begin
   ...
   begin
      ...
      // Caso o Resultado Final já esteja liberado, verificar permissões
      with DMDiario.qyAux1 do begin
         Close();
         SQL.Clear();
         Params.Clear();
         SQLList.Values['mysql-5'] := 'SELECT dt_resultado_final as dtf FROM cursos WHERE anosemestre = '+ UMGridNotas2.GetValorStr('nr_anosem') + ' AND codigo = ''' +  DMDiario.qTurmasCURSO.AsString + ''' LIMIT 1';
         SQLList.Values['oracle']  := 'SELECT dt_resultado_final as dtf FROM cursos WHERE anosemestre = '+ UMGridNotas2.GetValorStr('nr_anosem') + ' AND codigo = ''' +  DMDiario.qTurmasCURSO.AsString + ''' AND rownum = 1';
         Open();
      ...
      if (dtSituacao > Now()) then
      begin
         if DM.UsuarioLogado.TemPermissao(1028, npAcesso, False) then
         begin
            if not (Mensagem('O prazo do curso expirou, mesmo assim, deseja recalcular?', 'Atenção!!', MB_YESNO + MB_ICONQUESTION) = IDYES) then
            begin
               Exit;
            end;
         end else begin
...
CONST
   ...
   SQL_GET_PARAMETRO_AVALIACAO =
      ' SELECT apm.ds_frequencia_registro, apm.vl_hora_aula                                           ' +
      ' FROM turmas t                                                                                 ' +
...
begin
   ...
   Begin
      ...
      begin
         ...
         SQL.Add( '        LENGTH(REPLACE(TRIM(GROUP_CONCAT(daa.ds_freq)),'+QuotedStr(',')+','+QuotedStr('')+')) - LENGTH(REPLACE(REPLACE(TRIM(GROUP_CONCAT(daa.ds_freq)),'+QuotedStr(',')+','+QuotedStr('')+'), '+QuotedStr('F')+', '+QuotedStr('')+')) AS faltas ' );
         SQL.Add( ' FROM diario_aulas da ' );
         SQL.Add( ' LEFT JOIN fichaindividual as f ON (  ' );
         SQL.Add( '         da.turma = f.turma AND ' );
         SQL.Add( '         da.anosemestre = f.anosemestre AND ' );
         SQL.Add( '         da.disciplina = f.disciplina) ' );
         SQL.Add( '      LEFT JOIN diario_aulas_alunos daa ON ( ' );
         ...
         SQL.Add( '         AND daa.cd_pessoa = f.codigoaluno) ' );
         ...
         SQL.Add( '      ) ');
         SQL.Add( ' WHERE da.anosemestre = :ANOSEMESTRE AND da.bimestre = :BIMESTRE AND ' );
         SQL.Add( '       da.turma = :TURMA AND da.disciplina = :DISCIPLINA AND ' );
         SQL.Add( '       CASE WHEN :CD_GRUPO > 0 THEN dgp.cd_grupo = :CD_GRUPO ELSE 1=1 END AND ' );
         SQL.Add( '       daa.cd_pessoa IS NOT NULL ' );
         //grupo
         if TSpeedButton(Sender).Tag = 1 then
            SQL.Add( ' AND daa.cd_pessoa = :CD_PESSOA ' );

         SQL.Add( ' GROUP BY daa.cd_pessoa ' );
         ...
         begin
            ...
            begin
               ...
               begin
                  ...
                  SQL.Add( ' SET falta' + inttostr(bimestre) + ' = :FALTAS  ' );
