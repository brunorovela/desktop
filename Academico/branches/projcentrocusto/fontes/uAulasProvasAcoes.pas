...
type
  ...
  TfrmAulasProvasAcoes = class(TForm)
    ...
    dtDataInic: TDateTimePicker;
    txtDisciplina: TEdit;
    txtTurma: TEdit;
    Label1: TLabel;
    dtDataFin: TDateTimePicker;
    txtCurso: TEdit;
    ...
    lblFiltroFreq: TLabel;
    Label26: TLabel;
    cbAtualizaFI: TCheckBox;
    cbFreqForca: TCheckBox;
    Memo1: TMemo;
    txtPDiasBloq: TEdit;
    Label25: TLabel;
    ...
    Label43: TLabel;
    dtpCronogramasData: TDateTimePicker;
    Label49: TLabel;
    ...
    Label51: TLabel;
    Label52: TLabel;
    dtpDtInicioDiario: TDateTimePicker;
    dtpDtFimDiario: TDateTimePicker;
    Label53: TLabel;
    Label54: TLabel;
    Label55: TLabel;
    Label56: TLabel;
    ...
    procedure dtpDataAula1Change(Sender: TObject);
   ...
   private
    ...
    procedure verificarTrocaProfessor;
    ...
    procedure AtualizarDataAvaliacaoInstucionalCronograma;
     procedure IncluirAvlDisc(cd_turma : string; cd_disciplina : Integer );
     ...
     procedure CriaProvaUltimaAulaCronograma( const data: TDate;
                  const cd_turma: String; const cd_disciplina, cd_professor, nr_anosemestre, cd_grupo,bimestre: Integer );
   ...
   public
      ...
      function ValidarAvaliacao(): Boolean;
...
uses
   ...
   uTurmaProfessorAceite, uProfessores;
...
//   meConteudo.Enabled := not ckbConfGrade.Checked;
   ...
   dtDataFin.Enabled := ckbConfGrade.Checked;
...
procedure TfrmAulasProvasAcoes.ckbSerieClick(Sender: TObject);
begin
...
begin
   ...
   //txtAProf.Visible := bValor;
   //lblAProf.Visible := bValor;
   //sbAProf.Visible := bValor;
end;

procedure TfrmAulasProvasAcoes.dtpDataAula1Change(Sender: TObject);
begin
   ProcessarConflitosHorariosCronograma(
      StrToIntDef(StringReplace(TDateTimePicker(Sender).Name, 'dtpDataAula', '', [rfIgnoreCase]), 1)
   );
   RecarregaDatasDiario();
end;
...
begin
   if acao = Limpar then
   begin
      dtpDataAula1.Date := Now;
      dtpDataAula2.Date := Now;
   ...
   begin
      ...
      begin
         ...
         dtpDataAula := TDateTimePicker.Create( sbAulas );
         with dtpDataAula do
         begin
            Visible     := false;
            Parent      := sbAulas;
            ...
            OnChange    := dtpDataAula1Change;

            if (utilizaSegundaAula) then
            begin
               if (I MOD 2 = 0) then
               begin
                  dataNow := dtpDataAula2.Date;
                  IData := Trunc(I/2);
               end
               else
               begin
                  dataNow := dtpDataAula1.Date;
                  if I > 1 then
                     IData := Trunc((I+1)/2)
                  else
                     IData := I;
   ...
   begin
      ...
      begin
         ...
         begin
            ...
            Left        := lbAula.Left + lbAula.Width + 2;
            Top         := pnOrientacaoTopo.Top + espacoLinhas;
            Date        := qyAulasCronogramas.FieldByName( 'dt_cronograma' ).AsDateTime;
            OnChange    := dtpDataAula1Change;
         end;
      ...
      begin
         ...
         begin
            ...
            begin
               ...
               TDateTimePicker(sbAulas.Components[I]).Date := dtpDataAula2.Date + (numeroDias * (IData-1));
            end
            ...
            begin
               ...
               if (aula > 1) AND (utilizaSegundaAula()) then
                  ...
                  IData := aula;
               TDateTimePicker(sbAulas.Components[I]).Date := dtpDataAula1.Date + (numeroDias * (IData-1));
            end;
         end else if ( Pos( 'cbHorarioAula', nomeComponente ) > 0 ) then
         begin
...
begin
   ...
   begin
      ...
      SQL.Add(''+
         ...
         '  nr_qtd_diarias_local, vl_diaria_local, vl_total_local,                     '+
         '  nr_quilometragem, vl_km, vl_total_transporte, cd_grupo                     '+
         'FROM                                                                         '+
         '  diario_aulas                                                               '+
         'WHERE                                                                        '+
      ...
      begin
         ...
         dtDataInic.DateTime        := FieldByName('data').AsDateTime;
...
begin
   with qyAux do begin
      ...
      if not IsEmpty then begin
         dtPData.Date      := FieldByName('data').AsDateTime;
...
var
   ...
   data : TDateTime;
   nomeComponente, msg_cronograma, nm_disciplina, logDesc: String;
...
begin
   ...
   begin
      if ( Self.ActiveControl is TDateTimePicker ) then
      begin
      ...
      if ( chkAI.Checked and (not ValidarAvaliacao) ) then
      begin
         Mensagem('Não é possível vincular o cronograma a uma avaliação institucional, pois não foi encontrada nenhuma Avaliação Online por Disciplina.'+#13+'Sugerimos que seja criada uma Avaliação Online por Disciplina antes de criar o cronograma ou desmarque a opção "Gerar AI".', 'Atenção!!', MB_OK);
         Exit;
      end;

      IncluirAvlDisc( txtCronogramaTurma.Text , strtoint( TItemCombo(cbDisciplinas.Items.Objects[ cbDisciplinas.ItemIndex ]).getCodigo) );
         ...
         begin
            ...
            begin
               ...
               IncluirCronograma(
                  strtoint( sPAnoSem ), txtCronogramaTurma.Text , // Anosemestre, Turma
                  strtoint( TItemCombo( cbDisciplinas.Items.Objects[ cbDisciplinas.ItemIndex ]).getCodigo ), // Disciplina
                  strtoint( txtCronogramasProfessor.Text ), TDateTimePicker(sbAulas.Components[I]).Date, // Professor, Data
                  ckConfirmado.Checked, edmValor.getValorDouble, // Confirmado, Valor
                  horario, // Horário
               ...
               if ( TDateTimePicker(sbAulas.Components[I]).Date >= maior_data ) then
                  maior_data := TDateTimePicker(sbAulas.Components[I]).Date;
            end;
         end;

         if ckbCriaProvaUltimaAula.Checked then
            CriaProvaUltimaAulaCronograma( maior_data,
               txtCronogramaTurma.Text,
               strtoint( TItemCombo( cbDisciplinas.Items.Objects[ cbDisciplinas.ItemIndex ] ).getCodigo ),
               strtoint( txtCronogramasProfessor.Text ), strtoint( sPAnoSem ), sPGrupo, strtoint(edBimestre.Text)
            );
      ...
      begin
         ...
         AlterarCronograma(
            ...
            ckConfirmado.Checked, edmValor.getValorDouble, StrToInt(txtSubstituto.Text),
            edNomeSala.Tag, edNomeSala.Text, strtoint(edBimestre.Text)
         );
         ...
         begin
            ...
            begin
               AlterarCronograma(
                  ...
                  ckConfirmado.Checked, edmValor.getValorDouble , StrToInt(txtSubstituto.Text),// confirmado
                  edNomeSala.Tag, edNomeSala.Text, strtoint(edBimestre.Text)
               );
            end
            else
         ...
         begin
            ...
            begin
               ...
               begin
                  ...
                  if ( sbAulas.Components[I+1] is TDateTimePicker ) AND
                  ...
                  begin
                     ...
                     data := TDateTimePicker(sbAulas.Components[I+1]).Date;
                  end else if ( sbAulas.Components[I+1] is TUMComboBox ) AND
                              ( sbAulas.Components[I+2] is TDateTimePicker ) then
                  begin
                     horario := strtoint(
                        TItemCombo(
                     ...
                     data := TDateTimePicker(sbAulas.Components[I+2]).Date;
                  end;
                  // Recupera as informações de data e horário
                  ////////////////////////////////////////////
      ...
      CriaDiarioPrazos(
         ...
         strtoint( TItemCombo( cbDisciplinas.Items.Objects[ cbDisciplinas.ItemIndex ] ).getCodigo ),
         dtpDtInicioDiario.Date, dtpDtFimDiario.Date , strtoint(edBimestre.Text)
      );
...
procedure TfrmAulasProvasAcoes.IncluirAvlDisc( cd_turma : string; cd_disciplina : Integer );
var
  qryInsereDisc : TUMZQuery;
  qryGetAvaliacao : TUMZQuery;
begin
  dtAvaliacao.time := 0;

  DM.CriarConsulta(qryGetAvaliacao);
  DM.CriarConsulta(qryInsereDisc);
  ...
  ' FROM                         '+
	'   avl_avaliacoes avl         '+
  ' WHERE                        '+
  '   avl.cd_tipo_avaliacao = 2  '+
  ' order by                     '+
  ' 	cd_avaliacao desc          ';
  qryGetAvaliacao.Open;

  dm.CriarConsulta(qryInsereDisc);
  qryInsereDisc.SQL.Text := '' +
  ' SELECT                                '+
  '   dt_fim,                             '+
  '   cd_turma,                           '+
  '   cd_avaliacao,                       '+
  ...
  qryInsereDisc.ParamByName('cd_turma').AsString       := cd_turma;
  qryInsereDisc.ParamByName('cd_disciplina').AsInteger := cd_disciplina;
  qryInsereDisc.ParamByName('cd_avaliacao').AsInteger  := qryGetAvaliacao.FieldByName('cd_avaliacao').AsInteger;

  qryInsereDisc.Open;
  if chkAI.Checked = true then
  begin
    if qryInsereDisc.IsEmpty then
    begin
      qryInsereDisc.Insert;
      qryInsereDisc.FieldByName('dt_inicio').AsDateTime := dtAvaliacao.Date;
      qryInsereDisc.FieldByName('dt_fim').AsDateTime    := dtAvaliacao.Date+
                  qryGetAvaliacao.FieldByName('nr_dias_avaliacao').AsInteger;
      qryInsereDisc.FieldByName('cd_turma').AsString    := cd_turma;
      qryInsereDisc.FieldByName('cd_disciplina').AsInteger := cd_disciplina;
      qryInsereDisc.FieldByName('cd_avaliacao').AsInteger := qryGetAvaliacao.FieldByName('cd_avaliacao').AsInteger;
      qryInsereDisc.FieldByName('nr_anosemestre').AsInteger  := StrToInt(sPAnoSem);
      qryInsereDisc.Post;
    end
    else
    begin
  ...
  begin
      if not qryInsereDisc.IsEmpty then
         qryInsereDisc.Delete;
  end;

  FreeAndNil(qryGetAvaliacao);
  FreeAndNil(qryInsereDisc);
end;
...
begin
   ...
   with qyAux do begin
      if Active then begin
         ...
         FieldByName('data').AsDateTime             := dtDataInic.DateTime;
...
var
   ...
   cd_avaliacao, logDesc: String;
   cd_avaliacao_tipo, cd_tipo, sn_especial, sn_compoe, sn_proficiencia: Integer;
begin
   { Se não possui expediente, sai fora }                                         
   if not DM.temExpedienteInstituicao( dtPData.DateTime, False, Self.iCodColigada ) then
      Exit;
...
//            if chaveDuplicada(txtChave.text, qyAux.FieldByName('cd_prova').AsInteger) then begin
...
//            end;
         ...
         begin
            ...
            begin
               sValores := sValores + ', data = TO_DATE(''' + FormatDateTime('yyyy-mm-dd', dtPData.Date)+''', ''YYYY-MM-DD'') ';
            end else begin
               sValores := sValores + ', data = ''' + FormatDateTime('yyyy-mm-dd', dtPData.Date)+'''';
            end;
         ...
         // Inserir log de alteração de aulas
         logDesc := '';
         logDesc := logDesc + 'Nro. Prova: '+qyAux.ParamByName('nro_nota').AsString+', ';
         logDesc := logDesc + 'Data: '+FormatDateTime('yyyy-mm-dd', dtPData.Date)+', ';
         logDesc := logDesc + 'Peso: '+txtPPeso.Text+', ';
         logDesc := logDesc + 'Assunto: '+txtPAssunto.Text+', ';
         ...
         logDesc := logDesc + 'Anosemestre: '+qyAux.ParamByName('anosem').AsString+', ';
         logDesc := logDesc + 'Turma: '+qyAux.ParamByName('turma').AsString+', ';
         logDesc := logDesc + 'Disciplina: '+qyAux.ParamByName('disciplina').AsString+', ';
         logDesc := logDesc + 'Etapa: '+qyAux.ParamByName('bimestre').AsString+', ';
         logDesc := logDesc + 'Grupo: '+edGrupoProvas.Text+', ';
         logDesc := logDesc + 'Tipo de Avaliação: ' + Inttostr(cd_tipo) + ' ';
         logDesc := logDesc + CHR(13) + '[Acadêmico]';
...
begin
   ...
   begin
      ...
      if DM.temExpedienteInstituicao( dtDataInic.DateTime, False, DM.getColigadaTurma(txtTurma.Text,DM.GetAnoSemestreAtual)) then
      begin
...
begin
   ...
   begin
      ...
      begin
         data := TDateTimePicker(FindComponent('dtpDataAula' + IntToStr(I)) ).Date;
      ...
      begin
         data := TDateTimePicker(sbAulas.FindComponent('dtpDataAula' + IntToStr(I))).Date;
...
//      end;
   ...
   with qyAux2 do begin
      ...
      SQL.Add(''+
         sAux + '            '+
         '  diario_provas        '+
         '(turma, cd_professor, disciplina, anosemestre, bimestre, data, assunto, sn_bloqueado, sn_proficiencia, nro_nota, sn_especial, sn_compoe, nr_dias_bloqueio, ds_chave, vl_peso, nr_nota_minima, nr_nota_maxima, cd_avaliacao_tipo, cd_grupo)   '+
         'VALUES                 '+
      '');
      ...
      for i := 0 to clbPTD.Count-1 do begin
         ...
         if clbPTD.Checked[i] then begin
            ...
            // nao é especial   
               ...
               SplitString(slPTurmaDisc[i], ':', arrInfs);
            ...
            if Connection.Protocol = 'oracle' then //Oracle
            begin
               sAux := sAux + ' TO_DATE(''' + FormatDateTime('yyyy-mm-dd', dtPData.DateTime) + ''', ''YYYY-MM-DD''), ';
            end else begin
               //MySQL
               sAux := sAux + ' "' + FormatDateTime('yyyy-mm-dd', dtPData.DateTime) + '", ';
            end;
            ...
            logDesc := '';
            logDesc := logDesc + 'Nro. Prova: '+IntToStr(iProx)+', ';
            logDesc := logDesc + 'Data: '+FormatDateTime('dd-mm-yyyy', dtPData.DateTime)+', ';
            logDesc := logDesc + 'Peso: '+txtPPeso.Text+', ';
            logDesc := logDesc + 'Assunto: '+txtPAssunto.text+', ';
            ...
            logDesc := logDesc + 'Anosemestre: '+sPAnoSem+', ';
            logDesc := logDesc + 'Turma: '+Copy(slPTurmas.Strings[i], 1, Pos(',', slPTurmas.Strings[i]) - 1)+', ';
            logDesc := logDesc + 'Disciplina: '+Copy(slPTurmas.Strings[i], LastDelimiter(',', slPTurmas.Strings[i]) + 1, Length(slPTurmas.Strings[i]))+', ';
            logDesc := logDesc + 'Etapa: '+txtPBim.Text+', ';
            logDesc := logDesc + 'Grupo: '+edGrupoProvas.Text+', ';
            logDesc := logDesc + 'Tipo de Avaliação: ' + Inttostr(cd_tipo) + ' ';
            logDesc := logDesc + CHR(13) + '[Acadêmico]';
...
begin
   ...
   begin
      ...
      inclusoTurmaProfessor := qyTurmasProfessores.RecordCount = 0;
      ...
      begin
         ...
         begin
            ...
            if ( Mensagem( 'Esta turma já possui um professor, deseja substituir?', Application.Title, MB_YESNO + MB_ICONQUESTION ) = mrYes ) then
...
begin
   ...
   begin
      if not DM.temExpedienteInstituicao( dtDataInic.DateTime, False, DM.getColigadaTurma(txtTurma.Text,DM.GetAnoSemestreAtual) ) then
      begin
         Result := False;
         Exit;
      end;
      ...
      begin
         ...
         Open();
         if (not IsEmpty) AND (FieldByName('novaaula').AsInteger > 0) then begin
            iProxAu := FieldByName('novaaula').AsInteger;
         end else begin
            if (ckbAtividadeComplementares.Visible) AND (DM.tblAvaliacaonr_inicio_aulas_extras.AsInteger > 0 ) then begin
                  if ckbAtividadeComplementares.Checked then begin
      ...
      with qyInsereAulaDados do begin
         ParamByName('turma').AsString             := txtTurma.Text;
         ParamByName('anosemestre').AsInteger      := StrToInt(getAnoSemestre());
         ParamByName('disciplina').AsInteger       := StrToInt(txtDisciplina.text);
         ParamByName('b').AsInteger                := StrToInt(txtBim.Text);
         ParamByName('nro_aula').AsInteger         := iProxAu;
         ParamByName('data').AsString              := FormatDateTime('yyyy-mm-dd', dtDataInic.DateTime);
         ...
         logDesc := logDesc + 'Data: '+FormatDateTime('dd-mm-yyyy', dtDataInic.DateTime)+', ';
   ...
   begin
      iAux := DataDiif(dtDataInic.DateTime, dtDataFin.DateTime);
      ...
      begin
         ...
         begin
            ...
            dtAtual := DataSoma(dtDataInic.DateTime, i);
         ...
         begin
            ...
            begin
               ...
               with qyAux2 do begin
                  SQL.Clear();
                  Params.Clear();
                  SQL.Add('INSERT INTO diario_aulas (turma, anosemestre, disciplina, bimestre, nro_aula, data, qtd_aulas, sn_bloqueado, cd_professor, conteudo)   ');
                  SQL.Add('VALUES ');
                  SQL.Add('(:turma, :anosemestre, :disciplina, :b, :nro_aula, :data, :qtd_aulas, :bloq, :prof, :conteudo) ');
                  ParamByName('turma').AsString          := Aula.sTurma;
                  ParamByName('anosemestre').AsInteger   := StrToInt(getAnoSemestre());
                  ParamByName('disciplina').AsInteger    := Aula.iDisciplina;
                  ParamByName('b').AsInteger             := StrToInt(txtBim.Text);
                  ...
                  logDesc := logDesc + 'Etapa: '+txtBim.Text+', Geração de aulas conforme grade horários [Acadêmico]';
...
begin
   ...
   qyInserirAula.SQL.Text := ''+
      'INSERT INTO diario_aulas (anosemestre, turma, bimestre, disciplina, nro_aula, data, qtd_aulas, sn_bloqueado, conteudo, cd_professor, dt_envio) '+
      '  VALUES ( '+
      ...
      '     DATE_FORMAT(:data_aula, "%Y-%m-%d 00:00:00"), :qtd_aulas, :sn_bloqueado, :ds_conteudo, :cd_professor, DATE_FORMAT(:data_aula, "%Y-%m-%d 00:00:00") '+
      '  )';
   ...
   // SQL que monta a grade de horários agrupados para a criação das aulas
   qyGradeHorariosAula.SQL.Text := ''+
      ...
      '  cd_professor, '+
      '  COUNT(DISTINCT cd_horario) qtd_aulas, '+
   ...
   // Verifica se foi filtrada a serie para geração das aulas
   if (ckbSerie.checked = True) then
   begin
      qyGradeHorariosAula.SQL.Add(' AND t.serie = :nr_serie ');
   end;
   ...
   if (ckbSerie.checked = True) then
   begin
      qyGradeHorariosAula.ParamByName('nr_serie').AsInteger := StrToInt(txtSerie.Text);
   end;
...
begin
   ...
   begin
      ...
      if (ckbSerie.checked = True) then
      begin
         SQL.Add(' AND t.serie = ' + txtSerie.Text + ' ');
      end;
...
begin
   ...
   begin
      ...
      begin
         ...
         dtpDtInicioDiario.DateTime := FieldByName('DT_INICIO_ETAPA').AsDateTime;
         dtpDtFimDiario.DateTime := Now;
         dtpDtFimDiario.DateTime := FieldByName('DT_FIM_ETAPA').AsDateTime;
      end;
   end;
...
begin
   if ((iDiaSemana < 8) and (iDiaSemana > 0)) then begin
      //define chave
      sAux := sTurma + ':' + IntToStr(iDisciplina) + ':' + IntToStr(iProfessor);
...
begin
   ...
   begin
      ...
      dtpDtInicioDiario.Visible := false;
      dtpDtFimDiario.Visible := false;
   ...
   // Se não existir o parametro de avaliação definindo o prazo do diário, permite o usuário informar o próprio prazo
      ...
      dtpDtInicioDiario.Visible := true;
      dtpDtFimDiario.Visible := true;
      ...
      begin
         ...
         if ( Pos( 'dtpDataAula', nomeComponente ) > 0 ) then
         begin
            if TDateTimePicker(sbAulas.Components[I]).Date <= dataInicio then
               dataInicio := TDateTimePicker(sbAulas.Components[I]).Date;

            if TDateTimePicker(sbAulas.Components[I]).Date >= dataFim then
               dataFim := TDateTimePicker(sbAulas.Components[I]).Date;
         end;
      end;
      ...
      dtpDtInicioDiario.Date := now;
      if dataInicio > 0 then
      begin
         dtpDtInicioDiario.Date := dataInicio;
      end;
      dtpDtFimDiario.Date := dataFim;
...
begin
   ...
   dataUltimaAula := dtpDataAula1.Date;

   // Verifica se a segunda aula é maior que a primeira, caso esteja ativada
   if dtpDataAula2.Enabled and (dtpDataAula2.Date > dataUltimaAula) then
   begin
      dataUltimaAula := dtpDataAula2.Date;
   end;

   for I := (sbAulas.ComponentCount - 1) downto 0 do
   begin
      nomeComponente := sbAulas.Components[I].Name;
      if ( (Pos( 'dtpDataAula', nomeComponente ) > 0) and TDateTimePicker(sbAulas.Components[I]).Enabled ) then
      begin
         if TDateTimePicker(sbAulas.Components[I]).Date >= dataUltimaAula then
         begin
            dataUltimaAula := TDateTimePicker(sbAulas.Components[I]).Date;
         end;
      end;
   end;
   dtAvaliacao.Date := dataUltimaAula;
end;
...
begin
   ...
   dtDataInic.DateTime := Now();
   dtDataFin.DateTime := Now();
...
begin
   ...
   begin
      ...
      if IAula > 2 then
      begin
         dtpDataAula := TDateTimePicker(sbAulas.FindComponent('dtpDataAula' + IntToStr(IAula)));
         cbHorarioAula := TUMComboBox(sbAulas.FindComponent('cbHorarioAula' + IntToStr(IAula)));
      end else begin
         dtpDataAula := TDateTimePicker(Self.FindComponent('dtpDataAula' + IntToStr(IAula)));
...
begin
   ...
   if pcAulas.ActivePage = tsAulas then begin
      ...
      txtBim.Text          := sPBim;
      txtSerie.Text        := sPSerie;
      ckbSerie.checked     := sPCSerie;
      udSerie.Enabled      := sPCSerie;
   end
   else if pcAulas.ActivePage = tsProvas then begin
...
begin
   ...
   if pcAulas.ActivePage = tsCronogramasGerenciar then begin
      ...
      begin
         chkAI.Checked := True;
         dtAvaliacao.DateTime := qryGetAvaliacao.FieldByName('dt_inicio').AsDateTime;
      end else begin
         chkAI.Checked := False;
      end;
...
function TfrmAulasProvasAcoes.ValidarAvaliacao: Boolean;
var
   qyPossuiAvaliacao : TUMZQuery;
begin
   DM.CriarConsulta(qyPossuiAvaliacao);

   qyPossuiAvaliacao.SQL.Text :=
      ' SELECT                            '+
      '	 cd_avaliacao                    '+
      ' FROM                              '+
      '   avl_avaliacoes                  '+
   ...
   qyPossuiAvaliacao.Open;

   if not (qyPossuiAvaliacao.FieldByName('cd_avaliacao').AsInteger > 0) then
   begin
      Result := False;
   end else begin
      Result := True;
...
procedure TfrmAulasProvasAcoes.verificarTrocaProfessor();
var
   disciplina : String;
begin
   ...
   begin
      ...
      if (not qyProfessorAntigo.Eof) and
...
function TfrmAulasProvasAcoes.TrataPrazoCalculado(sql: String; const nr_anosemestre: Integer; const cd_turma: String;
  const cd_disciplina: Integer; tipo: TiposPrazos; isUpdate: Boolean): String;
   ...
   var
      ...
      chave_altera, chave_valor: String;
   ...
   begin
      ...
      begin
         ...
         chave_valor := QuotedStr(qyBuscaAula.FieldByName('dt_aula').AsString);      
      ...
      if trim(chave_valor) = '' then
         ...
         begin
            Result := StringReplace( valor_parametro, chave_altera,
                                             'ADDDATE( ' + chave_valor + ', INTERVAL ', [rfIgnoreCase] ) + ' DAY )';
         end else begin
            Result := chave_valor;
         end;
      end;
...
begin
   ...
   begin
      ...
      end else if dtpDtInicioDiario.Visible and dtpDtFimDiario.Visible then
      begin
         // Pega data dos campos preenchidos
         valor_inicio := QuotedStr(DateToStr(dtpDtInicioDiario.Date, ConfiguracaoData));
         valor_fim := QuotedStr(DateToStr(dtpDtFimDiario.Date, ConfiguracaoData));
      end;

      if((NivelAcessoDesc[tipo] = 'ds_cronograma_visualiza_') and (alterouDtInicio)) then
      begin
         valor_inicio := FormatDateTime('yyyy-mm-dd', dtpDtInicioDiario.Date);
      ...
      begin
         valor_fim := FormatDateTime('yyyy-mm-dd', dtpDtFimDiario.Date);
      ...
      begin
         valor_inicio := CalculaData( avl_param_dt_inicio );
      end else if dtpDtFimDiario.Visible then
      begin
         // Pega data dos campos preenchidos
         valor_inicio := QuotedStr(DateToStr(dtpDtFimDiario.Date, ConfiguracaoData));
      end;
   ...
   with qyGradeCurricular do begin
      ...
      begin
         ...
         begin
            ...
            slPTurmaDisc.Add( Concat(cd_turma, ':', cd_disciplina) );
...
begin
   ...
   cd_turma := Copy(slPTurmaDisc[i], 1, Pos(':', slPTurmaDisc[i])-1);
   cd_disciplina := StrToInt(Copy(slPTurmaDisc[i], Pos(':', slPTurmaDisc[i])+1, Length(slPTurmaDisc[i])));
...
begin
   ...
   begin
      ...
      begin
         ...
         SQL.Add('   DT_INICIO_NOTAS, DT_FIM_NOTAS, DT_LIBERACAO');
         SQL.Add(') VALUES (');
         ...
         SQL.Add('   {DS_CRONOGRAMA_NOTAS_INICIO}, {DS_CRONOGRAMA_NOTAS_FIM}, {DS_CRONOGRAMA_LIBERACAO} ');
         SQL.Add(')');
      ...
      begin
         ...
         begin
            ...
            SQL.Add('INSERT INTO DIA_PRAZOS ( ');
            SQL.Add('   NM_PRAZO, NR_ANOSEMESTRE, CD_TIPO_PRAZO, DT_LIBERACAO ');
            SQL.Add(') VALUES (');
            ...
            SQL.Add('   :NR_ANOSEMESTRE, -3, {DS_CRONOGRAMA_LIBERACAO}');
            SQL.Add(')');
...
procedure TfrmAulasProvasAcoes.CriaProvaUltimaAulaCronograma(const data: TDate;
                  const cd_turma: String; const cd_disciplina, cd_professor, nr_anosemestre, cd_grupo,bimestre: Integer);
var
...
begin
   ...
   begin
      ...
      SQL.Add('      bimestre = :bimestre AND CASE WHEN :cd_grupo IS NOT NULL THEN cd_grupo = :cd_grupo ELSE 1=1 END');
   ...
   begin
      ...
      begin
         ...
         SQL.Add('INSERT INTO diario_provas(nro_nota, data, anosemestre, turma, disciplina, bimestre, assunto, cd_professor, nr_tipo_digitacao, cd_grupo) '); // nr_tipo_digitacao = 1 (Conceito)
         SQL.Add('VALUES (:nro_nota, :data, :nr_anosemestre, :cd_turma, :cd_disciplina, :bimestre, :assunto, :cd_professor, 1, :cd_grupo)');
   ...
   begin
      ...
      SQL.Add('UPDATE diario_provas SET data = :data, cd_professor = :cd_professor ');
      ...
      SQL.Add('      AND bimestre = :bimestre AND nro_nota = :nro_nota');
...
begin
   ...
   begin
      // Se for selecionado um grupo de atividade, não permite selecionar o professor
      txtAProf.Enabled := not (sPGrupo > 0);
      sbAProf.Enabled := not (sPGrupo > 0);
      // Se for selecionado um grupo de atividade, não permite criar aulas conforme grade de horários
   ...
   begin
      // Se for selecionado um grupo de atividade, não permite selecionar o professor
      Self.cbResponsavel.Enabled := not (sPGrupo > 0);
      Self.spLimpaResp.Enabled := not (sPGrupo > 0);
...
//      pcAulas.ActivePage.SetFocus();
   ...
   begin
      ...
      ckbCriaProvaUltimaAula.Checked := False;
...
begin
   ...
   msg := '';
      ...
      qyVerificaConflitoHorarios.SQL.Add('     LEFT JOIN disciplinas d ON (d.codigo = DC.disciplina)');
      ...
      qyVerificaConflitoHorarios.ParamByName('HORA_INICIO').AsDateTime := hr_inicio;
      qyVerificaConflitoHorarios.ParamByName('HORA_FIM').AsDateTime := hr_fim;
...
begin
   nCount := 0;
   sAux := '(F.SITUACAO NOT IN (3,4,5,6,7,8)) ';
   sAux2 := '1 = 1';

      if (Length(sPAnoSem) > 0) then begin
         sAux     := sAux + ' AND F.ANOSEMESTRE = ' + sPAnoSem + ' ';
         sAux2    := sAux2 + ' AND DM.ANOSEMESTRE = ' + sPAnoSem + ' ';
         nCount   := nCount + 1;
      end;
      if (Length(sPCurso) > 0) then begin
         sAux     := sAux + ' AND F.CURSO = '''+ sPCurso + ''' ';
         //sAux2 := sAux2 + 'AND dm.curso = "' + sPCurso+ '" ';
         nCount   := nCount + 1;
      end;
      if (Length(sPTurma) > 0) then begin
         sAux  := sAux + ' AND F.TURMA = ''' + sPTurma + ''' ';
         sAux2 := sAux2 + 'AND DM.TURMA = ''' + sPTurma+ ''' ';
      end;
      if (Length(sPDisciplina) > 0) then begin
         sAux  := sAux + ' AND F.DISCIPLINA = ' + sPDisciplina + ' ';
         sAux2 := sAux2 + 'AND DM.DISCIPLINA = ' + sPDisciplina + ' ';
      end;
      if (Length(sPBim) > 0) then begin
         sAux     := sAux + ' AND DA.BIMESTRE = ' + sPBim + ' ';
         sAux2    := sAux2 + ' AND DT.BIMESTRE = ' + sPBim + ' ';
         nCount   := nCount + 1;
      end;
      if (Length(IntToStr(sPGrupo)) > 0) then begin
         sAux := sAux + ' AND DA.CD_GRUPO = ' + IntToStr(sPGrupo) + ' ';
         sAux2 := sAux2 + 'AND DT.CD_GRUPO = ' + IntToStr(sPGrupo) + ' ';
      end;
      if (sPData > 0) then begin
         sAux := sAux + ' AND DATE_FORMAT(DLP.DT_AULA, ''%Y-%m-%d'') = ''' + FormatDateTime('yyyy-mm-dd', sPData) + ''' ';
      end;
      ...
      if (nCount = 3) then begin
         ...
         Freq2.AjustarFrequencia2(cbFreqForca.Checked, sAux);
         if (cbAtualizaFI.Checked) then begin
            Freq2.RecalcularFrequencia(StrToInt(sPBim), sAux2);
         end;
         frmProg.FechaBarra();
         FreeAndNil(frmProg);
         FreeAndNil(Freq2);
