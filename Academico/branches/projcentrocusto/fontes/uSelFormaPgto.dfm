object frmSelFormaPgto: TfrmSelFormaPgto
  Left = 314
  Top = 221
  Caption = 'Selecionar Forma de Pagamento...'
  ClientHeight = 324
  ClientWidth = 430
  Color = clBtnFace
  Font.Charset = ANSI_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Arial'
  Font.Style = []
  KeyPreview = True
  OldCreateOrder = True
  Position = poScreenCenter
  OnCloseQuery = FormCloseQuery
  OnKeyDown = FormKeyDown
  OnPaint = FormPaint
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 14
  object Bevel1: TBevel
    Left = 11
    Top = 40
    Width = 406
    Height = 5
    Style = bsRaised
  end
  object Label3: TLabel
    Left = 15
    Top = 14
    Width = 52
    Height = 14
    Caption = 'Descri'#231#227'o:'
  end
  object btnFechar: TSpeedButton
    Left = 328
    Top = 296
    Width = 89
    Height = 22
    Caption = 'F12 Fechar'
    Flat = True
    Glyph.Data = {
      
      1F7C1F7C1F7C}
    OnClick = SpeedButton1Click
  end
  object grd: TDBGrid
    Left = 10
    Top = 56
    Width = 407
    Height = 233
    BorderStyle = bsNone
    DataSource = srcCaixa
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Arial'
    Font.Style = []
    Options = [dgTitles, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit]
    ParentFont = False
    PopupMenu = pmQtd
    TabOrder = 0
    TitleFont.Charset = ANSI_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'Arial'
    TitleFont.Style = []
    OnDblClick = grdDblClick
    OnKeyPress = grdKeyPress
    Columns = <
      item
        Expanded = False
        FieldName = 'cd_forma_pgto'
        Width = 60
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'ds_forma_pgto'
        Visible = True
      end>
  end
  object edDescricao: TEdit
    Left = 75
    Top = 10
    Width = 342
    Height = 22
    ParentShowHint = False
    ShowHint = True
    TabOrder = 1
    OnKeyPress = edDescricaoKeyPress
  end
  object qyFormaPgto: TUMZQuery
    Connection = DM.db
    Filtered = True
    SQL.Strings = (
      'SELECT                        '
      '  * '
      'FROM'
      '  fin_formas_pgto'
      'ORDER BY'
      '  ds_forma_pgto')
    Params = <>
    Left = 48
    Top = 296
    object qyFormaPgtocd_forma_pgto: TIntegerField
      DisplayLabel = 'C'#243'd. Forma'
      FieldName = 'cd_forma_pgto'
      Origin = 'WMESTRE12.fin_formas_pgto.cd_forma_pgto'
    end
    object qyFormaPgtods_forma_pgto: TStringField
      DisplayLabel = 'Descri'#231#227'o'
      FieldName = 'ds_forma_pgto'
      Origin = 'WMESTRE12.fin_formas_pgto.ds_forma_pgto'
      Size = 40
    end
    object qyFormaPgtosn_cadastra_cheque: TSmallintField
      FieldName = 'sn_cadastra_cheque'
      Origin = 'WMESTRE12.fin_formas_pgto.sn_cadastra_cheque'
    end
    object qyFormaPgtosn_ativo: TSmallintField
      FieldName = 'sn_ativo'
      Origin = 'WMESTRE12.fin_formas_pgto.sn_ativo'
    end
    object qyFormaPgtosn_compensa_auto: TSmallintField
      FieldName = 'sn_compensa_auto'
      Origin = 'WMESTRE12.fin_formas_pgto.sn_compensa_auto'
    end
  end
  object srcCaixa: TDataSource
    DataSet = qyFormaPgto
    Left = 16
    Top = 296
  end
  object pmQtd: TPopupMenu
    OnPopup = pmQtdPopup
    Left = 664
    Top = 264
  end
end
