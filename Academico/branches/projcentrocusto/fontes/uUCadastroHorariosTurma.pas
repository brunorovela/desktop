...
type
   ...
   TExecucaoHorarioTurma = class(AExecucao)
      strict private
         ...
         FAnosemestre, FHorario, FDiaSemana : Integer;
         FLegenda: String;
      ...
      public
         constructor Create(const LTipoExecucao: TTipoExecucao;
            ...
            const ds_legenda: String = ''; const sala: TSala = nil);
   ...
   THorarioDiaSemana = class(THorario)
      ...
      public
         constructor Create(const cd_horario: Integer; const ds_horario, hr_inicio: String); overload;
   ...
   THorariosTurmaGrid = class(THorariosTurma)
      ...
      public
         ...
         function GetListaExecucao( const nr_anosemestre: Integer ): TListaExecucao;
...
constructor THorarioDiaSemana.Create(const cd_horario: Integer;
  const ds_horario, hr_inicio: String);
var
   I : Integer;
begin
   inherited Create(cd_horario, ds_horario, hr_inicio);
...
begin
   ...
   New := THorarioDiaSemana.Create(Horario.Codigo, Horario.Descricao, Horario.HoraInicio);
...
function THorariosTurmaGrid.GetListaExecucao( const nr_anosemestre: Integer ): TListaExecucao;
var
   ...
   begin
      ...
      begin
         ...
         begin
            ...
            begin
               Result.Add(TExecucaoHorarioTurma.Create( teDelete, LListaDisciplinasBkp.Items[K], nr_anosemestre, HorarioDiaSemana.Codigo, KDiaSemana ));
            end;
         end;
         ...
         begin
            ...
            begin
               // Foi inserido
               Result.Add(TExecucaoHorarioTurma.Create( teInsert, LListaDisciplinas.Items[K], nr_anosemestre, HorarioDiaSemana.Codigo, KDiaSemana, LListaDisciplinas.adicionais.ds_legenda, LListaDisciplinas.adicionais.sala ));
            end else begin
               ...
               begin
                  Result.Add(TExecucaoHorarioTurma.Create( teUpdate, LListaDisciplinas.Items[K], nr_anosemestre, HorarioDiaSemana.Codigo, KDiaSemana, LListaDisciplinas.adicionais.ds_legenda, LListaDisciplinas.adicionais.sala ));
               end;
...
constructor TExecucaoHorarioTurma.Create(const LTipoExecucao: TTipoExecucao;
  const LDisciplina: TDisciplina; const nr_anosemestre, cd_horario, nr_dia_semana: Integer;
  const ds_legenda: String; const sala: TSala);
begin
...
const
   OBS_INCLUSAO = 'Incluído nova disciplina para a turma "%s"%s, no horário "%d", no dia da semana "%s", disciplina "%s"';
   OBS_EXCLUSAO = 'Exclusão da disciplina "%s" da turma "%s"%s, no horário "%d", no dia da semana "%s"';
   OBS_ALTERACAO = 'Alterada legenda do horário "%d", no dia da semana "%s", da turma "%s"%s, da disciplina "%s". Para: "%s"';
...
begin
   ...
   case FTipoExecucao of
      teInsert: begin
         Tipo := 'Inclusao';
         Obs := Format(OBS_INCLUSAO, [Turma, DescTurmaBase, FHorario, gaDiasSemana[FDiaSemana], FDisciplina.Descricao]);
      end;
      teDelete: begin        
         Tipo := 'Exclusao';
      ...
      teUpdate: begin
         Tipo := 'Alteracao';
         Obs := Format(OBS_ALTERACAO, [FHorario, gaDiasSemana[FDiaSemana], Turma, DescTurmaBase, FDisciplina.Descricao, FLegenda]);
