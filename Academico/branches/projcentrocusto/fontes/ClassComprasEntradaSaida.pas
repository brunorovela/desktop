   ...
   TClasseComprasEntradaSaida = class(TObject)
      private
         ...
         function GerarMensalidade(const cd_pessoa: Integer; const cd_resp_finan: Integer;
               const cd_compra: Integer = 0;  cd_produto: String = ''; const cd_kit : Integer = -1): Integer;
         ...
         function MensalidadeFromCompra( const cd_compra: Integer ): Integer;
      public
         ...
         function GerarTitulo( const cd_compra: Integer; const sn_gerar_mensalidade: Boolean = False ): Integer;
         function GerarTitulosAcumulados( cd_pessoa: Integer; cd_produto: String = ''; const cd_kit : Integer = -1 ): Integer;
...
function TClasseComprasEntradaSaida.GerarMensalidade( const cd_pessoa: Integer;
   const cd_resp_finan: Integer; const cd_compra: Integer = 0; cd_produto: String = ''; const cd_kit : Integer = -1): Integer;
const
   // Mysql
   SQL_GERAR_MENSALIDADE_CAMPOS =
      ' SELECT CA.CD_COMPRA, SUM(CE.VL_COMPRA) AS VL_COMPRA, SUM(CE.NR_QUANTIDADE) AS NR_QUANTIDADE, ' +
      '        CP.DS_PRODUTO, CC.CD_TIPO_TITULO ';
   SQL_GERAR_MENSALIDADE_CAMPOS_KIT =
      ' SELECT CA.CD_COMPRA, SUM(CE.VL_COMPRA) AS VL_COMPRA, ' +
      '        ROUND(SUM(CE.NR_QUANTIDADE) / (SELECT SUM(NR_QUANTIDADE) FROM COMP_KITS_ITENS WHERE CD_KIT = :CD_KIT)) AS NR_QUANTIDADE, ' +
      '        CK.NM_KIT AS DS_PRODUTO, CC.CD_TIPO_TITULO ';

   SQL_GERAR_MENSALIDADE_CAMPOS_COMPRA =
      ' SELECT CE.CD_COMPRA, SUM(CE.VL_COMPRA) AS VL_COMPRA, SUM(CE.NR_QUANTIDADE) AS NR_QUANTIDADE, ' +
      '        CP.DS_PRODUTO, CC.CD_TIPO_TITULO ';
...
begin
   ...
   try
      nr_anosemestre := StrToInt( DM.variavel_parametro('nr_anosem_atual') );
   except
   ...
   begin
      ...
      {if ( DM.variavel_parametro( 'agrupar_mensalidades_proximo_mes' ) = 'S' ) then
         dt_vencimento := GetDataVencimento( nr_anosemestre, cd_curso, cd_turma, cd_pessoa );}
   end else begin
...
function TClasseComprasEntradaSaida.GerarTitulo(
  const cd_compra: Integer; const sn_gerar_mensalidade: Boolean = False ): Integer;
const
   SQL_GERAR_TITULO =
...
begin
   ...
   if qyGerarTitulo.FieldByName('vl_compra').AsInteger = 0 then Exit;
   ...
   begin
      ...
      GerarMensalidade( qyGerarTitulo.FieldByName('cd_pessoa').AsInteger, 0, cd_compra );
   end else begin
...
function TClasseComprasEntradaSaida.GerarTitulosAcumulados(
  cd_pessoa: Integer; cd_produto: String = ''; const cd_kit : Integer = -1): Integer;
var
...
const
   SQL_GET_CUPONS_MENSALIDADE =
      ...
      '      LEFT JOIN mensalidades m ON (m.cd_mensalidade = ce.cd_mensalidade) ' +
...
const
   SQL_GET_CUPONS_MENSALIDADE =
      ...
      '        GET_PARAMETROS_COLIGADA( ''cliente_telefone'', c.cd_coligada ) AS cliente_telefone ' +
      ' FROM mensalidades m ' +
      '      INNER JOIN comp_estoque ce ON (ce.CD_MENSALIDADE = m.cd_mensalidade) ' +
...
const
   SQL_HAS_COMPRA_FROM_MENSALIDADE =
      ' SELECT cd_compra FROM comp_estoque WHERE cd_mensalidade = :cd_mensalidade ';
var
...
const
   SQL_HAS_MAIS_COMPRAS_MENSALIDADE =
      ...
      ' FROM comp_estoque ce ' +
      '      INNER JOIN comp_estoque cee ON (cee.CD_MENSALIDADE = ce.CD_MENSALIDADE) ' +
...
const
   SQL_IS_MENSALIDADE_FROM_PRODUTO =
      ' SELECT count(*) AS registros FROM comp_estoque ' +
...
const
   SQL_LISTA_COMPRAS =
      'SELECT CD_COMPRA FROM (SELECT CD_COMPRA FROM comp_estoque WHERE cd_mensalidade = (SELECT cd_mensalidade FROM comp_estoque WHERE CD_COMPRA = :CD_COMPRA) AND CD_COMPRA != :CD_COMPRA) TABELA';
   ...
   SQL_LIMPA_VINCULO_COMPRAS_MENSALIDADE_ESTOQUE =
      ' UPDATE COMP_ESTOQUE SET CD_MENSALIDADE = NULL WHERE CD_COMPRA IN ( ' + SQL_LISTA_COMPRAS + ' ) ';
var
...
function TClasseComprasEntradaSaida.MensalidadeBaixadaBanco(
  const cd_mensalidade: Integer): Boolean;
const
   SQL_MENSALIDADE_BAIXADA_BANCO =
      ' SELECT COUNT(*) AS registros FROM itensretorno ' +
      ' WHERE cd_mensalidade = :cd_mensalidade AND sn_baixado = 1 ';
var
   qyMensalidadeBaixadaBanco : TUMZQuery;
begin
   DM.CriarConsulta(qyMensalidadeBaixadaBanco);

   qyMensalidadeBaixadaBanco.SQL.Text := SQL_MENSALIDADE_BAIXADA_BANCO;
   qyMensalidadeBaixadaBanco.ParamByName('cd_mensalidade').AsInteger := cd_mensalidade;
   qyMensalidadeBaixadaBanco.Open();

   Result := ( qyMensalidadeBaixadaBanco.FieldByName('registros').AsInteger >= 1 );

   FreeAndNil(qyMensalidadeBaixadaBanco);
end;

function TClasseComprasEntradaSaida.MensalidadeFromCompra(
  const cd_compra: Integer): Integer;
const
   SQL_MENSALIDADE_FROM_COMPRA =
      ' SELECT CASE WHEN cp.SN_GERAR_TITULO = 1 AND ce.CD_MENSALIDADE IS NOT NULL THEN ' +
      '           ce.CD_MENSALIDADE ' +
      '        ELSE -1 END AS CD_MENSALIDADE ' +
      ' FROM comp_estoque ce ' +
      '      INNER JOIN comp_produtos cp ON ( cp.cd_produto = ce.cd_produto ) ' +
      ' WHERE CD_COMPRA = :CD_COMPRA ';
var
   qyMensalidadeFromCompra: TUMZQuery;
begin
   DM.CriarConsulta(qyMensalidadeFromCompra);
   qyMensalidadeFromCompra.SQL.Text := SQL_MENSALIDADE_FROM_COMPRA;
   qyMensalidadeFromCompra.ParamByName('CD_COMPRA').AsInteger := cd_compra;
   qyMensalidadeFromCompra.Open();

   Result := qyMensalidadeFromCompra.FieldByName('CD_MENSALIDADE').AsInteger;

   FreeAndNil(qyMensalidadeFromCompra);
end;
...
const
   ...
   SQL_PODE_IMPRIMIR_CUPOM_MENSALIDADE = 
      ' SELECT count(*) AS registros ' +
      ' FROM comp_estoque ce ' +
      ' WHERE ce.CD_MENSALIDADE = :cd_compra_mensalidade AND ' +
      '       ce.SN_PRIMEIRA_IMPRESSAO = 0 ';
var
...
begin
   ...
   // A programação abaixo foi retirada, pois não deve ser feita a verificação por permissão,
   // obrigatoriamente a mensalidade precisa ser estornada
   // OR
   //( DM.UsuarioLogado.TemPermissao( 0, 'UMFinanceiro.PlanilhaRecebimentos.Imprimir.CuponsProdutos', npEspecial, False ) )
...
const
   SQL_MENSALIDADE_COMPRA =
      ' UPDATE COMP_ESTOQUE SET CD_MENSALIDADE = :CD_MENSALIDADE WHERE CD_MENSALIDADE IS NULL AND TP_ENTRADA_SAIDA IN ( 2 ) AND ';
var
   qyMensalidadeCompra: TUMZQuery;
begin
   DM.CriarConsulta(qyMensalidadeCompra);

   qyMensalidadeCompra.Close();

   qyMensalidadeCompra.SQL.Text := SQL_MENSALIDADE_COMPRA;
   qyMensalidadeCompra.ParamByName('CD_MENSALIDADE').AsInteger := cd_mensalidade;

   if is_produto then
   begin
      qyMensalidadeCompra.SQL.Add('CD_PRODUTO = :CODIGO AND CD_PESSOA = :CD_PESSOA');
      qyMensalidadeCompra.ParamByName('CD_PESSOA').AsInteger := cd_pessoa;
   end else begin
      qyMensalidadeCompra.SQL.Add('CD_COMPRA = :CODIGO');
   end;
   
   qyMensalidadeCompra.ParamByName('CODIGO').AsString := cd_compra_produto;
...
const
   SQL_ZERA_IMPRESSAO =
      ' UPDATE COMP_ESTOQUE SET SN_PRIMEIRA_IMPRESSAO = 0 ' +
      ' WHERE CD_MENSALIDADE = :CD_MENSALIDADE ';
var
