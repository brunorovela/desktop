...
type
  TfrmCursos = class(TForm)
    ...
    procedure qyCursosColigadasAfterPost(DataSet: TDataSet);
...
//   property cd_coligada_atual: Integer read getColigadaAtual;
  public
    { Public declarations }
...
uses
   ...
   uSala, uTurma, uFSelecionarTurma, uProcessando;
...
begin
   ...
   if (Agrupado and (Coligada > 0)) or ((not Agrupado) and (Coligada = 0)) then
   begin
...
const
   SMsgAttentionCaption = 'Atenção';
   SMsgQuestCaption = 'Confirmação';
   SMsgInfoCaption = 'Informação';
   SMsgQuestChangeAll = 'Deseja alterar o código da turma para todos os anos/semestres?';
   SMsgQuestConfirmChangeAll = 'Tem certeza que deseja realizar a alteração do código da turma?'#13#13'De: %s'#13'Para: %s'#13#13'em todos os anos/semestres?';
   SMsgQuestConfirmChange = 'Tem certeza que deseja realizar a alteração do código da turma?'#13#13'De: %s'#13'Para: %s'#13#13'apenas no ano/semestre %d?';
   SMsgDuplicateClass = 'Já existe uma turma com esse código. Não será possível realizar a alteração';
   SMsgSuccess = 'Código da turma alterado com sucesso.';
   SMsgErrorInfo = 'Alteração do código da turma não concluído.'#13' A seguir, será apresentada uma tela de erro ao qual deverá ser encaminhada ao Atendimento.';
   SMsgError = 'Alteração do código da turma não concluído.'#13#13'Foram encontradas inconformidades nas seguintes tabelas:'#13'%s';
   ...
   SSQLDisableForeignKeys = 'SET FOREIGN_KEY_CHECKS=0;';
   SSQLEnableForeignKeys = 'SET FOREIGN_KEY_CHECKS=1;';
   SSQLGetTables = 'SHOW TABLE STATUS FROM %s';
   SSQLGetColumns = 'SHOW FIELDS FROM %s';
   SSQLSearchClass = 'SELECT CODIGO FROM TURMAS WHERE CODIGO LIKE ?';
   SSQLSearchClassPeriod = SSQLSearchClass + ' AND ANOSEMESTRE = ?';
   SSQLUpdClass = 'UPDATE %s SET %s = trim(%s) WHERE trim(%s) = trim(%s)';
   SSQLUpdClassPeriod = 'UPDATE %s SET %s = trim(%s) WHERE trim(%s) = trim(%s) AND %s = %d';

   SLogChangeAll = 'Mudança de código de turma. De: [%s] Para: [%s] em todos os anos/semestres';
   SLogChange = 'Mudança de código de turma. De: [%s] Para: [%s] apenas para o ano/semestre [%d]';
   SLogErroUpdateTurmas = '[ERRO] Mudança de código de turma. Erro ao alterar o código De: [%s] Para: [%s]. ' + #13 +
...
var
   ...
   begin
      ...
      FieldsCode.DelimitedText :=
         ...
         'TURMADEPENDENCIA;TURMAMAT;TURMAMATRICULA;TURMA_BASE;DS_CENTRO;';
...
begin
   ...
   Change := InputQueryEvento('UNIMESTRE', 'Digite o novo código da turma:', NewCode, realizaExpressaoRegular);
   ...
   if Change and (not SameText(CurrCode, NewCode)) then
   begin
      ListaTabelasErro := TStringList.Create;

      // Se utiliza ano/semestre pergunta ao usuário se deseja que a alteração
      // seja replicada para todos os ano/semestres
      
      if DM.UsaAnosemestre then
      begin
         All := MessageBox(Handle, PChar(SMsgQuestChangeAll),
            PChar(SMsgQuestCaption), MsgFlagsQuestion) = ID_YES;
      end
      else
         All := True;
      ...
      if All then
         MsgConfirm := Format(SMsgQuestConfirmChangeAll, [CurrCode, NewCode])
      else
         MsgConfirm := Format(SMsgQuestConfirmChange, [CurrCode, NewCode, Period]);

      if MessageBox(Handle, PChar(MsgConfirm), PChar(SMsgQuestCaption),
         MsgFlagsWarning) = ID_YES then
      begin
         ...
         if All then
            Statement := PrepareStatement(SSQLSearchClass)
         else
         begin
            Statement := PrepareStatement(SSQLSearchClassPeriod);
         ...
         begin
            ...
            Execute(SSQLDisableForeignKeys);
            ...
            // Percorre as tabelas para verificar quais
            // fazem referência ao código de turma
            ...
            begin
               ...
               begin
                  ...
                  IgnoreTables.Sort;
                  if not IgnoreTables.Find(ResultSet.GetStringByName('Name'), I) then
                  begin

                     // Recupera todas as colunas da tabela
                     RsFields := ExecuteQuery(Format(SSQLGetColumns,
                        [ResultSet.GetStringByName('Name')]));

                     // Se a alteração é para ano/semestre específico verifica se
                     // a tabela possui campo que armazena tal informação para
                     // aplicar o filtro na atualização do valor
                     ...
                     while RsFields.Next do
                     begin
                        if All or (KeyFieldSemester <> '') or (TabelasSQLsEspecificos.Values[ResultSet.GetStringByName('Name')] <> '') then
                        begin
                           ...
                           begin
                              ...
                              if All then
                                 ...
                                 begin
                                    SQLUpdate := Format(SSQLUpdClassPeriod, [
                                       ResultSet.GetStringByName('Name'),
                                       KeyField,
                                       QuotedStr(NewCode),
                                       KeyField,
                              ...
                              // Verifica se o nome do campo é diferente de
                              // código e força a atualização
                              // depois quebra a repetição
                              ...
                              begin
                                 try
                                    ...
                                    begin
                                       if (ResultSet.GetStringByName('Name') = 'fichaindividual') and All then
                                       begin
                                          SQLUpdate := Format(SSQLUpdClass, [
                                             ResultSet.GetStringByName('Name'),
                                             'turmamatricula',
                                       ...
                                       else if (ResultSet.GetStringByName('Name') = 'fichaindividual') and not All then
                                       begin
                                          SQLUpdate := Format(SSQLUpdClassPeriod, [
                                             ResultSet.GetStringByName('Name'),
                                             'turmamatricula',
                                    ...
                                    begin
                                       ...
                                       if (ResultSet.GetStringByName('Name') = 'turmas_horarios_config') and All then
                                       begin
                                          SQLUpdate := Format(SSQLUpdClass, [
                                             ResultSet.GetStringByName('Name'),
                                             'cd_turma_base',
                                       ...
                                       else if (ResultSet.GetStringByName('Name') = 'turmas_horarios_config') and not All then
                                       begin
                                          SQLUpdate := Format(SSQLUpdClassPeriod, [
                                             ResultSet.GetStringByName('Name'),
                                             'cd_turma_base',
                                    ...
                                    begin
                                       DM.setLog(
                                          1006, 'Alteracao', NewCode, tblTurmascd_coligada.AsInteger,
                                          Format(SLogErroUpdateTurmas, [CurrCode, NewCode, E.Message, SQLUpdate])
                                       );
            ...
            if All then
               MsgLog := Format(SLogChangeAll, [CurrCode, NewCode])
            else
               MsgLog := Format(SLogChange, [CurrCode, NewCode, Period]);
            ...
            begin
               ...
               Mensagem( SMsgErrorInfo, SMsgAttentionCaption, MB_OK + MB_ICONEXCLAMATION, Handle );
               raise Exception.Create(Format(SMsgError, [STabelasErro]));
            end else begin
               MessageBox(Handle, PChar(SMsgSuccess), PChar(SMsgInfoCaption),
                  MsgFlagsInfo);
            end;
         end
         else
            MessageBox(Handle, PChar(SMsgDuplicateClass),
               PChar(SMsgAttentionCaption), MsgFlagsAttention);
...
begin
   ...
   case TTabSheet(pcGeral.ActivePage).PageIndex of
      ...
      2 : Begin
         if not DM.UsuarioLogado.TemPermissao( 1112, npAlterar, True ) then Exit;
...
begin
   ...
   case TTabSheet(pcGeral.ActivePage).PageIndex of
      ...
      1 : begin
         ...
         if bAlterar then
         begin
            if qrySituacoesTurmascd_situacao.AsInteger in [3, 2] then
            begin
...
begin
   ...
   sCodigos := sCodigos + 'cd_curso_destino, grupo_curso, filtro_curso, ds_curso';
   ...
   begin
      ...
      flg := InputQuery('UNIMESTRE', 'Digite o novo código do Curso: ', sNovoCodigo);

      DM.CriarConsulta(qVerificaCodigoExistente);

      qVerificaCodigoExistente.SQL.Text := 'SELECT count(*) AS qtd FROM cursos_mestre ';
      qVerificaCodigoExistente.SQL.Add(Format(' WHERE cd_curso = %s ', [QuotedStr(sNovoCodigo)]));
...
begin
   ...
   begin
      case TTabSheet(pcGeral.ActivePage).PageIndex of
         ...
         // Delete Disciplinas
         2 : begin
            if not DM.UsuarioLogado.TemPermissao( 1112, npExcluir, True ) then Exit;
            // Verifica se disciplina está alocada para algum Professor ou Aluno
            if(DM.disciplinaAlocada(tblDisc.FieldByName('codigo').AsInteger, tblDisc.FieldByName('curso').AsString) = true) then
            begin
...
begin
   ...
   if (DM.UsaAnosemestre) then
   begin
      fCopiaTurma.carregarTela(qyColigadacd_coligada.AsInteger, tblTurmascd_coligada.AsInteger, coligadas, tblTurmasCurso.AsString, UMFiltroDataAnoSem.getAnosemestre())
   end
   else
   begin
      fCopiaTurma.carregarTela(qyColigadacd_coligada.AsInteger, tblTurmascd_coligada.AsInteger, coligadas, tblTurmasCurso.AsString, ANOSEMESTRE_PADRAO);
   end;
...
begin
   ...
   begin
      DM.setLog(1006, 'Inclusao', tblTurmasCodigo.AsString + ';' + tblTurmasAnoSemestre.AsString, tblTurmascd_coligada.AsInteger, tblTurmasdescricao.AsString);
   end else begin
      DM.setLog(1006, 'Alteracao', tblTurmasCodigo.AsString + ';' + tblTurmasAnoSemestre.AsString, tblTurmascd_coligada.AsInteger, tblTurmasdescricao.AsString);
   end;
   try
      if tblTurmascd_sala.AsInteger <> tblTurmascd_sala.OldValue then
      begin
...
begin
   ...
   if qyVerificarMatriculas.FieldByName('qtd_alunos').AsInteger > 0 then
   begin
      Result := False;
      // Apresenta mensagem do porque não é possível trocar a grade
      Mensagem('Não é possível trocar a grade curricular da turma.'+#13+'Existem alunos matriculados nessa turma em disciplinas que não estão sendo oferecidas na mesma série na grade curricular selecionada.', 'Aviso', MB_ICONEXCLAMATION+MB_OK);
      Exit;
   end;
...
begin
   ...
   cbNrEtapas.Enabled := (cbNrEtapas.Items.Count > 0);
   { Combo Nr. Etapa }
   ...
   begin
     ...
     qyListaProximaTurma.ParamByName('nr_anosemestre').AsString := DM.incrementar_ano_semestre(tblTurmas.FieldByName('anosemestre').AsString);
...
begin
  ...
  dbcbColigadas.Visible := (qyColigadacd_coligada.AsInteger < 1) AND (TDataSource( Sender ).State in [dsInsert]);
  lbColigada.Visible := not ((qyColigadacd_coligada.AsInteger < 1) AND (TDataSource( Sender ).State in [dsInsert]));
...
begin
   ...
   if Mensagem('Tem certeza que deseja excluir essa disciplina da grade?', 'Confirmação', MB_YESNO + MB_ICONQUESTION, Handle) <> mrYes then Exit;

   DM.qAux1.Close;
   DM.qAux1.Sql.Text := ' select codigograde from fichaindividual f INNER JOIN TURMAS t ON (t.codigo = f.turma and f.anosemestre = t.anosemestre) ' +
	                     ' INNER JOIN GRADES g ON (g.cd_grade = t.cd_grade and t.curso = g.CD_CURSO) where g.cd_grade = :cd_grade AND g.cd_curso = :cd_curso AND f.disciplina= '+qyGradesDisciplinasCD_DISCIPLINA.AsString+
   ...
   Else
      Mensagem('Não é possível excluir essa disciplina da grade porque contém uma matrícula relacionada', 'Atenção', MB_OK + MB_ICONWARNING, Handle);
...
begin
  case Column.Index of
    0 : strAux := 'order by Codigo';
    1 : strAux := 'Order by Sigla';
    2 : strAux := 'Order by Descricao';
  end;

  tblDisc.Close;
  tblDisc.SQL.Clear;
  tblDisc.SQL.Add( 'Select * From Disciplinas where Curso = :Codigo' );
  tblDisc.SQL.Add( strAux );
...
begin
  ...
  // Seleciona o Curso da coligada atual
  carregarCursoColigada();
...
//  qyListaGrades.close();
//  qyListaGrades.ParamByName('CD_CURSO').AsString := qyCursosMestreCD_CURSO.AsString;
//  qyListaGrades.Open();

  { Carregar combo de disciplinas mestre (pai) }
//   carregarDisciplinasPai();

  SqlAtosOficiais := 'SELECT * FROM cursos_atos_oficiais WHERE ';

  qyAtosOficiais.Close;

  qyAtosOficiais.SQL.Clear;

  SqlAtosOficiais := SqlAtosOficiais + '  cd_curso = ' + QuotedStr(qyCursosMestreCD_CURSO.AsString);

  //if not DM.VerificaUsuarioEstaTodasMatrizes( DM.iCdPessoaLogado ) then begin
      SqlAtosOficiais := SqlAtosOficiais + ' AND (cd_coligada IN (' + DM.GetColigadas + ') OR (cd_coligada IS NULL))';
  //end;

  SqlAtosOficiais := SqlAtosOficiais + ' ORDER BY nr_anosemestre DESC';

  qyAtosOficiais.SQL.Text := SqlAtosOficiais;
...
begin
   if pcEmenta.ActivePageIndex in [1, 2,3] then
   begin
      ...
      1: begin // Turmas
         ...
         begin
            ...
            Exit;
             ...
             lbCurso.Caption := qyCursosMestreDS_CURSO.AsString;
...
begin
  ...
  { Todas em maiúsculas }
   ...
   if qyVerificaMaisculo.FieldByName('ds_valor').asString = 'N' then
   begin
      DM.TodasMaiusculas(TForm(Sender), 'dbDescDisc');
   end
   else
   begin
   ...
   Label55.Top := 69;
   dbeProxCurso.Top := 85;
   dbeProxTurmaOutroCurso.Top := 85;
   sbBuscaProxTurmaOutroCurso.Top := 85;
   sbLimpaProxTurmaOutroCurso.Top := 85;
...
begin
   if (not dbEmentaDisc.Focused) AND (not dbreBibBasica.Focused) AND (not dbreBibComp.Focused) AND (not dbreBibSup.Focused) AND (not dbObjetivo.Focused) then
      DBEdit5.SetFocus;
end;
...
begin
   if (tblEmentas.State in dsEditModes) and
      (pcEmenta.ActivePageIndex in [1, 2,3]) then
   begin
...
begin
   ...
   resultado_filtro := TfrmSelecionarTurma.Filtrar([],
      StrToInt(DM.incrementar_ano_semestre(tblTurmasAnoSemestre.AsString)), tblTurmascd_coligada.AsInteger);
...
procedure TfrmCursos.qyCursosColigadasAfterPost(DataSet: TDataSet);
begin
   qyCursosMestre.Refresh;
end;
...
begin
   ...
   if (qyGradesDisciplinasNR_CARGA_HORARIA_PRATICA.AsFloat > 0) OR
      (qyGradesDisciplinasNR_CARGA_HORARIA_TEORICA.AsFloat > 0) then
   begin
...
begin
  qyAtosOficiais.FieldByName('dt_ato').AsString := DateToStr(frmAtosOficiais.dtpAto.DateTime);
end;
...
procedure TfrmCursos.ToolButton7Click(Sender: TObject);
begin
   if Mensagem('O sistema irá atualizar a grade curricular nas matrículas já realizadas.' + #13 + 'Deseja prosseguir?', 'Informação', MB_YESNO + MB_ICONEXCLAMATION, Handle) <> mrYes then Exit;

   qryUpdateGrade.ExecSQL;
   
   Mensagem('Códigos de grades atualizados com sucesso!', 'Informação', MB_OK, Handle);
end;
