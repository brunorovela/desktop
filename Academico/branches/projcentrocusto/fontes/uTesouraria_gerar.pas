...
uses
  ...
  uFinApropriaTesouraria, Contnrs;

type

  TEstado = (Inserir, Alterar, Estornar, Confirmar, AlteracaoSimplificada);
  ...
  private
    ...
    Function RegistrarApropriacao(CodigoMovimento : Integer)  : boolean;
    Function AtualizarApropriacao(CodigoMovimento : Integer)  : boolean;

    Function ProcuraCodigo(codigo : Integer; ListaCodigos : Array of Integer) : Integer;

    Procedure ResgataApropriacao(CodigoMovimento : Integer);
  ...
  public
    ...
    cd_coligada_selecionada: Integer;
...
uses uClassMovimento, Math, uSelConta, uEstorno, uUsuario, uListaColigadas,
   uColigada, uTesouraria_Planilha, UCompromisso_apropria_altera_caixa;
...
begin
   ...
   dtpLancamento.Date := Date;

   if tipoEstado in [Estornar, Alterar, Confirmar, AlteracaoSimplificada] then
   begin
       ...
       dtpLancamento.DateTime := DataLancamento;
...
begin
   ...
   if tipoEstado in [Alterar,AlteracaoSimplificada] then
   begin
      RegistrarApropriacao(CodigoMovimento);
      ...
      qyAltera.ParamByName('DtMovimento').AsDate := dtpLancamento.Date;
      ...
      if CodigoMensalidade > 0 then begin
         ...
         DM.qAux2.Close();
         DM.qAux2.SQL.Text :=
           ' UPDATE mensalidades SET dt_credito = :dt_credito ' +
           '  WHERE cd_mensalidade = ' + IntToStr(CodigoMensalidade) +
           '    AND situacao in (0,1) ';
         DM.qAux2.ParamByName('dt_credito').AsString := FormatDateTime('yyyy-mm-dd', dtpLancamento.DateTime);
         Dm.qAux2.ExecSQL();
      ...
      if CodigoTransfere > 0 then begin
         ...
         DM.qAux2.ParamByName('dt_movimento').AsString := FormatDateTime('yyyy-mm-dd', dtpLancamento.DateTime);
         Dm.qAux2.ExecSQL();

      end;

      sChave :=  IntToStr(CodigoMovimento) + ';';
   ...
   if tipoEstado = Estornar then begin
      ...
      qyAltera.SQL.Text :=
        ...
        ' WHERE cd_movimento_te in (' + IntToStr(CodigoMovimento) + ', ' + IntToStr(iCodMovimento) + ') ';
      qyAltera.ExecSQL();

      // O estorno de um lançamento, deve excluir suas apropriações
      qyAltera.SQL.Text :=
        ' DELETE from fin_apropria_te      ' +
        ' WHERE cd_movimento_te = ' + IntToStr(CodigoMovimento);
      qyAltera.ExecSQL();
...
begin
   ...
   // Bloqueio de Data;
   if DM.EstaBloqueado(dtpLancamento.Date, true) then Exit;



   if  not (tipoEstado in [Estornar, Alterar, Confirmar, AlteracaoSimplificada] )then
   begin
...
begin
   ...
   begin
      ...
      DataMovimento   := dtpLancamento.Date;
...
function Tfrm_Tesouraria_gerar.RegistrarApropriacao(CodigoMovimento : Integer): boolean;
var
   i : Integer;
   apropria : TUMZQuery;
begin
   ...
   apropria.ParamByName('cd_movimento_te').AsInteger := CodigoMovimento;
   apropria.ExecSQL;
   ...
   begin
     ...
     begin
       ...
       begin
           ...
           ParamByName('cd_movimento_te').AsInteger := CodigoMovimento;
...
procedure Tfrm_Tesouraria_gerar.ResgataApropriacao( CodigoMovimento: Integer );
var
   Apropria : TUMZQuery;
begin
...
begin
   ...
   begin
      ...
      // and (TFinApropriaTesouraria(ListaApropriacoes.Items[i]).getValorCampo('cd_coligada') = cd_coligada)
      and (TFinApropriaTesouraria(ListaApropriacoes.Items[i]).getValorCampo('cd_conta')    = cd_conta)
      and (TFinApropriaTesouraria(ListaApropriacoes.Items[i]).getValorCampo('cd_centro')   = cd_centro) then
      begin
...
begin
   ...
   begin
      ...
      if ListaColigadasFilhas.Items[i].Codigo = Self.cd_coligada_selecionada then
         Selecionada := i;
...
function Tfrm_Tesouraria_gerar.AtualizarApropriacao(CodigoMovimento: Integer): boolean;
var
   apropria : TUMZQuery;
...
begin
   ...
   apropria.ParamByName('CdMovimento').AsInteger := CodigoMovimento;
