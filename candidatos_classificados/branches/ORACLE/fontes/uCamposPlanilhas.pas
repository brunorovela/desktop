...
begin
  ...
  {Código para respeitar o parâmetro utilizado antes dessa implementação,
   ...
   begin
      // Define os campos padroes no OLHINHO 
      ds_campos := 'CD_PESSOA;DS_NOME;DS_CURSO;DS_TURMA;DS_PROPOSITO_IMPORTACAO;DS_EMAIL;DS_TELEFONE;DS_CLASSIFICACAO;NR_ANOSEMESTRE;dt_nascimento;DS_CELULAR';
      lstUtilizados := TStringList.Create;
      SplitString(ds_campos, ';', lstUtilizados);
      isNuPlanilhas := False;
   end;
   ...
   begin
      ...
      begin
         ...
         begin
            ...
            if Pos(grd.DataSource.DataSet.Fields[n].DisplayLabel, lbUtilizadas.Items.Strings[i]) > 0 then
            begin
               isUtilizado := true;
            end;
         end;
...
begin
   if lbUtilizadas.Items.Count = 0 then begin
      Mensagem(
         'Nenhuma coluna foi selecionada para compor a planilha',
         'Atenção',
