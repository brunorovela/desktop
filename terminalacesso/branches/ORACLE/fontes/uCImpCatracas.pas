...
uses
  General, ZAbstractRODataset, ZAbstractDataset, ZDataset, UZDataset, StrUtils, Windows, Messages, SysUtils, Variants, Classes, uDM,
  DB, ZConnection, uImportaDados, DateUtils, uCFuncString;
...
type
   ...
   public
      constructor Create(const usar_matricula: Boolean = True);
      function processaArquivo(sArquivo: string): TStringList; override;
   end;
...
constructor TImpCatracaBase.Create;
begin
  inherited;
...
begin
   ...
   for i := 0 to (ALinhas.Count-1) do begin
      ...
      // Teste de tamanho de linha só para descartar possíveis erros ou linhas em branco.
      if (Length(Trim(ALinhas.Strings[i])) > 40) then begin
         //Erro!!;
         Continue;
      end;

      // Separar a linha conforme os espaços
      // 1 01/11/2022 06:16 0000000059001124


      slAux := splitString(ALinhas.Strings[i], ' ');

      // Tem que ter pelo menos 4 colunas (entrada/saida; data; hora; codigoaluno) ( neste sequencia )
      if (slAux.Count >= 4 ) then begin
         //continua processando
         oItem := TImpRegistro.Create();

         //Tipo ( Entrada, Saída ).
         if (LeftStr(Trim(slAux.Strings[0]), 3) = 'ENT') or (LeftStr(Trim(slAux.Strings[0]), 3) = '010') or (LeftStr(Trim(slAux.Strings[0]), 1) = '1') then
         begin
...
constructor TImpCatracaMicrodin.Create;
begin
  inherited;
   AHistorico := TStringList.Create();
   AQuery := TUMZQuery.Create(DM);
   AQuery.Connection := DM.db;
   AQuery.SQL.Clear();
   AQuery.Params.Clear();
   AQuery.SQL.Add(''+
   'SELECT                                 '+
...
constructor TImpCatracaFlexpoint.Create(const usar_matricula: Boolean);
begin
   inherited Create();
   UsarMatricula := usar_matricula;
end;
...
begin
   ...
   for i := 0 to (ALinhas.Count-1) do begin
      ...
      oItem := TImpRegistro.Create(); 
      oItem.UsaMatricula := UsarMatricula;
...
constructor TImpCatracaInnersNew.create;
begin
   inherited;
   AHistorico := TStringList.Create;
   qryVerificaBase := TUMZQuery.Create(DM);
   qryVerificaBase.Connection := DM.db;
...
begin
   ...
   for i := 0 to (ALinhas.Count-1) do begin
      ...
      // Separar os campos pelos espaços
      slAux := splitString(Trim(ALinhas.Strings[i]), ' ');
...
begin
   ...
   for i := 0 to (ALinhas.Count-1) do begin
      ...
      if (slAux.Count >= 3 ) then begin
         ...
         //Codigo
         try
            oItem.Codigo := StrToInt64(Trim(slAux.Strings[2]));
         except
         end;
