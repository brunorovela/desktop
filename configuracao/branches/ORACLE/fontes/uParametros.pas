...
type
  ...
  public
    { Public declarations }
    procedure Filtra(tipo:Parametros; db_conexao : TZConnection; permAlterar : boolean);
  end;
...
procedure TfrmParametros.Filtra(tipo:Parametros; db_conexao : TZConnection; permAlterar : boolean);
begin

  tipoParametros := tipo;
  podeAlterar := permAlterar;

  if (db_conexao <> nil) then begin
    tblParam.Connection := db_conexao;

    qyCategorias.Connection := db_conexao;
  end;
